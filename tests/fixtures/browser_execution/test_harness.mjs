...
if (!packagePath || !["chrome", "safari"].includes(browser)) {
  throw new Error(
    "Usage: node test_harness.mjs <wasm-bindgen-package-directory> [chrome|safari] [omit=NAME|negative=NAME]",
  );
}
let omit = null;
let negative = null;
...
for (const name of ["f3d_runtime.js", "f3d_runtime_bg.wasm"]) {
  if (!existsSync(join(pkg, name)))
    throw new Error(`Missing compiled artifact: ${join(pkg, name)}`);
}
...
const NEGATIVE_RUNS = {
  "generation-check": {
    failsIn: "stale-result",
    message: "late child published into replaced region",
  },
};
if (negative && !NEGATIVE_RUNS[negative]) {
  throw new Error(
    `Unknown negative run "${negative}"; known: ${Object.keys(NEGATIVE_RUNS).join(", ")}`,
  );
}
const expected = [
  "timer",
...
async function finish(result) {
  ...
  const events =
    Array.isArray(result.events) && result.events.length > 0 ? result.events : streamed;
  ...
  if (omit) {
    ...
    const noOtherComplete = events.every(
      (e) => e.step !== "complete" || e.probe === "unsupported-host",
    );
    passed =
      result.passed === false &&
    ...
    const failedInProbe =
      !complete.has(spec.failsIn) &&
      events.some((e) => e.probe === spec.failsIn && e.step === "spawn");
  ...
  for (const event of events)
    evidence.log({
      lane: "integration",
      bead: "f3d-02-asupersync-browser-execution-58j.2",
      ...
      data: {
        ...
        observed_host_turn: event.host_turn,
      },
    });
  evidence.log({
    lane: "integration",
    ...
    test: omit
      ? "unsupported-host"
      : negative
        ? `${NEGATIVE_RUNS[negative].failsIn}-negative`
        : "browser-execution",
    browser: result.browser,
  ...
  console.log(
    JSON.stringify({ passed, browser, runDir, streamed_events: streamed.length, summary }),
  );
  settle(passed);
}
  ...
  if (req.method === "POST" && pathname === "/result") {
    ...
    req.on("end", async () => {
      try {
        ...
        res.writeHead(400);
        res.end(String(error));
        finish({ passed: false, detail: String(error) });
      }
    });
    return;
  }
...
const url =
  ...
  (omit ? `?omit=${omit}` : negative ? `?negative=${negative}` : "");
...
const timeout = setTimeout(
  () =>
    finish({
      passed: false,
      detail: `Browser probe timed out after 90 seconds; ${streamed.length} events streamed before the hang`,
      browser: streamed.length
        ? { userAgent: "streamed-before-timeout", platform: browser }
        : undefined,
    }),
  90_000,
);
