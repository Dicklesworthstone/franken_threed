...
export async function testDeviceReplacement(buildPacket) {
  ...
  const assertEmpty = () => {
    for (const name of resourceMaps)
      assert(host[name].size === 0, `${name} retained old-device residency`);
  };
  ...
  async function render() {
    ...
    const completion = host.executePacket(buildPacket(true), null, () => {
      ...
      assert(
        host.buffers.has(40) && host.buffers.has(41),
        "Submission observer preceded packet execution",
      );
      observed++;
    });
    ...
    assert(
      red[offset] > 200 && red[offset + 2] < 50 && blue[offset + 2] > 200 && blue[offset] < 50,
      "Recreated device did not render red-A/blue-B",
    );
    assert(
      red.deviceGeneration === host.deviceGeneration &&
        blue.deviceGeneration === host.deviceGeneration,
      "Readback has the wrong device generation",
    );
  }
  ...
  try {
    ...
    const requests = await Promise.allSettled([
      host.negotiateAndCreateDevice(),
      host.negotiateAndCreateDevice(),
    ]);
    if (host.device) devices.add(host.device);
    assert(
      requests[0].status === "rejected" && /superseded/.test(requests[0].reason.message),
    ...
    assert(
      invalid && !invalidObserved,
      "Malformed packet notified submission despite never reaching submit",
    );

    const asyncObserver = await host
      .executePacket(buildPacket(true), null, () => host.executePacket(malformed))
      .then(
        () => null,
        (error) => error,
      );
    assert(
      asyncObserver instanceof TypeError && /synchronously/.test(asyncObserver.message),
    ...
    assert(
      host.device !== firstDevice && host.deviceGeneration > firstGeneration,
    ...
    assert(
      missing && /unknown bufferId/.test(missing.message),
      "Old-device buffer remained readable after replacement",
    );
    await render();
    ...
    assert(
      host.device === replacement && host.buffers.size > 0,
    ...
    assert(
      host.device === null && host.capabilityRecord === null && host.deviceGeneration > beforeLoss,
      "Active device loss did not invalidate the host",
    );
    assertEmpty();
    ...
    assert(
      host.device === null && host.capabilityRecord === null,
