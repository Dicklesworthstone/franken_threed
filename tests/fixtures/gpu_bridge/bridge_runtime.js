...
import { ensureSafePacketBytes, isDetached } from "./memory_transport.js";
...
export class WebGpuBridgeHost {
  ...
  async negotiateAndCreateDevice(requiredProfile = {}) {
    ...
    for (const feat of requiredFeatures) {
      if (!adapterFeatures.has(feat)) {
        throw new Error(
          `Negotiation failed: required WebGPU feature '${feat}' is not supported by adapter`,
        );
      }
    }
    ...
    for (const [key, requestedVal] of Object.entries(reqLimits)) {
      ...
      if (key.startsWith("min")) {
        if (availableVal > requestedVal) {
          throw new Error(
            `Negotiation failed: ${key} alignment mismatch (requested ${requestedVal}, adapter requires ${availableVal})`,
          );
        }
      } else {
        if (availableVal < requestedVal) {
          throw new Error(
            `Negotiation failed: ${key} insufficient (requested ${requestedVal}, available ${availableVal})`,
          );
        }
      }
    ...
    try {
      ...
      for (const [key, requestedVal] of Object.entries(requiredLimits)) {
        ...
        if (key.startsWith("min")) {
          if (actualVal > requestedVal) {
            throw new Error(
              `Device limit verification failed: ${key} does not meet requested profile (requested ${requestedVal}, device has ${actualVal})`,
            );
          }
        } else {
          if (actualVal < requestedVal) {
      ...
      for (const feat of requiredFeatures) {
        if (!device.features.has(feat)) {
          throw new Error(
            `Device feature verification failed: required WebGPU feature '${feat}' is not enabled on device`,
          );
        }
      }
    } catch (error) {
  ...
  async withErrorScopes(kinds, syncAction) {
    if (this.errorScopeActive) {
      throw new Error(
        "Error-scope serialization violation: concurrent task attempted to interleave error scopes",
      );
    }
    ...
    try {
      ...
      if (syncResult && typeof syncResult.then === "function") {
        throw new Error(
          "Error-scope violation: syncAction returned a Promise. Awaits inside error-scope body are strictly forbidden",
        );
      }
    } catch (err) {
      syncError = err;
  ...
  async executePacket(packetBytes, canvasContext = null, onSubmitted = null) {
    ...
    if (magic !== PACKET_MAGIC) {
      throw new Error(
        `Invalid packet magic: 0x${magic.toString(16)} (expected 0x${PACKET_MAGIC.toString(16)})`,
      );
    }
      ...
      try {
        ...
        for (let i = 0; i < commandCount; i++) {
          ...
          if (
            ...
            opcode !== OPCODE_RENDER_PASS &&
            opcode !== OPCODE_RENDER_PASS_DEPTH &&
            opcode !== OPCODE_RENDER_PASS_RESOLVE
          ) {
            throw new Error(
              "SetDrawParameters must immediately precede a render-pass draw command",
            );
          }
          ...
          switch (opcode) {
            ...
            case OPCODE_WRITE_BUFFER: {
              ...
              if (dataOffset + dataLength > dataPayload.byteLength) {
                throw new Error(
                  `WriteBuffer: data slice out of bounds (offset ${dataOffset} + len ${dataLength} > payload ${dataPayload.byteLength})`,
                );
              }
            ...
            case OPCODE_WRITE_TEXTURE: {
              ...
              if (bytesPerRow < minRowBytes) {
                throw new Error(
                  `WriteTexture: bytesPerRow ${bytesPerRow} less than row width ${minRowBytes}`,
                );
              }
              const minDataLength =
                height > 1 ? (height - 1) * bytesPerRow + minRowBytes : minRowBytes;
              if (dataLength < minDataLength) {
                throw new Error(
                  `WriteTexture: dataLength ${dataLength} insufficient for dimensions ${width}x${height} and bytesPerRow ${bytesPerRow}`,
                );
              }

              if (dataOffset + dataLength > dataPayload.byteLength) {
                throw new Error(
                  `WriteTexture: data slice out of bounds (offset ${dataOffset} + len ${dataLength} > payload ${dataPayload.byteLength})`,
                );
              }
            ...
            case OPCODE_CREATE_PIPELINE_MULTISAMPLED: {
              ...
              if (codeOffset + codeLen > dataPayload.byteLength) {
                throw new Error(
                  `CreatePipeline: shader code slice out of bounds (offset ${codeOffset} + len ${codeLen} > payload ${dataPayload.byteLength})`,
                );
              }
              ...
              const uniformSize =
                explicitUniformSize > 0 ? explicitUniformSize : hasUniformBuffer ? 48 : 0;
                        ...
                        vertexStride === 32
                          ...
                          : [
                              { shaderLocation: 0, offset: 0, format: "float32x3" },
                              {
                                shaderLocation: 1,
                                offset: 12,
                                format: vertexStride === 28 ? "float32x4" : "float32x2",
                              },
                            ],
                    },
                  ]
            ...
            case OPCODE_CREATE_PIPELINE_DEPTH_CULL_COLOR: {
              ...
              if (codeOffset + codeLen > dataPayload.byteLength) {
                throw new Error(
                  `CreatePipelineDepth: shader code slice out of bounds (offset ${codeOffset} + len ${codeLen} > payload ${dataPayload.byteLength})`,
                );
              }
              ...
              if (depthCompareCode < 1 || depthCompareCode > 8) {
                throw new Error(
                  `CreatePipelineDepth: invalid depth_compare code ${depthCompareCode}`,
                );
              }
            ...
            case OPCODE_CREATE_PIPELINE_TEXTURED: {
              ...
              if (samplerFilterCode > 1 || !(samplerFilterCode in SAMPLER_FILTER_NAMES)) {
                throw new Error(
                  `CreatePipelineTextured: invalid sampler_filter ${samplerFilterCode}`,
                );
              }
              ...
              if (codeOffset + codeLen > dataPayload.byteLength) {
                throw new Error(
                  `CreatePipelineTextured: shader code slice out of bounds (offset ${codeOffset} + len ${codeLen} > payload ${dataPayload.byteLength})`,
                );
              }
            ...
            case OPCODE_RENDER_PASS_RESOLVE: {
              ...
              if (targetKind !== TARGET_OFFSCREEN && targetKind !== TARGET_CANVAS) {
                throw new Error(
                  `RenderPass: invalid target kind ${targetKind} in 0x${rawTargetType.toString(16)}`,
                );
              }
              ...
              if (hasResolve && targetKind !== TARGET_OFFSCREEN) {
                throw new Error(
                  "RenderPassResolve: multisampled canvas sources are not implemented",
                );
              }
              ...
              if (!currentPassEncoder || currentPassTargetKey !== targetKey || isNewPass) {
                ...
                if (hasResolve) {
                  ...
                  if (
                    !resolveTexture ||
                    sourceTexture.sampleCount !== 4 ||
                    resolveTexture.sampleCount !== 1 ||
                    sourceTexture.width !== resolveTexture.width ||
                    sourceTexture.height !== resolveTexture.height ||
                    ...
                    throw new Error(
                      "RenderPassResolve: requires a 4x source and matching single-sample render-attachment destination",
                    );
                  }
              ...
              if (vertexCount > 0) {
                ...
                if ((pipelineRecord.sampleCount ?? 1) !== currentPassSampleCount) {
                  throw new Error(
                    "RenderPass: pipeline sample count does not match the color attachment",
                  );
                }
                ...
                const isTextured = Boolean(
                  pipelineRecord.isTextured || (pipelineRecord.texture && pipelineRecord.sampler),
                );
                if (hasUniform || isTextured) {
                  ...
                  if (needsRebind) {
                    ...
                    if (hasUniform) {
                      ...
                      if (!uniformBuf) {
                        throw new Error(
                          `RenderPass: uniform buffer ${uniformBufferId} missing for pipeline`,
                        );
                      }
                    }
                        ...
                        bindGroupEntries.push(
                          {
                            binding: 1,
                            resource:
                              pipelineRecord.textureView || pipelineRecord.texture.createView(),
                          },
                          {
                            binding: 2,
            ...
            case OPCODE_RENDER_PASS_DEPTH: {
              ...
              if (targetKind !== TARGET_OFFSCREEN && targetKind !== TARGET_CANVAS) {
                throw new Error(
                  `RenderPassDepth: invalid target kind ${targetKind} in 0x${rawTargetType.toString(16)}`,
                );
              }
              const loadOpCode = (rawTargetType >> 8) & 0xff;
              if (loadOpCode > 2) {
              ...
              if (depthReadOnlyCode > 1) {
                throw new Error(
                  `RenderPassDepth: invalid depth_read_only code ${depthReadOnlyCode}`,
                );
              }
              if (reservedDepthFlags !== 0) {
                throw new Error(
                  `RenderPassDepth: reserved depth flags must be 0, received ${reservedDepthFlags}`,
                );
              }
              ...
              if (!currentPassEncoder || currentPassTargetKey !== targetKey || isNewPass) {
                ...
                if (targetKind === TARGET_CANVAS) {
                  ...
                  if (texture.sampleCount !== 1) {
                    throw new Error(
                      "RenderPassDepth: multisampled depth rendering is not implemented",
                    );
                  }
                  targetView = texture.createView();
                } else {
                ...
                if (hasUniform || isTextured) {
                  ...
                  if (needsRebind) {
                    ...
                    if (hasUniform) {
                      ...
                      if (!uniformBuf) {
                        throw new Error(
                          `RenderPassDepth: uniform buffer ${uniformBufferId} missing for pipeline`,
                        );
                      }
                      bindGroupEntries.push({
                        binding: 0,
            ...
            case OPCODE_COPY_BUFFER_TO_BUFFER: {
              ...
              if (srcOffsetBig > MAX_SAFE_INTEGER_BIGINT) {
                throw new Error(
                  `CopyBufferToBuffer: source_offset ${srcOffsetBig} exceeds Number.MAX_SAFE_INTEGER`,
                );
              }
              if (dstOffsetBig > MAX_SAFE_INTEGER_BIGINT) {
                throw new Error(
                  `CopyBufferToBuffer: destination_offset ${dstOffsetBig} exceeds Number.MAX_SAFE_INTEGER`,
                );
              }
              if (sizeBig > MAX_SAFE_INTEGER_BIGINT) {
                throw new Error(
                  `CopyBufferToBuffer: size ${sizeBig} exceeds Number.MAX_SAFE_INTEGER`,
                );
              }
              ...
              if (sourceOffset % 4 !== 0) {
                throw new Error(
                  `CopyBufferToBuffer: source_offset (${sourceOffset}) must be a multiple of 4`,
                );
              }
              if (destinationOffset % 4 !== 0) {
                throw new Error(
                  `CopyBufferToBuffer: destination_offset (${destinationOffset}) must be a multiple of 4`,
                );
              }
              if (size % 4 !== 0) {
              ...
              if (
                srcOffsetBig + sizeBig > MAX_SAFE_INTEGER_BIGINT ||
                !Number.isSafeInteger(sourceOffset + size)
              ) {
                throw new Error(
                  `CopyBufferToBuffer: source range calculation overflows safe integer bounds`,
                );
              }
              if (
                dstOffsetBig + sizeBig > MAX_SAFE_INTEGER_BIGINT ||
                !Number.isSafeInteger(destinationOffset + size)
              ) {
                throw new Error(
                  `CopyBufferToBuffer: destination range calculation overflows safe integer bounds`,
                );
              }
              ...
              if (!dstBuffer) {
                throw new Error(
                  `CopyBufferToBuffer: unknown destination bufferId ${destinationBufferId}`,
                );
              }

              const COPY_SRC =
                typeof GPUBufferUsage !== "undefined" && GPUBufferUsage.COPY_SRC
                  ? GPUBufferUsage.COPY_SRC
                  : 0x0004;
              const COPY_DST =
                typeof GPUBufferUsage !== "undefined" && GPUBufferUsage.COPY_DST
                  ? GPUBufferUsage.COPY_DST
                  : 0x0008;

              if ((srcBuffer.usage & COPY_SRC) === 0) {
                throw new Error(
                  `CopyBufferToBuffer: source buffer ${sourceBufferId} lacks COPY_SRC usage (usage: 0x${srcBuffer.usage.toString(16)})`,
                );
              }
              if ((dstBuffer.usage & COPY_DST) === 0) {
                throw new Error(
                  `CopyBufferToBuffer: destination buffer ${destinationBufferId} lacks COPY_DST usage (usage: 0x${dstBuffer.usage.toString(16)})`,
                );
              }

              if (sourceOffset + size > srcBuffer.size) {
                throw new Error(
                  `CopyBufferToBuffer: source range out of bounds (offset ${sourceOffset} + size ${size} = ${sourceOffset + size} > buffer size ${srcBuffer.size})`,
                );
              }
              if (destinationOffset + size > dstBuffer.size) {
                throw new Error(
                  `CopyBufferToBuffer: destination range out of bounds (offset ${destinationOffset} + size ${size} = ${destinationOffset + size} > buffer size ${dstBuffer.size})`,
                );
              }
            ...
            case OPCODE_RECORD_BUNDLE_BATCH: {
              ...
              cursor += fieldBytes;
              const lastOffset =
                drawCount === 0 ? dynamicOffset : dynamicOffset + (drawCount - 1) * offsetStride;
              ...
              if (pipelineRecord.hasDepth) {
                throw new Error(
                  "RecordBundle: bundles with depth attachments are not yet implemented",
                );
              }
              if ((pipelineRecord.sampleCount ?? 1) !== 1) {
                throw new Error("RecordBundle: multisampled bundles are not implemented");
              }
              const uniformBuf = pipelineRecord.hasUniformBuffer
                ? this.buffers.get(uniformBufferId)
                : null;
              if (pipelineRecord.hasUniformBuffer && !uniformBuf) {
                throw new Error(
                  `RecordBundle: uniform buffer ${uniformBufferId} missing for pipeline`,
                );
              }
              if (isBatch && pipelineRecord.hasUniformBuffer && drawCount > 0) {
                ...
                if (
                  ...
                  (drawCount > 1 && offsetStride % alignment !== 0) ||
                  lastOffset + (pipelineRecord.uniformSize || 48) > uniformBuf.size
                ) {
                  throw new Error(
                    "RecordBundleBatch: uniform offsets exceed buffer bounds or device alignment",
                  );
                }
              }
            ...
            case OPCODE_EXECUTE_BUNDLES: {
              if (currentPassHasDepth) {
                throw new Error(
                  "ExecuteBundles: bundles in passes with depth attachments are not yet implemented",
                );
              }
              ...
              if (!currentPassEncoder) {
                ...
                while (scanCursor + 2 <= dataBlockStart) {
                  ...
                  if (nextOp === OPCODE_RENDER_PASS) {
                    ...
                    throw new Error(
                      "ExecuteBundles: multisampled resolve passes are not implemented for bundles",
                    );
            ...
            case OPCODE_CREATE_COMPUTE_PIPELINE: {
              ...
              if (cursor + bindingCount * 16 > dataBlockStart) {
                throw new RangeError(
                  `Truncated CREATE_COMPUTE_PIPELINE binding list at command ${i} (needs ${bindingCount * 16} bytes)`,
                );
              }

              if (codeOffset + codeLen > dataPayload.byteLength) {
                throw new RangeError(
                  `CreateComputePipeline: WGSL code out of bounds (offset ${codeOffset} + len ${codeLen} > payload ${dataPayload.byteLength})`,
                );
              }
              const codeBytes = dataPayload.subarray(codeOffset, codeOffset + codeLen);
              ...
              if (entryPointLen > 0) {
                if (entryPointOffset + entryPointLen > dataPayload.byteLength) {
                  throw new RangeError(
                    `CreateComputePipeline: entry_point out of bounds (offset ${entryPointOffset} + len ${entryPointLen} > payload ${dataPayload.byteLength})`,
                  );
                }
                const epBytes = dataPayload.subarray(
                  entryPointOffset,
                  entryPointOffset + entryPointLen,
                );
              ...
              for (let b = 0; b < bindingCount; b++) {
                ...
                if (bindingType === BINDING_TYPE_UNIFORM) {
                  ...
                  throw new TypeError(
                    `CreateComputePipeline: invalid binding_type ${bindingType} at index ${b}`,
                  );
                }
              ...
              if (
                this.device.limits &&
                this.device.limits.maxStorageBuffersPerShaderStage !== undefined
              ) {
            ...
            case OPCODE_DISPATCH_COMPUTE: {
              ...
              if (cursor + bindingCount * 48 > dataBlockStart) {
                throw new RangeError(
                  `Truncated DISPATCH_COMPUTE binding records at command ${i} (needs ${bindingCount * 48} bytes)`,
                );
              }
              ...
              if (
                this.device.limits &&
                this.device.limits.maxComputeWorkgroupsPerDimension !== undefined
              ) {
                const maxDim = this.device.limits.maxComputeWorkgroupsPerDimension;
                if (
                  workgroupCountX > maxDim ||
                  workgroupCountY > maxDim ||
                  workgroupCountZ > maxDim
                ) {
                  throw new RangeError(
              ...
              for (let b = 0; b < bindingCount; b++) {
                ...
                if (!spec) {
                  throw new Error(
                    `DispatchCompute: pipeline ${pipelineId} has no binding specification for index ${bindingIndex}`,
                  );
                }

                if (bindingType !== spec.bindingType) {
                  throw new TypeError(
                    `DispatchCompute: binding index ${bindingIndex} type mismatch: dispatch has ${bindingType}, pipeline expects ${spec.bindingType}`,
                  );
                }

                if (offsetBig > MAX_SAFE_INTEGER_BIGINT) {
                  throw new RangeError(
                    `DispatchCompute: binding ${bindingIndex} offset exceeds Number.MAX_SAFE_INTEGER`,
                  );
                }
                if (sizeBig > MAX_SAFE_INTEGER_BIGINT) {
                  throw new RangeError(
                    `DispatchCompute: binding ${bindingIndex} size exceeds Number.MAX_SAFE_INTEGER`,
                  );
                }
                ...
                if (size === 0) {
                  throw new RangeError(
                    `DispatchCompute: binding ${bindingIndex} size must be greater than 0`,
                  );
                }
                if (
                  (bindingType === BINDING_TYPE_STORAGE_READ ||
                    bindingType === BINDING_TYPE_STORAGE_READ_WRITE) &&
                  size % 4 !== 0
                ) {
                  throw new RangeError(
                    `DispatchCompute: storage buffer binding ${bindingIndex} size ${size} must be a multiple of 4 bytes`,
                  );
                }
                if (spec.minBindingSize > 0 && size < spec.minBindingSize) {
                  throw new RangeError(
                    `DispatchCompute: binding ${bindingIndex} size ${size} is smaller than pipeline minBindingSize (${spec.minBindingSize})`,
                  );
                }
                ...
                if (!buffer) {
                  throw new Error(
                    `DispatchCompute: unknown bufferId ${bufferId} for binding ${bindingIndex}`,
                  );
                }

                if (bindingType === BINDING_TYPE_UNIFORM) {
                  const align =
                    (this.device.limits && this.device.limits.minUniformBufferOffsetAlignment) ||
                    256;
                  if (offset % align !== 0) {
                    throw new RangeError(
                      `DispatchCompute: uniform buffer ${bufferId} offset ${offset} must be aligned to ${align}`,
                    );
                  }
                  if (
                    this.device.limits &&
                    this.device.limits.maxUniformBufferBindingSize !== undefined &&
                    size > this.device.limits.maxUniformBufferBindingSize
                  ) {
                    throw new RangeError(
                      `DispatchCompute: uniform buffer ${bufferId} binding size ${size} exceeds maxUniformBufferBindingSize (${this.device.limits.maxUniformBufferBindingSize})`,
                    );
                  }
                } else if (
                  bindingType === BINDING_TYPE_STORAGE_READ ||
                  ...
                  const align =
                    (this.device.limits && this.device.limits.minStorageBufferOffsetAlignment) ||
                    256;
                  if (offset % align !== 0) {
                    throw new RangeError(
                      `DispatchCompute: storage buffer ${bufferId} offset ${offset} must be aligned to ${align}`,
                    );
                  }
                  if (
                    this.device.limits &&
                    this.device.limits.maxStorageBufferBindingSize !== undefined &&
                    size > this.device.limits.maxStorageBufferBindingSize
                  ) {
                    throw new RangeError(
                      `DispatchCompute: storage buffer ${bufferId} binding size ${size} exceeds maxStorageBufferBindingSize (${this.device.limits.maxStorageBufferBindingSize})`,
                    );
                  }
                } else {
                  throw new TypeError(
                    `DispatchCompute: invalid binding_type ${bindingType} at binding index ${b}`,
                  );
                }

                if (offset + size > buffer.size) {
                  throw new RangeError(
                    `DispatchCompute: buffer ${bufferId} binding range out of bounds (offset ${offset} + size ${size} = ${offset + size} > buffer size ${buffer.size})`,
                  );
                }

                if (bindingType === BINDING_TYPE_UNIFORM) {
                  const UNIFORM =
                    typeof GPUBufferUsage !== "undefined" && GPUBufferUsage.UNIFORM
                  ...
                  const STORAGE =
                    typeof GPUBufferUsage !== "undefined" && GPUBufferUsage.STORAGE
              ...
              if (seenBindingIndices.size !== pipelineRecord.bindingSpecs.size) {
                for (const reqIndex of pipelineRecord.bindingSpecs.keys()) {
                  if (!seenBindingIndices.has(reqIndex)) {
                    throw new Error(
                      `DispatchCompute: missing binding index ${reqIndex} required by pipeline ${pipelineId}`,
                    );
                  }
                }
              }
  ...
  async readbackBuffer(bufferId, byteLength) {
    ...
    try {
      if (
        !this.wrongImplSkipReadbackDeviceCheck &&
