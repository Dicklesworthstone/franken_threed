...
export async function testMultisampleResolve(host, buildPacket) {
  ...
  for (const mode of [0, 1]) {
    ...
    try {
      ...
      if (mismatch !== -1) {
        throw new Error(
          `Resolve mode ${mode}: byte ${mismatch} is ${pixels[mismatch]}, expected ${reference[mismatch]}`,
        );
      }
  ...
  for (const [offset, opcode] of [
    ...
    if (wire.getUint16(offset, true) !== opcode)
      throw new Error("Unexpected Rust MSAA fixture layout");
  }
  for (const [offset, value, expected] of [
    ...
    new DataView(changed.buffer, changed.byteOffset, changed.byteLength).setUint32(
      offset,
      value,
      true,
    );
    try {
      const error = await host.executePacket(changed).then(
        () => null,
        (error) => error,
      );
      if (!error || !expected.test(error.message)) {
        throw new Error(
          `MSAA mutation at ${offset}=${value}: expected ${expected}, received ${error}`,
        );
      }
    } finally {
      releaseProbe(host, previousTarget);
