...
MeshoptDecoder.decodeVertexBuffer = (target, elementCount, byteStride, source, filter) => {
  ...
  if (filter === "OCTAHEDRAL") {
    ...
    for (let i = 0; i < 4 * elementCount; i += 4) {
      ...
      const y = dst[i + 1] * s;
      const z = dst[i + 2] * s;
      const w = Math.sqrt(Math.max(0.0, 1.0 - x ** 2 - y ** 2 - z ** 2));
    ...
    const data =
      byteStride === 4
        ...
        : new Uint16Array(target.buffer, 0, elementCount * 4);
    const dataSigned =
      byteStride === 4
        ? new Int8Array(target.buffer)
        : new Int16Array(target.buffer, 0, elementCount * 4);
...
MeshoptDecoder.decodeIndexBuffer = (target, count, byteStride, source) => {
  ...
  const codeauxOffs = source.length - 0x10;
