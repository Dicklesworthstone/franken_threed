...
test('transcendental_expected.txt exists, has correct header, and exactly 48,888 cases', () => {
  ...
  assert.ok(header.includes('generator=tests/fixtures/math/gen_transcendental_expected.mjs'), 'Header must reference generator');
...
test('transcendental_expected.json exists, parses, and has ZERO raw numbers', () => {
  ...
  assert.equal(violations.length, 0, `Expected 0 raw JSON numbers, found ${violations.length}: ${violations.slice(0, 5).join(', ')}`);
...
test('Every line in transcendental_expected.txt strictly adheres to token and hex formatting', () => {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (unaryNames.has(op)) {
      ...
      assert.ok(HEX_REGEX.test(parts[1]), `Line ${lineNum}: x hex "${parts[1]}" must be 18-char lowercase hex`);
      assert.ok(HEX_REGEX.test(parts[2]), `Line ${lineNum}: result hex "${parts[2]}" must be 18-char lowercase hex`);
    } else if (binaryNames.has(op)) {
      assert.equal(
        parts.length, 4,
        `Line ${lineNum}: binary op '${op}' must have 4 space-separated tokens, got ${parts.length}: "${lines[i]}"`
      );
      assert.ok(HEX_REGEX.test(parts[1]), `Line ${lineNum}: arg1 hex "${parts[1]}" must be 18-char lowercase hex`);
      assert.ok(HEX_REGEX.test(parts[2]), `Line ${lineNum}: arg2 hex "${parts[2]}" must be 18-char lowercase hex`);
      assert.ok(HEX_REGEX.test(parts[3]), `Line ${lineNum}: result hex "${parts[3]}" must be 18-char lowercase hex`);
    } else {
...
test('transcendental_expected.txt and JSON carry identical cases in exact order', () => {
  ...
  for (const { name } of UNARY_OPS) {
    for (const c of data[name]) {
      ...
      assert.equal(line, `${name} ${c.x} ${c.result}`, `Mismatch at line ${idx + 2} for op ${name}`);
      idx++;
    }
  }
  ...
  for (const c of data.atan2) {
    const line = txtLines[idx];
    assert.equal(line, `atan2 ${c.y} ${c.x} ${c.result}`, `Mismatch at line ${idx + 2} for op atan2`);
    idx++;
  }
  ...
  for (const c of data.hypot) {
    const line = txtLines[idx];
    assert.equal(line, `hypot ${c.x} ${c.y} ${c.result}`, `Mismatch at line ${idx + 2} for op hypot`);
    idx++;
  }
