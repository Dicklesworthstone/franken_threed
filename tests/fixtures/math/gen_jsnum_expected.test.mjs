...
test('IEEE-754 64-bit bit pattern hex roundtrip', () => {
  ...
  for (const v of cases) {
    ...
    assert.match(hex, /^0x[0-9a-f]{16}$/, `Hex format for ${v} should be 0x followed by 16 hex chars`);
...
test('Inputs table contains all mandatory spec entries and 64 LCG doubles', () => {
  ...
  assert.ok(inputs.length >= 64 + 15, `Inputs count ${inputs.length} must include at least 15 spec entries + 64 LCG doubles`);

  // Check mandatory individual items
  assert.ok(inputs.some(v => Number.isNaN(v)), 'Must contain NaN');
  assert.ok(inputs.some(v => Object.is(v, 0.0)), 'Must contain +0.0');
  assert.ok(inputs.some(v => Object.is(v, -0.0)), 'Must contain -0.0');
  assert.ok(inputs.some(v => v === Infinity), 'Must contain +Infinity');
  assert.ok(inputs.some(v => v === -Infinity), 'Must contain -Infinity');
  assert.ok(inputs.some(v => v === Number.MIN_VALUE), 'Must contain subnormal Number.MIN_VALUE');
  assert.ok(inputs.some(v => v === -Number.MIN_VALUE), 'Must contain subnormal -Number.MIN_VALUE');
  assert.ok(inputs.some(v => v === 0.49999999999999994), 'Must contain 0.49999999999999994');
  assert.ok(inputs.some(v => v === 0.5), 'Must contain 0.5');
  assert.ok(inputs.some(v => v === -0.5), 'Must contain -0.5');
  assert.ok(inputs.some(v => v === 1.5), 'Must contain 1.5');
  assert.ok(inputs.some(v => v === -1.5), 'Must contain -1.5');
  assert.ok(inputs.some(v => v === Math.pow(2, 31)), 'Must contain 2^31');
  assert.ok(inputs.some(v => v === Math.pow(2, 32) + 1), 'Must contain 2^32+1');
  assert.ok(inputs.some(v => v === 1e15), 'Must contain 1e15');
  assert.ok(inputs.some(v => v === Math.pow(2, 52)), 'Must contain 2^52');
  assert.ok(inputs.some(v => v === Math.pow(2, 53)), 'Must contain 2^53');
  assert.ok(inputs.some(v => v === Number.MAX_VALUE), 'Must contain f64 max');
});
...
test('jsnum_expected.txt and jsnum_expected.json carry identical cases in identical order', () => {
  ...
  assert.ok(commentLine.includes('generator=tests/fixtures/math/gen_jsnum_expected.mjs'), 'Comment line must contain generator name');
  ...
  for (let i = 0; i < 11844; i++) {
    ...
    if (jc.second !== undefined) {
      assert.equal(tokens.length, 4, `Line ${i + 2}: expected 4 tokens for binary op ${jc.op}, got "${line}"`);
      ...
      assert.equal(tokens.length, 3, `Line ${i + 2}: expected 3 tokens for unary op ${jc.op}, got "${line}"`);
      assert.equal(tokens[0], jc.op, `Line ${i + 2}: op mismatch`);
      assert.equal(tokens[1], jc.x, `Line ${i + 2}: x mismatch`);
