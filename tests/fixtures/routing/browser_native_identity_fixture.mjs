...
function waitForContextRestored(canvas, timeoutMs = 5000) {
  return new Promise((resolve, reject) => {
    const timer = setTimeout(() => {
      ...
      reject(new Error(`Timed out waiting for 'webglcontextrestored' on ${canvas.id} after ${timeoutMs}ms`));
    }, timeoutMs);
...
export async function runBrowserNativeIdentityVerification() {
  ...
  try {
    ...
    for (const forceWebGL of [false, true]) {
      ...
      if (candidate.constructor !== source.constructor ||
          ...
          !(candidate instanceof PinnedWebGPURenderer) || candidate.isWebGLRenderer) {
        throw new Error('Opaque escape substituted a different source renderer class');
      }
      if (candidate.backend.constructor !== source.backend.constructor ||
      ...
      const beforeInit = renderer => {
        try { return { value: renderer.getContext()?.constructor.name }; }
      ...
      try {
        ...
        const nativeContext = forceWebGL
          ...
          : ((typeof GPUCanvasContext !== 'undefined' && context instanceof GPUCanvasContext) ||
             context instanceof WebGL2RenderingContext);
        if (!nativeContext || context === referenceContext ||
        ...
        branches.push({
          forceWebGL, backend: candidate.backend.constructor.name,
          nativeContext: context.constructor.name, route: getRendererRoute(candidate),
        });
      } finally {
        candidate.dispose();
        source.dispose();
  ...
  try {
    ...
    function assertExactBackendContract(renderer, glContext, label) {
      ...
      const isNativeGL =
        ...
        (typeof WebGLRenderingContext !== 'undefined' && glContext instanceof WebGLRenderingContext);
      if (!isNativeGL) {
        throw new Error(`${label}: acquired context is not an instance of native WebGL(2)RenderingContext`);
      }
    }
    ...
    if (!extRef || !extCand) {
      ...
      if (!allPixelsMatch) {
        throw new Error('Candidate and reference pixel bytes diverged across loss/restore checkpoints');
      }
  ...
  const allPassed = results.every(
    (r) => r.status === 'pass' || (r.status === 'host-blocked' && r.referenceBlocked && r.candidateBlocked)
  );
  const decisionLog = [
