...
test("Positive: Opaque escapes preserve the source WebGPURenderer constructor and options", () => {
  ...
  assert.ok(
    instance instanceof FakeWebGPURenderer,
    "Opaque escape must preserve the source constructor",
  );
  assert.equal(instance.isWebGLRenderer, undefined, "Must not substitute legacy WebGLRenderer");
  assert.equal(
    instance.opts.forceWebGL,
    undefined,
    "Must not force a backend the source did not select",
  );
  assert.equal(getRendererRoute(instance), ExecutionRoute.EXACT_BACKEND);
...
test("Positive: Connected groups propagate EXACT_BACKEND when sharing mutable resources (Order: Exact first)", () => {
  ...
  assert.equal(
    getRendererDecision(inst1).groupId,
    getRendererDecision(inst2).groupId,
    "Renderers must share connected group ID",
  );
});
...
test("Negative: Connected groups reject residency conflict when non-exact renderer is already committed (Order: Non-exact first)", () => {
  ...
  assert.throws(
    ...
    (err) => {
      ...
      assert.match(
        err.message,
        new RegExp(`already committed to route '${ExecutionRoute.RETAINED_UPSTREAM}'`),
      );
      return true;
    },
  );
...
test("Positive: Two renderers sharing only an immutable asset keep independent routes", () => {
  ...
  assert.notEqual(
    getRendererDecision(inst1).groupId,
    getRendererDecision(inst2).groupId,
    "Immutable asset sharing must NOT couple connected groups",
  );
});
...
test("Defect 1 regression: constructor increments side-effect counter and throws, proving single invocation and preserved error identity", () => {
  ...
  assert.equal(
    sideEffects,
    1,
    "Constructor must be invoked exactly once, never caught and retried",
  );
...
test("Binds-then-throws regression: canvas lock is retained after constructor acquires context then throws", () => {
  ...
  assert.equal(
    router.getCanvasLock("bound-then-failed-canvas")?.route,
...
test("Exact fallback without a source constructor requires a constructor-qualified implementation", () => {
  for (const unqualified of [
    undefined,
    AdmittedWebGLRenderer,
    { default: AdmittedWebGLRenderer },
  ]) {
    const router = new RendererConstructionRouter({
...
test("Positive: Real H1 module graph JSON (schema 1.0.0) consumed directly by decideRendererRoute", async () => {
  const h1Bundle = await buildModuleGraph(
...
test("Regression: Real H1 bundle with non-literal forceWebGL (! api.webgpu) records UNRESOLVED_FORCE_WEBGL and defers to runtime", async () => {
  ...
  assert.equal(
    site.options.forceWebGL,
    "unresolved",
    'AST extraction must classify !api.webgpu as "unresolved"',
  );
  ...
  assert.ok(
    runtimeForcedInstance instanceof MockWebGPURenderer,
    "Must preserve supplied WebGPURenderer constructor and prototype",
  );
  assert.ok(
    !(runtimeForcedInstance instanceof AdmittedWebGLRenderer),
    "Must not substitute legacy WebGLRenderer",
  );
...
test("Regression: caller-supplied constructorFn is invoked exactly once and not shadowed by registered implementations", () => {
  ...
  assert.equal(
    instance.isCallerSupplied,
    true,
    "Instance must be from caller constructorFn, not substituted implementation",
  );
  assert.equal(
    instance.isAdmitted,
    undefined,
    "Registered implementation must not shadow caller constructorFn",
  );
  ...
  assert.ok(
    absentConstructorInstance instanceof AdmittedBackendRenderer,
    "Registered implementation must be used when constructorFn is absent",
  );
...
test("Positive: H1 production route preserves WebGPURenderer, keeps diagnostics external, and supports backend query reload", async () => {
  ...
  assert.ok(
    forcedInstance instanceof MockRenderableWebGPURenderer,
    "Must preserve supplied WebGPURenderer constructor",
  );
  assert.ok(
    !(forcedInstance instanceof MockRenderableWebGLRenderer),
    "Must not substitute legacy WebGLRenderer",
  );
  ...
  assert.equal(
    Object.hasOwn(forcedInstance, "render"),
    false,
    "render must not be an own property",
  );
  assert.equal(
    forcedInstance.render,
    MockRenderableWebGPURenderer.prototype.render,
    "render must be the original prototype method",
  );
  ...
  assert.equal(
    forcedInstance.__f3d_renderer_id__,
    undefined,
    "No __f3d_renderer_id__ own property",
  );
  assert.deepEqual(
    Object.keys(forcedInstance).filter((k) => k.startsWith("__f3d_")),
    [],
    "No __f3d_* properties in Object.keys",
  );
  ...
  assert.equal(Object.hasOwn(unforcedInstance, "render"), false);
  assert.deepEqual(
    Object.keys(unforcedInstance).filter((k) => k.startsWith("__f3d_")),
    [],
  );
  ...
  assert.equal(
    router.getCanvasLock("canvas-h1-unforced")?.route,
  ...
  assert.equal(
    rejectedDecision.span,
    offendingSpan,
    "Rejected decision must record the offending source span",
  );
...
test("Independent generated H1 facade execution: module initialization, constructor identity, instanceof, subclassing/new.target, and import-map aliases", async () => {
  ...
  assert.equal(
    typeof facade.WebGPURenderer,
    "function",
    "Exported WebGPURenderer must be a constructor function",
  );
  assert.ok(
    facade.router instanceof RendererConstructionRouter,
    "Exported router must be an instance of RendererConstructionRouter",
  );
  ...
  assert.ok(
    instWebGPU instanceof facade.WebGPURenderer,
    "instWebGPU must be instanceof facade.WebGPURenderer",
  );
  assert.equal(
    instWebGPU.backend.constructor.name,
    "WebGPUBackend",
    "forceWebGL: false selects WebGPUBackend",
  );
  assert.equal(
    Object.keys(instWebGPU).filter((k) => k.startsWith("__f3d_")).length,
    0,
    "No __f3d_* own properties",
  );
  ...
  assert.ok(
    instWebGL instanceof facade.WebGPURenderer,
    "instWebGL must be instanceof facade.WebGPURenderer",
  );
  assert.equal(
    instWebGL.backend.constructor.name,
    "WebGLBackend",
    "forceWebGL: true selects WebGLBackend (never legacy WebGLRenderer)",
  );
  ...
  assert.ok(
    subInstance instanceof facade.WebGPURenderer,
    "Must be instanceof facade.WebGPURenderer",
  );
  assert.equal(subInstance.customFeature, "enabled", "Custom subclass fields must be preserved");
  assert.equal(
    subInstance.capturedNewTarget,
    CustomAppRenderer,
    "new.target must be preserved in derived constructor",
  );
  ...
  assert.deepEqual(overrideInstance.render("scene1", "cam1"), {
    overridden: true,
    scene: "scene1",
    camera: "cam1",
  });
  ...
  const upstreamModule = await import(
    path.resolve(repoRoot, "upstream/three.js/build/three.webgpu.js")
  );
  assert.equal(
...
test("6mv.4 report criterion: window-independent router decision log and attribution log for H1 WebGPU and forceWebGL branches", async () => {
  ...
  const generatedSource =
    ...
    "\n// test-nonce: 6mv.4-report-regression-" +
    Date.now();
  const dataUri = "data:text/javascript;base64," + Buffer.from(generatedSource).toString("base64");
  const facade = await import(dataUri);
  ...
  assert.ok(
    facade.router instanceof RendererConstructionRouter,
    "Facade must export router directly",
  );
  ...
  assert.ok(
    rendererWebGPU instanceof facade.WebGPURenderer,
    "WebGPU renderer must be instanceof facade.WebGPURenderer",
  );
  ...
  assert.ok(
    rendererWebGL instanceof facade.WebGPURenderer,
    "WebGL renderer must be instanceof facade.WebGPURenderer",
  );
  ...
  assert.equal(
    webgpuEntry.span,
    "webgpu_performance_renderbundle.html:188:13",
    "Span must match H1 constructor site",
  );
  assert.equal(
    webgpuEntry.route,
    ExecutionRoute.RETAINED_UPSTREAM,
    "WebGPU branch must route to retained-upstream",
  );
  assert.deepEqual(
    webgpuEntry.reasons,
    [EscapeReason.SPECIALIZATION_UNAVAILABLE],
    "Reasons must contain specialization-unavailable",
  );
  ...
  assert.equal(
    webglEntry.route,
    ExecutionRoute.EXACT_BACKEND,
    "forceWebGL branch must route to exact-backend",
  );
  assert.deepEqual(
    webglEntry.reasons,
    [EscapeReason.EXPLICIT_SOURCE_SELECTION],
    "Reasons must contain explicit-source-selection",
  );
  ...
  assert.notEqual(
    webgpuEntry.group,
    webglEntry.group,
    "Independent canvases must have distinct group memberships",
  );
  ...
  assert.equal(
    lockWebGPU?.route,
    ExecutionRoute.RETAINED_UPSTREAM,
    "Canvas lock for WebGPU canvas must match retained-upstream",
  );
  ...
  assert.equal(
    lockWebGL?.route,
    ExecutionRoute.EXACT_BACKEND,
    "Canvas lock for WebGL canvas must match exact-backend",
  );
});
...
test("Positive and negative: Exact backend router preserves route and permanent canvas lock across WebGL context loss and restore", () => {
  ...
  assert.equal(
    mockGl.isContextLost(),
    true,
    "Context must remain lost when restoration is skipped",
  );
...
test("Regression: nested successful construction surviving outer preflight failure", () => {
  ...
  assert.equal(
    router.getCanvasLock("canvas-nested-survival")?.route,
    ExecutionRoute.RETAINED_UPSTREAM,
  );
...
test("Regression: reentrant construction in callImplementations alters constraints and triggers preflight revalidation failure", () => {
  ...
  const dynamicCallImplementations = {
    get [ExecutionRoute.RETAINED_UPSTREAM]() {
      ...
      return function MockDynamicRetained() {
        this.isRetained = true;
      };
    },
  };
...
test("Regression: preflight RouteLockError does not poison shared resources for subsequent renderers", () => {
  ...
  assert.equal(
    router.getCanvasLock("canvas-fresh-unpoisoned")?.route,
    ExecutionRoute.RETAINED_UPSTREAM,
  );
});
...
test("Sunny regression: implementations[EXACT] getter builds retained inner sharing S, then outer fails; inner survives and blocks later exact sharing S", () => {
  ...
  assert.equal(
    connectedGroups._committedRoutes.get(innerRendererId),
    ExecutionRoute.RETAINED_UPSTREAM,
  );
...
test("Final review regression: refresh final group metadata after reentrant construction sharing resource", () => {
  ...
  assert.equal(
    outerDecision.groupId,
    innerDecision.groupId,
    "outer and inner renderers sharing S must report the same groupId",
  );
});
