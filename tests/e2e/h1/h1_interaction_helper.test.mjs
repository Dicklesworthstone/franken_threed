...
test("H1 Query Parser: Replicates H1 startup parameter semantics exactly, preserving count=0 and NaN", () => {
  ...
  // 6. Full URL parsing
  const qFullUrl = parseH1QueryParams(
    "http://localhost:8080/examples/webgpu_performance_renderbundle.html?backend=webgpu&renderBundle=true&count=8000",
  );
...
test("Inspector Control Locator & Toggle: Interacts with Three.js Inspector DOM structures", () => {
  ...
  const mockDoc = {
    querySelectorAll(selector) {
      ...
      if (selector === 'input[type="checkbox"]')
        return rows.map((r) => r.querySelector('input[type="checkbox"]'));
      return [];
    },
    querySelector() {
      return null;
...
test("Defect 4 Regression (Mail 7694 & 7862): detectCanvasContext operates correctly across iframe realms and rejects dummy objects", () => {
  ...
  assert.equal(
    detected,
    "webgpu",
    "detectCanvasContext must recognize cross-realm GPUCanvasContext",
  );
  ...
  assert.equal(
    detectCanvasContext(dummyCanvas),
    "none",
    "Arbitrary JS object must not be falsely recognized as webgpu",
  );
});
...
test("Defect 2 Regression (Mail 7694): Rejects false-pass when both canvases are missing or contextType is none", () => {
  ...
  assert.equal(
    res.pass,
    false,
    "Must reject false-pass when canvases are uninitialized or missing",
  );
  assert.ok(
    res.diffs.some((d) =>
      d.includes("Reference canvas must be initialized with valid GPU/GL context"),
    ),
  );
  assert.ok(
    res.diffs.some((d) =>
      d.includes("Candidate canvas must be initialized with valid GPU/GL context"),
    ),
  );
});
...
test("Defect 3 Regression (Mail 7694): Asserts dynamic control state parity and rejects mismatch", () => {
  ...
  assert.ok(
    res.diffs.some((d) =>
      d.includes("dynamic checked state mismatch: reference false vs candidate true"),
    ),
  );
});
...
test("State Comparison Negative 1: Mismatched backend query fails comparison", () => {
  ...
  assert.ok(
    res.diffs.some((d) =>
      d.includes("Backend query mismatch: reference 'webgpu' vs candidate 'webgl'"),
    ),
  );
});
...
test("State Comparison Negative 3: Dishonest candidate routing (claiming specialized-webgpu) fails comparison", () => {
  ...
  assert.ok(
    res.diffs.some((d) =>
      d.includes(
        "Candidate route dishonest: expected 'retained-upstream', got 'specialized-webgpu'",
      ),
    ),
  );
});
...
test("State Comparison Negative 4: Polluted reference containing router fails comparison", () => {
  ...
  assert.equal(res.pass, false);
  assert.ok(
    res.diffs.some((d) => d.includes("Reference snapshot unexpectedly contains __f3d_router__")),
  );
});
...
test("State Comparison Negative 7 (Mail 7862): Canvas context mismatch with requested backend branch fails comparison", () => {
  ...
  assert.ok(
    res.diffs.some((d) =>
      d.includes("does not match requested branch 'webgl2' for backend 'webgl'"),
    ),
  );
});
...
test("State Comparison Positive (Mail 7862): Preserves count NaN parity via Object.is", () => {
  ...
  assert.equal(
    res.pass,
    true,
    `Expected NaN count parity to pass via Object.is, got: ${res.diffs.join("; ")}`,
  );
});
...
test("Seeded Random Prelude: Injects deterministic PRNG before application scripts in HTML (ROOT H1 REVIEW)", () => {
  ...
  assert.ok(
    injected.indexOf('<script type="importmap">') <
      injected.indexOf('id="f3d-seeded-random-prelude"'),
  );
  assert.ok(
    injected.indexOf('id="f3d-seeded-random-prelude"') < injected.indexOf('<script type="module">'),
  );
