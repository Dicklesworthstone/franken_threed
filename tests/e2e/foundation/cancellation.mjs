...
export async function runCancellationSuite(packagePath, browser = "chrome") {
  if (!packagePath || !["chrome", "safari"].includes(browser)) {
    throw new Error(
      'Usage: runCancellationSuite(packagePath, browser) where browser is "chrome" or "safari"',
    );
  }
  ...
  const completedProbes = new Set(
    normalEvents.filter((e) => e.step === "complete").map((e) => e.test || e.probe),
  );
  ...
  if (missingProbes.length > 0) {
    throw new Error(
      `[f3d-58j.3] Missing required 58j.3 completed probe observations: ${missingProbes.join(", ")}`,
    );
  }
  ...
  console.log(
    `[f3d-58j.3] (1/2) PASS: Normal run completed with all 6 cancellation probes verified: ${PROBES_58J3_NORMAL.join(", ")}`,
  );
  ...
  const negFailureEvent = negativeEvents.find(
    (e) =>
      (e.test === "stale-result-negative" ||
        e.test === "stale-result" ||
        e.probe === "stale-result") &&
      e.level === "info",
  );
  console.log(
    `[f3d-58j.3] (2/2) PASS: Negative generation-check run verified (non-vacuous publication gate).`,
  );
  ...
  console.log(
    `[f3d-58j.3] PASS: All cancellation probes + negative generation-check verified on ${browser}. Evidence written under 02.3 (${suiteRunId}).`,
  );
...
if (process.argv[1] && fileURLToPath(import.meta.url) === resolve(process.argv[1])) {
  ...
  if (!packagePath || !["chrome", "safari"].includes(browser)) {
    console.error(
      "Usage: node tests/e2e/foundation/cancellation.mjs <wasm-bindgen-package-directory> [chrome|safari]",
    );
    process.exit(1);
  }
