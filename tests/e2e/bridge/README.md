...
| **9. Linear Memory Borrow Guards & Transport Policy Gate** | **Authoritative (05.7)** | Linear memory growth during active Rust borrows & view invalidation | Enforces that no WebAssembly memory growth or scheduler re-entry occurs while Rust linear memory borrows are held (`gpu_bridge_borrow_enter()`, `gpu_bridge_borrow_exit()`, `gpu_bridge_try_grow_memory()`). Idle memory growth succeeds; active borrow produces a non-zero `BorrowToken` during which growth attempts return `false`; invalid token exits return `false`; valid token exit succeeds and restores growth. Real broken control attempting growth inside an open borrow is demonstrably rejected by `assertMemoryGrowthAllowed`. Also establishes actual memory transport invariance: captures `WebAssembly.Memory` directly from `await wasm.default()`, verifies owned packet bytes (`gpu_bridge_build_triangle_packet()`) survive `WebAssembly.Memory.grow()`, verifies unrefreshed view detachment/invalidation, confirms wasm-bindgen internal view refresh on post-growth allocation, and proves real Rust callback reentry. |
| **10. AffineRows Wire Layout & WGSL Transform Evaluation** | **Authoritative (05.7)** | Wire layout errors and WGSL row dot-product transform evaluation | Combines full wire layout validation (codes 0, 1, 2, 4 via `gpu_bridge_validate_affine_rows()`) with authoritative browser WGSL evaluation via real binary transform packet (`gpu_bridge_build_affine_rows_transform_packet()`). Proves that the WGSL row dot-product vertex shader correctly evaluates the 48-byte `AffineRows` wire uniform, asserting hand-computed screen coordinates (transformed center (48, 32) Green [0,255,0,255], untransformed center (32, 32) Black, surrounding bounds Black). GPU-side broken control demonstrably rejected by `assertAffineRowsTransformMatch` and `assertAffineRowsLayoutValid`. |
| **11. Nested Pass Protocol & LoadOp Load Resume** | **Authoritative (05.7)** | Wiping intermediate render pass results across pass interleaving | Candidate bridge executes three logical passes on target 10: prefix clear plus red left draw, nested pass on target 11, resume with `loadOp: "load"` plus blue right draw, readback buffer 20 (64x64 rgba8unorm). Readback matches independent direct-JS oracle with Red at (24,32), Blue at (56,32), and Black at (2,2). Real GPU broken control (resuming with `loadOp: "clear"`, erasing Red) is demonstrably rejected by `assertNestedPassMatch`. Consumes real compiled Rust/Wasm export `gpu_bridge_build_nested_pass_packet` (alias `f3d_build_nested_pass_packet`) confirmed in Mail #6957. |
| **12. Nested Canvas Pass Protocol & Swapchain Reentrancy** | **Authoritative (05.7)** | Retaining intermediate offscreen render results across canvas swapchain reentrancy | Candidate bridge executes 14-command packet: canvas prefix clear plus red draw (pipeline 201), nested offscreen target 11 clear plus green draw (pipeline 200), canvas resume with loadOp: "load" plus blue draw, and copy of target 11 to buffer 20. Offscreen target 11 readback matches independent direct-JS oracle with Green at (24,32), Black at (56,32), and Black at (2,2). Asserts packet executes without WebGPU validation errors inside error scopes when canvas context is provided. Broken control: oracle-vs-candidate byte mismatch and missing export rejection. Consumes real compiled Rust/Wasm export `gpu_bridge_build_nested_canvas_pass_packet` (alias `f3d_build_nested_canvas_pass_packet`) confirmed in Mail #7088. |

### Pending Product Capabilities: ZERO PENDING (All 12 Authoritatively Implemented)
...
**All 12 bridge capabilities are fully backed by real Rust/Wasm product exports in `crates/f3d-runtime/src/gpu_host.rs`.** Zero pending items, zero mocks, zero synthetic JS fallbacks, zero unclassified items.
