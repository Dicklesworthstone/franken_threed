...
if (!existsSync(pkgDir)) {
  console.error(
    `[f3d-05.7-harness] FATAL: Specified Wasm package directory does not exist: ${pkgDir}`,
  );
  process.exit(1);
}
...
for (const artifact of requiredArtifacts) {
  ...
  if (!existsSync(artifactPath)) {
    console.error(
      `[f3d-05.7-harness] FATAL: Required Wasm artifact missing in ${pkgDir}: ${artifact}`,
    );
    process.exit(1);
  }
}
