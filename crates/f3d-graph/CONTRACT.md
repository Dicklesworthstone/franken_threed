...
| `CanvasOutput` / `CanvasId` / `CanvasEpochTracker` | Short-lived canvas representation bound to the host's acquisition interval. Multiple queue submissions may use it until `end_frame_interval`. |
...
3. **Pass Splitting Invariant**: Every split preserves all color destinations in their original slots, depth/stencil state, initial loads, final stores and final resolve publication. The current implementation admits a cross-draw buffer usage conflict only when two segments validate with those attachments unchanged and no inherited draw state is lost. Attachment-versus-sampling conflicts remain rejected with `HazardError::AttachmentSamplingConflict`: the current `Draw` representation cannot prove that removing an attachment preserves its outputs. Automatic attachment-aware splitting remains incomplete under bead `f3d-07-initial-webgpu-render-slice-2v8.2`; rejecting a conflict does not satisfy that positive capability requirement.
...
7. **Canvas Freshness and Epoch Isolation**: Canvas textures are valid inside the host's acquisition interval; queue submission does not end it. The host calls `end_frame_interval` when that texture expires. Plans carry captured output epochs validated against the `CanvasEpochTracker` at compilation and execution/publication boundaries. Using a canvas texture across output epochs returns `CanvasError::CanvasCachedAcrossEpochs`; use after explicit interval end returns `CanvasError::CanvasIntervalEnded`. Missing trackers on canvas use cannot bypass this guard.
...
- Unit tests in `crates/f3d-graph/tests/graph_tests.rs`:
  ...
  - Positive & Splitting: Cross-draw buffer usage changes split while preserving attachment slots, depth/stencil operations and final resolve publication. Attachment-feedback counterexamples are rejected without dropping outputs; automatic attachment-aware splitting still needs implementation and positive coverage.
