...
| `asupersync` | Plan citation `efa5798` (exists, 59 commits behind `f84ac21`); sibling `f84ac21`; sibling current HEAD `4232e302bfd971ffe4875dc44e29a31c32d7fa39` (Cargo manifests, Cargo.lock and rust-toolchain identical between `f84ac21` and `4232e30`); its toolchain is `nightly-2026-08-31`, same as ours; workspace pin decision pending root | Structured concurrency regions, cooperative task polling, timer wakeups, explicit cancellation | Not run | Not run | **No integration has been compiled.** Version divergence between plan citation and sibling repo HEAD must be resolved before workspace integration. |
