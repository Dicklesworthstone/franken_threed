...
| `asupersync` | Plan citation `efa5798` (exists, 59 commits behind `f84ac21`); sibling `f84ac21`; sibling current HEAD `4232e302bfd971ffe4875dc44e29a31c32d7fa39` (Cargo manifests, Cargo.lock and rust-toolchain identical between `f84ac21` and `4232e30`); its toolchain is `nightly-2026-08-31`, same as ours; selected workspace revision: `4232e302bfd971ffe4875dc44e29a31c32d7fa39` (controller default 2026-09-09T22:48Z, manifests identical to `f84ac21`; root may override before any crate depends on it) | Structured concurrency regions, cooperative task polling, timer wakeups, explicit cancellation | Not run | Not run | **No integration has been compiled.** Version divergence between plan citation and sibling repo HEAD must be resolved before workspace integration. |
| `wasm-bindgen` | `0.2.126` (declared `0.2` in asupersync) | External browser boundary, JS exception handling, Wasm export plumbing | Not run | Not run | Resolved by asupersync lockfile, not yet a workspace dependency |
| `js-sys` | `0.3.103` (declared `0.3` in asupersync) | Direct JavaScript global object bindings (`Function`, `Uint8Array`, `Promise`) | Not run | Not run | Resolved by asupersync lockfile, not yet a workspace dependency |
| `web-sys` | `0.3.103` (declared `0.3` in asupersync) | WebGPU context, canvas acquisition, DOM event listener bindings | Not run | Not run | Resolved by asupersync lockfile, not yet a workspace dependency |
| `wasm-bindgen-futures` | `0.4.76` (declared `0.4` in asupersync) | Bridge between Rust futures and JavaScript promises | Not run | Not run | Resolved by asupersync lockfile, not yet a workspace dependency |
