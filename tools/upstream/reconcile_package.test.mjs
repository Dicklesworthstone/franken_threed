...
async function createTestDir(name) {
  const dir = path.join(
    BASE_TEST_TEMP,
    `f3d_reconcile_test_${name}_${Date.now()}_${Math.random().toString(36).slice(2)}`,
  );
...
test("reconcilePackage: clean synthetic package with matching files passes with zero unexplained discrepancies", async () => {
  ...
  await fs.writeFile(
    path.join(testDir, "build", "three.module.js"),
    'export const REVISION = "186";',
  );
  await fs.writeFile(
    path.join(testDir, "build", "three.cjs"),
    'module.exports = { REVISION: "186" };',
  );
  await fs.writeFile(path.join(testDir, "build", "three.webgpu.js"), "export const WebGPU = true;");
  await fs.writeFile(
    path.join(testDir, "examples", "jsm", "controls", "OrbitControls.js"),
...
test("reconcilePackage: detects missing export targets and directories pretending to be modules", async () => {
  ...
  const missing = report.discrepancies.missing_export_targets.find(
    (m) => m.exportKey === "./missing",
  );
  assert.ok(missing);
  assert.equal(missing.target, "./build/three.missing.js");
  assert.equal(missing.explained, false);

  const isDir = report.discrepancies.missing_export_targets.find(
    (m) => m.exportKey === "./is_a_dir",
  );
  assert.ok(isDir);
...
test("reconcilePackage: wildcard pattern with suffix matching (e.g. ./*.js -> ./*.js)", async () => {
  ...
  await fs.writeFile(
    path.join(testDir, "src", "nodes", "ShaderNode.js"),
...
test("reconcilePackage: identifies explained alias duplicates vs unexplained duplicates", async () => {
  ...
  const customAliases = new Map([["./build/three.webgpu.js", "Intentional WebGPU alias"]]);
  ...
  const explainedDup = report.discrepancies.alias_duplicates.find(
    (a) => a.targetPath === "build/three.webgpu.js",
  );
  assert.ok(explainedDup);
  assert.equal(explainedDup.explained, true);
  assert.equal(explainedDup.reason, "Intentional WebGPU alias");

  const unexplainedDup = report.discrepancies.alias_duplicates.find(
    (a) => a.targetPath === "build/unknown.js",
  );
...
test("reconcilePackage: deterministic output formatting produces identical bytes without timestamp hacks", async () => {
  ...
  assert.equal(
    json1,
    json2,
    "Report serialization is 100% byte-for-byte deterministic across runs",
  );
});
...
test("reconcilePackage: glob pattern in package.json files is reported as unsupported", async () => {
  ...
  assert.equal(report.summary.missing_files_entries_count, 1);
  const globEntry = report.discrepancies.missing_files_entries.find(
    (e) => e.filesEntry === "src/**/*.js",
  );
