 ...
 */
export async function loadGpuGltfAnimationScene(
  device,
  source,
  {
    assets = {},
    ...
    exporting = false,
    sourceExport = false,
    signal = assets.signal ?? textures.signal ?? output?.signal,
  } = {},
) {
  if (decode.resolveTexture != null)
    throw new GltfAssetError(
      "GLTF_MODEL_LOAD_OPTIONS",
      "The owning loader supplies resolveTexture",
    );
  for (const nested of [assets.signal, textures.signal, output?.signal])
    if (nested !== undefined && nested !== signal)
      throw new GltfAssetError("GLTF_MODEL_LOAD_OPTIONS", "Use one construction AbortSignal");
  if (
    sourceExport !== false &&
    sourceExport !== true &&
    (!sourceExport || typeof sourceExport !== "object" || Array.isArray(sourceExport))
  )
    throw new GltfAssetError(
      "GLTF_MODEL_LOAD_OPTIONS",
      "sourceExport must be false, true or a limits object",
    );
  const sourceOptions =
    sourceExport === false ? null : sourceExport === true ? {} : { ...sourceExport };
  if (sourceOptions) {
    for (const key of Object.keys(sourceOptions))
      if (!["maxBytes", "maxJsonBytes", "maxResources"].includes(key))
        throw new GltfAssetError(
          "GLTF_MODEL_LOAD_OPTIONS",
          `Unsupported sourceExport option: ${key}`,
        );
    for (const [key, min, max] of [
      ["maxBytes", 20, 0xffffffff],
      ["maxJsonBytes", 1, sourceOptions.maxBytes ?? 128 * 1024 * 1024],
      ["maxResources", 1, 65536],
    ])
      if (
        sourceOptions[key] !== undefined &&
        (!Number.isSafeInteger(sourceOptions[key]) ||
          sourceOptions[key] < min ||
          sourceOptions[key] > max)
      )
  ...
  if (output !== null && (!output || typeof output !== "object" || Array.isArray(output)))
    throw new GltfAssetError(
      "GLTF_MODEL_LOAD_OPTIONS",
      "output must be a presentation options object or null",
    );
  const outputOptions = output === null ? null : { ...output, signal };
  if (outputOptions) {
    if (
      outputOptions.readback &&
      typeof outputOptions.readback === "object" &&
      !Array.isArray(outputOptions.readback)
    )
      outputOptions.readback = { ...outputOptions.readback };
    for (const key of ["sampleCount", "depthFormat"]) {
      if (
        outputOptions[key] !== undefined &&
        sceneOptions.renderer[key] !== undefined &&
        outputOptions[key] !== sceneOptions.renderer[key]
      )
        throw new GltfAssetError(
          "GLTF_MODEL_LOAD_OPTIONS",
          "Output and scene attachment settings conflict",
        );
      if (outputOptions[key] === undefined && sceneOptions.renderer[key] !== undefined)
        outputOptions[key] = sceneOptions.renderer[key];
    }
  }
  const basisu =
    decodeOptions.basisu === undefined ? textureOptions.ktx2Loader != null : decodeOptions.basisu;
  if (
    typeof basisu !== "boolean" ||
    (basisu && typeof textureOptions.ktx2Loader?.parse !== "function")
  )
    throw new GltfAssetError(
      "GLTF_MODEL_LOAD_OPTIONS",
      "BasisU selection requires a configured textures.ktx2Loader; decode.basisu must be boolean",
    );
  ...
  try {
    if (outputOptions) {
      ...
      const { createGpuAnimationPresentation } = await import("./animation_presentation.mjs");
      abort(signal);
      presentation = await createGpuAnimationPresentation(device, outputOptions);
      abort(signal);
      Object.assign(sceneOptions.renderer, presentation.rendererOptions);
    }
    asset = await loadGltfAsset(source, assetOptions);
    abort(signal);
    if (sourceOptions) {
      const { exportGltfAssetGLB } = await import("./gltf_asset_export.mjs");
      abort(signal);
      sourceSnapshot = await exportGltfAssetGLB(asset, { ...sourceOptions, signal });
      abort(signal);
      sourceWriter = exportGltfAssetGLB;
    }
    const prepared = prepareGltfAnimationModel(asset.json, asset.buffers, decodeOptions);
    resources = await createGltfTextureResources(
      device,
      prepared.textureRequests,
      asset.readImage,
      textureOptions,
    );
    abort(signal);
    if (exporting !== false) {
      ...
      for (const request of prepared.textureRequests) {
        if (!images.has(request.imageIndex))
          images.set(request.imageIndex, await asset.readImage(request.imageIndex));
        const image = images.get(request.imageIndex),
          resource = resources.resolveTexture(request);
        const sampler = {
          ...request.sampler,
        ...
        exportSources.push({
          resource,
          encoded: { bytes: image.bytes, mimeType: image.mimeType, sampler },
        });
      }
    }
    model = await createGpuDecodedAnimationScene(
      device,
      prepared.resolveTextures(resources.resolveTexture),
      { ...sceneOptions, picking, exporting },
    );
    abort(signal);
  } catch (error) {
    try {
  ...
  function checkTextures() {
    if (resources.failed) {
      release();
      throw new GltfTextureError("GLTF_TEXTURE_DEVICE_LOST", "Model texture device was lost");
    }
    if (presentation?.failed) {
      release();
      throw new GltfAssetError("GLTF_MODEL_OUTPUT_FAILED", "Model presentation failed");
    }
  }
  function query(operation) {
    ...
    try {
      ...
      if (model.failed || resources.failed || presentation?.failed) release();
      throw error;
    }
  }
  ...
  const result = Object.freeze({
    ...
    get bufferBytes() {
      return model.bufferBytes;
    },
    get textureBytes() {
      return resources.textureBytes;
    },
    get disposed() {
      return model.disposed;
    },
    get failed() {
      return model.failed || resources.failed || Boolean(presentation?.failed);
    },
    get outputEnabled() {
      return presentation !== undefined;
    },
    get outputTextureBytes() {
      return presentation?.textureBytes ?? 0;
    },
    get outputBufferBytes() {
      return presentation?.bufferBytes ?? 0;
    },
    get readbackEnabled() {
      return presentation?.readbackEnabled ?? false;
    },
    get readbackPending() {
      return presentation?.readbackPending ?? 0;
    },
    get readbackBufferBytes() {
      return presentation?.readbackBufferBytes ?? 0;
    },
    get readbackReservedBytes() {
      return presentation?.readbackReservedBytes ?? 0;
    },
    async readPixels(settings = {}) {
      checkTextures();
      if (model.disposed)
        throw new GltfAssetError("GLTF_MODEL_DISPOSED", "Model has been disposed");
      if (!presentation)
        throw new GltfAssetError(
          "GLTF_MODEL_READBACK_DISABLED",
          "Enable output.readback at model construction",
        );
      try {
        const pixels = await presentation.readPixels(settings);
        checkTextures();
        return pixels;
      } catch (error) {
        if (model.failed || resources.failed || presentation.failed) release();
    ...
    get sourceExportEnabled() {
      return sourceOptions !== null;
    },
    get sourceExportBytes() {
      return sourceSnapshot?.byteLength ?? 0;
    },
    async exportSourceGLB(settings = {}) {
      return query(() => {
        if (model.disposed)
          throw new GltfAssetError("GLTF_EXPORT_DISPOSED", "Model has been disposed");
        if (!sourceSnapshot)
          throw new GltfAssetError(
            "GLTF_EXPORT_DISABLED",
            "Enable sourceExport at model construction",
          );
        if (
          !settings ||
          typeof settings !== "object" ||
          Array.isArray(settings) ||
          Object.keys(settings).some((key) => key !== "signal")
        )
          throw new GltfAssetError(
            "GLTF_EXPORT_OPTIONS",
            "Source export accepts only an optional AbortSignal",
          );
        abort(settings.signal);
        if (model.disposed || !sourceSnapshot)
          throw new GltfAssetError(
            "GLTF_EXPORT_DISPOSED",
            "Model was disposed during source export",
          );
        return sourceSnapshot.slice(0);
      });
    },
    async exportAnimationGLB(clips, settings = {}) {
      return query(() => {
        if (model.disposed)
          throw new GltfAssetError("GLTF_EXPORT_DISPOSED", "Model has been disposed");
        if (!sourceSnapshot)
        ...
        for (const key of Object.keys(settings)) {
          if (
            !["signal", "animationMode", "maxBytes", "maxJsonBytes", "maxResources"].includes(key)
          )
            throw new GltfAssetError(
              "GLTF_EXPORT_OPTIONS",
              `Unsupported animation export option: ${key}`,
            );
          const property = Object.getOwnPropertyDescriptor(settings, key);
          if (!property || !Object.hasOwn(property, "value"))
            throw new GltfAssetError(
              "GLTF_EXPORT_OPTIONS",
              "Animation export settings must be data properties",
            );
          options[key] = property.value;
        }
        abort(options.signal);
        if (!Array.isArray(clips))
          throw new GltfAssetError("GLTF_EXPORT_ANIMATION", "Supply an explicit clip array");
        if (
          options.animationMode !== undefined &&
          !["append", "replace"].includes(options.animationMode)
        )
          throw new GltfAssetError(
            "GLTF_EXPORT_OPTIONS",
            "animationMode must be append or replace",
          );
        const byteCeiling = sourceOptions.maxBytes ?? 128 * 1024 * 1024;
        const ceilings = {
          maxBytes: byteCeiling,
          maxJsonBytes: sourceOptions.maxJsonBytes ?? Math.min(byteCeiling, 16 * 1024 * 1024),
          maxResources: sourceOptions.maxResources ?? 4096,
        };
        for (const [key, ceiling] of Object.entries(ceilings)) {
          if (options[key] === undefined)
            options[key] = key === "maxJsonBytes" ? Math.min(ceiling, options.maxBytes) : ceiling;
          if (
            !Number.isSafeInteger(options[key]) ||
            options[key] < (key === "maxBytes" ? 20 : 1) ||
            options[key] > ceiling
          )
            throw new GltfAssetError(
              "GLTF_EXPORT_LIMIT",
              `Invalid or increased animation export limit: ${key}`,
            );
        }
        if (options.maxJsonBytes > options.maxBytes)
          throw new GltfAssetError("GLTF_EXPORT_LIMIT", "JSON limit exceeds GLB limit");
        if (model.disposed || !sourceSnapshot)
          throw new GltfAssetError(
            "GLTF_EXPORT_DISPOSED",
            "Model was disposed during animation export",
          );
    ...
    get exportingEnabled() {
      return model.exportingEnabled;
    },
    exportPoseGLB(settings = {}) {
      return query(() => {
        if (!settings || typeof settings !== "object" || Array.isArray(settings))
          throw new GltfAssetError("GLTF_EXPORT_OPTIONS", "Invalid export settings");
        ...
        const sources = exportSources.slice(),
          options = { ...settings };
        ...
        options.resolveTexture ??= (resource) => {
          const found = sources.find(
            (s) => s.resource.view === resource.view && s.resource.sampler === resource.sampler,
          );
          if (!found)
            throw new GltfAssetError("GLTF_EXPORT_TEXTURE", "Texture was not loaded by this model");
    ...
    get pickingEnabled() {
      return model.pickingEnabled;
    },
    get pickingStats() {
      return model.pickingStats;
    },
    raycast(ray, settings) {
      return query(() => model.raycast(ray, settings));
    },
    pick(ndc, cameraSettings, querySettings) {
      return query(() => model.pick(ndc, cameraSettings, querySettings));
    },
    update(dt, settings) {
      return invoke(() => model.update(dt, settings));
    },
    upload() {
      return invoke(() => model.upload());
    },
    render(frame) {
      return invoke(() => (presentation ? presentation.render(model, frame) : model.render(frame)));
    },
    renderCamera(frame, settings) {
      return invoke(() =>
        presentation
          ? presentation.renderCamera(model, frame, settings)
          : model.renderCamera(frame, settings),
      );
    },
    async whenIdle() {
      checkTextures();
      try {
        await Promise.all([model.whenIdle(), presentation?.whenIdle()]);
        checkTextures();
        return result;
      } catch (error) {
        if (model.failed || resources.failed || presentation?.failed) release();
