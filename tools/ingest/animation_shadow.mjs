...
const fail = (message, code = "ANIMATION_SHADOW_INPUT") => {
...
const fields = (value, allowed) => {
  ...
  for (const key of Object.keys(value))
    if (!allowed.includes(key)) fail(`Unsupported shadow field: ${key}`);
};
function matrix(value) {
  ...
  if (
    ...
    (!(value.buffer instanceof ArrayBuffer) || value.buffer.resizable)
  )
    fail("Matrix requires fixed unshared storage");
  const copy = Array.from(value);
  if (copy.some((v) => typeof v !== "number" || !Number.isFinite(Math.fround(v))))
    fail("Matrix exceeds finite f32");
  return copy;
}
...
export async function createGpuAnimationShadowMap(device, options = {}) {
  ...
  if (
    ...
    ![width, height].every((v) => Number.isSafeInteger(v) && v >= 1 && v <= maximum) ||
    !Number.isSafeInteger(maxBytes) ||
    maxBytes < 1 ||
    typeof label !== "string"
  )
    fail("Invalid shadow extent or byte budget");
  const textureBytes = width * height * 4;
  if (!Number.isSafeInteger(textureBytes) || textureBytes >= maxBytes)
    fail("Depth map exceeds byte budget", "ANIMATION_SHADOW_LIMIT");
  const renderer = await createGpuAnimationRenderer(device, {
    format: null,
    depthFormat: "depth32float",
    sampleCount: 1,
    maxBytes: maxBytes - textureBytes,
    maxDraws,
    maxMeshes,
    label,
  });
  ...
  function live() {
    ...
    if (renderer.failed) {
      terminal = new AnimationRenderError("ANIMATION_SHADOW_DEVICE", "Shadow rendering failed");
      release();
      throw terminal;
    }
  }
  function exclusive(operation) {
    live();
    if (busy) fail("Shadow operation cannot be reentered", "ANIMATION_SHADOW_REENTRANT");
    busy = true;
    try {
      return operation();
    } finally {
  ...
  function current(dependency) {
    ...
    if (gpu.disposed || gpu.failed || gpu.version !== version || gpu.poseVersion !== poseVersion)
      return false;
    if (world) {
  ...
  try {
    ...
    const errors = await Promise.race([
      pending,
      device.lost.then(() => {
        fail("Device lost during shadow initialization", "ANIMATION_SHADOW_DEVICE");
      }),
    ]);
    if (caught) throw caught;
    if (errors.some(Boolean))
      fail(errors.find(Boolean).message || "Shadow allocation failed", "ANIMATION_SHADOW_DEVICE");
    live();
  } catch (error) {
    disposed = true;
    release();
  ...
  const shadow = Object.freeze({
    ...
    get allocatedBytes() {
      return renderer.allocatedBytes + (texture ? textureBytes : 0);
    },
    get disposed() {
      return disposed;
    },
    get failed() {
      return terminal !== null || renderer.failed;
    },
    async addMesh(gpu, material = {}) {
      live();
      if (busy) fail("Cannot add a caster during a shadow operation", "ANIMATION_SHADOW_REENTRANT");
      ...
      const handle = Object.freeze({
        ...
        get disposed() {
          return mesh.disposed || disposed;
        },
        dispose() {
          if (busy)
            fail("Cannot dispose a caster during a shadow operation", "ANIMATION_SHADOW_REENTRANT");
          mesh.dispose();
        },
      });
      owned.set(handle, { mesh, gpu });
      return handle;
    },
    render(frame) {
      return exclusive(() => {
        ...
        snapshot = Object.freeze({
          view,
          sampler,
          viewProjection: Object.freeze(viewProjection),
          width,
          height,
    ...
    sample(borrowedDevice) {
      return exclusive(() => {
        ...
        if (!dependencies.every(current))
          fail("Caster pose changed; render the shadow map again", "ANIMATION_SHADOW_STALE");
        return snapshot;
      });
    },
    async whenIdle() {
