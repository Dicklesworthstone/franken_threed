...
test("lifecycle emits self-contained hashed runtime assets and executable native Wasm", async (t) => {
  ...
  assert.deepEqual(record.importedBindings[record.imports[0]], [
    "createNumericDispatch",
    "dispatchNumericCall",
  ]);
  const { module } = await materialize(run.bundle, "app.mjs");
  const actual = new Float64Array([1, 2]),
    expected = actual.slice();
  for (let i = 0; i < 300; i++) {
    module.tick(actual, 1 / 60);
    module.step(expected, 1 / 60);
...
test("nested and dynamically imported chunks share assets and resolve relative runtime imports", async () => {
  const run = lifecycle();
  run.chunk(CODE, "chunks/deep/update.mjs");
  ...
  const update = await module.load();
  ...
  (await moved.load()).tick(a, 2);
  assert.equal(a[0], 6);
});

test("final reports follow Rollup filename hash substitutions", () => {
  const run = lifecycle();
  const preliminary = "app-!~{001}~.mjs";
  const { record } = run.chunk(CODE, preliminary);
  delete run.bundle[preliminary];
  record.fileName = "app-finalhash.mjs";
  run.bundle[record.fileName] = record;
  const report = run.finish();
  assert.equal(report.units[0].fileName, record.fileName);
  assert.equal(report.units[0].inputSha256, sha256(CODE));
  assert.equal(
    report.units[0].coordinateSpace,
    "renderChunk-before-specialization-and-hash-substitution",
  );
});

test("refused and uncalled kernels emit no runtime and keep code untouched", () => {
  const run = lifecycle();
  const code = CODE.replace("a[i] += dt", "a[i] = Math.sin(a[i])");
  assert.equal(run.chunk(code).result, null);
  const report = run.finish();
  assert.equal(report.compiledKernels, 0);
  assert.deepEqual(report.runtimeAssets, []);
...
test("a new output generation resets kernel/asset/report state", () => {
  const run = lifecycle();
  run.chunk();
  assert.equal(run.finish().compiledKernels, 1);
  run.plugin.renderStart();
  for (const key of Object.keys(run.bundle)) delete run.bundle[key];
...
test("source map and non-ES contracts are retained rather than silently changed", () => {
  for (const output of [{ format: "cjs" }, { format: "es", sourcemap: true }]) {
    const run = lifecycle();
    assert.equal(run.chunk(CODE, "app.mjs", output).result, null);
    const report = run.finish();
    assert.equal(report.compiledKernels, 0);
    assert.deepEqual(report.runtimeAssets, []);
    assert.equal(
      report.units[0].refusal.code,
      output.format === "cjs" ? "NON_ES_OUTPUT" : "SOURCE_MAP_SPECIALIZATION_UNAVAILABLE",
    );
  }
});
...
test("runtime and report collisions fail before silently shipping incompatible assets", () => {
  let run = lifecycle();
  run.chunk();
  const fileName = Object.keys(run.bundle).find((name) => name.includes("numeric-kernel"));
  run.bundle[fileName].source = "wrong runtime";
  assert.throws(() => run.finish(), /runtime asset collision/);
  run = lifecycle();
  run.chunk();
  run.bundle["f3d-numeric-specialization.json"] = { type: "asset", source: "{}" };
...
test("runtime resolution is lazy and configuration mistakes are rejected", () => {
  ...
  assert.equal(
    specializeNumericModule("export const empty = true;", { runtimeModule }).changed,
    false,
  );
  assert.equal(calls, 0);
  assert.equal(specializeNumericModule(CODE, { runtimeModule }).changed, true);
