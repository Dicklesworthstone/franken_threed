...
const modelBoundary =
  encode(`export async function createGpuDecodedAnimationScene(device,decoded,options){
...
const presentationBoundary =
  encode(`export async function createGpuAnimationPresentation(device,options){
...
function loader(packer = new URL("./gltf_asset_export.mjs", import.meta.url).href) {
  ...
  for (const [name, url] of Object.entries({
    ...
    "animation_presentation.mjs": presentationBoundary,
...
function fixture() {
  ...
  const json = {
    asset: { version: "2.0", copyright: "keep" },
    scene: 0,
    ...
    buffers: [{ byteLength: positions.byteLength, uri: "mesh.bin" }],
    ...
    images: [{ uri: "visible.png" }, { uri: "unselected.png" }],
  ...
  s.asset = {
    ...
    buffers: [positions],
    get bytesLoaded() {
      return 100 + positions.byteLength + s.cache.size * png.length;
    },
    readImage: (index) => {
      s.reads.push(index);
      s.events.push(["image", index]);
      if (!s.cache.has(index))
        s.cache.set(
          index,
          Promise.resolve().then(() =>
            s.imageHook ? s.imageHook(index) : { bytes: png, mimeType: "image/png" },
          ),
        );
  ...
  s.device = {
    state: s,
    destroy() {
      assert.fail("borrowed device is never destroyed");
    },
  };
  return s;
}
...
test("opt-in source export closes all images before GPU allocation and exports independent offline buffers", async () => {
  ...
  assert.equal(m.sourceExportEnabled, true);
  assert.equal(m.exportingEnabled, false);
  assert.ok(m.sourceExportBytes > 0);
  assert.deepEqual(s.reads, [0, 1, 0]);
  assert.ok(
    s.events.findIndex((e) => Array.isArray(e) && e[0] === "image" && e[1] === 1) <
      s.events.indexOf("textures"),
  );
  const before = s.reads.length,
    a = await m.exportSourceGLB(),
    b = await m.exportSourceGLB();
  assert.notEqual(a, b);
  assert.deepEqual(a, b);
  assert.equal(a.byteLength, m.sourceExportBytes);
  assert.equal(s.reads.length, before);
  const json = parse(a);
  assert.deepEqual(json.scenes, s.asset.json.scenes);
  assert.deepEqual(json.nodes, s.asset.json.nodes);
  assert.ok(json.images.every((i) => i.uri === undefined));
  assert.equal(json.buffers.length, 1);
  new Uint8Array(a).fill(0);
  assert.deepEqual(await m.exportSourceGLB(), b);
  m.dispose();
...
test("default construction does not import the exporter or read inactive-scene images", async () => {
  const { loadGpuGltfAnimationScene: noExportLoad } = await loader(
    encode("throw Error('unexpected exporter import');"),
  );
  for (const option of [{}, { sourceExport: false }]) {
    const s = fixture(),
      m = await noExportLoad(s.device, s, option);
    assert.deepEqual(s.reads, [0]);
    assert.equal(m.sourceExportEnabled, false);
    assert.equal(m.sourceExportBytes, 0);
    await assert.rejects(m.exportSourceGLB(), { code: "GLTF_EXPORT_DISABLED" });
    m.dispose();
  }
});
...
test("authored export is independent of live updates and later source JSON/buffer mutations", async () => {
  ...
  m.update(1);
  s.asset.json.nodes[0].translation = [9, 9, 9];
  s.asset.buffers[0].fill(99);
  s.asset.readImage = () => assert.fail("export must stay offline");
  assert.deepEqual(await m.exportSourceGLB(), original);
  assert.equal(m.poseVersion, 1);
  m.dispose();
});

for (const sourceExport of [
  null,
  0,
  "yes",
  [],
  ...
  { maxBytes: Infinity },
  { maxJsonBytes: 0 },
  { maxBytes: 20, maxJsonBytes: 21 },
  { maxResources: -1 },
  { maxResources: 65537 },
])
  test(
    "invalid source-export settings fail before load or native work: " + String(sourceExport),
    async () => {
      const s = fixture();
      await assert.rejects(load(s.device, s, { sourceExport }), optionsError);
...
test("source-export configuration is snapshotted before asset loading yields", async () => {
  const s = fixture();
  let finish;
  s.waitForLoad = new Promise((r) => {
    finish = r;
  });
  const sourceExport = { maxBytes: 8192, maxResources: 8 };
  const pending = load(s.device, s, { sourceExport });
  sourceExport.maxBytes = 1;
  sourceExport.maxResources = 0;
  finish();
  const m = await pending;
  assert.ok(m.sourceExportBytes > 20);
  m.dispose();
});

test("source byte limit is enforced before model decoding or texture allocation", async () => {
  const s = fixture();
  await assert.rejects(load(s.device, s, { sourceExport: { maxBytes: 128 } }), {
    code: "GLTF_EXPORT_LIMIT",
  });
  assert.deepEqual(s.events, ["load"]);
...
test("a missing inactive-scene image aborts source-export construction rather than producing a partial asset", async () => {
  const s = fixture();
  s.imageHook = (index) => {
    if (index === 1) throw Error("inactive image missing");
    return { bytes: png, mimeType: "image/png" };
  };
  await assert.rejects(load(s.device, s, { sourceExport: true }), /inactive image missing/);
  assert.equal(s.events.includes("prepare"), false);
  assert.equal(s.events.includes("textures"), false);
  assert.equal(s.events.includes("model"), false);
});
...
test("the same missing inactive image is not requested without source export", async () => {
  ...
  const m = await load(s.device, s);
  assert.equal(s.cache.size, 1);
  m.dispose();
});
...
test("cancelling source closure cleans up optional presentation and observes a late image rejection", async () => {
  ...
  const pending = load(s.device, s, { sourceExport: true, output: {}, signal: controller.signal });
  await entered;
  controller.abort();
  await assert.rejects(pending, { name: "AbortError" });
  assert.equal(s.presentationDisposals, 1);
  assert.equal(s.resources, undefined);
  reject(Error("late image"));
  await new Promise((r) => setImmediate(r));
});

test("a late GPU-scene construction failure disposes already-created texture resources and presentation", async () => {
  const s = fixture();
  s.modelError = Error("GPU construction failed");
  await assert.rejects(
    load(s.device, s, { sourceExport: true, output: {} }),
    /GPU construction failed/,
  );
  assert.equal(s.textureDisposals, 1);
  assert.equal(s.presentationDisposals, 1);
});
...
test("late cancellation after model creation cleans up the complete owned pipeline", async () => {
  const s = fixture(),
    controller = new AbortController();
  s.afterModel = () => controller.abort();
  await assert.rejects(
    load(s.device, s, { sourceExport: true, output: {}, signal: controller.signal }),
    { name: "AbortError" },
  );
  assert.equal(s.modelDisposals, 1);
  assert.equal(s.textureDisposals, 1);
...
test("source and posed exports remain independently enabled and can coexist", async () => {
  for (const [sourceExport, exporting] of [
    [true, true],
    [false, true],
    [true, false],
  ]) {
    const s = fixture(),
      m = await load(s.device, s, { sourceExport, exporting });
    assert.equal(m.sourceExportEnabled, sourceExport);
    assert.equal(m.exportingEnabled, exporting);
    ...
    if (exporting) {
      assert.equal((await m.exportPoseGLB()).byteLength, 16);
      assert.equal(s.poseExportOptions.sceneView, m.view);
      assert.deepEqual(s.poseImage.bytes, png);
      assert.equal(s.poseImage.sampler.wrapS, 33071);
    } else await assert.rejects(m.exportPoseGLB(), { code: "ANIMATION_EXPORT_DISABLED" });
    assert.equal(Object.hasOwn(s.sceneOptions, "sourceExport"), false);
    m.dispose();
  }
});
...
test("export cancellation is local and cannot poison an initialized model or the saved snapshot", async () => {
  ...
  assert.ok(await m.exportSourceGLB());
...
test("invalid per-export settings do not change the saved source or model state", async () => {
  const s = fixture(),
    m = await load(s.device, s, { sourceExport: true });
  for (const settings of [null, [], { maxBytes: 10 }, { resolveTexture() {} }])
    await assert.rejects(m.exportSourceGLB(settings), { code: "GLTF_EXPORT_OPTIONS" });
  assert.equal(m.disposed, false);
  assert.ok(await m.exportSourceGLB());
  m.dispose();
});
...
test("returned export copy survives immediate model disposal while the owned snapshot is released", async () => {
  const s = fixture(),
    m = await load(s.device, s, { sourceExport: true });
  const pending = m.exportSourceGLB(),
    size = m.sourceExportBytes;
  m.dispose();
  const bytes = await pending;
  assert.equal(bytes.byteLength, size);
  assert.equal(m.sourceExportBytes, 0);
  assert.ok(parse(bytes).scenes);
});
...
test("terminal texture and presentation failures release the retained source with the owned pipeline", async () => {
  for (const target of ["resources", "presentation"]) {
    const s = fixture(),
      m = await load(s.device, s, { sourceExport: true, output: {} });
    s[target].failed = true;
    await assert.rejects(m.exportSourceGLB());
    assert.equal(m.sourceExportBytes, 0);
