...
for (const Float of [Float32Array, Float64Array])
  for (const Index of [Uint16Array, Uint32Array]) {
    test(`relocatable indexed normals package: ${Float.name}/${Index.name}`, async () => {
      ...
      const types = [
        Float === Float32Array ? "f32[]" : "f64[]",
        Index === Uint16Array ? "u16[]" : "u32[]",
        Float === Float32Array ? "f32[]" : "f64[]",
      ];
...
test("indexed entry packages close immutable scalar helpers", async () => {
  ...
  const entry = sourceFile(
    root,
    `function offset(i){return i*2;}
export function update(a,index){for(let i=0;i<index.length;i++)a[offset(index[i])]+=3;}`,
  );
  const result = buildNumericKernel(entry, output, {
    parameterTypes: ["f32[]", "u16[]"],
    functionName: "update",
  });
  assert.deepEqual(
    result.selectedFunction.scalarHelpers.map((helper) => helper.name),
    ["offset"],
  );
  const kernel = (await load(output)).createKernel(),
...
test("no-Wasm package retains execution and deeply freezes checked-index descriptors", async () => {
  ...
  for (const value of [
    kernel.manifest,
    kernel.manifest.lengthParameters,
    kernel.manifest.loops,
    ...kernel.manifest.loops,
  ]) {
    assert.ok(Object.isFrozen(value));
  }
  assert.throws(() => {
    kernel.manifest.loops[0].boundParameter = 99;
  }, TypeError);
  kernel.dispose();
...
test("unsafe source and explicitly disabled checked indexing fail before creating output", () => {
  const cases = [
    ...
    [
      "function f(a,b){for(let i=0;i<b.length;i++)a[b[i]]=external();}",
      { parameterTypes: ["f64[]", "u16[]"] },
    ],
    [
      "function f(a,b){for(let i=0;i<b.length;i++)a[b[i]]=1;}",
      { parameterTypes: ["f64[]", "f64[]"], checkedIndexing: false },
    ],
    [
      "function f(a){for(let i=0;i<a.length;i++)a[i]=1;}",
      { parameterTypes: ["f64[]"], checkedIndexing: "yes" },
    ],
  ];
...
test("legacy prefix kernels keep deterministic module bytes and manifests", () => {
  ...
  for (const source of sources)
    for (const storage of ["f64[]", "f32[]"]) {
      const root = fresh(),
        output = path.join(root, "output");
      const options = {
        parameterTypes: [storage, storage],
        sourceName: "input.mjs",
        allowMath: true,
      };
      const original = compileNumericKernel(source, options),
        candidate = compileNumericCandidate(source, options);
      ...
      assert.deepEqual(
        new Uint8Array(fs.readFileSync(path.join(output, "kernel.wasm"))),
        original.wasm,
      );
    }
});
