...
export async function runAnimationWebGpuChecks(device) {
  ...
  const buffer = (options) => {
    const result = device.createBuffer(options);
    owned.add(result);
    return result;
  };
  ...
  async function read(target, Type = Float32Array) {
    try {
      ...
      return new Type(target.getMappedRange().slice(0));
    } finally {
      target.unmap();
      target.destroy();
      owned.delete(target);
    ...
    for (let i = 0; i < actual.length; i++) {
      check(
        Number.isFinite(actual[i]) &&
          Math.abs(actual[i] - reference[i]) <= 2e-5 * Math.max(1, Math.abs(reference[i])),
        `GPU/CPU deformation differs at ${i}: ${actual[i]} versus ${reference[i]}`,
      );
      comparisons++;
    }
  }
  try {
    const nodes = [
      { translation: [0.25, 0, 0], weights: [0, 0] },
      ...Array.from({ length: 32 }, (_, i) => ({
        translation: [i / 64, 0, 0],
        rotation: [0, 0, Math.sin(i / 64), Math.cos(i / 64)],
        scale: [1 + i / 64, 1, 1],
      })),
    ...
    const pose = createAnimationPlayer({
      ...
      clips: [
        ...
        {
          channels: [
            { node: 2, path: "rotation", times: [0, 2], values: [0, 0, 0, 1, 0, 0, 1, 0] },
          ],
        },
      ],
    });
    ...
    for (let v = 0; v < count; v++) {
      source.positions.push((v % 7) / 32, (v % 11) / 64, (v % 3) / 32);
      source.normals.push(1, 0, 0);
      source.tangents.push(0, 1, 0, v % 2 ? 1 : -1);
      source.morphTargets[0].positions.push(0.125, 0.03125, 0);
      source.morphTargets[0].normals.push(0, 0.125, 0);
      source.morphTargets[0].tangents.push(0, 0, 0.25);
      source.morphTargets[1].positions.push(0, -0.0625, 0.125);
      for (let k = 0; k < 32; k++) {
        source.joints.push((v + k) % 32);
        source.weights.push(1 / 32);
      }
    }
    for (const node of [0, 33]) {
      const cpu = createAnimationDeformer(pose, { ...source, node }),
        gpu = await createGpuAnimationDeformer(device, pose, { ...source, node });
      cpuMeshes.push(cpu);
      gpuMeshes.push(gpu);
    }
    for (const time of [0, 0.5, 1.5, 2]) {
      pose.sample(time);
      for (let i = 0; i < 2; i++) {
        const gpu = gpuMeshes[i],
          cpu = cpuMeshes[i];
        cpu.update();
        gpu.update();
        compare(await read(enqueueCopy(gpu.vertexBuffer, count * 40)), expected(cpu));
        await gpu.whenIdle();
        check(
          gpu.worldMatrix.every((value, j) => value === cpu.worldMatrix[j]),
          "Instance world transform differs",
        );
      }
    }
    pose.blend([
      { clip: 0, time: 0.75, weight: 0.5 },
    ...
    for (let i = 0; i < 2; i++) {
      ...
      gpuMeshes[i].update();
      compare(
        await read(enqueueCopy(gpuMeshes[i].vertexBuffer, count * 40)),
        expected(cpuMeshes[i]),
      );
    }
    ...
    cpuMeshes[0].update();
    gpuMeshes[0].update();
    const referenceA = expected(cpuMeshes[0]),
      a = enqueueCopy(gpuMeshes[0].vertexBuffer, count * 40);
    pose.sample(1.75);
    cpuMeshes[0].update();
    gpuMeshes[0].update();
    const referenceB = expected(cpuMeshes[0]),
      b = enqueueCopy(gpuMeshes[0].vertexBuffer, count * 40);
    compare(await read(a), referenceA);
    compare(await read(b), referenceB);
    check(
      referenceA.some((value, i) => value !== referenceB[i]),
      "Same-frame versions were not distinct",
    );
    ...
    const trianglePose = createAnimationPlayer({
      format: "f3d-animation-v1",
      nodes: [{ translation: [0.125, 0, 0], weights: [0] }, {}],
      skins: [{ joints: [1] }],
    ...
    const triangle = {
      node: 0,
      positions: [-0.25, -0.25, 0, 0.25, -0.25, 0, 0, 0.25, 0],
      morphTargets: [{ positions: [0, 0.125, 0, 0, 0.125, 0, 0, 0.125, 0] }],
      joints: Array(12).fill(0),
      weights: [1, 0, 0, 0, 1, 0, 0, 0, 1, 0, 0, 0],
    };
    const gpu = await createGpuAnimationDeformer(device, trianglePose, triangle),
      cpu = createAnimationDeformer(trianglePose, triangle);
    gpuMeshes.push(gpu);
    cpuMeshes.push(cpu);
    ...
    const pipeline = await device.createRenderPipelineAsync({
      layout: "auto",
      vertex: { module: shader, entryPoint: "vs", buffers: [gpu.vertexLayout] },
    ...
    const group = device.createBindGroup({
      ...
      entries: [{ binding: 0, resource: { buffer: world } }],
    });
    ...
    for (const time of [0.5, 1.5]) {
      trianglePose.sample(time);
      cpu.update();
      gpu.update();
      device.queue.writeBuffer(world, 0, new Float32Array(gpu.worldMatrix));
      const vertices = expected(cpu),
        referenceBuffer = buffer({
          size: vertices.byteLength,
          usage: GPUBufferUsage.VERTEX,
          mappedAtCreation: true,
        });
      new Float32Array(referenceBuffer.getMappedRange()).set(vertices);
      referenceBuffer.unmap();
      const rendered = render(gpu.vertexBuffer),
        reference = render(referenceBuffer);
      const actual = await read(rendered, Uint8Array),
        expectedPixels = await read(reference, Uint8Array);
      check(
        actual.every((value, i) => value === expectedPixels[i]),
        "Rendered GPU deformation differs from CPU reference",
      );
      check(
        actual.some((value, i) => i % 4 === 1 && value === 255),
        "Triangle rendered no colored pixels",
      );
      if (previous)
        check(
          actual.some((value, i) => value !== previous[i]),
          "Animated triangle did not move",
        );
      previous = actual;
    }
    await gpu.whenIdle();
    ...
    return {
      status: "passed",
      numericComponentsCompared: comparisons,
      verticesPerSkin: count,
      jointInfluences: 32,
      sharedSkinInstances: 2,
      sameFrameVersions: true,
      renderedFrames: 2,
      renderedPixelEquality: true,
      execution: "actual WebGPU; CPU reference is not a renderer substitute",
      speedupClaim: false,
    };
  } finally {
    device.removeEventListener("uncapturederror", recordError);
    for (const gpu of gpuMeshes) gpu.dispose();
    for (const cpu of cpuMeshes) cpu.dispose();
