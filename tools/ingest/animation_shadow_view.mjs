...
export class AnimationShadowViewError extends Error {
  constructor(message) {
    super(`ANIMATION_SHADOW_VIEW: ${message}`);
    this.name = "AnimationShadowViewError";
    this.code = "ANIMATION_SHADOW_VIEW";
  }
}
const fail = (message) => {
  throw new AnimationShadowViewError(message);
};
const EPS = 2 ** -23;
...
function vector(value, length, label) {
  ...
  if (ArrayBuffer.isView(value)) {
    if (!(value.buffer instanceof ArrayBuffer) || value.buffer.resizable)
      fail(`${label} needs fixed unshared storage`);
    try {
      new Uint8Array(value.buffer, 0, 0);
    } catch {
      fail(`${label} is detached`);
    }
  }
  const copy = Array.from(value);
  if (copy.some((v) => typeof v !== "number" || !Number.isFinite(v)))
...
function bounds(input) {
  const min = vector(input?.min, 3, "bounds minimum"),
    max = vector(input?.max, 3, "bounds maximum");
  if (min.some((v, i) => v > max[i])) fail("Bounds minimum exceeds maximum");
  return { min, max };
}
const corners = (box) =>
  Array.from({ length: 8 }, (_, n) =>
    [0, 1, 2].map((a) => (n & (1 << a) ? box.max[a] : box.min[a])),
  );
...
export function animationShadowWorldBounds(entries) {
  if (!Array.isArray(entries) || !entries.length || entries.length > 4096)
    fail("Expected 1..4096 bounds entries");
  const min = [Infinity, Infinity, Infinity],
    max = [-Infinity, -Infinity, -Infinity];
  for (const entry of entries) {
    const box = bounds(entry?.bounds),
      world = vector(entry?.worldMatrix, 16, "world matrix");
    ...
    for (let row = 0; row < 3; row++) {
      ...
      for (let axis = 0; axis < 3; axis++) {
        const a = world[axis * 4 + row] * box.min[axis],
          b = world[axis * 4 + row] * box.max[axis];
        lo += Math.min(a, b);
        hi += Math.max(a, b);
        magnitude += Math.max(Math.abs(a), Math.abs(b));
      }
      const slack = 64 * EPS * magnitude + 64 * 2 ** -126;
      min[row] = Math.min(min[row], lo - slack);
      max[row] = Math.max(max[row], hi + slack);
    }
  }
  if ([...min, ...max].some((v) => !Number.isFinite(Math.fround(v))))
    fail("World bounds exceed finite f32");
...
export function fitAnimationShadowView(light, inputBounds, options = {}) {
  if (!options || typeof options !== "object" || Array.isArray(options))
    fail("Expected fit options");
  for (const key of Object.keys(options))
    if (!["padding", "minNear"].includes(key)) fail(`Unsupported fit field: ${key}`);
  ...
  if (
    ...
    padding < 0 ||
    padding > 1 ||
    typeof minNear !== "number" ||
    !Number.isFinite(Math.fround(minNear)) ||
    !(Math.fround(minNear) > 0)
  )
    fail("Invalid padding or minNear");
  const type = light?.type;
  if (type !== "directional" && type !== "spot")
    fail("Projected shadows require a directional or spot light");
  const box = bounds(inputBounds),
    direction = unit(vector(light.direction ?? [0, 0, -1], 3, "light direction"));
  const right = unit(cross(direction, Math.abs(direction[1]) < 0.9 ? [0, 1, 0] : [1, 0, 0]));
  const up = cross(right, direction),
    back = direction.map((v) => -v);
  ...
  const magnitude = Math.max(
    ...box.min.map(Math.abs),
    ...box.max.map(Math.abs),
    ...eye.map(Math.abs),
  );
  ...
  if (type === "directional") {
    ...
    near = margin;
    far = hi[2] - lo[2] + 3 * margin;
    const halfX = Math.max(
      floor,
      Math.max(Math.abs(lo[0]), Math.abs(hi[0])) * (1 + 2 * padding) + floor,
    );
    const halfY = Math.max(
      floor,
      Math.max(Math.abs(lo[1]), Math.abs(hi[1])) * (1 + 2 * padding) + floor,
    );
    projection = [
      1 / halfX,
      0,
    ...
    const angle = light.outerConeAngle ?? Math.PI / 4,
      range = light.range;
    if (typeof angle !== "number" || !Number.isFinite(angle) || angle <= 0 || angle >= Math.PI / 2)
      fail("Spot half-angle must be strictly between 0 and PI/2");
    if (
      range !== undefined &&
      (typeof range !== "number" || !Number.isFinite(Math.fround(range)) || range <= minNear)
    )
      fail("Spot range must exceed minNear");
    near = Math.max(minNear, lo[2] - margin);
    far = Math.max(hi[2] + margin, near * 2);
    if (range !== undefined) {
      far = Math.min(far, range);
      near = Math.min(near, Math.max(minNear, far / 2));
    }
  ...
  for (let column = 0; column < 4; column++)
    for (let row = 0; row < 4; row++) {
      viewProjection[column * 4 + row] = [0, 1, 2, 3].reduce(
        (sum, k) => sum + projection[k * 4 + row] * view[column * 4 + k],
        0,
      );
    }
  // Reject singular f32 projections too, rather than clearing a useless map.
  if (
    Math.fround(projection[0]) === 0 ||
    Math.fround(projection[5]) === 0 ||
    Math.fround(projection[10]) === 0
  )
    fail("Shadow projection underflows f32");
  return Object.freeze({
    ...
    projectionMatrix: frozenMatrix(projection),
    viewProjection: frozenMatrix(viewProjection),
