...
test("Absolute module URLs normalize identity while retaining queries and fragments", () => {
  ...
  assert.throws(
    () => resolveModuleSpecifier(missing.replace("file:", "FILE:"), referrer),
...
test("File URL conversion preserves host validation and ignores only query and fragment", () => {
  ...
  if (process.platform === "win32") {
    ...
    assert.throws(
      () => resolveModuleSpecifier(remote.href, import.meta.url),
      IngestionResolutionError,
    );
  }
});
...
test("Import-map scope matching uses normalized priority and exact or directory boundaries", () => {
  ...
  const resolve = (referrer, scopes) =>
    resolveModuleSpecifier("lib", referrer, { imports, scopes }, options);

  assert.equal(
    resolve("https://example.com/app/a", {
      "./a": exact,
      "https://example.com/app/": broad,
    }),
    exact.lib,
  );
  ...
  assert.equal(
    resolve("https://example.com/app/a/module.js", {
      "./a/": {},
      "https://example.com/app/": broad,
    }),
    broad.lib,
...
test("Positive: Ingest real H1 (webgpu_performance_renderbundle.html) with 0 unresolved static imports", async () => {
  ...
  assert.ok(
    graph.summary.total_modules >= 20,
    `Expected at least 20 modules, got ${graph.summary.total_modules}`,
  );
  assert.ok(
    graph.summary.total_static_imports >= 50,
    `Expected at least 50 static imports, got ${graph.summary.total_static_imports}`,
  );
  ...
  for (const [modId, mod] of Object.entries(graph.modules)) {
    for (const imp of mod.static_imports) {
      ...
      assert.ok(
        imp.resolved_id.startsWith("file://"),
        `Resolved ID must be a file URL: ${imp.resolved_id}`,
      );
  ...
  assert.ok(
    webgpuImport.resolved_id.endsWith("build/three.webgpu.js"),
    "three/webgpu must resolve to build/three.webgpu.js",
  );

  // Verify cycles detected and preserved
  assert.ok(
    graph.cycles.length > 0,
    `Expected cycles to be detected, found ${graph.cycles.length}`,
  );

  // Verify Settings.js nonliteral dynamic import classification
  const settingsModEntry = Object.entries(graph.modules).find(([url]) =>
    url.endsWith("Settings.js"),
  );
...
test("Positive: Ingest real H2 (webgl_marchingcubes.html) with 0 unresolved static imports", async () => {
  ...
  assert.ok(
    threeImport.resolved_id.endsWith("build/three.module.js"),
    "three must resolve to build/three.module.js",
  );
...
`,
  ...
  assert.equal(
    errorCaught.span.start.line,
    3,
    "Error must identify line 3 where the import occurred",
  );
...
`,
  ...
  assert.ok(
    modA.mutable_exported_bindings.includes("counter"),
    "counter must be tracked as mutable exported binding",
  );
...
`;
  ...
  assert.equal(
    code.slice(span1.start.offset, span1.end.offset),
    "new URL('./textures/wood.png', import.meta.url)",
  );
  ...
  assert.equal(
    code.slice(span2.start.offset, span2.end.offset),
    "new URL('./models/chair.glb', import.meta['url'])",
  );
  ...
  assert.equal(
    code.slice(span3.start.offset, span3.end.offset),
    'new URL(`./data/scene.json`, import.meta["url"])',
  );
});
...
test("Asset references: function body var declarations do not scope default parameter initializers (Root Error 2)", () => {
  ...
  const positiveCode =
    "function f(x = new URL('./real.png', import.meta.url)) { var URL; return x; }";
  ...
  const span =
    posAnalysis.assetReferences[0].source_span || posAnalysis.assetReferences[0].sourceSpan;
  assert.equal(
    positiveCode.slice(span.start.offset, span.end.offset),
    "new URL('./real.png', import.meta.url)",
  );
...
`,
  ...
  assert.equal(errorCaught.name, "IngestionResolutionError");
  assert.ok(
    errorCaught.message.includes("mapped to null") || errorCaught.message.includes("blocked"),
  );
});
...
`,
  ...
  const graph = await buildModuleGraph(rootFile);
  assert.ok(
    graph.modules[targetFileUrl],
    "Explicit absolute file:// URL must be resolved directly",
  );
  assert.equal(graph.summary.total_modules, 2);
});
...
`,
  ...
  assert.ok(
    bundleResult.code.includes("hello from dependency"),
    "Bundle must contain bundled dependency code",
  );
  assert.ok(
    bundleResult.modules.length >= 2,
    `Bundle must bundle at least 2 modules, got ${bundleResult.modules.length}`,
  );
});
...
`,
  ...
  assert.equal(
    libImp.resolved_id,
    pathToFileURL(libScoped).href,
    "Scoped import must resolve to lib_scoped.js",
  );
});
...
`,
  ...
  assert.ok(
    threeImp && threeImp.resolved_id.endsWith("build/three.module.js"),
    "three must resolve via package fallback",
  );

  const addonsImp = rootNode.static_imports.find((i) => i.specifier.includes("OrbitControls"));
  assert.ok(
    addonsImp && addonsImp.resolved_id.endsWith("examples/jsm/controls/OrbitControls.js"),
    "three/addons/* must resolve via package fallback",
  );
});
...
test("Routing facts: Ingestion extracts renderer construction sites and WebGL escapes", async () => {
  ...
  assert.ok(
    h1Root.renderer_construction_sites.length >= 1,
    "H1 must have at least 1 renderer construction site",
  );
  const h1Site = h1Root.renderer_construction_sites[0];
  assert.equal(h1Site.constructor_name, "WebGPURenderer");
  assert.equal(
    h1Site.has_force_webgl,
    "unresolved",
    "H1 must detect forceWebGL: !api.webgpu as unresolved",
  );
  ...
  assert.ok(
    h2Root.renderer_construction_sites.length >= 1,
    "H2 must have at least 1 renderer construction site",
  );
...
`,
  ...
  assert.equal(
    modA.sharedToken,
    modB.sharedToken,
    "Shared dependency singleton symbol must be identical",
  );
...
`,
  ...
  assert.equal(
    dyn.classification,
    "literal",
    "Template literal without expressions must be classified as literal",
  );
  ...
  assert.ok(
    graph.modules[targetUrl],
    "Target module referenced via literal template must be in graph",
  );
  assert.equal(graph.summary.unresolved_dynamic_imports, 0);
});
...
`,
  ...
  assert.equal(
    dyn.finite_set,
    undefined,
    "Must not claim finite set closure for identifier argument",
  );
...
`,
  ...
  assert.equal(
    dyn.unresolved,
    true,
    "Partial candidate resolution failure must mark dynamic import unresolved",
  );
  assert.equal(
    dyn.claims_closure,
    false,
    "Partial candidate resolution failure must not claim closure",
  );
  ...
  assert.equal(
    graph.summary.unresolved_dynamic_imports,
    1,
    "Unresolved candidate must contribute to summary counter",
  );
...
test("Dynamic import: unsafe unknown expressions and non-string coercions remain unresolved", () => {
  ...
  for (let i = 0; i < analysis.dynamicImports.length; i++) {
    ...
    assert.equal(
      dyn.classification,
      "nonliteral",
      `Expression ${i} must be classified as nonliteral`,
    );
    assert.equal(dyn.specifier, null);
    assert.equal(dyn.unresolved, true);
...
test("Dynamic import: Cartesian expansion exceeding 128 candidates stays nonliteral", () => {
  ...
  assert.equal(
    dyn.classification,
    "nonliteral",
    "Cartesian expansion exceeding limit must stay nonliteral",
  );
  assert.equal(dyn.unresolved, true);
  assert.equal(dyn.finite_set, undefined);
});
...
`,
  ...
  assert.ok(
    threeImp.resolved_id.includes("upstream/three.js/"),
    "three must resolve inside upstream/three.js/",
  );
  ...
  assert.ok(
    webgpuImp.resolved_id.includes("upstream/three.js/"),
    "three/webgpu must resolve inside upstream/three.js/",
  );
  ...
  assert.ok(
    tslImp.resolved_id.includes("upstream/three.js/"),
    "three/tsl must resolve inside upstream/three.js/",
  );
  ...
  const addonsImp = entryMod.static_imports.find(
    (i) => i.specifier === "three/addons/controls/OrbitControls.js",
  );
  assert.ok(addonsImp, "Must have static import for three/addons/controls/OrbitControls.js");
  assert.ok(
    addonsImp.resolved_id.includes("upstream/three.js/"),
    "three/addons/* must resolve inside upstream/three.js/",
  );
  ...
  assert.ok(
    srcImp.resolved_id.includes("upstream/three.js/"),
    "three/src/* must resolve inside upstream/three.js/",
  );
...
test("Package exports: Subpath patterns respect import and default conditions in package.json exports", async () => {
  ...
  fs.writeFileSync(
    path.join(pkgDir, "addons_esm/controls/Orbit.js"),
    "export const isOrbit = true;\n",
    "utf-8",
  );
  fs.writeFileSync(
    path.join(pkgDir, "custom_default/util.js"),
    "export const isUtil = true;\n",
    "utf-8",
  );
...
`,
  ...
  const addonsImp = entryMod.static_imports.find(
    (i) => i.specifier === "three/addons/controls/Orbit.js",
  );
  assert.ok(
    addonsImp && addonsImp.resolved_id.endsWith("custom_three/addons_esm/controls/Orbit.js"),
  );
...
`,
  ...
  await assert.rejects(
    ...
    (err) => {
      assert.ok(
        err instanceof IngestionResolutionError,
        `Expected IngestionResolutionError, got ${err?.constructor?.name}`,
      );
      assert.ok(
...
`,
  ...
  assert.ok(
    reDyn.error.includes('subpath "./src/math/Vector3.js" is not exported by package.json'),
  );
});
...
`;
  ...
  assert.ok(
    graph.external_modules.includes(dataStatic),
    "external_modules must include static data URL",
  );
  assert.ok(
    graph.external_modules.includes(dataDyn),
    "external_modules must include dynamic data URL",
  );
  assert.equal(
    graph.summary.total_external_modules,
    2,
    "total_external_modules must match count of distinct external URLs",
  );

  // Assert external modules are NOT queued as filesystem reads and have NO entries in modules map
  assert.equal(
    graph.modules[dataStatic],
    undefined,
    "External data: URL must never create a node in modules map",
  );
  assert.equal(
    graph.modules[dataDyn],
    undefined,
    "External data: URL must never create a node in modules map",
  );
  ...
  assert.equal(
    dyn.claims_closure,
    false,
    "Dynamic import with external candidate must not claim closure",
  );
  assert.equal(dyn.claimsClosure, false);
});
...
test("Positive: Dynamic closure status drops through transitive local-to-external edge while standalone closed local branch stays closed", async () => {
  ...
  fs.writeFileSync(path.join(scratch, "standalone_helper.js"), `export const helper = 'HELP';\n`);
  fs.writeFileSync(
...
`;
  ...
  const dynTransitive = mainNode.dynamic_imports.find(
    (d) => d.specifier === "./transitive_intermediate.js",
  );
  ...
  const dynStandalone = mainNode.dynamic_imports.find(
    (d) => d.specifier === "./standalone_closed.js",
  );
...
test("HTML graph resolves import-map addresses and scopes against the effective relative base", async () => {
  ...
  fs.writeFileSync(
    ...
    'import { answer } from "choice"; export { answer };\n',
  );
...
<body></body>
  ...
  assert.equal(
    ...
    "Empty baseHref resolves effective base to document URL",
  );
...
<body></body>
  ...
  assert.equal(
    parsed.baseUrl,
    "file:///app/dir/index.html",
    "Effective base must resolve to document URL",
  );
  assert.equal(parsed.moduleScripts.length, 1);
  assert.equal(
    parsed.moduleScripts[0].id,
...
<body></body>
  ...
  assert.equal(
    parsedWithSubsequent.baseHref,
    "/sub/",
    "Target-only base must not record, allowing subsequent base href to win",
  );
  assert.equal(
    parsedWithSubsequent.baseUrl,
    "file:///sub/",
    "Effective base must resolve to /sub/",
  );
...
<body></body>
  ...
  assert.equal(
    parsedTargetOnly.baseHref,
    null,
    "baseHref must remain null when no base element has an href attribute",
  );
  assert.equal(
    parsedTargetOnly.baseUrl,
    "file:///app/dir/index.html",
    "Effective base must default to document URL",
  );
...
test("Regression: resolveExportTargetValue default always matches and nested no-match continues parent", () => {
  ...
  assert.equal(resolveExportTargetValue({ default: "./fallback.js" }, ["import"]), "./fallback.js");
  assert.equal(
    resolveExportTargetValue({
      node: "./node.js",
      require: "./require.cjs",
...
test("Regression: resolveExportTargetValue null target blocks fallback and throws in resolvePackageExports", () => {
  assert.equal(resolveExportTargetValue({ import: null, default: "./default.js" }), null);
  assert.throws(
    () =>
      resolvePackageExports(
        ".",
...
test("Regression (6mv.1): WHATWG import-map URL key normalization and request matching", () => {
  ...
  assert.equal(
    resolveModuleSpecifier(
      "./dep.js",
      "https://example.com/app/nested/main.js",
      { imports },
      { mapBaseUrl },
    ),
  ...
  assert.equal(
    resolveModuleSpecifier(
      "https://example.com/app/modules/sub/util.js",
      "https://example.com/other/main.js",
      { imports },
      { mapBaseUrl },
    ),
    "https://example.com/app/vendor/modules/sub/util.js",
  ...
  assert.throws(
    () =>
      resolveModuleSpecifier(
        "https://example.com/app/blocked.js",
        "https://example.com/app/main.js",
        { imports },
        { mapBaseUrl },
      ),
...
test("Import-map prefix targets normalize before slash validation and suffix resolution", () => {
  ...
  for (const target of ["./vendor/.", "./vendor/sub/..", "./vendor/"]) {
    ...
    assert.equal(
      resolveModuleSpecifier("pkg/", referrer, map, options),
      "https://example.com/app/vendor/",
    );
    assert.equal(
      resolveModuleSpecifier("pkg/sub/../child.js?q=1#part", referrer, map, options),
...
test("Import-map prefix resolution blocks suffixes that escape the mapped URL", () => {
  ...
  for (const specifier of ["pkg/../escape.js", "pkg/%2e%2e/escape.js", "pkg//escape.js"]) {
    assert.throws(
      () => resolveModuleSpecifier(specifier, referrer, map, { span }),
      (err) => {
...
test("Import-map prefix targets with queries do not concatenate suffixes into the query", () => {
  ...
  assert.throws(
    () => resolveModuleSpecifier("pkg/child.js", referrer, map),
...
test("Import-map invalid prefix targets block exact matches as well as subpaths", () => {
  ...
  for (const specifier of ["pkg/", "pkg/child.js"]) {
    assert.throws(
      () => resolveModuleSpecifier(specifier, referrer, map),
...
test("Import-map directory keys match subpaths after URL normalization", () => {
  ...
  assert.equal(
    resolveModuleSpecifier("./dir/child.js", referrer, map),
