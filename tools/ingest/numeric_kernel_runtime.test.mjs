...
test("executes real Wasm repeatedly and publishes into original array identities", () => {
  ...
  const expectedPosition = position.slice(),
    expectedVelocity = velocity.slice();
...
test("reacquires memory views after growth and accepts changing batch sizes", () => {
  ...
  for (const length of [1, 8193, 0, 3, 20000, 16]) {
    const a = new Float64Array(length).fill(2),
      b = new Float64Array(length).fill(3);
    engine.run(a, b, 0.5);
...
test("all argument guards run before publication, including late invalid arguments", async (t) => {
  const cases = [
    ...
    {
      value: new Float64Array(new ArrayBuffer(24, { maxByteLength: 48 })),
...
test("memory budget refusal is transactional and a later smaller batch still runs", () => {
  ...
  const a = new Float64Array(5000).fill(2),
    b = new Float64Array(5000).fill(3);
  ...
  assert.throws(
    () => unalignedBudget.run(new Float64Array(4097), new Float64Array(4097), 1),
    /KERNEL_MEMORY_LIMIT/,
  );
...
test("ignores overridable array methods and publishes only declared writes", () => {
  const compiled = compileNumericKernel(
    "function fill(x, y) { for(let i=0;i<x.length;i++) x[i]=y[i]+1; }",
    { parameterTypes: ["f64[]", "f64[]"] },
  );
  const a = new Float64Array([1, 2]),
    b = new Float64Array([3, 4]);
  a.set = b.subarray = () => {
    throw new Error("User method must not be invoked");
  };
...
test("validates binary ABI and options; disposal releases the private memory reference", () => {
  assert.throws(
    () => instantiateNumericKernel(new Uint8Array([0, 97, 115, 109, 1, 0, 0, 0])),
    /KERNEL_ABI_MISMATCH/,
  );
...
test("runtime module imports with DOM, GPU and WebAssembly globals unavailable", async () => {
  ...
  Object.defineProperty(globalThis, "WebAssembly", {
    configurable: true,
    get() {
      throw new Error("eager Wasm access");
    },
  });
  try {
