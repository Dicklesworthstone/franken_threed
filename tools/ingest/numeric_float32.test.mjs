...
test("float32 storage uses Number arithmetic and rounds at every store, not every operator", () => {
  ...
  const a = new Float32Array([16777216]),
...
test("mixed f32/f64 ABI aligns odd-length views and preserves tails over repeated frames", () => {
  ...
  for (let i = 0; i < 400; i++) {
    run.run(a, v, 1 / 60);
    integrate(expected, v, 1 / 60);
    assert.deepEqual(a, expected);
  }
  assert.equal(v[3], 123);
  assert.equal(run.diagnostics.wasmCalls, 400);
...
test("f32 boundary values, subnormals, overflow and signed zeros match JavaScript", () => {
  function arithmetic(a, b, divisor) {
    for (let i = 0; i < a.length; i++) {
      const previous = a[i];
      a[i] = previous * b[i];
      b[i] = a[i] / divisor;
  ...
  for (const multiplier of [-2, -0, 0, 0.5, 1, 2, Infinity, NaN]) {
    const a = new Float32Array(samples),
      b = new Float32Array(samples.length).fill(multiplier),
      aa = a.slice(),
      bb = b.slice();
    run.run(a, b, 3);
    arithmetic(aa, bb, 3);
...
test("conditional float32 stores preserve skipped elements across changing input arrays", () => {
  ...
  for (let frame = 0; frame < 30; frame++) {
    const a = new Float32Array([frame, -frame, frame / 3]),
...
test("float32 typed-array identity, ownership and scalar coercion guards retain JS", () => {
  ...
  run.run(a, new Float32Array([2, 3]), dt);
  assert.deepEqual([...a], [5, 8]);
  assert.equal(conversions, 2);
  class Derived extends Float32Array {}
  const b = new Derived([1, 2]);
  run.run(b, new Float32Array([2, 3]), 2);
  assert.deepEqual([...b], [5, 8]);
  const shared = new Float32Array(new SharedArrayBuffer(8));
  run.run(shared, new Float32Array([2, 3]), 2);
  assert.deepEqual([...shared], [4, 6]);
  assert.equal(run.diagnostics.fallbackCalls, 3);
});

test("mixed-width overlapping prefixes use original sequential JavaScript semantics", () => {
  const run = kernel(integrate, ["f32[]", "f64[]", "f64"]);
  const buffer = new ArrayBuffer(32),
    expected = new ArrayBuffer(32);
  new Float64Array(buffer).set([1, 2, 3, 4]);
  new Uint8Array(expected).set(new Uint8Array(buffer));
  const a = new Float32Array(buffer, 8, 2),
    v = new Float64Array(buffer, 0, 2);
  run.run(a, v, 2);
  integrate(new Float32Array(expected, 8, 2), new Float64Array(expected, 0, 2), 2);
...
test("float32 growth recreates scratch views and enforces byte budgets", () => {
  ...
  for (const count of [0, 3, 9000, 5, 20000]) {
    const a = new Float32Array(count).fill(1),
      v = new Float32Array(count).fill(2);
    run.run(a, v, 3);
    assert.ok(a.every((value) => value === 7));
  }
  assert.equal(run.diagnostics.memoryBytes, 2 * 65536);
  assert.equal(run.diagnostics.wasmCalls, 4);
  assert.equal(run.diagnostics.fallbackCalls, 1);
...
test("AOT dispatch switches f32/f64 variants lazily and reuses both native instances", () => {
  const double = compileNumericKernel(integrate.toString(), {
    parameterTypes: ["f64[]", "f64[]", "f64"],
  });
  const single = compileNumericKernel(integrate.toString(), {
    parameterTypes: ["f32[]", "f32[]", "f64"],
  });
  const token = createNumericDispatch(integrate, double.wasm, [
    { parameterTypes: ["f32[]", "f32[]", "f64"], bytes: single.wasm },
  ]);
  assert.equal(numericDispatchDiagnostics(token).initialized, false);
  for (let frame = 0; frame < 60; frame++) {
    const ArrayType = frame % 2 ? Float32Array : Float64Array;
    const a = new ArrayType([1, 2]),
      v = new ArrayType([2, 3]);
  ...
  assert.deepEqual(
    diagnostics.variants.map((variant) => variant.kernel.wasmCalls),
    [30, 30],
  );
...
test("mismatched variant metadata cannot select a binary with the wrong storage ABI", () => {
  ...
  const token = createNumericDispatch(integrate, artifact.wasm, [
    { parameterTypes: ["f32[]", "f32[]", "f64"], bytes: artifact.wasm },
  ]);
