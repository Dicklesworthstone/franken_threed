...
export function packAnimationEnvironment(device, input, output, fail) {
  ...
  if (!input || typeof input !== "object" || Array.isArray(input))
    reject("Expected an environment descriptor");
  for (const key of Object.keys(input))
    if (!["map", "intensity", "rotation"].includes(key))
      reject(`Unsupported environment field: ${key}`);
  const { map, intensity = 1, rotation = IDENTITY } = input;
  if (typeof intensity !== "number" || !Number.isFinite(Math.fround(intensity)) || intensity < 0)
    reject("Intensity must be nonnegative finite f32");
  if ((!Array.isArray(rotation) && !ArrayBuffer.isView(rotation)) || rotation.length !== 9)
    reject("Expected a 3x3 world-to-environment rotation");
  if (ArrayBuffer.isView(rotation)) {
    if (!(rotation.buffer instanceof ArrayBuffer) || rotation.buffer.resizable)
      reject("Rotation requires fixed unshared storage");
    try {
      new Uint8Array(rotation.buffer, 0, 0);
    } catch {
      reject("Rotation is detached");
    }
  }
  const matrix = Array.from(rotation);
  if (matrix.some((v) => typeof v !== "number" || !Number.isFinite(Math.fround(v))))
    reject("Rotation must fit finite f32");
  for (let a = 0; a < 3; a++)
    for (let b = a; b < 3; b++) {
      const dot =
        matrix[a * 3] * matrix[b * 3] +
        matrix[a * 3 + 1] * matrix[b * 3 + 1] +
        matrix[a * 3 + 2] * matrix[b * 3 + 2];
  ...
  if (typeof map?.sample !== "function" || typeof map?.whenIdle !== "function")
    reject("Expected a prepared animation environment");
  const snapshot = map.sample(device);
  if (
    !snapshot ||
    !Object.isFrozen(snapshot) ||
    snapshot.profile !== "f3d-animation-environment-v1" ||
    snapshot.version !== 1 ||
    !["diffuseView", "specularView", "brdfView", "sampler"].every(
      (key) => snapshot[key] && typeof snapshot[key] === "object",
    ) ||
    !Number.isSafeInteger(snapshot.mipLevelCount) ||
    snapshot.mipLevelCount < 2 ||
    snapshot.mipLevelCount > 16
  )
    reject("Invalid environment snapshot");
  const check = () => {
    if (map.sample(device) !== snapshot) reject("Environment changed while preparing the receiver");
  };
  check();
  output.fill(0);
  for (let column = 0; column < 3; column++)
    output.set(matrix.slice(column * 3, column * 3 + 3), column * 4);
