...
export class AnimationEnvironmentError extends Error {
  constructor(code, message) {
    super(`${code}: ${message}`);
    this.name = "AnimationEnvironmentError";
    this.code = code;
  }
}
const fail = (code, message) => {
  throw new AnimationEnvironmentError("ANIMATION_ENVIRONMENT_" + code, message);
};
 ...
 */
export function planAnimationEnvironment({
  size = 128,
  diffuseSize = 32,
  lutSize = 128,
  samples = 1024,
  ...
  integer(maxDimension, 1, 32768, "dimension limit");
  integer(size, 2, maxDimension, "specular size");
  integer(diffuseSize, 1, maxDimension, "diffuse size");
  integer(lutSize, 2, maxDimension, "LUT size");
  integer(samples, 1, 4096, "sample count");
  integer(maxTextureBytes, 1, Number.MAX_SAFE_INTEGER, "texture budget");
  integer(maxSampleWork, 1, Number.MAX_SAFE_INTEGER, "sample work budget");
  integer(alignment, 16, 65536, "uniform alignment");
  if ((size & (size - 1)) !== 0 || (alignment & (alignment - 1)) !== 0)
    fail("LIMIT", "Specular size and alignment must be powers of two");
  const levels = 1 + Math.log2(size),
    passes = [];
  let textureBytes = 0,
    sampleWork = 0;
  for (let level = 0; level < levels; level++) {
    ...
    for (let face = 0; face < 6; face++)
      passes.push({
        ...
        samples: level === 0 ? 1 : samples,
      });
  }
  for (let face = 0; face < 6; face++)
    passes.push({ kind: 1, face, level: 0, width: diffuseSize, roughness: 1, samples });
  passes.push({ kind: 2, face: 0, level: 0, width: lutSize, roughness: 0, samples });
  for (const pass of passes) {
    textureBytes += pass.width ** 2 * 8;
    sampleWork += pass.width ** 2 * pass.samples;
    Object.freeze(pass);
  }
  if (textureBytes > maxTextureBytes || sampleWork > maxSampleWork)
    fail("LIMIT", "Environment texture storage or filtering work exceeds budget");
  return Object.freeze({
    ...
    textureBytes,
    sampleWork,
    uniformBytes: passes.length * Math.max(32, alignment),
    stride: Math.max(32, alignment),
...
export function animationEnvironmentShader(cube = false) {
  return ANIMATION_ENVIRONMENT_WGSL.replace(
    "// SOURCE_DECLARATION",
    `@group(0) @binding(2) var input_texture: texture_${cube ? "cube" : "2d"}<f32>;`,
  ).replace(
    "// SOURCE_SAMPLE",
    cube
      ? "return textureSampleLevel(input_texture,input_sampler,d,0.0).rgb;"
...
export async function createGpuAnimationEnvironment(device, source, options = {}) {
  if (!options || typeof options !== "object" || Array.isArray(options))
    fail("OPTIONS", "Expected environment options");
  for (const key of Object.keys(options))
    if (
      ...
      fail("OPTIONS", `Unknown environment option: ${key}`);
  ...
  for (const key of ["writeBuffer", "submit", "onSubmittedWorkDone"])
    if (typeof device.queue?.[key] !== "function") fail("DEVICE", `Missing GPU queue.${key}`);
  ...
  const plan = planAnimationEnvironment({
    ...requested,
  ...
  if (
    ...
    plan.uniformBytes > device.limits.maxBufferSize
  )
    fail("LIMIT", "Filter parameter buffer exceeds device limit");
  if (
    !source ||
    source.dimension !== "2d" ||
    ![1, 6].includes(source.depthOrArrayLayers) ||
    source.sampleCount !== 1 ||
    !formats.includes(source.format) ||
    !Number.isInteger(source.usage) ||
    !(source.usage & 4) ||
    typeof source.createView !== "function"
  )
    fail("SOURCE", "Lend a linear rgba16float/rgba8unorm panorama or cubemap");
  const width = integer(source.width, 1, device.limits.maxTextureDimension2D, "source width"),
    height = integer(source.height, 1, device.limits.maxTextureDimension2D, "source height");
  const cube = source.depthOrArrayLayers === 6;
  if (cube ? width !== height : width !== 2 * height)
    fail("SOURCE", "Expected square cube faces or a 2:1 equirectangular panorama");
  const owned = new Set();
  let parameters,
    disposed = false,
    terminal = null;
  ...
  if (device.lost && typeof device.lost.then === "function")
    device.lost.then(
      (info) =>
        stop(
          new AnimationEnvironmentError(
            "ANIMATION_ENVIRONMENT_LOST",
  ...
  async function checked(operation) {
    ...
    const work = Promise.all([value, v, m]).then(([result, invalid, oom]) => {
      if (error) throw error;
      if (invalid || oom) fail("GPU", (invalid || oom).message ?? "Environment GPU failure");
      live();
      return result;
    });
  ...
  try {
      ...
      layout = device.createBindGroupLayout({
        entries: [
          ...
          { binding: 1, visibility: 2, sampler: { type: "filtering" } },
          {
            binding: 2,
            visibility: 2,
            texture: { sampleType: "float", viewDimension: cube ? "cube" : "2d" },
          },
        ],
      });
      const module = device.createShaderModule({
        label: "Animation environment filter",
        code: animationEnvironmentShader(cube),
      });
    ...
    await checked(() => {
      const make = (size, layers, mipLevelCount) => {
      ...
      specular = make(plan.size, 6, plan.levels);
      diffuse = make(plan.diffuseSize, 6, 1);
      lut = make(plan.lutSize, 1, 1);
      parameters = device.createBuffer({
        label: "Animation IBL filter parameters",
        size: plan.uniformBytes,
        usage: 8 | 64,
      });
      const bytes = new ArrayBuffer(plan.uniformBytes),
        view = new DataView(bytes);
      for (const [i, p] of plan.passes.entries()) {
        const at = i * plan.stride;
        view.setUint32(at, p.face, true);
        view.setUint32(at + 4, p.kind, true);
        view.setUint32(at + 8, p.samples, true);
        view.setUint32(at + 12, p.width, true);
        view.setFloat32(at + 16, p.roughness, true);
      }
      ...
      const inputSampler = device.createSampler({
        minFilter: "linear",
        magFilter: "linear",
        addressModeU: cube ? "clamp-to-edge" : "repeat",
        addressModeV: "clamp-to-edge",
      });
      const inputView = source.createView({
        dimension: cube ? "cube" : "2d",
        baseMipLevel: 0,
        mipLevelCount: 1,
        baseArrayLayer: 0,
        arrayLayerCount: cube ? 6 : 1,
      });
      const group = device.createBindGroup({
        layout,
        entries: [
          { binding: 0, resource: { buffer: parameters, size: 32 } },
      ...
      for (const [i, p] of plan.passes.entries()) {
        const target = [specular, diffuse, lut][p.kind].createView({
          dimension: "2d",
          baseArrayLayer: p.face,
          arrayLayerCount: 1,
          baseMipLevel: p.level,
          mipLevelCount: 1,
        });
        ...
        pass.setBindGroup(0, group, [i * plan.stride]);
        pass.draw(3);
        pass.end();
      }
      device.queue.submit([encoder.finish()]);
    });
    await Promise.race([device.queue.onSubmittedWorkDone(), stopped]);
    live();
    parameters.destroy();
    parameters = undefined;
    const snapshot = await checked(() =>
      Object.freeze({
        ...
        version: 1,
        diffuseView: diffuse.createView({ dimension: "cube" }),
        specularView: specular.createView({ dimension: "cube" }),
        brdfView: lut.createView(),
        sampler,
    ...
    const result = Object.freeze({
      ...
      get textureBytes() {
        return owned.size ? plan.textureBytes : 0;
      },
      sample(borrowedDevice) {
        available();
        if (borrowedDevice !== device)
          fail("DEVICE", "Environment and receiver must use the same GPU device");
        return snapshot;
      },
      async whenIdle() {
        available();
