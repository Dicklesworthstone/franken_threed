...
const fail = (message) => {
  const error = new Error(`ANIMATION_SCENE_SHADOW: ${message}`);
  error.code = "ANIMATION_SCENE_SHADOW";
  throw error;
};
const positive = (v, name) => {
  if (!Number.isSafeInteger(v) || v < 1) fail(`Invalid ${name}`);
  return v;
};
function array(value, length, name) {
  ...
  if (ArrayBuffer.isView(value)) {
    if (!(value.buffer instanceof ArrayBuffer) || value.buffer.resizable)
      fail(`${name} needs fixed unshared storage`);
    try {
      new Uint8Array(value.buffer, 0, 0);
    } catch {
      fail(`${name} is detached`);
    }
  }
...
function snapshotLighting(input) {
  if (!input || typeof input !== "object" || Array.isArray(input))
    fail("Automatic shadows require frame lighting");
  const copy = { ...input };
  for (const key of ["cameraPosition", "viewDirection"])
    if (copy[key] !== undefined) copy[key] = array(copy[key], 3, key);
  if (!Array.isArray(copy.lights) || copy.lights.length > 8)
    fail("Expected up to eight frame lights");
...
export function prepareAnimationSceneShadows(pose, inputs, options) {
  ...
  for (const key of Object.keys(options))
    if (!allowed.includes(key)) fail(`Unsupported shadow option: ${key}`);
  const {
    lightIndex = 0,
    width = 1024,
    height = width,
    ...
    strength = 1,
    blend = "reject",
    label = "f3d-scene-shadow",
  } = options;
  if (!Number.isInteger(lightIndex) || lightIndex < 0 || lightIndex > 7)
    fail("lightIndex must be in 0..7");
  for (const [name, value] of Object.entries({
    ...
    positive(value, name);
  if (!Number.isSafeInteger(width * height * 4) || width * height * 4 >= maxBytes)
    fail("Shadow texture exceeds its GPU budget");
  if (typeof label !== "string" || !["reject", "skip"].includes(blend))
    fail("Invalid label or blend policy");
  if (
    [bias, normalBias, strength].some(
    ...
    fail("Invalid shadow bias or strength");
  // Exercise exactly the same fit-option admission before allocating a map.
  fitAnimationShadowView(
    { type: "directional" },
    { min: [0, 0, 0], max: [1, 1, 1] },
    { padding, minNear },
  );
  if (!Array.isArray(inputs) || !inputs.length || inputs.length > 4096)
    fail("Expected 1..4096 scene meshes");
  const selected =
    options.casters === undefined
      ? inputs.map(({ material }) => material.alphaMode !== "BLEND" || blend !== "skip")
  ...
  let materials = inputs.map(({ material }, i) => {
    ...
    if (material.alphaMode === "BLEND")
      fail('BLEND has no depth-only shadow policy; select blend:"skip" or exclude it in casters');
    const caster = {};
    for (const key of [
      "indices",
      "baseColor",
    ...
    if (material.mapCoordinates?.baseColorTexture)
      caster.mapCoordinates = { baseColorTexture: material.mapCoordinates.baseColorTexture };
    return caster;
  });
  ...
  function release() {
    map?.dispose();
    for (const summary of summaries) summary.dispose();
    handles = [];
    deformers = null;
  }
  function live() {
    if (disposed || pose.disposed) fail("Scene shadows or pose have been disposed");
  }
  try {
    for (const { geometry } of inputs) {
      const summary = createAnimationBounds(pose, geometry, {
        maxBytes: positive(maxBoundsBytes - summaryBytes, "remaining bounds bytes"),
        maxComponents: positive(maxBoundsComponents - components, "remaining bounds components"),
      });
      summaries.push(summary);
      summaryBytes += summary.byteLength;
      components += summary.sourceComponents;
    }
  ...
  const state = Object.freeze({
    get allocatedBytes() {
      return map?.allocatedBytes ?? 0;
    },
    get boundsBytes() {
      return disposed ? 0 : summaryBytes;
    },
    get failed() {
    ...
    async initialize(device, values) {
      live();
      if (initializing || ready) fail("Scene shadows have already been initialized");
      initializing = true;
      try {
        if (!Array.isArray(values) || values.length !== meshCount)
          fail("Mismatched scene deformers");
        deformers = values.slice();
        if (
          pose.version !== initialVersion ||
          deformers.some((g) => g.poseVersion !== initialVersion || g.disposed || g.failed)
        )
          fail("Pose changed before shadow binding");
        map = await createGpuAnimationShadowMap(device, {
          width,
          height,
          maxBytes,
          maxDraws: Math.max(1, materials.filter(Boolean).length),
          maxMeshes: Math.max(1, materials.filter(Boolean).length),
          label,
        });
        live();
        for (let i = 0; i < materials.length; i++)
          if (materials[i]) {
            handles[i] = await map.addMesh(deformers[i], materials[i]);
            live();
          }
        if (
          pose.version !== initialVersion ||
          ...
          fail("Pose changed during shadow binding");
        materials = [];
        ready = true;
        return state;
      } catch (error) {
    ...
    render(inputLighting, drawIndices = null) {
      live();
      if (!ready) fail("Initialize the scene shadow map before rendering");
      let indices = Array.from({ length: meshCount }, (_, i) => i);
      if (drawIndices !== null) {
        if (!Array.isArray(drawIndices) || !drawIndices.length || drawIndices.length > meshCount)
          fail("Invalid active draw selection");
        ...
        for (const index of drawIndices) {
          if (
            ...
            fail("Invalid or duplicate active draw index");
          selected.add(index);
        }
      ...
      const lighting = snapshotLighting(inputLighting),
        light = lighting.lights[lightIndex];
      if (!light) fail("Selected lightIndex is absent from frame lighting");
      const versions = deformers.map((g) => g.version);
      const world = animationShadowWorldBounds(
        indices.map((i) => ({
          bounds:
            summaries[i].snapshot.poseVersion === version
              ? summaries[i].snapshot
              : summaries[i].update(),
          worldMatrix: deformers[i].worldMatrix,
        })),
      );
      const view = fitAnimationShadowView(light, world, { padding, minNear });
      live();
      if (
        pose.version !== version ||
        deformers.some(
          (g, i) =>
            g.poseVersion !== version || g.version !== versions[i] || g.disposed || g.failed,
        )
      )
        fail("Upload a stable complete pose before automatic shadows");
      ...
      return {
        lighting,
        shadow: { map, lightIndex, bias, normalBias, strength },
        stats: Object.freeze({
          poseVersion: version,
          lightIndex,
          casterCount: activeHandles.length,
