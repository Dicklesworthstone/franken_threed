...
test("pyramid includes odd borders, stops at 1x1, accounts for both half-float textures", () => {
  ...
  assert.equal(p.textureBytes, 144);
  assert.equal(p.allocatedBytes, 160);
...
test("limits and settings reject malformed, overflowing and unsupported values", () => {
  for (const value of [0, -1, 1.5, NaN, Infinity, 32769, "4"])
    assert.throws(() => animationBloomPlan(value, 2), code("LIMIT"));
  for (const levels of [0, 7, 1.5])
    assert.throws(() => animationBloomPlan(8, 8, { levels }), code("LIMIT"));
  ...
  for (const input of [
    ...
    assert.throws(() => animationBloomSettings(input), code("VALUE"));
...
test("compiles explicit-layout HDR pipelines without filtering features or samplers", async () => {
  ...
  assert.ok(
    composite.entries
      .filter((e) => e.texture)
      .every((e) => e.texture.sampleType === "unfilterable-float"),
  );
...
test("executes extract/downsample and ordered horizontal/vertical/composite passes in one submission", async () => {
  ...
  const commands = f.calls.submissions[0];
  assert.equal(commands.length, 1);
  const passes = commands[0].passes;
  assert.deepEqual(
    passes.map((p) => p.pipeline.fragment.entryPoint),
    [
      "extract_main",
      "horizontal_main",
      "vertical_main",
  ...
  assert.equal(f.bloom.lastRender.passes, 10);
  assert.equal(f.bloom.allocatedBytes, 688);
  assert.deepEqual(
    f.calls.textures.map((t) => [t.width, t.height]),
    [
      [8, 4],
      [8, 4],
      [4, 2],
  ...
  assert.equal(await f.bloom.whenIdle(), f.bloom);
  f.bloom.dispose();
});

test("unchanged extents and source reuse textures/pipelines/bind groups", async () => {
  const f = await setup({ levels: 2 });
  f.bloom.render(f.frame);
  const groups = f.calls.groups.length;
  f.bloom.render({ ...f.frame, strength: 2 });
  assert.equal(f.calls.textures.length, 4);
  assert.equal(f.calls.groups.length, groups);
  assert.equal(f.calls.pipelines.length, 5);
  assert.equal(f.calls.submissions.length, 2);
  assert.equal(f.bloom.version, 2);
  ...
  assert.equal(f.calls.groups.length, groups + 2);
  assert.equal(f.calls.textures.length, 4);
  await f.bloom.whenIdle();
  f.bloom.dispose();
});

test("zero strength is one copy/composite pass with no pyramid allocation even at a 16-byte budget", async () => {
  const f = await setup({ strength: 0, maxBytes: 16 });
  f.bloom.render(f.frame);
  assert.equal(f.calls.textures.length, 0);
  assert.equal(f.bloom.allocatedBytes, 16);
  assert.equal(f.bloom.lastRender.passes, 1);
  assert.equal(f.bloom.lastRender.levels, 0);
...
test("single-pixel inputs bind unused levels safely and publish actual level count", async () => {
  const f = await setup({ levels: 6 });
  f.bloom.render({ source: f.texture(1, 1), target: f.texture(1, 1) });
  assert.equal(f.bloom.lastRender.levels, 1);
  assert.equal(f.bloom.lastRender.passes, 4);
...
test("recoverable input failures issue no native writes and preserve the last successful snapshot", async () => {
  const f = await setup();
  f.bloom.render(f.frame);
  const before = f.bloom.lastRender,
    writes = f.calls.writes.length;
  const badFrames = [
    { ...f.frame, strength: -1 },
    { ...f.frame, unknown: true },
    { ...f.frame, target: f.frame.source },
    { ...f.frame, target: f.texture(8, 8) },
    { ...f.frame, source: f.texture(16, 8, "rgba8unorm-srgb") },
  ...
  assert.equal(f.bloom.lastRender, before);
  assert.equal(f.bloom.failed, false);
  f.bloom.render(f.frame);
  await f.bloom.whenIdle();
  f.bloom.dispose();
});
...
test("resize checks old/new peak payload before allocation and leaves existing render usable", async () => {
  ...
  const small = { source: f.texture(8, 8), target: f.texture(8, 8) };
  f.bloom.render(small);
...
test("successful resize releases old storage, replaces dimension-dependent bindings, and stays bounded", async () => {
  ...
  const old = f.calls.textures.slice();
  f.bloom.render(f.frame);
  assert.ok(old.every((t) => t.destroyed === 1));
  assert.equal(f.calls.textures.length, 8);
  assert.equal(f.bloom.allocatedBytes, 656);
  await f.bloom.whenIdle();
  f.bloom.dispose();
});
...
test("partial allocation failure releases candidate and existing storage and makes native failure terminal", async () => {
  ...
  assert.throws(
    () => f.bloom.render({ source: f.texture(32, 16), target: f.texture(32, 16) }),
    /allocation failure/,
  );
  assert.equal(f.bloom.failed, true);
  assert.equal(f.bloom.allocatedBytes, 0);
...
test("scoped asynchronous validation failure is observed by whenIdle and frees owned resources", async () => {
  const f = await setup();
  f.controls.scopeError = { message: "injected validation failure" };
  f.bloom.render(f.frame);
  ...
  assert.ok(f.calls.textures.every((t) => t.destroyed === 1));
  assert.equal(f.calls.scopes.length, 0);
  assert.equal(f.frame.source.destroyed, 0);
...
test("abort before and during initialization does not publish an unusable bloom object", async () => {
  ...
  const b = gpuPostprocessFixture(),
    gate = deferred(),
    late = new AbortController();
  b.controls.compile = gate;
  const pending = createGpuAnimationBloom(b.device, { signal: late.signal });
  late.abort(new Error("cancel compile"));
  await assert.rejects(pending, /cancel compile/);
  gate.resolve();
  await Promise.resolve();
  assert.equal(b.calls.buffers.length, 0);
  assert.equal(b.calls.textures.length, 0);
});
...
test("device loss rejects a pending completion wait and prevents further submissions", async () => {
  ...
  f.controls.completion = deferred();
  const idle = f.bloom.whenIdle();
  f.lost.resolve({ message: "device unavailable" });
  await assert.rejects(idle, code("DEVICE_LOST"));
  assert.equal(f.bloom.failed, true);
  assert.throws(() => f.bloom.render(f.frame), code("DEVICE_LOST"));
...
test("source getters cannot dispose or recursively render during frame preparation", async () => {
  ...
  assert.equal(f.bloom.disposed, false);
  f.bloom.render(f.frame);
  await f.bloom.whenIdle();
  f.bloom.dispose();
});
...
test("device limits are checked before shader construction", async () => {
  const f = gpuPostprocessFixture();
  f.device.limits.maxSampledTexturesPerShaderStage = 2;
