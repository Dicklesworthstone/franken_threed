 ...
 */
import {
  AnimationOutputError,
  animationOutputSettings,
  createGpuAnimationOutput,
} from "./animation_output.mjs";

const fail = (code, message) => {
  throw new AnimationOutputError("ANIMATION_PRESENTATION_" + code, message);
};
...
export async function createGpuAnimationPresentation(device, options = {}) {
  ...
  if (![1, 4].includes(sampleCount) || !depthBytes.has(depthFormat))
    fail("FORMAT", "Unsupported sample count or depth format");
  ...
  if (outputOptions.inputAlpha !== undefined && outputOptions.inputAlpha !== "premultiplied")
    fail("ALPHA", "Managed scene output is premultiplied linear color");
  ...
  if (
    ...
    fail("OPTIONS", "readback must be false, true or a limits object");
  const readbackOptions = readback === false ? null : readback === true ? {} : { ...readback };
  let reader, output;
  const checkAbort = () => {
    if (outputOptions.signal?.aborted)
      throw outputOptions.signal.reason ?? new DOMException("Aborted", "AbortError");
  };
  try {
    checkAbort();
    if (readbackOptions) {
    ...
    output = await createGpuAnimationOutput(device, outputOptions);
    checkAbort();
  } catch (error) {
    reader?.dispose();
    output?.dispose();
  ...
  function release() {
    ...
    current?.dispose();
    current = null;
    lastTarget = lastScene = lastOutput = null;
    output.dispose();
  }
  function stop(error) {
  ...
  function live() {
    ...
    if (disposed) fail("DISPOSED", "Presentation is disposed");
    if (output.failed || reader?.failed) fail("GPU", "Output or readback failed");
  }
  if (device.lost && typeof device.lost.then === "function")
    device.lost.then(
      (info) =>
        stop(
          new AnimationOutputError(
            "ANIMATION_PRESENTATION_DEVICE_LOST",
  ...
  function targetInfo(target) {
    object(target, "output target");
    const width = positive(target.width, "target width"),
      height = positive(target.height, "target height");
    if (width > maxDimension || height > maxDimension)
      fail("LIMIT", "Target exceeds the device dimension limit");
    if (
      target.dimension !== "2d" ||
      target.depthOrArrayLayers !== 1 ||
      target.sampleCount !== 1 ||
      !Number.isInteger(target.usage) ||
      !(target.usage & 16) ||
      typeof target.createView !== "function"
    )
      fail("TARGET", "Target requires a single-sample 2D render attachment");
    if (
      target.format !== output.format &&
      !(output.format.endsWith("-srgb") && target.format === output.format.slice(0, -5))
    )
      fail("FORMAT", "Target format differs from output format");
    const bytes =
      width *
      height *
      (8 * (sampleCount === 4 ? 5 : 1) + depthBytes.get(depthFormat) * sampleCount);
    if (!Number.isSafeInteger(bytes) || bytes > maxTextureBytes)
      fail("LIMIT", "HDR attachments exceed the byte budget");
  ...
  function allocate({ width, height, bytes }) {
    ...
    if (bytes + (current?.bytes ?? 0) > maxTextureBytes)
      fail("LIMIT", "Resize peak exceeds the intermediate byte budget");
    const owned = [];
    let complete = false;
    const make = (format, count, usage) => {
      const texture = device.createTexture({
        label: "Animation HDR " + format,
        size: [width, height, 1],
        format,
        sampleCount: count,
        usage,
    ...
    try {
      const color = make("rgba16float", 1, 4 | 16 | (reader ? 1 : 0)),
      ...
      result = {
        ...
        bytes,
        color,
        colorView: multisample ? multisample.createView() : colorView,
        ...(multisample ? { resolveTarget: colorView } : {}),
        ...(depth ? { depthView: depth.createView() } : {}),
        dispose() {
          if (!complete) {
            complete = true;
    ...
    const check = Promise.all([device.popErrorScope(), device.popErrorScope()]).then(
      ([invalid, oom]) => {
        if (invalid || oom) fail("GPU", (invalid || oom).message ?? "HDR allocation failed");
      },
    );
    validation = Promise.all([validation, check]);
  ...
  function submit(scene, input, camera, cameraSettings) {
    live();
    if (busy) fail("REENTRANT", "Presentation operation cannot be reentered");
    busy = true;
    try {
      object(input, "frame");
      object(scene, "borrowed scene/renderer");
      const method = camera ? "renderCamera" : "render";
      if (typeof scene[method] !== "function" || scene.disposed || scene.failed)
        fail("SCENE", "A live scene/renderer is required");
      for (const [key, value] of Object.entries(rendererOptions))
        if (scene[key] !== undefined && scene[key] !== value)
          fail("FORMAT", "Borrowed renderer configuration differs from HDR attachments");
      const { target, output: overrides = {}, ...frame } = { ...input };
      object(overrides, "output overrides");
      for (const key of Object.keys(overrides))
        if (!["toneMapping", "exposure", "outputAlpha"].includes(key))
          fail("OPTIONS", `Unknown output override: ${key}`);
      for (const key of ["colorView", "depthView", "resolveTarget"])
        if (Object.hasOwn(frame, key))
          fail("TARGET", "Managed presentation supplies color/depth/resolve views");
      const selected = animationOutputSettings(
          { ...overrides, inputAlpha: "premultiplied" },
          output.defaults,
        ),
        info = targetInfo(target);
      const resize = !current || current.width !== info.width || current.height !== info.height;
      if (resize && (frame.loadOp === "load" || (depthFormat && frame.depthLoadOp === "load")))
        fail("HISTORY", "Clear attachments on first use or resize");
      ...
      if (frame.clearColor !== undefined) {
        ...
        if ((!Array.isArray(color) && !ArrayBuffer.isView(color)) || color.length !== 4)
          fail("COLOR", "Expected linear RGBA clear color");
        const values = Array.from(color);
        if (
          values.some((v) => typeof v !== "number" || !Number.isFinite(v) || v < 0 || v > 65504) ||
          values[3] > 1
        )
          fail("COLOR", "Invalid half-float clear color or alpha");
        frame.clearColor = [
          values[0] * values[3],
          values[1] * values[3],
          values[2] * values[3],
      ...
      try {
        const prepared = {
          ...frame,
          colorView: next.colorView,
          ...(depthFormat ? { depthView: next.depthView } : {}),
          ...(sampleCount === 4 ? { resolveTarget: next.resolveTarget } : {}),
        };
        const completion = camera
          ? scene.renderCamera(prepared, cameraSettings)
          : scene.render(prepared);
        if (completion && typeof completion.then === "function") {
          try {
            Promise.prototype.then.call(completion, undefined, () => {});
          } catch {}
          submitted = true;
          fail("ASYNC", "Scene rendering must submit synchronously");
        }
        submitted = true;
  ...
  async function readPixels(settings = {}) {
    live();
    if (busy) fail("REENTRANT", "Cannot capture during render submission");
    ...
    if (source !== "hdr" && source !== "output")
      fail("OPTIONS", "Capture source must be hdr or output");
    if (
      (options.mipLevel !== undefined && options.mipLevel !== 0) ||
      (options.layer !== undefined && options.layer !== 0)
    )
      fail("OPTIONS", "Capture reads the rendered base mip/layer only");
    ...
    if (Object.hasOwn(options, "completion"))
      fail("OPTIONS", "Presentation supplies producer validation");
    const observedVersion = version,
      observedScene = lastScene;
    const metadata = {
      source,
      presentationVersion: observedVersion,
    ...
    try {
      ...
      pixels = reader.readPixels(texture, { ...options, completion });
      pixels.catch(() => {});
      const verified = Promise.all([validation, output.whenIdle(), observedScene.whenIdle?.()]);
      verified.then(resolveCompletion, (error) => {
    ...
    const result = await Promise.race([pixels, stopped]);
    live();
  ...
  const presentation = Object.freeze({
    ...
    get readbackPending() {
      return reader?.pending ?? 0;
    },
    get readbackBufferBytes() {
      return reader?.bufferBytes ?? 0;
    },
    get readbackReservedBytes() {
      return reader?.reservedBytes ?? 0;
    },
    get version() {
      return version;
    },
    ...
    get bufferBytes() {
      return output.allocatedBytes + (reader?.bufferBytes ?? 0);
    },
    get width() {
    ...
    get failed() {
      return terminal !== null || output.failed || Boolean(reader?.failed);
    },
    render(scene, frame) {
      return submit(scene, frame, false);
    },
    renderCamera(scene, frame, settings) {
      return submit(scene, frame, true, settings);
    },
    async whenIdle() {
      live();
      try {
        await Promise.race([
          Promise.all([validation, output.whenIdle(), reader?.whenIdle()]),
          stopped,
        ]);
        live();
        return presentation;
    ...
    dispose() {
      if (busy) fail("REENTRANT", "Cannot dispose during a render");
      if (!disposed) {
        disposed = true;
        release();
        rejectStop(
          new AnimationOutputError("ANIMATION_PRESENTATION_DISPOSED", "Presentation disposed"),
        );
      }
    },
  });
