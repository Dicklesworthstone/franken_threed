...
export class AnimationBloomError extends Error {
  constructor(code, message) { super(`${code}: ${message}`); this.name = 'AnimationBloomError'; this.code = code; }
}
const fail = (code, message) => { throw new AnimationBloomError(`ANIMATION_BLOOM_${code}`, message); };
...
const scalar = (value, max, label) => {
  if (typeof value !== 'number' || !Number.isFinite(value) || value < 0 || value > max) fail('VALUE', `Invalid ${label}`);
  return value;
};
const keys = (value, allowed) => {
  for (const key of Object.keys(value)) if (!allowed.includes(key)) fail('OPTIONS', `Unknown option: ${key}`);
};
 ...
 */
export function animationBloomPlan(width, height, { levels = 5, maxBytes = 128 * 1024 * 1024 } = {}) {
  integer(width, 1, 32768, 'width'); integer(height, 1, 32768, 'height');
  integer(levels, 1, 6, 'levels'); integer(maxBytes, 16, Number.MAX_SAFE_INTEGER, 'maxBytes');
  const sizes = [];
  let textureBytes = 0;
  ...
  return Object.freeze({ sizes: Object.freeze(sizes), textureBytes, allocatedBytes: textureBytes + 16 });
}
...
export function animationBloomCompositeWgsl(levels) {
  ...
  const bindings = Array.from({ length: levels }, (_, i) => `@group(0) @binding(${i + 2}) var bloom${i}: texture_2d<f32>;`).join('\n');
  const samples = Array.from({ length: levels }, (_, i) => `if (info.levels > ${i}u) { glow += bilinear(bloom${i}, uv); }`).join('\n  ');
  return /* wgsl */`
${INFO}
@group(0) @binding(0) var source: texture_2d<f32>;
@group(0) @binding(1) var<uniform> info: BloomInfo;
...
export async function createGpuAnimationBloom(device, options = {}) {
  object(options, 'bloom options'); keys(options, [...SETTINGS, 'levels', 'maxBytes', 'signal']);
  const levels = integer(options.levels ?? 5, 1, 6, 'levels');
  const maxBytes = integer(options.maxBytes ?? 128 * 1024 * 1024, 16, Number.MAX_SAFE_INTEGER, 'maxBytes');
  const defaults = animationBloomSettings(options), signal = options.signal;
  ...
  aborted();
  for (const name of ['createTexture', 'createBuffer', 'createShaderModule', 'createBindGroupLayout', 'createPipelineLayout', 'createRenderPipelineAsync', 'createBindGroup', 'createCommandEncoder', 'pushErrorScope', 'popErrorScope']) {
  ...
  if (device.limits?.maxSampledTexturesPerShaderStage < levels + 1) fail('DEVICE', 'Insufficient sampled texture bindings');
  let uniform, pack, filterLayout, compositeLayout, pipelines, disposed = false, terminal = null, busy = false;
  let validation = Promise.resolve(), version = 0, lastRender = null;
  let rejectStop;
  const stopped = new Promise((_, reject) => { rejectStop = reject; }); stopped.catch(() => {});
  const destroyPack = value => { for (const texture of value?.textures ?? []) texture.destroy(); };
  function release() { destroyPack(pack); pack = undefined; uniform?.destroy(); uniform = undefined; pipelines = undefined; }
  function stop(error) { if (!disposed && !terminal) { terminal = error; release(); rejectStop(error); } }
  function live() { if (terminal) throw terminal; if (disposed) fail('DISPOSED', 'Bloom is disposed'); }
  ...
  if (device.lost && typeof device.lost.then === 'function') device.lost.then(info => stop(new AnimationBloomError('ANIMATION_BLOOM_DEVICE_LOST', info?.message ?? 'Device lost')), stop);
  const onAbort = () => stop(signal.reason ?? new DOMException('Aborted', 'AbortError'));
  signal?.addEventListener('abort', onAbort, { once: true });
  try {
    const compiled = checked(() => {
      const textureEntry = binding => ({ binding, visibility: 2, texture: { sampleType: 'unfilterable-float', viewDimension: '2d' } });
      const base = [textureEntry(0), { binding: 1, visibility: 2, buffer: { type: 'uniform', minBindingSize: 16 } }];
      filterLayout = device.createBindGroupLayout({ entries: base });
      compositeLayout = device.createBindGroupLayout({ entries: [...base, ...Array.from({ length: levels }, (_, i) => textureEntry(i + 2))] });
      const module = device.createShaderModule({ label: 'Animation bloom filters', code: ANIMATION_BLOOM_FILTER_WGSL });
      const composite = device.createShaderModule({ label: 'Animation bloom composite', code: animationBloomCompositeWgsl(levels) });
      const make = (module, entryPoint, layout) => device.createRenderPipelineAsync({
        label: `Animation bloom ${entryPoint}`, layout: device.createPipelineLayout({ bindGroupLayouts: [layout] }),
        vertex: { module, entryPoint: 'vertex_main' }, fragment: { module, entryPoint, targets: [{ format: 'rgba16float' }] },
        primitive: { topology: 'triangle-list' },
      });
      return Promise.all(['extract_main', 'downsample_main', 'horizontal_main', 'vertical_main'].map(entry => make(module, entry, filterLayout))
        .concat(make(composite, 'composite_main', compositeLayout)));
    });
    pipelines = await Promise.race([compiled, stopped]);
    await Promise.race([validation, stopped]); live(); aborted();
    checked(() => { uniform = device.createBuffer({ label: 'Animation bloom parameters', size: 16, usage: 8 | 64 }); });
    await Promise.race([validation, stopped]); live(); aborted();
  } catch (error) { stop(error); release(); throw error; }
  finally { signal?.removeEventListener('abort', onAbort); }

  const view = texture => texture.createView({ dimension: '2d', baseMipLevel: 0, mipLevelCount: 1, baseArrayLayer: 0, arrayLayerCount: 1 });
  const group = (layout, views) => device.createBindGroup({ layout, entries: [
    { binding: 0, resource: views[0] }, { binding: 1, resource: { buffer: uniform, size: 16 } },
    ...views.slice(1).map((resource, i) => ({ binding: i + 2, resource })),
  ] });
  function allocate(width, height, plan) {
    const next = { width, height, bytes: plan.textureBytes, textures: [], stages: [], source: null, extract: null, composite: null };
    try {
      for (const size of plan.sizes) {
        const pair = [];
        for (let i = 0; i < 2; i++) {
          const texture = device.createTexture({ label: `Animation bloom ${size.width}x${size.height}`, size, format: 'rgba16float', usage: 4 | 16 });
          next.textures.push(texture); pair.push(view(texture));
        }
        const previous = next.stages.at(-1);
        next.stages.push({ a: pair[0], b: pair[1], horizontal: group(filterLayout, [pair[0]]), vertical: group(filterLayout, [pair[1]]),
          downsample: previous ? group(filterLayout, [previous.a]) : null });
      }
      return next;
    } catch (error) { destroyPack(next); throw error; }
  }
  function texture(value, label, usage) {
    object(value, label);
    const limit = Math.min(device.limits?.maxTextureDimension2D ?? 8192, 32768);
    const width = integer(value.width, 1, limit, `${label} width`), height = integer(value.height, 1, limit, `${label} height`);
    if (value.dimension !== '2d' || value.depthOrArrayLayers !== 1 || value.sampleCount !== 1 ||
        !Number.isInteger(value.usage) || (value.usage & usage) !== usage || typeof value.createView !== 'function') fail('TEXTURE', `Invalid ${label} storage, usage or samples`);
    return { value, width, height, format: value.format };
  }
  function pass(encoder, target, pipeline, bindings) {
    const p = encoder.beginRenderPass({ label: 'Animation bloom', colorAttachments: [{ view: target, loadOp: 'clear', storeOp: 'store', clearValue: { r: 0, g: 0, b: 0, a: 0 } }] });
    p.setPipeline(pipeline); p.setBindGroup(0, bindings); p.draw(3); p.end();
  }
  const result = Object.freeze({ defaults, levels,
    get version() { return version; }, get lastRender() { return lastRender; },
    get allocatedBytes() { return uniform ? 16 + (pack?.bytes ?? 0) : 0; },
    get disposed() { return disposed; }, get failed() { return terminal !== null; },
    render(frame) {
      live(); if (busy) fail('REENTRANT', 'Bloom operation cannot be reentered'); busy = true;
      try {
        object(frame, 'bloom frame'); keys(frame, ['source', 'target', ...SETTINGS]);
        const source = texture(frame.source, 'source', 4), target = texture(frame.target, 'target', 16);
        const settings = animationBloomSettings(frame, defaults);
        if (!['rgba16float', 'rgba8unorm', 'bgra8unorm'].includes(source.format) || target.format !== 'rgba16float') fail('FORMAT', 'Bloom requires linear source and rgba16float target');
        if (source.value === target.value || source.width !== target.width || source.height !== target.height) fail('TEXTURE', 'Source/target must be distinct and equally sized');
        const enabled = settings.strength > 0;
        const resize = enabled && (!pack || pack.width !== source.width || pack.height !== source.height);
        const plan = resize ? animationBloomPlan(source.width, source.height, { levels, maxBytes }) : null;
        if (resize && plan.allocatedBytes + (pack?.bytes ?? 0) > maxBytes) fail('LIMIT', 'Bloom resize old/new overlap exceeds maxBytes');
        checked(() => {
          if (resize) { const next = allocate(source.width, source.height, plan); destroyPack(pack); pack = next; }
          let composite;
          if (enabled) {
            if (pack.source !== source.value) {
              const sourceView = view(source.value);
              const extract = group(filterLayout, [sourceView]);
              composite = group(compositeLayout, [sourceView, ...Array.from({ length: levels }, (_, i) => pack.stages[Math.min(i, pack.stages.length - 1)].a)]);
              pack.source = source.value; pack.extract = extract; pack.composite = composite;
            } else composite = pack.composite;
          } else { const sourceView = view(source.value); composite = group(compositeLayout, Array(levels + 1).fill(sourceView)); }
          const bytes = new ArrayBuffer(16), data = new DataView(bytes);
          data.setFloat32(0, settings.threshold, true); data.setFloat32(4, settings.threshold * settings.softKnee, true);
          data.setFloat32(8, settings.strength, true); data.setUint32(12, enabled ? pack.stages.length : 0, true);
          device.queue.writeBuffer(uniform, 0, bytes);
          const encoder = device.createCommandEncoder({ label: 'Animation bloom frame' });
          if (enabled) for (let i = 0; i < pack.stages.length; i++) {
            const stage = pack.stages[i];
            pass(encoder, stage.a, pipelines[i === 0 ? 0 : 1], i === 0 ? pack.extract : stage.downsample);
            pass(encoder, stage.b, pipelines[2], stage.horizontal); pass(encoder, stage.a, pipelines[3], stage.vertical);
          }
          pass(encoder, view(target.value), pipelines[4], composite);
          device.queue.submit([encoder.finish()]);
        });
        version++;
        lastRender = Object.freeze({ version, width: source.width, height: source.height, levels: enabled ? pack.stages.length : 0,
          passes: enabled ? 3 * pack.stages.length + 1 : 1, settings });
        return result;
      } finally { busy = false; }
    },
    async whenIdle() {
      live(); if (busy) fail('REENTRANT', 'Cannot await during a bloom operation');
      try { await Promise.race([Promise.all([validation, device.queue.onSubmittedWorkDone()]), stopped]); live(); return result; }
      catch (error) { stop(error); throw error; }
    },
    dispose() {
      if (busy) fail('REENTRANT', 'Cannot dispose during a bloom operation');
      if (!disposed) { disposed = true; release(); rejectStop(new AnimationBloomError('ANIMATION_BLOOM_DISPOSED', 'Bloom is disposed')); }
    },
  });
  return result;
}
