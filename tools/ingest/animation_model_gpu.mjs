 ...
 */
import {decodeGltfAnimationModel,createGltfSceneView,GltfSceneViewError,createAnimationModelPicker,AnimationRaycastError} from './animation_model.mjs';
 ...
 */
export async function createGpuGltfAnimationScene(device, json, buffers, {decode={},scene:options={},picking=false}={}) {
  return createGpuDecodedAnimationScene(device,decodeGltfAnimationModel(json,buffers,decode),{...options,picking});
}
 ...
 */
export async function createGpuDecodedAnimationScene(device,prepared,{picking=false,...options}={}) {
  const pose=createAnimationPlayer(prepared.definition);
  let scene,view,picker,busy=false;
  try {
  ...
  } catch(error){picker?.dispose();pose.dispose();throw error;}
  ...
  function read(operation) {
    ...
    catch(error){if(scene.failed){picker.dispose();pose.dispose();}throw error;}
    finally {busy=false;}
  }
  ...
  const result=Object.freeze({pose,view,cameras:view.cameras,lights:view.lights,
    ...
    async whenIdle(){try{await scene.whenIdle();return result;}catch(error){if(scene.failed){picker.dispose();pose.dispose();}throw error;}},
    dispose(){if(busy)fail('REENTRANT','Cannot dispose during model operation');scene.dispose();picker.dispose();pose.dispose();},
  });
  return result;
}
