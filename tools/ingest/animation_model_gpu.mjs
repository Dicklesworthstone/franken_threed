 ...
 */
import {decodeGltfAnimationModel,createGltfSceneView,GltfSceneViewError,createAnimationModelPicker,AnimationRaycastError,createAnimationModelExporter,AnimationExportError} from './animation_model.mjs';
 ...
 */
export async function createGpuGltfAnimationScene(device, json, buffers, {decode={},scene:options={},picking=false,exporting=false}={}) {
  return createGpuDecodedAnimationScene(device,decodeGltfAnimationModel(json,buffers,decode),{...options,picking,exporting});
}
 ...
 */
export async function createGpuDecodedAnimationScene(device,prepared,{picking=false,exporting=false,...options}={}) {
  const pose=createAnimationPlayer(prepared.definition);
  let scene,view,picker,exporter,busy=false;
  try {
    ...
    exporter=createAnimationModelExporter(pose,prepared.drawables,prepared.source,exporting,null,prepared.copyright);
    scene=await createGpuAnimationScene(device,pose,prepared.drawables,options);
  } catch(error){exporter?.dispose();picker?.dispose();pose.dispose();throw error;}
  ...
  function read(operation) {
    ...
    catch(error){if(scene.failed){exporter.dispose();picker.dispose();pose.dispose();}throw error;}
    finally {busy=false;}
  }
  function invoke(operation){read(operation);return result;}
  function query(operation){return read(()=>{
    if(scene.disposed||scene.failed||pose.disposed)throw new AnimationRaycastError('ANIMATION_PICK_POSE','Model is not usable');
    if(pose.version!==scene.poseVersion)throw new AnimationRaycastError('ANIMATION_PICK_STALE','Upload the current pose before picking');
    return operation();
  });}
  const result=Object.freeze({pose,view,cameras:view.cameras,lights:view.lights,
    ...
    exportPoseGLB(settings){return read(()=>{
      ...
      if(pose.version!==scene.poseVersion)throw new AnimationExportError('ANIMATION_EXPORT_STALE','Upload the current pose before exporting');
      return exporter.exportPoseGLB(settings);
    });},
    get pickingEnabled(){return picker.enabled;},get pickingStats(){return picker.lastQuery;},
    raycast(ray,settings){return query(()=>picker.raycast(ray,settings));},
    pick(ndc,cameraSettings,querySettings){return query(()=>picker.pick(ndc,cameraSettings,querySettings));},
    ...
    dispose(){if(busy)fail('REENTRANT','Cannot dispose during model operation');scene.dispose();exporter.dispose();picker.dispose();pose.dispose();},
  });
  return result;
}
