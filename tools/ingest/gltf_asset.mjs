...
export class GltfAssetError extends Error {
  constructor(code, message) {
    super(`${code}: ${message}`);
    this.name = "GltfAssetError";
    this.code = code;
  }
}
const fail = (code, message) => {
  throw new GltfAssetError("GLTF_ASSET_" + code, message);
};
...
function bytes(value) {
  ...
  if (!(buffer instanceof ArrayBuffer) || buffer.resizable)
    fail("BYTES", "Expected fixed, unshared bytes");
  try {
    return ArrayBuffer.isView(value)
      ? new Uint8Array(buffer, value.byteOffset, value.byteLength)
      : new Uint8Array(buffer);
  } catch {
    fail("BYTES", "Detached input bytes");
  }
}
...
export function parseGltfAsset(input, { maxBytes = 64 * 1024 * 1024 } = {}) {
  ...
  if (raw.length > maxBytes) fail("LIMIT", "Model file exceeds byte limit");
  ...
  let json;
  try {
    json = JSON.parse(new TextDecoder("utf-8", { fatal: true }).decode(source));
  } catch {
    fail("JSON", "Invalid UTF-8 model JSON");
  }
  if (
    !json ||
    Array.isArray(json) ||
...
function dataUri(uri, limit) {
  ...
  let output;
  if (match[2]) {
    if (!/^[A-Za-z0-9+/]*={0,2}$/.test(body) || body.replace(/=+$/, "").length % 4 === 1)
      fail("URI", "Invalid base64");
    ...
    if (btoa(decoded).replace(/=+$/, "") !== body.replace(/=+$/, ""))
      fail("URI", "Noncanonical base64");
    output = Uint8Array.from(decoded, (c) => c.charCodeAt(0));
  } else {
    let length = 0;
    for (let i = 0; i < body.length; i++, length++) {
      if (body[i] === "%") {
        if (!/^[\da-f]{2}$/i.test(body.slice(i + 1, i + 3))) fail("URI", "Invalid escaped octet");
        i += 2;
    ...
    for (let i = 0, j = 0; i < body.length; i++, j++) {
      if (body[i] === "%") {
        output[j] = parseInt(body.slice(i + 1, i + 3), 16);
        i += 2;
...
function imageType(data) {
  if (data.length >= 8 && [137, 80, 78, 71, 13, 10, 26, 10].every((v, i) => data[i] === v))
    return "image/png";
  ...
  if (
    data.length >= 12 &&
    [171, 75, 84, 88, 32, 50, 48, 187, 13, 10, 26, 10].every((v, i) => data[i] === v)
  )
    return "image/ktx2";
...
export async function loadGltfAsset(
  ...
  positive(maxBytes, "total byte limit");
  positive(maxResourceBytes, "resource byte limit");
  positive(maxResources, "resource count");
  if (!Array.isArray(allowedOrigins) || allowedOrigins.some((v) => typeof v !== "string"))
    fail("URI", "Invalid allowed origins");
  ...
  if (baseURL !== undefined) {
    try {
      ...
      fail("URI", "baseURL must be absolute");
    }
  }
  const networkSource = typeof source === "string" || source instanceof URL;
  if (networkSource) {
    try {
      base = new URL(source, base ?? undefined);
    } catch {
      fail("URI", "Model URL must be absolute or have baseURL");
    }
  }
  if (base) {
    if (!["http:", "https:"].includes(base.protocol) || base.username || base.password)
      fail("URI", "Use credential-free HTTP(S) URLs");
    origins.add(base.origin);
  }
  const cache = new Map();
  function charge(length) {
    if (length > maxBytes - total) fail("LIMIT", "Total input byte budget exceeded");
    total += length;
  }
  async function read(uri) {
    ...
    if (!/^data:/i.test(uri)) {
      let url;
      try {
        url = new URL(uri, base ?? undefined);
      } catch {
        fail("URI", "External resources need an absolute baseURL");
      }
      if (
        ...
        fail("URI", "Resource origin or scheme is not allowed");
      url.hash = "";
      key = url.href;
    }
    if (!cache.has(key)) {
      ...
      cache.set(
        ...
        Promise.resolve().then(async () => {
          abort(signal);
          if (/^data:/i.test(key)) {
            const result = dataUri(key, Math.min(maxResourceBytes, maxBytes - total));
            charge(result.bytes.length);
            return result;
          }
          ...
          try {
            ...
            if (!response?.ok || response.redirected)
              fail("HTTP", `Resource request failed: ${response?.status ?? "no response"}`);
            if (response.url) {
              let actual;
              try {
                ...
                fail("HTTP", "Invalid response URL");
              }
            ...
            if (
              ...
              header !== undefined &&
              /^\d+$/.test(header) &&
              Number(header) > Math.min(maxResourceBytes, maxBytes - total)
            )
              fail("LIMIT", "Resource exceeds byte budget");
            if (!response.body || typeof response.body.getReader !== "function")
              fail("FETCH", "A streaming response body is required");
          } catch (error) {
            cancel();
            throw error;
          }
          ...
          try {
            while (true) {
              ...
              if (data.length > maxResourceBytes - length)
                fail("LIMIT", "Stream exceeds resource byte budget");
              charge(data.length);
              length += data.length;
  ...
  if (networkSource) input = (await read(base.href)).bytes;
    ...
    input = bytes(source);
    if (input.length > maxResourceBytes) fail("LIMIT", "Model file exceeds resource limit");
    charge(input.length);
  }
  const parsed = parseGltfAsset(input, { maxBytes: maxResourceBytes }),
    bin = parsed.bin;
  let json = parsed.json;
  const definitions = json.buffers ?? [],
    images = structuredClone(json.images ?? []);
  let views = structuredClone(json.bufferViews ?? []);
  if (
    !Array.isArray(definitions) ||
    !Array.isArray(images) ||
    !Array.isArray(views) ||
    definitions.length > maxResources ||
    images.length > maxResources ||
    views.length > maxResources * 16
  )
    fail("LIMIT", "Invalid or excessive resources");
  // Expansion and fallback decisions precede dependency I/O, not just decoding.
  const meshopt = prepareMeshoptBuffers(json, {
    decoder: meshoptDecoder,
    binaryBuffer: bin !== null,
    maxEncodedBytes: maxBytes,
    maxDecodedBytes,
    maxDecodedBufferBytes,
    maxBufferViews: maxResources * 16,
  });
  const dracoOptions = {
    decoder: dracoDecoder,
    maxEncodedBytes: maxBytes,
    maxDecodedBytes: maxDecodedBytes - meshopt.decodedBytes,
    maxDecodedBufferBytes,
    maxPrimitives: maxResources,
  };
  ...
  for (let i = 0; i < definitions.length; i++) {
    ...
    if (
      ...
      item.byteLength < 1 ||
      (!skipped.has(i) && item.byteLength > maxResourceBytes)
    )
      fail("BUFFER", "Invalid buffer byteLength");
  }
  let buffers = [];
  for (let i = 0; i < definitions.length; i++) {
    abort(signal);
    ...
    if (item.uri === undefined) {
      if (
        i !== 0 ||
        bin === null ||
        bin.length < item.byteLength ||
        bin.length - item.byteLength > 3
      )
        fail("BUFFER", "Missing or invalid GLB BIN buffer");
      for (let j = item.byteLength; j < bin.length; j++)
        if (bin[j] !== 0) fail("BUFFER", "Nonzero BIN padding");
      data = bin;
    } else {
  ...
  const decoded = await meshopt.decode(buffers, { signal });
  abort(signal);
  ...
  const preparedDraco =
    decoded.json === json ? draco : prepareDracoMeshes(decoded.json, dracoOptions);
  const geometry = await preparedDraco.decode(decoded.buffers, { signal });
  abort(signal);
  const sourceJson = parsed.json;
  json = geometry.json;
  buffers = geometry.buffers;
  views = structuredClone(json.bufferViews ?? []);
  const imageCache = new Map();
  function readImage(index) {
    try {
      abort(signal);
      if (!Number.isSafeInteger(index) || index < 0 || index >= images.length)
        fail("IMAGE", "Invalid image index");
    } catch (error) {
    ...
    if (!imageCache.has(index))
      imageCache.set(
        ...
        Promise.resolve().then(async () => {
          ...
          if (!image || (image.uri === undefined) === (image.bufferView === undefined))
            fail("IMAGE", "Image needs exactly one URI or bufferView");
          if (image.extensions && Object.keys(image.extensions).length)
            fail("IMAGE", "Image extensions need the source loader");
          let data,
            declared = image.mimeType;
          if (image.uri !== undefined) {
            const resource = await read(image.uri);
            data = resource.bytes;
            ...
            const view =
              Number.isSafeInteger(image.bufferView) && image.bufferView >= 0
                ? views[image.bufferView]
                : null;
            const buffer =
              view && Number.isSafeInteger(view.buffer) && view.buffer >= 0
                ? buffers[view.buffer]
                : null;
            ...
            if (
              ...
              Object.keys(view.extensions ?? {}).length
            )
              fail("IMAGE", "Invalid embedded image range");
          ...
          if (declared !== undefined && declared !== mimeType)
            fail("IMAGE", "Image MIME type disagrees with image bytes");
  ...
  return Object.freeze({
    ...
    readImage,
    decodedBytes: decoded.decodedBytes + geometry.decodedBytes,
    decodedBufferViews: decoded.decodedBufferViews,
