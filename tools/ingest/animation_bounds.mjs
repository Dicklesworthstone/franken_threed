...
export class AnimationBoundsError extends Error {
  constructor(code, message) {
    super(`${code}: ${message}`);
    this.name = "AnimationBoundsError";
    this.code = code;
  }
}
const fail = (code, text) => {
  throw new AnimationBoundsError(`ANIMATION_BOUNDS_${code}`, text);
};
const EPS = 2 ** -23,
  MIN_NORMAL = 2 ** -126,
...
const scalar = new Float32Array(1),
  bits = new Uint32Array(scalar.buffer);
...
function expand(lo, hi, error = 0) {
  ...
  lo = adjacent(Math.fround(lo), false);
  hi = adjacent(Math.fround(hi), true);
  if (lo > -MIN_NORMAL && hi < MIN_NORMAL) {
    lo = Math.min(lo, 0);
    hi = Math.max(hi, 0);
...
function dot(matrix, row, box) {
  ...
  return expand(
    terms.reduce((sum, t) => sum + t[0], 0),
    terms.reduce((sum, t) => sum + t[1], 0),
...
function fixed(value, length, label) {
  ...
  if (ArrayBuffer.isView(value)) {
    if (!(value.buffer instanceof ArrayBuffer) || value.buffer.resizable)
      fail("STORAGE", `${label} must be fixed and unshared`);
    try {
...
export function animationBoundsVisible(bounds, viewProjection, worldMatrix) {
  ...
  if (
    ...
    fail("VALUE", "World matrix must be affine");
  if (
    !bounds ||
    ...
    fail("SHAPE", "Expected a bounds snapshot");
  ...
  for (let c = 0; c < 4; c++)
    for (let r = 0; r < 4; r++) {
      clip[c * 4 + r] =
        viewProjection[r] * worldMatrix[c * 4] +
        viewProjection[4 + r] * worldMatrix[c * 4 + 1] +
        viewProjection[8 + r] * worldMatrix[c * 4 + 2] +
        viewProjection[12 + r] * worldMatrix[c * 4 + 3];
    }
 ...
 */
export function createAnimationBounds(
  pose,
  geometry,
  { maxComponents = 16777216, maxBytes = 16 * 1024 * 1024 } = {},
) {
  positive(maxComponents, "source component limit");
  positive(maxBytes, "summary byte limit");
  if (
    !pose ||
    ...
    fail("POSE", "Expected a packed pose");
  ...
  if (
    ...
    fail("SHAPE", "Expected node and XYZ positions");
  ...
  if (
    ...
    end > (pose.morphWeights?.length ?? -1)
  )
    fail("POSE", "Invalid morph range");
  const targets = geometry.morphTargets ?? [];
  if (!Array.isArray(targets) || targets.length !== end - start || targets.length > 4096)
    fail("SHAPE", "Mismatched morph target count");
  ...
  if (
    ...
    (!Number.isSafeInteger(skin.offset) ||
      ...
      skin.jointCount < 1 ||
      skin.offset + skin.jointCount * 16 > (pose.jointMatrices?.length ?? -1))
  )
    fail("POSE", "Invalid instance palette");
  const influences = skin ? (geometry.influences ?? 4) : 0;
  if (skin && (!Number.isInteger(influences) || influences < 1 || influences > 32))
    fail("SHAPE", "Expected 1..32 influences");
  if (
    !skin &&
    ...
    fail("SHAPE", "Skin attributes require an instance");
  ...
  for (const target of targets) {
    if (!target || typeof target !== "object" || Array.isArray(target))
      fail("SHAPE", "Invalid morph target");
  ...
  let summary = new Float64Array((targetCount + 1) * 6),
    usedJoints = null,
    smin = 1,
    smax = 1;
  function scan(values, at) {
    fixed(values, count, "Positions");
    summary.fill(Infinity, at, at + 3);
    summary.fill(-Infinity, at + 3, at + 6);
    for (let i = 0; i < count; i++) {
      const f = finite(values[i], "Position"),
        range = inputInterval(f),
        axis = i % 3;
  ...
  for (let i = 0; i < targets.length; i++)
    if (targets[i].positions !== undefined) scan(targets[i].positions, (i + 1) * 6);
  if (skin) {
    const length = vertices * influences,
      joints = fixed(geometry.joints, length, "Joint indices"),
      weights = fixed(geometry.weights, length, "Skin weights");
    const used = new Set();
    smin = Infinity;
    smax = 0;
    for (let vertex = 0; vertex < vertices; vertex++) {
      ...
      for (let k = 0; k < influences; k++) {
        const at = vertex * influences + k,
          j = joints[at],
          w = finite(weights[at], "Skin weight");
        if (!Number.isInteger(j) || j < 0 || j >= skin.jointCount || weights[at] < 0)
          fail("SHAPE", "Invalid joint or negative skin weight");
        original += weights[at];
        sum += w;
        low += w < MIN_NORMAL ? 0 : w;
        if (w > 0) used.add(j);
      }
  ...
  const morphLength = pose.morphWeights.length,
    paletteLength = pose.jointMatrices?.length;
  let disposed = false,
    busy = false,
    current = null;
  function live() {
    if (disposed) fail("DISPOSED", "Bounds have been disposed");
    if (pose.disposed || !Number.isSafeInteger(pose.version) || pose.version < 0)
      fail("POSE", "Pose is unavailable");
  }
  if (pose.version !== initialVersion) fail("CHANGED", "Pose changed while preparing bounds");
  function update() {
    live();
    if (busy) fail("REENTRANT", "Bounds update cannot be reentered");
    busy = true;
    ...
    try {
      ...
      for (let t = 0; t < targetCount; t++) {
        ...
        for (let a = 0; a < 3; a++)
          box[a] = add(
            box[a],
            product(inputInterval(w), [summary[(t + 1) * 6 + a], summary[(t + 1) * 6 + 3 + a]]),
          );
      }
      if (skin) {
        ...
        for (const joint of usedJoints) {
          ...
          for (let i = 0; i < 16; i++)
            m[i] = finite(pose.jointMatrices[offset + i], "Joint matrix");
          if (m[3] !== 0 || m[7] !== 0 || m[11] !== 0 || m[15] !== 1)
            fail("POSE", "Joint matrix must be affine");
          for (let a = 0; a < 3; a++) {
            const range = dot(m, a, box);
            hull[a][0] = Math.min(hull[a][0], range[0]);
            hull[a][1] = Math.max(hull[a][1], range[1]);
          }
        }
        ...
        box = hull.map((range) => {
          ...
          return expand(
            Math.min(...values),
            Math.max(...values),
            gamma * Math.max(Math.abs(range[0]), Math.abs(range[1])) * smax +
              4 * influences * MIN_NORMAL,
          );
        });
      }
      live();
      if (pose.version !== version) fail("CHANGED", "Pose changed during bounds update");
      current = Object.freeze({
        min: Object.freeze(box.map((x) => x[0])),
        max: Object.freeze(box.map((x) => x[1])),
        poseVersion: version,
        bounded: box.every((x) => Number.isFinite(x[0]) && Number.isFinite(x[1])),
      });
      return current;
    } finally {
      busy = false;
  ...
  const result = Object.freeze({
    ...
    get byteLength() {
      return disposed ? 0 : byteLength;
    },
    get disposed() {
      return disposed;
    },
    dispose() {
      if (busy) fail("REENTRANT", "Cannot dispose during bounds update");
      disposed = true;
      summary = null;
      usedJoints = null;
      current = null;
