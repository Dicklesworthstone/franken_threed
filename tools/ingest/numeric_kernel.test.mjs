...
function compare(reference, types, args) {
  ...
  arrays.forEach((array, index) => {
    ...
    array.forEach((value, i) =>
      assert.ok(
        Object.is(value, outputs[index][i]),
...
test("emits an import-free deterministic module with an embedded matching ABI", () => {
  const source =
    "export function integrate(x, v, dt) { for (let i=0; i<x.length; i++) { x[i] += v[i]*dt; } }";
...
test("retains signed zero, infinities and NaN numeric semantics without reassociation", () => {
  ...
  const result = execute(
    "function f(x) { for (let i=0; i<x.length; i++) { x[i] = (1e16 + -1e16) + 1; } }",
    ["f64[]"],
    [new Float64Array(1)],
  );
...
test("matches native JavaScript across deterministic randomized inputs and multiple memory pages", () => {
  ...
  for (const size of [0, 1, 7, 257, 8193]) {
    compare(
      ...
      ["f64[]", "f64[]", "f64"],
      [
        Float64Array.from({ length: size }, random),
        Float64Array.from({ length: size }, random),
        0.125,
      ],
    );
...
test("supports more than 127 locals and instructions with multi-byte binary lengths", () => {
  ...
  assert.deepEqual(
    [...execute(source, ["f64[]"], [new Float64Array([2, 3])]).arrays[0]],
    [141, 142],
  );
});
...
test("rejects unclosed and effectful source before producing executable code", async (t) => {
  const refused = [
    ...
    "function f(x) { for (let i=0; i<x.length; i+=17) x[i]=1; }",
...
test("validates ABI, source bounds and refusal source locations", () => {
  ...
  for (const options of [
    ...
    { parameterTypes: ["i32[]"] },
    { parameterTypes: [] },
    { parameterTypes: ["f64[]"], maxMemoryPages: 0 },
    { parameterTypes: ["f64[]"], maxMemoryPages: 16385 },
  ]) {
  ...
  assert.throws(
    () =>
      compileNumericKernel("function f(x) { for (let i=0; i<x.length; i++) x[i]=unknown; }", {
        parameterTypes: ["f64[]"],
      }),
    (error) => error.span.line === 1 && error.span.end > error.span.start,
  );
});
