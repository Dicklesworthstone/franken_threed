...
const STEP = 'export function step(a, v, dt) { for (let i = 0; i < a.length; i++) a[i] += v[i] * dt; }';
...
function directory() {
  ...
  for (const name of runtimeFiles) fs.copyFileSync(new URL(name, import.meta.url), path.join(dir, name));
  return dir;
}
...
test('ordinary frame calls execute one persistent Wasm instance, with unaltered exports and function identity', async t => {
  ...
  const actual = new Float64Array([0, 1]), expected = actual.slice(), v = new Float64Array([1, -2]);
...
test('shadowed same-name callee uses its own body, arguments, receiver and return value', async () => {
  ...
  const actual = module.shadow(function (...args) { assert.equal(this, undefined); return args; }, 7, 8);
  assert.deepEqual(actual, [7, 8]);
  const sentinel = new Error('retained');
  assert.throws(() => module.shadow(() => { throw sentinel; }), e => e === sentinel);
});
...
export function optional(a, v, dt) { return step?.(a, v, dt); }`);
  const a = new Float64Array([1]), v = new Float64Array([2]);
  module.tick(a, v, 3);
  assert.equal(a[0], 7);
...
export function tick(a, v, events) {
  ...
  const a = new Float64Array([0]), v = new Float64Array([1]), events = [];
...
test('early calls through cyclic imports retain original JS until registration', async () => {
  ...
  fs.writeFileSync(path.join(dir, 'b.mjs'), `import { tick } from './a.mjs';
  ...
  const a = new Float64Array([1]); module.tick(a, new Float64Array([2]), 3);
  assert.equal(a[0], 7);
});
...
export function replace(fn) { step = fn; } export const value = early;`;
  ...
  fs.writeFileSync(path.join(dir, 'b.mjs'), `import { replace, tick } from './a.mjs';
...
export function tick(a, v, dt) { let __f3d_numeric_call_3 = 9; return step(a, v, dt); }`);
  const a = new Float64Array([0]); module.tick(a, new Float64Array([2]), 3);
  assert.equal(a[0], 6);
  assert.equal(counts.calls, 1);
});
...
test('unsupported argument families keep functioning instead of becoming feature refusals', async () => {
  ...
  for (const make of [xs => xs, xs => new Float32Array(xs), xs => new Float64Array(xs)]) {
    const a = make([1, 2]), v = make([2, 3]); module.tick(a, v, 2);
...
test('custom length and scalar coercion effects run only in the original fallback', async () => {
  const { module } = await load(`${STEP}\n${TICK}`);
  let lengths = 0, coercions = 0;
  const a = new Float64Array([1, 2]), v = new Float64Array([2, 3]);
  Object.defineProperty(a, 'length', { get() { lengths++; return 2; } });
  module.tick(a, v, { valueOf() { coercions++; return 2; } });
...
test('Wasm-unavailable applications still import and perform their normal update calls', async t => {
  ...
  t.after(() => { globalThis.WebAssembly = native; });
...
test('conditional particle updates preserve exact arithmetic across hundreds of calls', async t => {
  ...
  const actual = new Float64Array([0, -0, NaN, Infinity, -Infinity, 0.2]), expected = actual.slice();
  const v = new Float64Array([1, -1, 2, 1, 1, -2]), ev = v.slice();
  for (let i = 0; i < 300; i++) {
...
export function tick(a) { step(a); }`;
  ...
  const a = [1]; module.tick(a); assert.equal(a[0], Math.sin(1));
});
...
test('hashbang, directive prologue and deterministic output survive insertion', () => {
  ...
  const a = specializeNumericModule(source), b = specializeNumericModule(source);
