...
export function createAnimationDrawOrder(entries, {
  pose=null,sortObjects=true,frustumCulling=false,maxBoundsBytes=16*1024*1024,maxBoundsComponents=16777216,
}={}) {
  if (!Array.isArray(entries) || entries.length > 4096) fail('Invalid scene draw count');
  if(typeof sortObjects!=='boolean'||typeof frustumCulling!=='boolean')fail('Sorting and culling options must be boolean');
  if(frustumCulling&&(!pose||!Number.isSafeInteger(maxBoundsBytes)||maxBoundsBytes<1||!Number.isSafeInteger(maxBoundsComponents)||maxBoundsComponents<1))fail('Invalid bounds limits or pose');
  let bytes=0,components=0,disposed=false,busy=false,stats=null,cameraSnapshot=null;
  const records=[];
  try {
    for(const [index,entry] of entries.entries()) {
      ...
      let bounds=null;
      if(frustumCulling) {
        if(bytes>=maxBoundsBytes||components>=maxBoundsComponents)fail('Aggregate bounds limit exceeded');
        bounds=createAnimationBounds(pose,entry.geometry,{maxBytes:maxBoundsBytes-bytes,maxComponents:maxBoundsComponents-components});
        bytes+=bounds.byteLength;components+=bounds.sourceComponents;
      }
      records.push({mesh:entry.mesh,deformer:entry.deformer,alphaMode:entry.alphaMode,index,bounds});
    }
  }catch(error){for(const r of records)r.bounds?.dispose();throw error;}
  function live() {if(disposed)fail('Draw ordering has been disposed');}
  function synchronized(record) {
    if(pose.disposed||record.deformer.disposed||record.deformer.failed||
      record.deformer.poseVersion!==pose.version||record.bounds.snapshot.poseVersion!==pose.version)fail('Bounds and geometry need the same uploaded pose');
  }
  return Object.freeze({
    get boundsBytes(){return disposed?0:bytes;},get lastCulling(){return stats;},
    get viewProjection(){return cameraSnapshot;},
    updateBounds() {
    ...
    order(viewProjection) {
      ...
      try {
        ...
        if(!sortObjects)result=visible.map(record=>record.mesh);
          ...
          const opaque=visible.filter(record=>record.alphaMode!=='BLEND').map(record=>record.mesh);
          const sorted=visible.filter(record=>record.alphaMode==='BLEND').map(record=>{
    ...
    dispose(){if(busy)fail('Cannot dispose during ordering');if(!disposed){for(const record of records)record.bounds?.dispose();disposed=true;cameraSnapshot=null;stats=null;}},
  });
}
