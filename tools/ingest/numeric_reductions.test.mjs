...
for (const [ArrayType, type] of [
  ...
  test(`${type} dot products preserve the exact sequential f64 accumulation order`, () => {
    ...
    for (const n of [0, 1, 7, 8193, 100000]) {
      const a = ArrayType.from({ length: n }, (_, i) => ((i % 41) - 20) / 7),
        b = ArrayType.from({ length: n }, (_, i) => ((i % 29) - 14) / 13);
      assert.ok(Object.is(kernel.run(a, b), dot(a, b)));
    }
    assert.equal(kernel.manifest.version, 5);
...
test("cancellation-sensitive sums are never reassociated into pairwise or f32 reductions", () => {
  ...
  for (const values of [[1e16, 1, -1e16], [1e16, -1e16, 1], [1, 1e100, -1e100, 2], []]) {
    ...
    assert.ok(Object.is(kernel.run(a), mean(a)));
  }
...
test("conditional extrema and counters update mutable locals without converting booleans to numbers", () => {
  ...
  for (const values of [[], [-1, -2], [NaN, -Infinity, Infinity, 0, -0], [9, 2, 7, 1]]) {
    const a = new Float64Array(values);
    assert.ok(Object.is(kernel.run(a, Infinity), positiveMinimum(a, Infinity)));
  }
...
test("mutable locals reset per invocation and per-iteration locals reset per record", () => {
  ...
  assert.equal(kernel.run(new Float32Array(0)), 0);
  assert.equal(kernel.diagnostics.wasmCalls, 51);
});
...
test("scalar parameters are mutable by value and scalar ++/-- follow Number semantics", () => {
  ...
  for (const seed of [0, -0, NaN, Infinity, -Infinity, 1e-300, 16777216]) {
    ...
    assert.ok(Object.is(kernel.run(a, seed), recurrence(a, seed)));
  }
  assert.equal(kernel.diagnostics.fallbackCalls, 0);
});
...
test("in-place prefix scans publish f32 stores and return the unrounded f64 accumulator", () => {
  ...
  const a = new Float32Array([16777216, 1, 1, -16777216]),
    expected = a.slice();
  const result = scan(expected);
  assert.equal(kernel.run(a), result);
  assert.equal(result, 2);
  assert.deepEqual(a, expected);
  assert.deepEqual([...a], [16777216, 16777216, 16777218, 2]);
...
test("ordered array updates without a return keep the void result contract", () => {
  ...
  const kernel = compiled(scan, ["f64[]"]);
  const a = new Float64Array([1, 2, 3]);
  assert.equal(kernel.run(a), undefined);
  assert.deepEqual([...a], [1, 3, 6]);
  assert.equal(kernel.manifest.resultType, "void");
  assert.equal(kernel.manifest.iterationSemantics, "ordered");
});
...
test("geometry reductions combine vec3 records with small uniform arrays and a numeric return", () => {
  ...
  const a = Float32Array.from({ length: 30000 }, (_, i) => ((i % 47) - 23) / 19),
    center = new Float64Array([1, 2, 3]);
...
test("nested shadowing distinguishes const bindings, mutable bindings and loop-scope lifetimes", () => {
  ...
  const kernel = compiled(sum, ["f64[]"]);
  assert.equal(kernel.run(new Float64Array([1, 2, 3])), 6);
  const refused = [
  ...
  for (const body of refused)
    assert.throws(() =>
      compileNumericKernel(`function sum(a) { ${body} }`, { parameterTypes: ["f64[]"] }),
    );
});
...
test("uninitialized or undeclared scalar assignments never become Wasm zero-initialized bindings", () => {
  for (const body of [
    ...
    assert.throws(() =>
      compileNumericKernel(`function bad(a) { ${body} }`, { parameterTypes: ["f64[]"] }),
    );
});
...
test("mean length guards preserve getter effects and original fallback return values", () => {
  ...
  assert.equal(kernel.run(a), 2);
  assert.equal(reads, 5);
  assert.equal(kernel.diagnostics.fallbackCalls, 1);
  function firstOrMinimum(a) {
    let minimum = a[0];
    for (let i = 0; i < a.length; i++) if (a[i] < minimum) minimum = a[i];
    return minimum;
  }
  const first = compiled(firstOrMinimum, ["f64[]"]);
  assert.equal(first.run(new Float64Array(0)), undefined);
...
test("fallback keeps nonnumeric JS returns and exceptions exactly, without retrying", () => {
  ...
  assert.equal(kernel.run([], []), sentinel);
  assert.equal(calls, 1);
  const throwing = instantiateNumericKernel(artifact.wasm, {
    fallback() {
      calls++;
      throw sentinel;
    },
...
test("write/read aliasing still falls back before publication when a reduction mutates arrays", () => {
  ...
  assert.equal(kernel.run(actual.subarray(1), actual.subarray(0, 3)), reference);
...
test("ordered result manifests reject missing, unsupported or reordered contracts", (t) => {
  const artifact = compileNumericKernel(dot.toString(), { parameterTypes: ["f64[]", "f64[]"] });
  const native = WebAssembly.Module.customSections;
  t.after(() => {
    WebAssembly.Module.customSections = native;
  });
  for (const change of [
    (m) => {
      delete m.resultType;
    ...
    (m) => {
      delete m.iterationSemantics;
    },
    (m) => {
      m.iterationSemantics = "parallel";
    },
  ]) {
    const invalid = structuredClone(artifact.manifest);
    change(invalid);
...
test("automatic specialization returns native results, including nested calls and mixed uniform storage", async (t) => {
  ...
  WebAssembly.Instance = (...args) => {
    ...
    return {
      exports: {
        ...
        run(...xs) {
          calls++;
          return instance.exports.run(...xs);
        },
      },
    };
  };
  function weighted(a, m) {
    const scale = m[0];
    let total = 0;
    for (let i = 0; i < a.length; i++) total += a[i] * scale;
    return total;
  }
  const source = `${dot.toString()}\n${weighted.toString()}\nexport const evaluate=(a,m)=>dot(a,a)+weighted(a,m);`;
  const result = specializeNumericModule(source);
  assert.equal(result.report.compiledKernels, 2);
  assert.ok(
    result.report.candidates.every(
      (item) => item.resultType === "f64" && item.iterationSemantics === "ordered",
    ),
  );
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "f3d-reduction-"));
  for (const name of ["numeric_dispatch.mjs", "numeric_kernel_runtime.mjs"])
  ...
  assert.equal(module.evaluate(new Float32Array([1, 2, 3]), new Float64Array([2])), 26);
  assert.equal(calls, 2);
});

test("dispatcher preserves f64 results and reports native execution", () => {
  const artifact = compileNumericKernel(dot.toString(), { parameterTypes: ["f64[]", "f64[]"] });
  const token = createNumericDispatch(dot, artifact.wasm),
...
test("relocatable standalone packages return scalars in native and Wasm-unavailable hosts", async (t) => {
  ...
  const entry = path.join(dir, "mean.mjs");
  fs.writeFileSync(entry, mean.toString());
  const result = buildNumericKernel(entry, path.join(dir, "package"), {
    parameterTypes: ["f32[]"],
  });
  const relocated = path.join(dir, "moved");
  fs.cpSync(result.outDir, relocated, { recursive: true });
  ...
  const retained = module.createKernel();
  assert.equal(retained.run(a), 2);
