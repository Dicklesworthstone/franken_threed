...
const fail = (code, message) => {
  throw new AnimationPoseError("GLTF_INSTANCING_" + code, message);
};
...
function indexed(values, index, label) {
  list(values, label);
  integer(index, label, 0, values.length - 1);
 ...
 */
export function expandGltfInstances(
  model,
  suppliedBuffers,
  ...
  function charge(count) {
    ...
    if (count > maxComponents - expandedComponents)
      fail("LIMIT", "Expanded instance data exceeds component budget");
  ...
  for (let node = 0; node < original.length; node++) {
    ...
    if (Object.keys(object(extension.extensions ?? {}, "extensions")).length)
      fail("UNSUPPORTED", "Extended instance behavior needs the source route");
    const mesh = indexed(model.meshes, source.mesh, "instanced mesh");
    if (source.skin !== undefined)
      fail("UNSUPPORTED", "Skinned instancing needs the source skinning route");
    const attributes = object(extension.attributes, "instance attributes"),
    ...
    for (const [semantic, index] of entries) {
      if (!["TRANSLATION", "ROTATION", "SCALE"].includes(semantic))
        fail("UNSUPPORTED", `Instance attribute needs the source route: ${semantic}`);
      const a = indexed(model.accessors, index, "instance accessor");
      integer(a.count, "instance count", 1, maxInstances);
      if (count !== undefined && count !== a.count)
        fail("COUNT", "All instance attributes must have the same count");
      count = a.count;
      const rotation = semantic === "ROTATION";
      if (
        a.type !== (rotation ? "VEC4" : "VEC3") ||
        !(a.componentType === 5126
          ? a.normalized === undefined || a.normalized === false
    ...
    if (count > maxInstances - instanceCount || count > 65536 - original.length - instanceCount)
      fail("LIMIT", "Aggregate instance/pose-node budget exceeded");
    ...
    if (
      width > 4096 ||
      primitives.some((p) => list(p?.targets ?? [], "morph targets").length !== width)
    )
      fail("SHAPE", "Inconsistent or excessive instance morph targets");
    if (source.weights !== undefined) {
      const weights = list(source.weights, "node morph weights");
      if (
        weights.length !== width ||
        weights.some((v) => typeof v !== "number" || !Number.isFinite(v))
      )
        fail("SHAPE", "Invalid instance morph weights");
    }
    charge(count * (10 + width));
    const first = original.length + instanceCount;
    const plan = { node, source, count, first, entries };
    plans.push(plan);
    byNode.set(node, plan);
    instanceCount += count;
  }
  const declarations = {};
  for (const key of ["extensionsUsed", "extensionsRequired"])
    if (model[key] !== undefined) {
      ...
      if (names.some((name) => typeof name !== "string"))
        fail("SHAPE", "Extension names must be strings");
      if (names.includes(EXT)) declarations[key] = names.filter((name) => name !== EXT);
    }
  if (!plans.length)
    return Object.freeze({
      json: Object.keys(declarations).length ? { ...model, ...declarations } : model,
      instanceOrigins: Object.freeze({}),
      instanceCount: 0,
      accessorComponents: 0,
      expandedComponents: 0,
    });
  ...
  for (const node of original)
    for (const child of list(node.children ?? [], "children")) sourceNode(child);
  for (const scene of list(model.scenes ?? [], "scenes"))
    for (const root of list(scene.nodes ?? [], "scene roots")) sourceNode(root);
  ...
  const animationPlans = animations.map((animation) => {
    ...
    for (const channel of channels) {
      ...
      integer(times.count, "animation input count", 1);
      integer(values.count, "animation output count", 1);
      if (times.type !== "SCALAR" || values.type !== "SCALAR")
        fail("ATTRIBUTE", "Morph channels require scalar accessors");
  ...
  for (const plan of plans) {
    plan.attributes = Object.fromEntries(
      plan.entries.map(([semantic, index]) => [semantic, reader.read(index)]),
    );
    const rotation = plan.attributes.ROTATION;
    if (rotation)
      for (let i = 0; i < plan.count; i++) {
        const q = rotation.values.subarray(i * 4, i * 4 + 4),
          norm = Math.hypot(...q);
        ...
        if (!(norm > 0) || Math.abs(norm - 1) > (rotation.componentType === 5126 ? 1e-3 : 0.01))
          fail("ROTATION", "Instance quaternion must be normalized");
      }
  }
  ...
  for (const plan of plans) {
    ...
    const parent = { ...source, extensions, children };
    delete parent.mesh;
    delete parent.weights;
    nodes[node] = parent;
    for (let i = 0; i < count; i++) {
      const child = { mesh: source.mesh };
      for (const [semantic, field, width] of [
  ...
  if (animationPlans.some((p) => p.changed))
    json.animations = animationPlans.map(({ animation, channels, changed }) => {
      if (!changed) return animation;
      const output = [];
      for (const channel of channels) {
        const target = channel.target,
          plan = target?.path === "weights" ? byNode.get(target.node) : undefined;
        if (!plan) output.push(channel);
        else
          for (let i = 0; i < plan.count; i++)
            output.push({ ...channel, target: { ...target, node: plan.first + i } });
      }
      return { ...animation, channels: output };
    });
  return Object.freeze({
    json,
    instanceOrigins: Object.freeze(origins),
    instanceCount,
