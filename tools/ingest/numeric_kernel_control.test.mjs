...
test("conditional write-only arrays preserve skipped elements across reused Wasm memory", () => {
  ...
  const compiled = compileNumericKernel(selective.toString(), {
  ...
  const out = new Float64Array([1, 2, 3, 4]),
    mask = new Float64Array([1, 0, 1, 0]);
...
test("strict numeric comparisons preserve ordered and unordered IEEE semantics", () => {
  ...
  runComparison(
    ...
    [
      ...
      new Float64Array([-0, 0, NaN, Infinity, Infinity, -1, 1, 1]),
      new Float64Array(8),
    ],
  );
});
...
test("conditional and logical lowering are lazy in the actual Wasm engine", () => {
  ...
  for (const [predicate, initial] of [
    ["x[i] > 0 && y[i] > 0", 0],
    ["x[i] > 0 || y[i] > 0", 1],
  ]) {
...
test("refuses effects in untaken branches, boolean-as-number confusion, and invalid scopes", async (t) => {
  for (const body of [
    ...
    await t.test(body, () => {
      assert.throws(
        () =>
          compileNumericKernel(`function f(x) { for(let i=0;i<x.length;i++) { ${body} } }`, {
            parameterTypes: ["f64[]"],
          }),
