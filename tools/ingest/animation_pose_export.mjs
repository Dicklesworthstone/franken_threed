...
const fail = (code, message) => {
  throw new AnimationExportError("ANIMATION_EXPORT_" + code, message);
};
...
function array(value, count, label) {
  ...
  if (ArrayBuffer.isView(value)) {
    if (!(value.buffer instanceof ArrayBuffer) || value.buffer.resizable)
      fail("STORAGE", `${label} needs fixed unshared storage`);
    try {
...
function vector(value, count, label, min = -Infinity, max = Infinity) {
  return Array.from(array(value, count, label), (n) => {
    finite(n, label);
    if (n < min || n > max) fail("VALUE", `Invalid ${label}`);
    return n;
  });
}
...
export async function exportAnimationPoseGLB(pose, entries, options = {}) {
  fields(
    options,
    ["resolveTexture", "signal", "maxBytes", "maxVertices", "copyright", "sceneView"],
    "export option",
  );
  const {
    resolveTexture,
  ...
  integer(maxBytes, 64, 0xffffffff, "GLB byte limit");
  integer(maxVertices, 1, 0xfffffffe, "vertex limit");
  if (resolveTexture !== undefined && typeof resolveTexture !== "function")
    fail("TEXTURE", "resolveTexture must be a function");
  checkAbort(signal);
  if (!pose || pose.disposed || !Number.isSafeInteger(pose.version) || pose.version < 0)
    fail("POSE", "A live animation pose is required");
  const poseVersion = pose.version;
  if (!Array.isArray(entries) || entries.length < 1 || entries.length > 4096)
    fail("LIMIT", "Expected 1..4096 drawables");
  const json = {
    asset: { version: "2.0", generator: "FrankenThreeD static pose export" },
    scene: 0,
    scenes: [{ nodes: [] }],
    nodes: [],
    meshes: [],
  ...
  function requireExtension(name) {
    for (const key of ["extensionsUsed", "extensionsRequired"]) {
      json[key] ??= [];
      if (!json[key].includes(name)) json[key].push(name);
    }
  }
  if (options.copyright !== undefined) {
    if (typeof options.copyright !== "string" || options.copyright.length > maxBytes / 4)
      fail("LIMIT", "Invalid or excessive copyright text");
  ...
  function reserve(bytes) {
    if (
      ...
      bytes < 1 ||
      aligned(byteLength) + aligned(bytes) + 28 > maxBytes
    )
      fail("LIMIT", "Binary data exceeds GLB byte budget");
  }
  ...
  function floats(count, width, read, bounds = false) {
    const size = count * width * 4;
    reserve(size);
    const bytes = new Uint8Array(size),
      view = new DataView(bytes.buffer);
    const min = Array(width).fill(Infinity),
      max = Array(width).fill(-Infinity);
  ...
  function textureInfo(descriptor, field) {
    ...
    if (!view || typeof view !== "object" || !sampler || typeof sampler !== "object")
      fail("TEXTURE", "Texture needs a borrowed view and sampler");
    ...
    if (!keys.has(sampler)) {
      keys.set(sampler, textures.length);
      textures.push(Object.freeze({ view, sampler }));
      textureUses.push(new Set());
    }
    const index = keys.get(sampler);
    textureUses[index].add(
      field === "baseColorTexture" || field === "emissiveTexture" ? "srgb" : "linear",
    );
    return { index };
  }
  ...
  for (const entry of entries) {
    checkAbort(signal);
    fields(entry, ["deformer", "drawable", "source"], "export entry");
    const { deformer: d, drawable: material, source = {} } = entry;
    if (!d || d.disposed || d.poseVersion !== poseVersion)
      fail("STALE", "Deformation must match the current pose");
    const count = integer(d.vertexCount, 1, maxVertices, "vertex count");
    vertices += count;
    if (vertices > maxVertices) fail("LIMIT", "Aggregate vertex limit exceeded");
    fields(
      material,
      [
        "geometry",
    ...
    if (m[3] !== 0 || m[7] !== 0 || m[11] !== 0 || m[15] !== 1)
      fail("GEOMETRY", "World matrix must be affine");
    ...
    if ((normals || tangents) && det === 0)
      fail("GEOMETRY", "Cannot export normals through a singular world transform");
    ...
    const attributes = {
      POSITION: floats(
        ...
        3,
        (i) => {
          const x = finite(positions[i * 3], "position"),
            y = finite(positions[i * 3 + 1], "position"),
            z = finite(positions[i * 3 + 2], "position");
          return [
    ...
    function normal(i) {
      const x = finite(normals[i * 3], "normal"),
        y = finite(normals[i * 3 + 1], "normal"),
        z = finite(normals[i * 3 + 2], "normal"),
        n = normalMatrix;
      return unit(
        n[0] * x + n[3] * y + n[6] * z,
        n[1] * x + n[4] * y + n[7] * z,
    ...
    if (tangents)
      attributes.TANGENT = floats(count, 4, (i) => {
        const a = i * 4,
          x = finite(tangents[a], "tangent"),
          y = finite(tangents[a + 1], "tangent"),
          z = finite(tangents[a + 2], "tangent");
        ...
        return [
          ...
          tangents[a + 3] * (det < 0 ? -1 : 1),
        ];
      });
    const primitive = { attributes, mode: 4, material: json.materials.length };
    const sourceIndices = material.indices;
    const indexCount =
      sourceIndices == null ? count : integer(sourceIndices.length, 1, maxBytes / 2, "index count");
    ...
    if (sourceIndices != null || det < 0) {
      ...
      for (let i = 0; i < indexCount; i++)
        max = Math.max(
          max,
          integer(sourceIndices == null ? i : sourceIndices[i], 0, count - 1, "vertex index"),
        );
      // Maximum representable indices are forbidden primitive-restart values.
      const width = max < 65535 ? 2 : 4;
      reserve(indexCount * width);
      const data = new Uint8Array(indexCount * width),
        view = new DataView(data.buffer);
      for (let i = 0; i < indexCount; i++) {
        const at = det < 0 && i % 3 !== 0 ? i + (i % 3 === 1 ? 1 : -1) : i;
        view[width === 2 ? "setUint16" : "setUint32"](
          i * width,
          sourceIndices == null ? at : sourceIndices[at],
          true,
        );
      }
      primitive.indices =
        json.accessors.push({
          bufferView: append(data, 34963),
    ...
    if (!["unlit", "metallic-roughness"].includes(shading))
      fail("UNSUPPORTED", "This material has no lossless core glTF representation");
    const unlit = shading === "unlit",
      rgba = vector(material.baseColor ?? [1, 1, 1, 1], 4, "base color", 0, 1);
    const out = {
    ...
    if (
      !["OPAQUE", "MASK", "BLEND"].includes(out.alphaMode) ||
      typeof out.doubleSided !== "boolean"
    )
      fail("VALUE", "Invalid alpha mode or side");
    const cutoff = vector([material.alphaCutoff ?? 0.5], 1, "alpha cutoff", 0, 1)[0];
    if (out.alphaMode === "MASK") out.alphaCutoff = cutoff;
    if (unlit) {
      if (MAPS.slice(1).some((f) => material[f] != null))
        fail("UNSUPPORTED", "Unlit export cannot silently discard lit maps");
      out.extensions = { KHR_materials_unlit: {} };
      requireExtension("KHR_materials_unlit");
    } else {
      out.pbrMetallicRoughness.metallicFactor = vector(
      ...
      out.pbrMetallicRoughness.roughnessFactor = vector(
      ...
      if (emission.some((value) => !Number.isFinite(Math.fround(value))))
        fail("VALUE", "Emission exceeds the renderer Float32 range");
    ...
    const coated =
      COAT_FIELDS.some((key) => material[key] !== undefined) ||
      COAT_MAPS.some((key) => material[key] != null);
    let coating;
    if (coated) {
      if (unlit) fail("UNSUPPORTED", "Unlit export cannot preserve clearcoat");
      if (material.clearcoatNormalScale !== undefined && material.clearcoatNormalTexture == null)
        fail("SHAPE", "Clearcoat normal scale needs its normal map");
      coating = {
        clearcoatFactor: vector(
          [material.clearcoatFactor === undefined ? 0 : material.clearcoatFactor],
          1,
          "clearcoat factor",
          0,
          1,
        )[0],
        clearcoatRoughnessFactor: vector(
          [material.clearcoatRoughnessFactor === undefined ? 0 : material.clearcoatRoughnessFactor],
          1,
          "clearcoat roughness",
          0,
          1,
      ...
      out.extensions ??= {};
      out.extensions.KHR_materials_clearcoat = coating;
    ...
    if (material.vertexColors != null) {
      ...
      attributes.COLOR_0 = floats(count, width, (i) =>
        vector(
          Array.from({ length: width }, (_, c) => values[i * width + c]),
          width,
          "vertex color",
          0,
          1,
    ...
    const overrides = material.mapCoordinates ?? {};
    fields(overrides, MAPS, "map coordinates");
    for (const key of Object.keys(overrides))
      if (material[key] == null) fail("SHAPE", "Coordinate override needs a texture");
    const shared = vector(material.uvTransform ?? IDENTITY_UV, 6, "shared UV transform");
    let channel = 0;
    for (const field of MAPS)
      if (material[field] != null) {
        const info = textureInfo(material[field], field),
          override = overrides[field] ?? {};
        fields(override, ["texCoords", "uvTransform"], "map coordinate");
        const values = array(
          override.texCoords === undefined ? material.texCoords : override.texCoords,
          count * 2,
          "texture coordinates",
        );
        const local = vector(override.uvTransform ?? IDENTITY_UV, 6, "local UV transform");
        attributes["TEXCOORD_" + channel] = floats(count, 2, (i) => {
          const x = finite(values[i * 2], "UV"),
            y = finite(values[i * 2 + 1], "UV");
          const u = local[0] * x + local[2] * y + local[4],
            v = local[1] * x + local[3] * y + local[5];
          return [
            shared[0] * u + shared[2] * v + shared[4],
            shared[1] * u + shared[3] * v + shared[5],
          ];
        });
        info.texCoord = channel++;
        if (field === "normalTexture")
          info.scale = finite(material.normalScale ?? 1, "normal scale");
        if (field === "occlusionTexture")
          info.strength = vector(
            [material.occlusionStrength === undefined ? 1 : material.occlusionStrength],
            1,
            "occlusion strength",
            0,
            1,
        ...
        if (field === "clearcoatNormalTexture") {
          info.scale = finite(
            material.clearcoatNormalScale === undefined ? 1 : material.clearcoatNormalScale,
            "clearcoat normal scale",
          );
          if (!Number.isFinite(Math.fround(info.scale)))
            fail("VALUE", "Clearcoat normal scale exceeds the renderer Float32 range");
        }
        (COAT_MAPS.includes(field)
          ? coating
          : field === "baseColorTexture" || field === "metallicRoughnessTexture"
    ...
    for (const [key, value] of Object.entries(source))
      ids[key] = value === null ? null : integer(value, 0, Number.MAX_SAFE_INTEGER, "source ID");
  ...
  if (options.sceneView != null) {
    ...
    function instance(source) {
      if (!instances.has(source.node)) {
        const node = {
          extras: { f3dSource: { node: source.node } },
          ...(source.nodeName ? { name: source.nodeName } : {}),
        };
        json.scenes[0].nodes.push(json.nodes.length);
        json.nodes.push(node);
        instances.set(source.node, node);
      }
    ...
    for (const camera of view.cameras) {
      ...
      const frame = view.sample({ cameraNode: camera.node, aspectRatio: 1 }),
        v = frame.viewMatrix;
      const node = instance(camera),
        description = {
          type: camera.type,
      ...
      json.cameras ??= [];
      if (!cameras.has(key)) {
        cameras.set(key, json.cameras.length);
        json.cameras.push(description);
      }
      node.camera = cameras.get(key);
      node.extras.f3dSource.camera = camera.camera;
    ...
    if (evaluated.length) {
      ...
      for (const key of ["extensionsUsed", "extensionsRequired"]) {
        json[key] ??= [];
        if (!json[key].includes("KHR_lights_punctual")) json[key].push("KHR_lights_punctual");
      }
    }
    for (let i = 0; i < view.lights.length; i++) {
      const source = view.lights[i],
        value = evaluated[i],
      ...
      const description = {
        ...
        ...(value.type === "spot"
          ? { spot: { innerConeAngle: value.innerConeAngle, outerConeAngle: value.outerConeAngle } }
          : {}),
      };
      const key = JSON.stringify(description),
        output = json.extensions.KHR_lights_punctual.lights;
      if (!lights.has(key)) {
        lights.set(key, output.length);
        output.push(description);
      }
      node.extensions = { KHR_lights_punctual: { light: lights.get(key) } };
      node.extras.f3dSource.light = source.light;
      if (!node.matrix) {
        ...
        node.translation = Array.from(
          pose.worldMatrices.subarray(source.node * 16 + 12, source.node * 16 + 15),
        );
        if (value.direction) {
          const [x, y, z] = value.direction.map((n) => -n),
            axis = Math.hypot(x, y),
            angle = Math.atan2(axis, z);
          const sine = Math.sin(angle / 2);
          node.rotation =
            axis > 0
              ? [(-y / axis) * sine, (x / axis) * sine, 0, Math.cos(angle / 2)]
              : z < 0
                ? [0, 1, 0, 0]
                : [0, 0, 0, 1];
        }
  ...
  if (
    pose.disposed ||
    pose.version !== poseVersion ||
    entries.some((e) => e.deformer.disposed || e.deformer.poseVersion !== poseVersion)
  )
    fail("STALE", "Pose changed while capturing export");
  ...
  if (textures.length) {
    ...
    for (const [textureIndex, descriptor] of textures.entries()) {
      ...
      if (
        ...
        !["image/png", "image/jpeg", "image/ktx2"].includes(mimeType)
      )
        fail("TEXTURE", "Supply encoded PNG/JPEG or BasisU KTX2 bytes");
      ...
      if ((mimeType === "image/png" && !png) || (mimeType === "image/jpeg" && !jpeg))
        fail("TEXTURE", "MIME type disagrees with encoded image");
      if (mimeType === "image/ktx2") {
        ...
        const header = inspectGltfKtx2(bytes, {
          maxBytes,
          maxImagePixels: 0xffffffff,
          maxDimension: 0xffffffff,
        });
        const dfd = new DataView(bytes.buffer, bytes.byteOffset, bytes.byteLength).getUint32(
          48,
          true,
        );
        const primaries = bytes[dfd + 13];
        if (
          colorSpaces.some((space) => space !== header.colorSpace) ||
          primaries !== (header.colorSpace === "srgb" ? 1 : 0)
        ) {
          fail("TEXTURE", "KTX2 color metadata disagrees with a material use");
        }
        for (const key of ["extensionsUsed", "extensionsRequired"]) {
          json[key] ??= [];
          if (!json[key].includes("KHR_texture_basisu")) json[key].push("KHR_texture_basisu");
        }
      }
      fields(sampler, ["wrapS", "wrapT", "magFilter", "minFilter"], "sampler");
      const s = { wrapS: sampler.wrapS ?? 10497, wrapT: sampler.wrapT ?? 10497 };
      if (![33071, 33648, 10497].includes(s.wrapS) || ![33071, 33648, 10497].includes(s.wrapT))
        fail("TEXTURE", "Invalid wrapping");
      for (const [key, choices] of [
        ["magFilter", [9728, 9729]],
        ["minFilter", [9728, 9729, 9984, 9985, 9986, 9987]],
      ]) {
        if (sampler[key] !== undefined) {
          if (!choices.includes(sampler[key])) fail("TEXTURE", "Invalid filtering");
          s[key] = sampler[key];
        }
      }
      const key = JSON.stringify(s);
      if (!samplers.has(key)) {
        samplers.set(key, json.samplers.length);
        json.samplers.push(s);
      }
      ...
      if (!image) {
        ...
        image = { index: json.images.length, mimeType, bytes: snapshot };
        versions.push(image);
      ...
      json.textures.push({
        ...
        ...(mimeType === "image/ktx2"
          ? { extensions: { KHR_texture_basisu: { source: image.index } } }
          : { source: image.index }),
      });
    }
  }
  ...
  const text = new TextEncoder().encode(JSON.stringify(json)),
    jsonBytes = aligned(text.length),
    binBytes = aligned(byteLength);
  ...
  header.setUint32(12, jsonBytes, true);
  header.setUint32(16, 0x4e4f534a, true);
  output.fill(32, 20, 20 + jsonBytes);
  output.set(text, 20);
  header.setUint32(20 + jsonBytes, binBytes, true);
  header.setUint32(24 + jsonBytes, 0x004e4942, true);
