...
function fixture() {
  ...
  const buffers = [],
    attributes = model.meshes[0].primitives[0].attributes;
...
const decode = (f, options = {}) =>
  decodeGltfAnimationModel(f.model, f.buffers, { resolveTexture: resources, ...options });
const close = (actual, expected) => {
  assert.equal(actual.length, expected.length);
  actual.forEach((v, i) =>
    assert.ok(Math.abs(v - expected[i]) < 1e-12, `${i}: ${v} != ${expected[i]}`),
  );
};
...
test("occlusion imports as a linear map, preserving zero, fractional and default strength", () => {
  for (const strength of [undefined, 0, 0.25, 1]) {
    ...
    assert.deepEqual([...d.texCoords], UV0);
    assert.equal(requests.length, 1);
    assert.equal(requests[0].colorSpace, "linear");
    assert.deepEqual(requests[0].image, { uri: "orm.png" });
    assert.ok(Object.isFrozen(requests[0]));
    assert.ok(Object.isFrozen(d.occlusionTexture));
  }
});
...
test("packed ORM shares one resolution with metallic-roughness but not sRGB material uses", () => {
  const f = fixture(),
    requests = [];
  f.material.pbrMetallicRoughness = {
    baseColorTexture: { index: 0 },
    metallicRoughnessTexture: { index: 0 },
  };
  f.material.normalTexture = { index: 0 };
  ...
  assert.equal(d.mapCoordinates, undefined);
  assert.equal(d.normalScale, 1);
});
...
test("occlusion keeps its own UV set and KHR_texture_transform independently of the packed MR texture", () => {
  ...
  assert.deepEqual(d.uvTransform, [1, 0, 0, 1, 0, 0]);
...
test("an occlusion-only material uses its authored shared transform without requiring TEXCOORD_0", () => {
  const f = fixture();
  delete f.attributes.TEXCOORD_0;
  f.material.occlusionTexture = {
    index: 0,
    texCoord: 1,
  ...
  assert.deepEqual([...d.texCoords], UV1);
  close(d.uvTransform, [1, 0, 0, 1, 0.5, 0.25]);
  assert.equal(d.mapCoordinates, undefined);
});

test("all occlusion metadata and coordinates are snapshotted before texture resolution", () => {
  const f = fixture();
  f.material.occlusionTexture.strength = 0.25;
  const prepared = prepareGltfAnimationModel(f.model, f.buffers);
  f.material.occlusionTexture.strength = 1;
  f.model.images[0].uri = "changed.png";
  for (const b of f.buffers) b.fill(99);
  const d = prepared.resolveTextures((r) => {
    assert.equal(r.image.uri, "orm.png");
    return resources();
  }).drawables[0];
  assert.equal(d.occlusionStrength, 0.25);
  assert.deepEqual([...d.texCoords], UV0);
});
...
for (const strength of [-0.1, 1.1, NaN, Infinity, -Infinity, "1", null])
  test(`invalid occlusion strength ${String(strength)} fails before any texture resolver`, () => {
    const f = fixture();
    f.material.occlusionTexture.strength = strength;
    let calls = 0;
    assert.throws(
      () =>
        decode(f, {
...
for (const [name, mutate] of [
  [
    "missing UV",
    (f) => {
      f.material.occlusionTexture.texCoord = 7;
    },
  ],
  [
    "bad transform",
    (f) => {
      f.material.occlusionTexture.extensions = { KHR_texture_transform: { scale: [NaN, 1] } };
    },
  ],
  [
    "unknown field",
    (f) => {
      f.material.occlusionTexture.scale = 2;
    },
  ],
  [
    "unknown extension",
    (f) => {
      f.material.occlusionTexture.extensions = { EXT_unknown: {} };
    },
  ],
  [
    "missing texture",
    (f) => {
      f.material.occlusionTexture.index = 9;
    },
  ],
])
...
test("unlit materials ignore occlusion fallback data without requiring UVs or image resolution", () => {
  ...
  f.material.occlusionTexture = { index: 999, strength: -99 };
  delete f.attributes.TEXCOORD_0;
  delete f.attributes.TEXCOORD_1;
  const d = decode(f, { resolveTexture: () => assert.fail("ignored lit texture was requested") })
    .drawables[0];
  assert.equal(d.shading, "unlit");
  assert.equal(d.occlusionTexture, undefined);
  assert.equal(d.occlusionStrength, undefined);
});

test("BasisU occlusion follows explicit route selection and keeps a linear request", () => {
  for (const basisu of [false, true]) {
    const f = fixture();
    f.model.images.push({ uri: "orm.ktx2", mimeType: "image/ktx2" });
...
test("occlusion resolver failure can be retried without reparsing or publishing a placeholder", () => {
  const f = fixture(),
    ...
    error = new Error("image unavailable");
  assert.throws(
    () =>
  ...
  const borrowed = resources(),
    d = prepared.resolveTextures(() => borrowed).drawables[0];
  assert.equal(d.occlusionTexture.view, borrowed.view);
  assert.equal(d.occlusionTexture.sampler, borrowed.sampler);
...
test("materials without occlusion keep the previous drawable shape", () => {
  const f = fixture();
  delete f.material.occlusionTexture;
  ...
  for (const key of [
    ...
    assert.equal(Object.hasOwn(d, key), false);
});
