...
const source=path.join(root,'source'),output=path.join(root,'export','app.html');
const files={
  'index.html':'<!doctype html><meta charset="utf-8"><title>Packed off-thread jobs</title><script type="module" src="app.mjs"></script>',
  ...
  'jobs/add.wasm':Buffer.from([0,97,115,109,1,0,0,0,1,7,1,96,2,127,127,1,127,3,2,1,0,7,7,1,3,97,100,100,0,0,10,9,1,7,0,32,0,32,1,106,11]),
  'jobs/model.gltf':'{"asset":{"version":"2.0"},"buffers":[{"uri":"geometry.bin","byteLength":12}]}',
...
for(const [name,data]of Object.entries(files)){const dest=path.join(source,name);fs.mkdirSync(path.dirname(dest),{recursive:true});fs.writeFileSync(dest,data);}
...
try{
  const address=await new Promise((resolve,reject)=>{
    let log='';timer=setTimeout(()=>reject(new Error('Browser did not start: '+log)),10000);
    child.on('error',reject);child.stderr.on('data',bytes=>{log+=bytes;const m=/DevTools listening on (ws:\/\/[^\s]+)/.exec(log);if(m){clearTimeout(timer);resolve(m[1]);}});
    child.on('exit',code=>reject(new Error('Browser exited: '+code+'\n'+log)));
  });
  socket=new WebSocket(address);
  await new Promise((resolve,reject)=>{socket.addEventListener('open',resolve,{once:true});socket.addEventListener('error',reject,{once:true});});
  let sequence=0,workerTargets=0;const pending=new Map(),network=[],errors=[],attachments=[];
  function command(method,params={},sessionId){const id=++sequence;return new Promise((resolve,reject)=>{pending.set(id,{resolve,reject});socket.send(JSON.stringify({id,method,params,sessionId}));});}
  socket.addEventListener('message',event=>{
    const m=JSON.parse(event.data);
    if(m.id){const request=pending.get(m.id);if(!request)return;pending.delete(m.id);if(m.error)request.reject(new Error(JSON.stringify(m.error)));else request.resolve(m.result);}
      ...
      workerTargets++;const session=m.params.sessionId;
      attachments.push((async()=>{await command('Network.enable',{},session);
        ...
        await command('Runtime.runIfWaitingForDebugger',{},session);})().catch(error=>errors.push(String(error))));
    }
  });
  ...
  for(let tries=0;tries<200;tries++){
    const response=await command('Runtime.evaluate',{expression:'({result:globalThis.result,failure:globalThis.failure})',returnByValue:true},sessionId);
    actual=response.result.value?.result;failure=response.result.value?.failure;if(actual||failure||errors.length)break;
    await new Promise(resolve=>setTimeout(resolve,50));
  }
  await Promise.all(attachments);
  assert.equal(failure,undefined);assert.deepEqual(errors,[]);assert.ok(actual,'Worker application did not complete');
  assert.equal(actual.native,true);assert.equal(actual.terminated,true);assert.match(actual.error,/expected packed worker error/);
  assert.deepEqual(actual.events,['name','name','name']);
  for(const [index,result]of [...actual.results,actual.restarted].entries()){
    assert.equal(result.detached,true);assert.equal(result.correct,true);assert.equal(result.length,30000);
    assert.equal(result.wasm,42);assert.equal(result.config,42);assert.equal(result.name,'geometry');
    assert.deepEqual(result.order,['math','decoder','entry-relative','math']);assert.equal(result.loads,2);
    assert.deepEqual(result.port,{id:index+1,name:'geometry'});assert.deepEqual(result.geometry,[1,2,3]);
    assert.equal(result.state,index===2?2:1);assert.equal(result.left,index===2?2:1);assert.equal(result.right,9);
  }
  assert.equal(workerTargets,5);
  const externalRequests=network.filter(url=>!url.startsWith('data:')&&!url.startsWith('blob:')&&url!=='about:blank');
  assert.deepEqual(externalRequests,[]);
  console.log(JSON.stringify({browser:spawnSync(browser,['--version'],{encoding:'utf8'}).stdout.trim(),
    transport:'document injection; page offline; DNS disabled; worker network observed; source relocated',workerTargets,
    externalRequests,result:actual,...report},null,2));
