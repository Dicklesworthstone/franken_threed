...
async function application(t, source, files = {}) {
  ...
  const result = specializeNumericModule(source, {
    sourceName: "app.mjs",
    runtimeModule: runtimeUrl,
  });
  const tokens = [...result.code.matchAll(/var (__f3d_numeric_token_\d+) =/g)].map(
    (match) => match[1],
  );
...
const rootSource =
  "export function update(a) { for(let i=0;i<a.length;i++) a[i]=Math.sqrt(a[i]); }";
...
for (const name of [
  ...
  test(`ordinary application calls discover Math.${name} and select f32/f64 native variants`, async (t) => {
    ...
    const { module, result } = await application(
      t,
...
for (const Storage of [Float32Array, Float64Array])
  for (const Uniform of [Float32Array, Float64Array]) {
    test(`automatic 10k-vertex + 40k-color pipeline: ${Storage.name} / ${Uniform.name}`, async (t) => {
      ...
      assert.equal(candidate.variants.length, 4);
      assert.deepEqual(candidate.mathIntrinsics, ["fround", "max", "min", "round", "sqrt"]);
      assert.deepEqual(
        candidate.scalarHelpers.map((helper) => helper.name),
        ["magnitude", "quantize"],
      );
      const p = new Storage(30000),
        c = new Storage(40000),
        u = new Uniform([1.1, 0.9, 1.2]);
      ...
      for (let frame = 0; frame < 30; frame++) {
        ...
        assert.ok(Object.is(module.frame(p, c, u, 1.013), expected));
        same(p, ep);
        same(c, ec);
        same(u, eu);
      }
      const stats = module.__stats()[0];
      assert.equal(stats.kernel.wasmCalls, 30);
      assert.equal(stats.kernel.fallbackCalls, 0);
      assert.equal(
        stats.kernel.copiedBytes,
        30 * (p.byteLength * 2 + c.byteLength * 2 + u.byteLength),
      );
...
test("live module-local Math identity follows replacement and restoration", async (t) => {
  ...
  module.run(a);
  same(a, [2, 3]);
  module.setMath({ sqrt: (x) => x + 7 });
  module.run(a);
  same(a, [9, 10]);
  assert.equal(module.__stats()[0].kernel.lastGuardFailure, "KERNEL_MATH_BINDING");
  module.setMath(Math);
  module.run(a);
  same(a, [3, Math.sqrt(10)]);
  assert.equal(module.__stats()[0].kernel.wasmCalls, 2);
  assert.equal(module.__stats()[0].kernel.fallbackCalls, 1);
});

test("a renamed import named Math uses the original module binding, not the runtime global", async (t) => {
  const { module } = await application(
    t,
    `import {methods as Math} from './methods.mjs';\n${rootApp}`,
    { "methods.mjs": "export const methods={sqrt:x=>x+40};" },
  );
  const a = new Float64Array([4, 9]);
  module.run(a);
  same(a, [44, 49]);
  assert.equal(module.__stats()[0].kernel.wasmCalls, 0);
  assert.equal(module.__stats()[0].kernel.fallbackCalls, 1);
});
...
test("caller-local Math does not replace the top-level function/helper environment", async (t) => {
  ...
  module.run(a, {
    sqrt() {
      throw new Error("caller binding must not be used");
    },
  });
  same(a, [2, 3]);
...
test("registration does not read a TDZ Math binding; early empty calls preserve behavior", async (t) => {
  ...
  assert.equal(module.early, undefined);
  assert.ok(module.failure instanceof ReferenceError);
  same(module.partial, [4]);
  assert.equal(module.__stats()[0].kernel.fallbackCalls, 2);
  module.run(module.partial);
  same(module.partial, [2]);
  assert.equal(module.__stats()[0].kernel.wasmCalls, 1);
});
...
test("ESM-cycle calls before registration use original code; subsequent calls use native Math", async (t) => {
  const { module } = await application(
    t,
    `import {early} from './cycle.mjs';\n${rootApp}\nexport {early};`,
    {
      "cycle.mjs": `import {run} from './app.mjs';export const early=new Float64Array([4,9]);run(early);`,
    },
  );
  same(module.early, [2, 3]);
  assert.equal(module.__stats()[0].initialized, false);
  module.run(module.early);
  same(module.early, [Math.sqrt(2), Math.sqrt(3)]);
  assert.equal(module.__stats()[0].kernel.wasmCalls, 1);
});
...
test("argument evaluation mutating Math happens before invocation guards, in source order", async (t) => {
  ...
  const saved = Math.sqrt,
    log = [],
    a = new Float64Array([4, 9]);
  try {
    module.run(
      a,
      () => {
  ...
  same(a, [5, 10]);
  assert.deepEqual(log, ["argument", "change", "sqrt", "sqrt"]);
  assert.equal(module.__stats()[0].kernel.wasmCalls, 0);
  assert.equal(module.__stats()[0].kernel.fallbackCalls, 1);
});
...
test("overridden method getter before lazy instantiation is invoked only by retained code", async (t) => {
  ...
  const saved = Object.getOwnPropertyDescriptor(Math, "sqrt");
  let calls = 0;
  const a = new Float64Array([4, 9]);
  try {
...
test("callee identity misses and missing ESM-cycle tokens do not evaluate Math resolvers", () => {
  ...
  const token = createNumericDispatch(target, artifact.wasm, [], () => {
...
async function packageOf(t, source, parameterTypes, options = {}) {
  const dir = temp(t),
    entry = path.join(dir, "source.mjs"),
...
for (const Storage of [Float32Array, Float64Array])
  test(`standalone Math helpers/pipeline package relocates and executes: ${Storage.name}`, async (t) => {
    ...
    assert.equal(module.retained, retained.update);
    assert.equal(retained.default, retained.update);
    assert.equal(result.sourceSha256, sha256(source));
    assert.equal(result.wasmSha256, sha256(fs.readFileSync(path.join(out, "kernel.wasm"))));
    ...
    const a = new Storage([3, 4, 0, 0, 0, 0, -1, -2, 3]),
      c = new Storage([-2, -0.5, 0, 0.5, 2]),
      u = new Float64Array([1, 2, 3]);
    const ea = a.slice(),
      ec = c.slice();
    for (let i = 0; i < 5; i++) {
      assert.ok(Object.is(kernel.run(a, c, u, 0.9), module.retained(ea, ec, u, 0.9)));
      same(a, ea);
      same(c, ec);
    }
...
test("no-Wasm standalone fallback exposes frozen intrinsic metadata and original exceptions", async (t) => {
  ...
  kernel.dispose();
  assert.throws(() => kernel.run(a), { code: "KERNEL_DISPOSED" });
});
...
for (const suffix of [
  ...
  test(
    "standalone rejects shadowed Math even when declaration is unused/mutable: " + suffix,
    (t) => {
      const dir = temp(t),
        entry = path.join(dir, "source.mjs"),
        out = path.join(dir, "package");
      fs.writeFileSync(entry, rootSource + "\n" + suffix);
      assert.throws(() => buildNumericKernel(entry, out, { parameterTypes: ["f64[]"] }), {
...
for (const replacement of ["function", "getter", "bound", "proxy"])
  test(`runtime loaded after a Math method ${replacement} replacement refuses intrinsic admission`, async (t) => {
    const artifact = compileNumericKernel(
      "function update(a){for(let i=0;i<a.length;i++)a[i]=Math.sqrt(a[i]);}",
      { parameterTypes: ["f64[]"], allowMath: true },
    );
    const saved = Object.getOwnPropertyDescriptor(Math, "sqrt");
    let gets = 0;
    try {
      ...
      const { instantiateNumericKernel } = await import(
        runtimeKernelUrl + "?prepatched=" + replacement
      );
      assert.equal(gets, 0);
      const kernel = instantiateNumericKernel(artifact.wasm, {
        resolveMath: () => Math,
        fallback(a) {
          for (let i = 0; i < a.length; i++) a[i] = Math.sqrt(a[i]);
        },
      });
      const a = new Float64Array([4, 9]);
      kernel.run(a);
