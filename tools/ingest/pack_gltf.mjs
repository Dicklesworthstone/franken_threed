...
export function embedGltfAssets(bytes, binary, embed, fail) {
  ...
  if (binary) {
    ...
    while (cursor < bytes.length) {
      ...
      if (length % 4 || cursor + 8 + length > bytes.length) fail('GLB_CHUNK', 'Invalid GLB chunk length/alignment');
      if (cursor !== 12 && bytes.readUInt32LE(cursor + 4) === 0x4e4f534a) fail('GLB_CHUNK', 'Duplicate GLB JSON chunk');
      cursor += 8 + length;
    }
    const end = 20 + bytes.readUInt32LE(12);
    jsonBytes = bytes.subarray(20, end); tail = bytes.subarray(end);
  }
  let model;
  try { model = JSON.parse(new TextDecoder('utf-8', {fatal:true}).decode(jsonBytes)); }
  catch { fail('GLTF_JSON', 'Invalid glTF JSON'); }
  if (!model || Array.isArray(model) || model.asset?.version !== '2.0') fail('GLTF_VERSION', 'Single-file model packing requires glTF 2.0');
  ...
  for (const field of ['buffers', 'images']) {
    ...
    for (const item of model[field]) {
      if (!item || typeof item !== 'object' || Array.isArray(item)) fail('GLTF_RESOURCE', 'Invalid glTF resource record');
      if (!Object.hasOwn(item, 'uri')) continue; // BIN/bufferView resources remain in place.
      if (typeof item.uri !== 'string' || !item.uri) fail('GLTF_RESOURCE', 'Invalid glTF resource URI');
      coreReferences.add(item);
      if (!item.uri.startsWith('data:')) { item.uri = embed(item.uri); changed = true; }
    }
  }
  function inspect(value, depth = 0) {
    ...
    for (const [key, child] of Object.entries(value)) {
      ...
      if (key === 'uri' && !coreReferences.has(value) && typeof child === 'string' && !child.startsWith('data:')) {
  ...
  function encode(value, depth = 0) {
    ...
    if (typeof value === 'number') {
      if (!Number.isFinite(value)) fail('GLTF_NUMBER', 'Non-finite model data cannot be repackaged faithfully');
      return Object.is(value, -0) ? '-0' : JSON.stringify(value);
    }
    if (Array.isArray(value)) return '[' + value.map(item => encode(item, depth + 1)).join(',') + ']';
    if (value && typeof value === 'object') return '{' + Object.entries(value).map(([key, item]) => JSON.stringify(key) + ':' + encode(item, depth + 1)).join(',') + '}';
  ...
  header.writeUInt32LE(json.length + padding, 12); header.writeUInt32LE(0x4e4f534a, 16);
