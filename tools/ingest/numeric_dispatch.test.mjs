...
test('lazy dispatcher executes real Wasm for repeated ordinary updates', () => {
  ...
  const actual = new Float64Array([0, 1, -1]), expected = actual.slice(), velocity = new Float64Array([1, -3, 2]);
...
test('callee identity misses do not initialize Wasm and retain receiver/result/exception semantics', () => {
  const token = createNumericDispatch(integrate, artifact.wasm);
  function shadow(...args) { assert.equal(this, undefined); return args; }
  assert.deepEqual(dispatchNumericCall(token, shadow, [1, 2]), [1, 2]);
  const sentinel = new Error('source exception');
  assert.throws(() => dispatchNumericCall(token, () => { throw sentinel; }, []), error => error === sentinel);
...
test('undefined cycle token invokes the actual callee', () => {
  let calls = 0;
  assert.equal(dispatchNumericCall(undefined, function () { calls++; assert.equal(this, undefined); return 42; }, []), 42);
...
test('coercions occur only in fallback, in original per-element order', () => {
  const token = createNumericDispatch(integrate, artifact.wasm);
  let conversions = 0;
  const dt = { valueOf() { conversions++; return 2; } };
  const a = new Float64Array([0, 0]), v = new Float64Array([1, 2]);
...
test('blocked Wasm with hostile thrown values does not escape fallback', t => {
  ...
  let attempts = 0;
  t.after(() => { WebAssembly.Module = native; });
  WebAssembly.Module = function () {
    attempts++;
    throw { get code() { throw new Error('must not inspect policy exception'); } };
  };
  const token = createNumericDispatch(integrate, artifact.wasm);
  const a = new Float64Array([0]), v = new Float64Array([1]);
