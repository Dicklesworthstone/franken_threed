...
const decode = (url) =>
  Buffer.from(url.slice(url.indexOf(",") + 1).split("#")[0], "base64").toString();
function fixture(
  files = {},
  main = `export function start() { return new Worker(new URL('./worker.mjs', import.meta.url), {type:'module'}); }`,
) {
  ...
  const inputs = {
    ...
    "app.mjs": main,
    "worker.mjs": "export const answer = 42;",
    ...files,
  };
  ...
  return {
    ...
    inputs,
    pack(options = {}) {
      return packHtml(path.join(source, "index.html"), output, options);
    },
  };
}
function modules(output) {
  ...
  return Object.values(
    JSON.parse(/<script type="importmap">(.*?)<\/script>/s.exec(html)[1]).imports,
  );
}
function factories(output) {
  return modules(output)
    .flatMap(
      (url) => acorn.parse(decode(url), { ecmaVersion: "latest", sourceType: "module" }).body,
    )
    .filter((node) => node.type === "ImportDeclaration" && node.source.value.startsWith("data:"))
...
test("embeds a worker module graph including shared live bindings, reexports and lazy targets", async () => {
  const f = fixture({
    ...
    "left.mjs": `export {value} from './state.mjs';`,
    "right.mjs": `export const value=99;`,
  });
  const report = f.pack();
  assert.equal(report.workerCount, 1);
  assert.equal(report.workerScriptCount, 5);
  const code = await body(factories(f.output)[0]);
  const module = await import(
    "data:text/javascript;base64," + Buffer.from(code).toString("base64")
  );
  assert.deepEqual(module.run(), [1, 1]);
  assert.deepEqual(module.run(), [2, 2]);
  assert.equal((await module.lazy(true)).value, 2);
  assert.equal((await module.lazy(false)).value, 99);
  for (const [name, source] of Object.entries(f.inputs))
    assert.deepEqual(fs.readFileSync(path.join(f.source, name)), Buffer.from(source));
});

test("resolves string Worker URLs from the document, and new URL from its source module", async () => {
  const f = fixture({
    "app.mjs": `export {a,b} from './nested/start.mjs';`,
  ...
  f.pack();
  const sources = await Promise.all(factories(f.output).map(body));
  assert.ok(sources.includes("export const answer = 42;"));
  assert.ok(sources.includes("export const answer=7;"));
});

test("does not import document import-map rules into worker resolution", () => {
  const f = fixture({
    "index.html": `<script type="importmap">{"imports":{"dependency":"./dep.mjs"}}</script><script type="module" src="app.mjs"></script>`,
    "worker.mjs": `import 'dependency';`,
    "dep.mjs": "export const x=1;",
  });
  assert.throws(() => f.pack(), { code: "WORKER_IMPORT" });
...
test("shares one factory across constructor sites and preserves distinct query/fragment identities", async () => {
  const f = fixture(
    {},
    `export function a(){return new Worker('./worker.mjs',{type:'module'});}
  ...
  const r = f.pack(),
    urls = factories(f.output);
  assert.equal(r.workerCount, 2);
  assert.equal(urls[0], urls[1]);
  assert.notEqual(urls[0], urls[2]);
  const a = await import(urls[0]),
    b = await import(urls[2]);
...
test("module-relative assets and JSON modules stay fetchable and streaming-compatible", async () => {
  ...
  const { instance } = await WebAssembly.instantiateStreaming(fetch(m.binary));
...
test("factory scope is isolated from application Blob locals and generated names do not collide", async () => {
  const f = fixture(
    {},
    `const Blob=null;const __f3d_packed_worker_0=9;export const start=()=>new Worker('./worker.mjs',{type:'module'});`,
  );
  f.pack();
  await body(factories(f.output)[0]);
...
test("leaves non-URL constructor arguments and function/callee identity unchanged", async () => {
  const f = fixture(
    {},
    `export function start(log){return new Worker(new URL('./worker.mjs',import.meta.url),{type:'module',name:(log.push('name'),'decoder')});}`,
  );
  f.pack();
  const main = await import(modules(f.output)[0]);
  const prior = Object.getOwnPropertyDescriptor(globalThis, "Worker");
  const events = [];
  class NativeTestSeam {
  ...
  try {
    const worker = main.start(events);
    assert.ok(worker instanceof NativeTestSeam);
    assert.deepEqual(events, ["name", "construct"]);
    assert.deepEqual(worker.options, { type: "module", name: "decoder" });
...
for (const [label, main, code] of [
  ...
  [
    "bound Worker",
    `function f(Worker){return new Worker('./worker.mjs',{type:'module'});}`,
    "WORKER_BINDING",
  ],
  [
    "bound URL",
    `const URL=globalThis.URL;new Worker('./worker.mjs',{type:'module'});`,
    "WORKER_BINDING",
  ],
  [
  ...
  test(`retains the normal build: ${label}`, () => {
    const f = fixture({}, main);
    assert.throws(() => f.pack(), { code });
    assert.equal(fs.existsSync(f.output), false);
  });
...
for (const [label, files, code] of [
  [
    "module cycle",
    { "worker.mjs": `import './a.mjs';`, "a.mjs": `import './worker.mjs';` },
    "WORKER_MODULE_CYCLE",
  ],
  [
    "nested worker",
    { "worker.mjs": `new Worker('./child.mjs',{type:'module'});` },
    "WORKER_PARENT",
  ],
  ...
  [
    "remote import",
    { "worker.mjs": `import 'https://example.com/module.mjs';` },
    "EXTERNAL_RESOURCE",
  ],
])
  test(`worker closure refusal: ${label}`, () => {
...
test("budget, root containment, symlink and output protections apply to workers", () => {
  let f = fixture();
  assert.throws(() => f.pack({ maxBytes: 100 }), { code: "PACK_LIMIT" });
  assert.equal(fs.existsSync(f.output), false);
  f = fixture();
  assert.throws(() => f.pack({ maxFiles: 2 }), { code: "PACK_LIMIT" });
  f = fixture();
  fs.writeFileSync(path.join(f.root, "outside.mjs"), "postMessage(1)");
  fs.symlinkSync(path.join(f.root, "outside.mjs"), path.join(f.source, "link.mjs"));
  fs.writeFileSync(path.join(f.source, "worker.mjs"), `import './link.mjs';`);
  assert.throws(() => f.pack(), { code: "ROOT_ESCAPE" });
  f = fixture();
  f.pack();
  const bytes = fs.readFileSync(f.output);
  assert.throws(() => f.pack(), { code: "OUTPUT_EXISTS" });
