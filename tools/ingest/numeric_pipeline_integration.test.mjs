...
async function application(t, source, options = {}) {
  ...
  const result = specializeNumericModule(source, {
    sourceName: 'application.mjs', runtimeModule: observer(root), ...options,
  });
  ...
  return { result, module, reference, diagnostics: (await import(url(root, 'observer.mjs'))).diagnostics };
}
const source = `
...
test('ordinary three-pass application calls execute one native transaction per frame without changing exported identities', async t => {
  ...
  for (const pass of item.loops) {
    assert.equal(pass.boundParameter, 0); assert.equal(pass.loopStride, 1);
  ...
  for (const ArrayType of [Float32Array, Float64Array]) {
    ...
    for (let frame = 0; frame < 120; frame++) {
      ...
      assert.ok(Object.is(app.module.frame(x, velocity, dt), app.reference.frame(expected, velocity, dt)));
    ...
    assert.equal(app.diagnostics()[0].kernel.copiedBytes, 3 * n * ArrayType.BYTES_PER_ELEMENT * 120);
  }
});
...
test('native discovery handles independent bounds and strides in all four streamed/uniform layouts', async t => {
  ...
  const app = await application(t, text), item = app.result.report.candidates[0];
  assert.equal(item.loopCount, 2);
  assert.deepEqual(item.boundParameters, [0, 1]);
  assert.deepEqual(item.loops.map(loop => loop.loopStride), [3, 4]);
  assert.equal(item.variants.length, 4);
  assert.deepEqual(item.scalarHelpers.map(helper => helper.name), ['transform']);
  for (const StreamType of [Float32Array, Float64Array]) for (const UniformType of [Float32Array, Float64Array]) {
    const v = StreamType.from({ length: 30000 }, (_, i) => (i % 37) / 11);
    const c = new StreamType(800).fill(0.5), m = new UniformType([0.01, -0.02, 0.03, 0.99]);
    const expectedV = v.slice(), expectedC = c.slice();
    for (let frame = 0; frame < 20; frame++) {
      assert.ok(Object.is(app.module.frame(v, c, m), app.reference.frame(expectedV, expectedC, m)));
      assert.deepEqual(v, expectedV); assert.deepEqual(c, expectedC);
    }
...
test('parenthesized and spread call sites retain argument order and actual callee identity', async t => {
  ...
  const order = [], a = new Float64Array([1, 3]), b = new Float64Array([2, 4]);
  assert.equal(app.module.evaluated(() => { order.push('a'); return a; },
    () => { order.push('b'); return b; }, () => { order.push('c'); return 0.5; }), 3.5);
  assert.deepEqual(order, ['a', 'b', 'c']);
  assert.deepEqual([...a], [-1.5, 1.5]);
  assert.equal(app.module.spread([a, b, 0]), 0);
  const marker = {};
  assert.equal(app.module.shadow((x, y) => { assert.equal(x, a); assert.equal(y, b); return marker; }, [a, b]), marker);
  ...
  assert.throws(() => app.module.evaluated(() => { throw failure; }, () => b, () => 0), error => error === failure);
  assert.equal(app.diagnostics()[0].kernel.wasmCalls, 2);
});
...
test('nested calls to different multi-pass kernels keep original evaluation order and use one token per function', async t => {
  ...
  const a = new Float64Array([1, 2]), b = new Float64Array([2, 3]);
  assert.equal(app.module.frame(a, b), 10); assert.deepEqual([...a], [11, 21]);
  assert.equal(app.result.report.compiledKernels, 2);
  assert.deepEqual(app.diagnostics().map(item => item.kernel.wasmCalls), [1, 1]);
});
...
test('later-pass type and length guards fall back to the whole original sequence with original coercions and exceptions', async t => {
  ...
  const app = await application(t, text);
  const a = new Float64Array([2]), b = new Float64Array([3, 4]), input = new Float64Array([5]);
  assert.equal(app.module.frame(a, b, input, 2), 1);
  assert.deepEqual([...a], [4]); assert.deepEqual([...b], [8, NaN]);
  assert.equal(app.diagnostics()[0].kernel.lastGuardFailure, 'KERNEL_ARRAY_LENGTH');
  let coercions = 0;
  app.module.frame(a, new Float64Array(), new Float64Array(), { valueOf() { coercions++; return 3; } });
  assert.equal(coercions, 1); assert.deepEqual([...a], [12]);
  const failure = new Error('later original length failed'), later = new Float64Array([1]);
  let reads = 0;
  Object.defineProperty(later, 'length', { get() { reads++; throw failure; } });
  assert.throws(() => app.module.frame(a, later, input, 2), error => error === failure);
  assert.deepEqual([...a], [24]); assert.equal(reads, 1);
...
test('unavailable Wasm preserves all passes and their numeric result', async t => {
  ...
  try {
    const a = new Float64Array([1, 3]), b = new Float64Array([2, 4]);
...
test('a later-pass unclosed effect retains the entire original function and creates no runtime dependency', async t => {
  ...
  let resolutions = 0;
  const result = specializeNumericModule(text, { runtimeModule() { resolutions++; return './never.mjs'; } });
  assert.equal(result.changed, false); assert.equal(result.code, text);
  assert.equal(result.report.compiledKernels, 0); assert.equal(resolutions, 0);
  assert.equal(result.report.candidates[0].reason, 'KERNEL_NOT_CLOSED');
  const app = await application(t, text), a = new Float64Array([1, 2]);
  assert.equal(app.module.frame(a), 2); assert.deepEqual([...a], [4, 6]);
...
test('mutable helpers, direct eval, captured state and dynamic later loops cannot bypass whole-function closure', () => {
  ...
  for (const text of inputs) {
    ...
    assert.equal(result.changed, false, text); assert.equal(result.code, text);
...
test('ESM cycle calls before token initialization use original source, then later calls use native pipelines', async t => {
  const root = fixture(t), runtime = observer(root);
  ...
  fs.writeFileSync(path.join(root, 'application.mjs'), result.code);
  fs.writeFileSync(path.join(root, 'cycle.mjs'), `import {frame} from './application.mjs';
  ...
  const a = new Float64Array([2, 3]); mod.frame(a);
  assert.deepEqual([...a], [6, 8]); assert.equal(diagnostics()[0].kernel.wasmCalls, 1);
});
...
test('pipeline budgets count functions, not passes, and discovery output is deterministic', async t => {
  ...
  const a = specializeNumericModule(text, options), b = specializeNumericModule(text, options);
  assert.deepEqual(a, b); assert.equal(a.report.compiledKernels, 1);
  assert.equal(a.report.candidates[0].loopCount, 2);
  assert.equal(a.report.candidates[1].reason, 'KERNEL_BUDGET');
  const app = await application(t, text, { maxKernels: 1 }), x = new Float64Array([1, 2]);
  app.module.frame(x); assert.deepEqual([...x], [24, 30]);
  assert.equal(app.diagnostics().length, 1); assert.equal(app.diagnostics()[0].kernel.wasmCalls, 1);
});
...
function packageFixture(t, text = packageSource) {
  const root = fixture(t), entry = path.join(root, 'input.mjs'), out = path.join(root, 'package');
  fs.writeFileSync(entry, text);
  return { root, entry, out };
}
function frozenPipeline(manifest) {
  assert.ok(Object.isFrozen(manifest)); assert.ok(Object.isFrozen(manifest.boundParameters));
  assert.ok(Object.isFrozen(manifest.loops)); assert.ok(manifest.loops.every(Object.isFrozen));
  assert.ok(manifest.parameters.every(param => !param.access || Object.isFrozen(param.access.loopBounds)));
}
...
test('standalone packages preserve complete multi-pass source and helpers, relocate, and execute without runtime compiler dependencies', async t => {
  for (const [ArrayType, type] of [[Float32Array, 'f32[]'], [Float64Array, 'f64[]']]) {
    ...
    const moved = path.join(root, 'moved'); fs.renameSync(out, moved);
    ...
    const engine = mod.createKernel(); frozenPipeline(engine.manifest);
    const actual = new ArrayType([1, 2, 3]), expected = actual.slice();
    assert.ok(Object.is(engine.run(actual), mod.retained(expected))); assert.deepEqual(actual, expected);
    assert.equal(engine.diagnostics.wasmCalls, 1);
    assert.equal(engine.diagnostics.copiedBytes, actual.byteLength * 2);
    const plain = [1, 2, 3]; assert.equal(engine.run(plain), 14);
...
test('unavailable-Wasm package fallback retains all passes, results, disposal and deeply immutable pipeline metadata', async t => {
  ...
  Object.defineProperty(globalThis, 'WebAssembly', { configurable: true, value: undefined });
  try {
    const engine = createKernel(); frozenPipeline(engine.manifest);
    assert.throws(() => engine.manifest.boundParameters.push(1), TypeError);
    assert.throws(() => { engine.manifest.loops[0].loopStride = 2; }, TypeError);
    assert.throws(() => engine.manifest.parameters[0].access.loopBounds.push(1), TypeError);
    const x = new Float64Array([3, 4]); assert.equal(engine.run(x), 25);
    assert.deepEqual([...x], [3 * (1 / 25), 4 * (1 / 25)]);
    assert.equal(engine.diagnostics.wasmCalls, 0); assert.equal(engine.diagnostics.fallbackCalls, 1);
    engine.dispose(); assert.throws(() => engine.run(x), /KERNEL_DISPOSED/);
  } finally { Object.defineProperty(globalThis, 'WebAssembly', prior); }
});
...
test('pipeline packages select among multiple entries explicitly and retain fresh-output safety and deterministic artifacts', async t => {
  ...
  const second = path.join(root, 'second'); buildNumericKernel(entry, second, options);
