...
import {
  AnimationEnvironmentError,
  createGpuAnimationEnvironment,
  planAnimationEnvironment,
} from "./animation_environment.mjs";
import { AnimationHdrError, decodeAnimationHdr } from "./animation_hdr.mjs";
...
function cancelBody(response, reason) {
  try {
    Promise.resolve(response?.body?.cancel(reason)).catch(ignore);
  } catch {
    /* A borrowed implementation may already have closed it. */
  }
}
function sourceUrl(source, baseURL) {
  ...
  try {
    ...
    fail("URL", "Relative HDR URLs require baseURL");
  }
  if (!["http:", "https:"].includes(url.protocol) || url.username || url.password)
    fail("URL", "Expected HTTP(S) URL without embedded credentials");
  url.hash = "";
  return url.href;
}
...
export async function loadGpuAnimationEnvironment(device, source, options = {}) {
  if (!options || typeof options !== "object" || Array.isArray(options))
    fail("OPTIONS", "Expected HDR loading options");
  ...
  for (const key of Object.keys(options))
    if (
      ...
      fail("OPTIONS", `Unknown HDR loading option: ${key}`);
  const {
    baseURL,
  ...
  for (const key of ["writeTexture", "writeBuffer", "submit", "onSubmittedWorkDone"])
  ...
  const alignment = positive(
    device.limits?.minUniformBufferOffsetAlignment,
    "device uniform alignment",
  );
  const filter = Object.fromEntries(
    filterKeys.filter((key) => options[key] !== undefined).map((key) => [key, options[key]]),
  );
  const plan = planAnimationEnvironment({ ...filter, maxTextureBytes, maxDimension, alignment });
  if (plan.uniformBytes > positive(device.limits?.maxBufferSize, "device buffer limit"))
    fail("LIMIT", "Filter uniforms exceed device limit");
  const url =
    typeof source === "string" || source instanceof URL ? sourceUrl(source, baseURL) : null;
  if (url !== null && typeof fetcher !== "function")
    fail("OPTIONS", "Expected Fetch API implementation");
  ...
  device.lost.then(
    (info) =>
      stop(
        new AnimationEnvironmentError("ANIMATION_ENVIRONMENT_LOST", info?.message ?? "Device lost"),
      ),
    stop,
  );
  function check() {
    if (ended) throw stopReason;
  }
  ...
  function releaseReader(cancel) {
    ...
    if (current) {
      if (cancel) {
        try {
          Promise.resolve(current.cancel(stopReason)).catch(ignore);
        } catch {}
      }
      try {
        current.releaseLock();
  ...
  async function readUrl() {
    ...
    const request = Promise.resolve(
      fetcher(url, { credentials: "omit", redirect: "error", signal: controller.signal }),
    ).then((response) => {
    ...
    try {
      check();
      if (!response?.ok)
        fail("HTTP", `HDR request failed with status ${response?.status ?? "unknown"}`);
      if (response.redirected || (response.url && sourceUrl(response.url) !== url))
        fail("HTTP", "HDR redirects are not allowed");
      const length = response.headers?.get?.("content-length");
      if (
        length != null &&
        (!/^\d+$/.test(length) ||
          !Number.isSafeInteger(Number(length)) ||
          Number(length) > maxInputBytes)
      )
        fail("LIMIT", "HDR Content-Length exceeds byte budget or is invalid");
      if (typeof response.body?.getReader !== "function")
        fail("HTTP", "HDR loading requires a streaming response body");
    ...
    for (;;) {
      ...
      if (
        ...
        chunk.buffer.resizable
      )
        fail("HTTP", "Expected fixed, unshared response bytes");
      const next = total + chunk.byteLength;
      if (!Number.isSafeInteger(next) || next > maxInputBytes)
        fail("LIMIT", "Streamed HDR exceeds byte budget");
      if (next > storage.length) {
        const grown = new Uint8Array(
          Math.min(maxInputBytes, Math.max(next, 65536, storage.length * 2)),
        );
        grown.set(storage.subarray(0, total));
        storage = grown;
      }
  ...
  try {
    ...
    let encoded = url === null ? source : await readUrl();
    check();
    let image = decodeAnimationHdr(encoded, {
      maxInputBytes,
      maxDecodedBytes,
      maxHeaderBytes,
      maxDimension,
    ...
    const inputBytes = encoded.byteLength;
    encoded = null;
    source = null;
    if (image.width !== image.height * 2)
      fail("SOURCE", "Environment HDR must be a 2:1 equirectangular panorama");
    const peakTextureBytes = image.byteLength + plan.textureBytes;
    if (!Number.isSafeInteger(peakTextureBytes) || peakTextureBytes > maxTextureBytes)
      fail("LIMIT", "Panorama plus filtered environment exceeds GPU texture budget");
    const sourceInfo = Object.freeze({
      width: image.width,
      height: image.height,
      inputBytes,
      exposure: image.exposure,
      clampedComponents: image.clampedComponents,
      uploadTextureBytes: image.byteLength,
      peakTextureBytes,
    });
    check();
    ...
    try {
      ...
      device.queue.writeTexture(
        ...
        image.data,
        { offset: 0, bytesPerRow: image.width * 8, rowsPerImage: image.height },
    ...
    image = null;
    const uploadValidation = Promise.all([device.popErrorScope(), device.popErrorScope()]).then(
      (errors) => {
        if (uploadError) throw uploadError;
        const error = errors.find(Boolean);
        if (error) fail("GPU", error.message ?? "HDR upload failed");
      },
    );
    await wait(uploadValidation);
    check();
    const filtering = createGpuAnimationEnvironment(device, uploadTexture, {
      ...filter,
      maxTextureBytes: maxTextureBytes - sourceInfo.uploadTextureBytes,
      signal: controller.signal,
    }).then((environment) => {
      if (ended) environment.dispose(); // A late custom driver resolution cannot leak a completed map.
      return environment;
    });
    prepared = await wait(filtering);
    check();
    ...
    const result = Object.freeze({
      ...
      sourceInfo,
      get disposed() {
        return environment.disposed;
      },
      get failed() {
        return environment.failed;
      },
      get textureBytes() {
        return environment.textureBytes;
      },
      async whenIdle() {
        await environment.whenIdle();
        return result;
      },
    });
    return result;
