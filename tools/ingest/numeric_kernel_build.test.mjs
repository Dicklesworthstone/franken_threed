...
async function load(out) {
  return import(pathToFileURL(path.join(out, "kernel.mjs")).href);
}
...
test("build is deterministic and accepts plain declarations, including a retained-named function", async (t) => {
  const { root, entry, out } = fixture(
    t,
    "function retained(x) { for(let i=0; i<x.length; i++) x[i]*=2; } // trailing comment",
  );
  ...
  for (const name of result.emittedFiles) {
    assert.deepEqual(
      ...
      fs.readFileSync(path.join(other, name)),
    );
  }
  const { createKernel } = await load(out);
...
test("CLI rejects missing signatures, conflicting modes and invalid budgets before output", (t) => {
  ...
  for (const extra of [
    ...
    const result = spawnSync(process.execPath, [...common, ...extra], {
      encoding: "utf8",
      timeout: 10000,
    });
  ...
  const missingMode = spawnSync(
    process.execPath,
    [cli, "--entry", entry, "--parameter-types", "f64[]"],
    { encoding: "utf8" },
  );
