...
const index = (array, i, label) => {
  if (!Number.isSafeInteger(i) || i < 0 || i >= array.length)
...
const positive = (value, label) => {
  finite(value, label);
  if (value <= 0) fail("VALUE", `${label} must be positive`);
  return value;
};
const name = (value) => {
  if (value !== undefined && typeof value !== "string") fail("SHAPE", "Names must be strings");
  return value ?? "";
};
function fields(value, allowed, label) {
  ...
  if (Object.keys(object(value.extensions ?? {}, "extensions")).length)
    fail("UNSUPPORTED", `Extended ${label} needs the source route`);
}
function projection(camera) {
  ...
  if (type === "perspective") {
    ...
    fields(p, ["yfov", "znear", "zfar", "aspectRatio"], "perspective");
    const yfov = positive(p.yfov, "vertical field of view"),
      znear = positive(p.znear, "near plane");
    if (yfov >= Math.PI) fail("CAMERA", "Perspective field of view must be less than PI");
    const result = { yfov, znear };
    if (p.zfar !== undefined) {
      if (finite(p.zfar, "far plane") <= znear) fail("CAMERA", "Far plane must exceed near plane");
      result.zfar = p.zfar;
    }
    if (p.aspectRatio !== undefined)
      result.aspectRatio = positive(p.aspectRatio, "camera aspect ratio");
    return Object.freeze(result);
  }
  if (type === "orthographic") {
    if (camera.perspective !== undefined) fail("CAMERA", "Camera has two projection types");
    const p = camera.orthographic;
    fields(p, ["xmag", "ymag", "znear", "zfar"], "orthographic");
    const result = {
      xmag: positive(p.xmag, "orthographic xmag"),
      ymag: positive(p.ymag, "orthographic ymag"),
      znear: finite(p.znear, "near plane", 0),
      zfar: finite(p.zfar, "far plane"),
    };
...
function lightProperties(light) {
  ...
  if (!["point", "directional", "spot"].includes(light.type))
    fail("LIGHT", "Unsupported light type");
  const color = list(light.color ?? [1, 1, 1], "light color");
  if (color.length !== 3) fail("LIGHT", "Light color needs three components");
  const result = {
    type: light.type,
    color: Object.freeze(color.map((v) => finite(v, "light color", 0, 1))),
    intensity: finite(light.intensity ?? 1, "intensity", 0),
  };
  ...
  if (light.type === "spot") {
    ...
    result.outerConeAngle = finite(
      light.spot.outerConeAngle ?? Math.PI / 4,
      "outer cone",
      0,
      Math.PI / 2,
    );
    if (result.innerConeAngle >= result.outerConeAngle)
      fail("LIGHT", "Spot cones must satisfy inner < outer");
...
export function decodeGltfSceneView(model, { scene = model?.scene ?? 0 } = {}) {
  ...
  const nodes = list(model.nodes ?? [], "nodes"),
    scenes = list(model.scenes, "scenes");
  ...
  for (let n = 0; n < nodes.length; n++) {
    ...
    for (const child of children[n]) {
      ...
      if (child === n || parents[child] !== -1)
        fail("HIERARCHY", "Self-parent, repeated child or multiple parents");
      parents[child] = n;
    }
  }
  const forest = [];
  ...
  for (let i = 0; i < forest.length; i++)
    for (const child of children[forest[i]]) forest.push(child);
  ...
  const cameraDefs = list(model.cameras ?? [], "cameras"),
    rootExtension = model.extensions?.KHR_lights_punctual;
  if (rootExtension !== undefined) fields(rootExtension, ["lights"], "punctual lights extension");
  const lightDefs = list(rootExtension?.lights ?? [], "lights"),
    cameras = [],
    lights = [];
  for (const nodeIndex of order) {
    const node = nodes[nodeIndex],
      ref = node.extensions?.KHR_lights_punctual;
    ...
    if (ref !== undefined) {
      ...
      if (lights.length >= 8)
        fail("LIMIT", "Selected scene exceeds the renderer limit of eight punctual lights");
...
function world(pose, node) {
  ...
  if (!(data instanceof Float64Array) && !(data instanceof Float32Array))
    fail("POSE", "Expected packed world matrices");
  if (
    !(data.buffer instanceof ArrayBuffer) ||
    data.buffer.resizable ||
    data.length !== pose.nodeCount * 16
  )
    fail("POSE", "Invalid world matrix storage");
  let m;
  try {
    m = Array.from(data.subarray(node * 16, node * 16 + 16));
  } catch {
    fail("POSE", "Detached world matrix storage");
  }
  for (const x of m) finite(x, "world matrix");
  if (m[3] !== 0 || m[7] !== 0 || m[11] !== 0 || m[15] !== 1)
    fail("TRANSFORM", "World transform must be affine");
  return m;
}
const unit = (v) => {
  const n = Math.hypot(...v);
  if (!Number.isFinite(n) || n === 0) fail("TRANSFORM", "Undefined camera/light direction");
  return v.map((x) => x / n);
};
...
function cameraMatrices(camera, m, aspectRatio) {
  ...
  if (camera.type === "perspective") {
    ...
    if (aspect === undefined)
      fail("ASPECT", "Perspective camera without aspectRatio needs a viewport aspect ratio");
    projection[0] = 1 / Math.tan(p.yfov / 2) / aspect;
    projection[5] = 1 / Math.tan(p.yfov / 2);
    projection[10] = f === undefined ? -1 : -1 / (1 - n / f);
    projection[11] = -1;
    projection[14] = n * projection[10];
  } else {
    projection[0] = 1 / p.xmag;
    projection[5] = 1 / p.ymag;
    projection[10] = -1 / (f - n);
    projection[14] = -n / (f - n);
    projection[15] = 1;
  }
  ...
  for (let c = 0; c < 4; c++)
    for (let r = 0; r < 4; r++)
      for (let k = 0; k < 4; k++)
        viewProjection[c * 4 + r] += projection[k * 4 + r] * view[c * 4 + k];
  for (const matrix of [view, projection, viewProjection]) {
    for (const v of matrix)
      if (!Number.isFinite(Math.fround(v)))
        fail("VALUE", "Camera matrix cannot be represented by the renderer");
...
export function createGltfSceneView(pose, definition) {
  if (
    definition?.format !== "f3d-gltf-scene-view-v1" ||
    !Number.isSafeInteger(definition.nodeCount) ||
    definition.nodeCount < 0 ||
    definition.nodeCount > 65536 ||
    !pose ||
    pose.nodeCount !== definition.nodeCount
  )
    fail("POSE", "Scene view needs its matching animation pose");
  ...
  const cameras = list(definition.cameras, "camera instances"),
    lights = list(definition.lights, "light instances");
  if (cameras.length > 4096 || lights.length > 8) fail("LIMIT", "Excessive scene view instances");
  const seen = new Set();
  const node = (n) => {
    if (!Number.isSafeInteger(n) || n < 0 || n >= nodeCount) fail("INDEX", "Invalid instance node");
    return n;
  };
  const ids = (entry, key) => {
    if (!Number.isSafeInteger(entry[key]) || entry[key] < 0) fail("INDEX", `Invalid ${key} ID`);
    return { [key]: entry[key] };
  };
  const preparedCameras = cameras.map((c) => {
    node(c.node);
    if (seen.has(c.node)) fail("CAMERA", "Duplicate camera node");
    seen.add(c.node);
    return Object.freeze({
      node: c.node,
      ...ids(c, "camera"),
      name: name(c.name),
      nodeName: name(c.nodeName),
      type: c.type,
      projection: projection({ type: c.type, [c.type]: c.projection }),
    });
  });
  seen.clear();
  const preparedLights = lights.map((l) => {
    node(l.node);
    if (seen.has(l.node)) fail("LIGHT", "Duplicate light node");
    seen.add(l.node);
    const source = {
      type: l.type,
      color: l.color,
      ...
      ...(l.type === "spot"
        ? { spot: { innerConeAngle: l.innerConeAngle, outerConeAngle: l.outerConeAngle } }
        : {}),
    };
    return Object.freeze({
      node: l.node,
      ...ids(l, "light"),
      name: name(l.name),
      nodeName: name(l.nodeName),
      ...lightProperties(source),
    });
  });
  let busy = false;
  function live() {
    if (pose.disposed) fail("POSE", "Animation pose has been disposed");
    if (pose.nodeCount !== nodeCount || !Number.isSafeInteger(pose.version) || pose.version < 0)
      fail("POSE", "Invalid animation pose");
  }
  function snapshot(operation) {
    live();
    if (busy) fail("REENTRANT", "Scene view sampling cannot be reentered");
    busy = true;
    const version = pose.version;
    try {
      const result = operation(version);
      live();
      if (pose.version !== version) fail("CHANGED", "Pose changed during scene view sampling");
      return result;
    } finally {
      busy = false;
    }
  ...
  function evaluateLights() {
    return Object.freeze(
      preparedLights.map((l) => {
        const m = world(pose, l.node),
          out = { type: l.type, color: l.color, intensity: l.intensity };
        ...
        if (l.type === "spot") {
          out.innerConeAngle = l.innerConeAngle;
          out.outerConeAngle = l.outerConeAngle;
        }
        for (const v of [...out.color, ...(out.position ?? []), out.intensity, out.range ?? 1])
          if (!Number.isFinite(Math.fround(v)))
            fail("VALUE", "Light cannot be represented by the renderer");
        return Object.freeze(out);
      }),
    );
  }
  return Object.freeze({
    cameras: Object.freeze(preparedCameras),
    lights: Object.freeze(preparedLights),
    sampleLights() {
      return snapshot(() => evaluateLights());
    },
    sample(options = {}) {
      return snapshot((poseVersion) => {
        const { cameraNode, aspectRatio } = object(options, "camera options");
        for (const key of Object.keys(options))
          if (!["cameraNode", "aspectRatio"].includes(key))
            fail("CAMERA", `Unsupported camera option: ${key}`);
        const camera =
          cameraNode === undefined
            ? preparedCameras.length === 1
              ? preparedCameras[0]
              : null
            : preparedCameras.find((c) => c.node === cameraNode);
        if (!camera) fail("CAMERA", "Select an instantiated cameraNode in this scene");
        const matrices = cameraMatrices(camera, world(pose, camera.node), aspectRatio),
          lights = evaluateLights();
