...
export class GltfDracoError extends Error {
  constructor(code, message, options) {
    ...
    this.name = "GltfDracoError";
    this.code = code;
  }
}
const EXT = "KHR_draco_mesh_compression";
const fail = (code, message) => {
  throw new GltfDracoError("GLTF_DRACO_" + code, message);
};
...
const index = (values, i, label) => {
  integer(i, label);
  if (i >= values.length) fail("LAYOUT", `Invalid ${label}`);
...
const formats = new Map([
  [5120, [Int8Array, 1, "setInt8"]],
  [5121, [Uint8Array, 1, "setUint8"]],
  [5122, [Int16Array, 2, "setInt16"]],
  [5123, [Uint16Array, 2, "setUint16"]],
  [5125, [Uint32Array, 4, "setUint32"]],
  [5126, [Float32Array, 4, "setFloat32"]],
]);
...
function unusedBuffers(model, records, available) {
  ...
  const replaced = new Set(
    available
      ? records.flatMap((r) => [
          ...r.attributes.map((a) => a.at),
          ...(r.indices ? [r.indices.at] : []),
        ])
      : [],
  );
  const keepAccessors = new Set(accessors.map((_, i) => i).filter((i) => !replaced.has(i)));
  const ignoredViews = new Set(
    available
      ? [...replaced].map((i) => accessors[i].bufferView).filter((i) => i !== undefined)
      : records.map((r) => r.source),
  );
  for (const mesh of model.meshes ?? [])
    for (const p of mesh.primitives) {
      const draco = available && p.extensions?.[EXT];
      for (const [name, at] of Object.entries(p.attributes ?? {}))
        if (!draco || !Object.hasOwn(draco.attributes, name)) keepAccessors.add(at);
      if (p.indices !== undefined && !draco) keepAccessors.add(p.indices);
      for (const target of p.targets ?? [])
        for (const at of Object.values(target)) keepAccessors.add(at);
    }
  for (const skin of model.skins ?? [])
    if (skin.inverseBindMatrices !== undefined) keepAccessors.add(skin.inverseBindMatrices);
  ...
  for (const at of keepAccessors) {
    const a = accessors[at];
    if (!a) continue;
    if (a.bufferView !== undefined) keepViews.add(a.bufferView);
    if (a.sparse) {
      keepViews.add(a.sparse.indices?.bufferView);
      keepViews.add(a.sparse.values?.bufferView);
    }
  }
  for (const image of model.images ?? [])
    if (image.bufferView !== undefined) keepViews.add(image.bufferView);
  ...
  return [...new Set([...ignoredViews].map((i) => views[i]?.buffer))]
    .filter((i) => Number.isSafeInteger(i) && i >= 0 && !needed.has(i))
    .sort((a, b) => a - b);
}
function attributeLayout(attribute, count, width, Type = null) {
  object(attribute, "decoded attribute");
  if (attribute.itemSize !== width || attribute.count !== count)
    fail("DECODE", "Decoded attribute count/width disagrees with accessor");
  ...
  if (!ArrayBuffer.isView(array) || array instanceof DataView || (Type && !(array instanceof Type)))
    fail("DECODE", "Decoder returned an incorrect component type");
  byteView(array);
  ...
  if (
    offset + width > stride ||
    !Number.isSafeInteger((count - 1) * stride + offset + width) ||
    (count - 1) * stride + offset + width > array.length
  )
    fail("DECODE", "Decoded attribute exceeds its storage");
 ...
 */
export function prepareDracoMeshes(
  model,
  {
    decoder = null,
  ...
  integer(maxEncodedBytes, "encoded budget");
  integer(maxDecodedBytes, "decoded budget");
  integer(maxDecodedBufferBytes, "decoded buffer budget", 1);
  integer(maxPrimitives, "primitive budget", 1);
  if (model?.asset?.version !== "2.0") fail("LAYOUT", "Expected glTF 2.0");
  const meshes = list(model.meshes ?? [], "meshes");
  const hasDraco = meshes.some(
    (mesh) =>
      Array.isArray(mesh?.primitives) &&
      mesh.primitives.some((p) => Object.hasOwn(p?.extensions ?? {}, EXT)),
  );
  const required = list(model.extensionsRequired ?? [], "required extensions").includes(EXT);
  if (!hasDraco && !required)
    return Object.freeze({
      decodedBytes: 0,
      skippedBuffers: Object.freeze([]),
      async decode(buffers, { signal } = {}) {
        abort(signal);
        return { json: model, sourceJson: model, buffers, decodedBytes: 0, decodedPrimitives: 0 };
      },
    });
  ...
  if (hasDraco && available && typeof decoder.decodeGeometry !== "function")
    fail("DECODER", "Expected DRACOLoader.decodeGeometry");
  list(model.extensionsUsed ?? [], "used extensions");
  const snapshot = structuredClone(model),
    accessors = list(snapshot.accessors ?? [], "accessors");
  const views = list(snapshot.bufferViews ?? [], "bufferViews"),
    buffers = list(snapshot.buffers ?? [], "buffers");
  const records = [];
  let encodedBytes = 0,
    reservedBytes = 0;
  const reserve = (size) => {
    if (
      ...
      size > maxDecodedBufferBytes ||
      size > maxDecodedBytes - reservedBytes
    )
      fail("LIMIT", "Decoded Draco storage exceeds byte budget");
    reservedBytes += size;
  };
  function accessor(at, label, isIndex = false) {
    const a = index(accessors, at, label),
      format = formats.get(a.componentType),
      width = widths.get(a.type);
    if (
      !format ||
      !width ||
      (isIndex &&
        (width !== 1 || ![5121, 5123, 5125].includes(a.componentType) || a.normalized === true))
    )
      fail("LAYOUT", "Unsupported Draco accessor format");
    ...
    if (
      ...
      fail("LAYOUT", "Invalid normalization");
    if (a.sparse !== undefined && available)
      fail("LAYOUT", "Compressed accessors cannot also supply sparse overrides");
  ...
  for (let mesh = 0; mesh < (snapshot.meshes ?? []).length; mesh++) {
    ...
    for (let primitive = 0; primitive < primitives.length; primitive++) {
      ...
      const ext = object(p.extensions[EXT], "Draco extension"),
        attrs = object(p.attributes, "primitive attributes");
      const mappings = object(ext.attributes, "Draco attribute map"),
        names = Object.keys(mappings);
      if (!names.length || names.length > 64 || Object.keys(attrs).length > 64)
        fail("LIMIT", "Invalid Draco attribute count");
      if (available && (p.mode ?? 4) !== 4)
        fail("TOPOLOGY", "DRACOLoader triangle-list decoding requires TRIANGLES");
      if (![4, 5].includes(p.mode ?? 4)) fail("TOPOLOGY", "Draco requires triangle topology");
      const source = index(views, ext.bufferView, "Draco bufferView"),
        sourceBuffer = index(buffers, source.buffer, "Draco buffer");
      integer(sourceBuffer.byteLength, "source buffer length", 1);
      integer(source.byteOffset ?? 0, "source offset");
      integer(source.byteLength, "source length", 1);
      if (
        source.byteLength > sourceBuffer.byteLength - (source.byteOffset ?? 0) ||
        source.byteStride !== undefined
      )
        fail("LAYOUT", "Invalid compressed view range/stride");
      if (available && source.byteLength > maxEncodedBytes - encodedBytes)
        fail("LIMIT", "Compressed Draco snapshots exceed budget");
      if (available) encodedBytes += source.byteLength;
      const positions = index(accessors, attrs.POSITION, "POSITION accessor");
      integer(positions.count, "vertex count", 1);
      for (const at of Object.values(attrs))
        if (index(accessors, at, "attribute accessor").count !== positions.count)
          fail("LAYOUT", "Primitive attribute counts disagree");
      const attributes = names.map((name, slot) => {
        if (!Object.hasOwn(attrs, name))
          fail("LAYOUT", "Draco attribute is not a primitive attribute");
      ...
      if (indices && (p.mode ?? 4) === 4 && indices.count % 3)
      ...
      if (!available)
        for (const field of [...attributes, ...(indices ? [indices] : [])]) {
          const a = accessors[field.at];
          if (a.bufferView === undefined && a.sparse === undefined)
            fail("FALLBACK", "Optional Draco needs backed or sparse fallback accessors");
        }
      records.push({
        mesh,
        primitive,
        source: ext.bufferView,
        attributes,
        indices,
  ...
  let consumed = false;
  return Object.freeze({
    decodedBytes: reservedBytes,
    skippedBuffers: Object.freeze(skipped),
    async decode(supplied, { signal } = {}) {
      if (consumed) fail("STATE", "Draco plan is single-use");
      consumed = true;
      abort(signal);
      if (!Array.isArray(supplied) || supplied.length !== buffers.length)
        fail("BUFFER", "Supply buffers in original index order");
      if (!available)
        for (const r of records)
          for (const field of [...r.attributes, ...(r.indices ? [r.indices] : [])]) {
            const a = accessors[field.at];
            const references = [
              a.bufferView,
            ...
            for (const at of references) {
              const view = index(views, at, "fallback view"),
                declaration = index(buffers, view.buffer, "fallback buffer");
              const data = byteView(supplied[view.buffer]),
                offset = integer(view.byteOffset ?? 0, "fallback offset");
              integer(view.byteLength, "fallback length", 1);
              if (
                data.length < declaration.byteLength ||
                view.byteLength > declaration.byteLength - offset
              )
                fail("FALLBACK", "Truncated fallback storage");
            }
          }
      const sources = available
        ? records.map((record) => {
            const view = views[record.source];
            if (Object.keys(view.extensions ?? {}).length)
              fail("LAYOUT", "Resolve compressed bufferView extensions before Draco decoding");
            const data = byteView(supplied[view.buffer]),
              start = view.byteOffset ?? 0;
            if (
              data.length < buffers[view.buffer].byteLength ||
              view.byteLength > data.length - start
            )
              fail("BUFFER", "Truncated Draco buffer");
            return data.slice(start, start + view.byteLength).buffer;
          })
        : [];
      const json = structuredClone(snapshot),
        output = supplied.slice();
      ...
      if (json.meshes)
        json.meshes = json.meshes.map((mesh) => ({
          ...mesh,
      ...
      function allocate(size) {
        if (
          !Number.isSafeInteger(size) ||
          size > maxDecodedBufferBytes ||
          size > maxDecodedBytes - decodedBytes
        )
          fail("LIMIT", "Decoded Draco storage exceeds byte budget");
        decodedBytes += size;
      ...
      function publish(data, definition, stride, target) {
        ...
        const a = { ...definition, bufferView, byteOffset: 0 };
        delete a.sparse;
      ...
      for (let i = 0; i < records.length; i++) {
        ...
        const record = records[i],
          p = json.meshes[record.mesh].primitives[record.primitive];
        if (available) {
          const taskConfig = {
            attributeIDs: {},
            attributeTypes: {},
          ...
          for (const a of record.attributes) {
            taskConfig.attributeIDs[a.key] = a.uniqueID;
            taskConfig.attributeTypes[a.key] = formats.get(a.componentType)[0].name;
          }
          let geometry;
          try {
            ...
            geometry = await receive(decoder.decodeGeometry(sources[i], taskConfig), signal);
            abort(signal);
            object(geometry, "decoded geometry");
            object(geometry.attributes, "decoded attributes");
            for (const a of record.attributes) {
            ...
            if (indexAttribute) {
              ...
              if (count % 3 || (declared && count !== declared.count))
                fail("DECODE", "Decoded index count disagrees with triangle accessor");
              const source = attributeLayout(indexAttribute, count, 1);
              if (
                ![Uint8Array, Uint16Array, Uint32Array].some(
                  (Type) => source.array instanceof Type,
                ) ||
                indexAttribute.normalized === true
              )
                fail("DECODE", "Decoded indices must be unsigned integers");
              const componentType = declared?.componentType ?? 5125,
                [, size, setter] = formats.get(componentType);
              const data = allocate(aligned(count * size)),
                view = new DataView(data.buffer),
                restart = 2 ** (size * 8) - 1;
              for (let k = 0; k < count; k++) {
                const value = source.array[source.offset + k * source.stride];
                if (value >= record.count || value >= restart)
                  fail("DECODE", "Decoded index is outside the accessor domain");
                view[setter](k * size, value, true);
              }
              p.indices = publish(
                data,
                declared ? accessors[declared.at] : { type: "SCALAR", componentType, count },
                size,
                34963,
              );
            }
            ...
            abort(signal);
            if (error instanceof GltfDracoError) throw error;
            throw new GltfDracoError(
              "GLTF_DRACO_DECODE",
              `Draco failed for mesh ${record.mesh} primitive ${record.primitive}`,
              { cause: error },
            );
          } finally {
      ...
      for (const key of ["extensionsRequired", "extensionsUsed"])
        if (json[key]) json[key] = json[key].filter((name) => name !== EXT);
      abort(signal);
      return { json, sourceJson: snapshot, buffers: output, decodedBytes, decodedPrimitives };
    },
  });
}
