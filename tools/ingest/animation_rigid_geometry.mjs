...
export class AnimationRigidError extends Error {
  constructor(code, message) {
    super(`${code}: ${message}`);
    this.name = "AnimationRigidError";
    this.code = code;
  }
}
const fail = (code, message) => {
  throw new AnimationRigidError("ANIMATION_RIGID_" + code, message);
};
...
function finite(value) {
  if (typeof value !== "number" || !Number.isFinite(value) || !Number.isFinite(Math.fround(value)))
    fail("VALUE", "Expected finite f32-representable data");
  return value;
}
...
export function canUseRigidAnimationGeometry(pose, geometry) {
  return (
    ...
    (geometry.morphTargets === undefined ||
      (Array.isArray(geometry.morphTargets) && geometry.morphTargets.length === 0)) &&
    Number.isSafeInteger(geometry.node) &&
    geometry.node >= 0 &&
    geometry.node < pose?.nodeCount &&
    Array.isArray(pose.instances) &&
    !pose.instances.some((s) => s.node === geometry.node) &&
    pose.morphOffsets?.length === pose.nodeCount + 1 &&
    Number.isSafeInteger(pose.morphOffsets[geometry.node]) &&
    pose.morphOffsets[geometry.node] >= 0 &&
    pose.morphOffsets[geometry.node] === pose.morphOffsets[geometry.node + 1]
  );
}

export function createGpuRigidGeometryPool(
  device,
  pose,
  {
  ...
  integer(maxMeshes, 1, 4096, "mesh budget");
  integer(maxComponents, 1, Number.MAX_SAFE_INTEGER, "component budget");
  if (
    typeof label !== "string" ||
    !device?.limits ||
    ...
    fail("DEVICE", "Lend a WebGPU device");
  const nodeCount = integer(pose?.nodeCount, 0, 65536, "pose node count"),
    worlds = pose.worldMatrices;
  fixed(worlds, nodeCount * 16, "pose matrices");
  fixed(pose.morphOffsets, nodeCount + 1, "morph offsets");
  ...
  device.lost
    .then(
      (info) => {
        if (!disposed)
          stopWith(
            new AnimationRigidError("ANIMATION_RIGID_LOST", info?.message || "WebGPU device lost"),
          );
      },
      (error) => {
        if (!disposed) stopWith(error);
      },
    )
    .catch(() => {});
  function live() {
    if (terminal) throw terminal;
    if (disposed) fail("DISPOSED", "Rigid geometry pool was disposed");
    if (pose.disposed) fail("POSE", "Borrowed pose was disposed");
  }
  function world(node, out) {
    live();
    if (pose.nodeCount !== nodeCount || pose.worldMatrices !== worlds)
      fail("POSE", "Pose storage identity changed");
    fixed(worlds, nodeCount * 16, "pose matrices");
    const stamp = integer(pose.version, 0, Number.MAX_SAFE_INTEGER, "pose version");
    for (let i = 0; i < 16; i++) out[i] = finite(worlds[node * 16 + i]);
    if (out[3] !== 0 || out[7] !== 0 || out[11] !== 0 || out[15] !== 1)
      fail("TRANSFORM", "World matrix must be affine");
    if (pose.version !== stamp || pose.disposed)
      fail("CHANGED", "Pose changed during transform capture");
    return stamp;
  }
  function pack(geometry) {
    if (!canUseRigidAnimationGeometry(pose, geometry))
      fail("DYNAMIC", "This geometry needs the ordinary deformation path");
    const node = geometry.node,
      length = geometry.positions?.length;
    integer(length, 3, maxComponents, "position extent");
    if (length % 3) fail("GEOMETRY", "Positions must be XYZ");
    const count = length / 3,
      positions = numbers(geometry.positions, length, "positions");
    const normals =
      geometry.normals === undefined ? null : numbers(geometry.normals, length, "normals");
    const tangents =
      geometry.tangents === undefined ? null : numbers(geometry.tangents, count * 4, "tangents");
    if (length + (normals ? length : 0) + (tangents ? count * 4 : 0) > maxComponents)
      fail("LIMIT", "Geometry exceeds component budget");
    const size = count * 40;
    if (
      size > maxBytes ||
      !Number.isSafeInteger(device.limits.maxBufferSize) ||
      size > device.limits.maxBufferSize
    )
      fail("LIMIT", "Packed vertices exceed byte/device budget");
    ...
    for (let v = 0; v < count; v++) {
      ...
      const w = tangents ? finite(tangents[v * 4 + 3]) : 1;
      if (Math.abs(w) !== 1) fail("GEOMETRY", "Tangent handedness must be -1 or 1");
      packed[v * 10 + 9] = w;
    }
    const words = new Uint32Array(packed.buffer);
    let hash = 2166136261;
    for (const word of words) hash = Math.imul(hash ^ word, 16777619) >>> 0;
    const mask = (normals ? 1 : 0) | (tangents ? 2 : 0),
      key = `${mask}:${count}:${hash}`;
    ...
    const vertexLayout = Object.freeze({
      arrayStride: 40,
      stepMode: "vertex",
      attributes: Object.freeze(attributes.map(Object.freeze)),
    });
    return { node, count, words, key, vertexLayout };
  }
  async function allocate(input) {
    const entry = { ...input, buffer: null, refs: 0, retired: false };
    entries.add(entry);
    bytes += input.words.byteLength;
    let syncError,
      depth = 0;
    try {
      ...
      entry.buffer = device.createBuffer({
        label,
        size: input.words.byteLength,
        usage: 32 | 4,
        mappedAtCreation: true,
      });
      new Uint32Array(entry.buffer.getMappedRange()).set(input.words);
      entry.buffer.unmap();
    } catch (error) {
      syncError = error;
    }
  ...
  function instance(entry, node, initialWorld, stamp) {
    let released = false,
      ...
      version = 0,
      poseVersion = stamp;
    const worldMatrix = initialWorld,
      scratch = new Float64Array(16);
    const handle = Object.freeze({
      node,
      vertexCount: entry.count,
      vertexLayout: entry.vertexLayout,
      vertexBuffer: entry.buffer,
      bufferBytes: 0,
      sharedBufferBytes: entry.words.byteLength,
      worldMatrix,
      get version() {
        return version;
      },
      ...
      update() {
        live();
        if (released) fail("DISPOSED", "Rigid mesh was disposed");
        if (updating) fail("REENTRANT", "Rigid mesh update cannot be reentered");
        updating = true;
        try {
          fixed(worldMatrix, 16, "mesh transform");
          const next = world(node, scratch);
          worldMatrix.set(scratch);
          poseVersion = next;
          version++;
          return handle;
        } finally {
      ...
      dispose() {
        if (updating) fail("REENTRANT", "Cannot dispose while updating");
        if (!released) {
          released = true;
          handles.delete(handle);
          if (--entry.refs === 0) retire(entry);
        }
      },
    });
    entry.refs++;
  ...
  const result = Object.freeze({
    ...
    async addMesh(geometry, { maxAdditionalBytes = maxBytes } = {}) {
      live();
      if (busy) fail("REENTRANT", "Register rigid meshes sequentially");
      busy = true;
      capturing = true;
      let entry,
        created = false;
      try {
        ...
        const input = pack(geometry),
          matrix = new Float64Array(16),
          stamp = world(input.node, matrix);
        entry = buckets
          .get(input.key)
          ?.find((e) => e.words.every((word, i) => word === input.words[i]));
        if (!entry) {
          if (input.words.byteLength > Math.min(maxBytes - bytes, maxAdditionalBytes))
            fail("LIMIT", "Unique rigid vertices exceed GPU budget");
          created = true;
          entry = await allocate(input);
          if (pose.version !== stamp || pose.disposed)
            fail("CHANGED", "Pose changed while preparing rigid geometry");
          const bucket = buckets.get(input.key) ?? [];
          bucket.push(entry);
          buckets.set(input.key, bucket);
        }
        live();
        return instance(entry, input.node, matrix, stamp);
      } catch (error) {
        if (created && entry && entry.refs === 0) retire(entry);
        throw error;
      } finally {
        capturing = false;
        busy = false;
    ...
    async whenIdle() {
      live();
      try {
        await Promise.race([Promise.all([pending, device.queue.onSubmittedWorkDone()]), stopped]);
      } catch (error) {
        throw stopWith(error);
      }
      live();
    ...
    dispose() {
      if (capturing) fail("REENTRANT", "Cannot dispose during synchronous capture");
      if (!disposed) {
        disposed = true;
        release();
        stop(
          new AnimationRigidError("ANIMATION_RIGID_DISPOSED", "Rigid geometry pool was disposed"),
        );
      }
    },
  });
