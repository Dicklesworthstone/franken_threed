...
const files = {
  'index.html': `<!doctype html><html><head><title>Offline application</title>
    ...
    </head><body><h1 id="title">Packed application</h1><img id="pixel" src="pixel.png">
  ...
  'entry.mjs': `import { bump, value, object } from './shared.mjs';
    ...
    const choose = flag => import(flag ? './left.mjs' : './right.mjs');
    const binary = await (await fetch(new URL('./add.wasm', import.meta.url))).arrayBuffer();
    ...
    globalThis.result={events:globalThis.events, live:value, cyclic:read(), shared:lazy.object===object,
      ...
      wasm:instance.exports.add(20,22), pixel:document.querySelector('#pixel').naturalWidth, style};
...
const report = packHtml(path.join(input, 'index.html'), output);
...
try {
  ...
  assert.deepEqual(actual,{events:['classic','entry'],live:2,cyclic:2,shared:true,distinct:true,lazy:7,left:'left',right:'right',wasm:42,pixel:1,style:'rgb(12, 34, 56)'});
