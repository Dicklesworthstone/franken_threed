...
function transport(resources, calls) {
  return async (url, options) => {
    ...
    const bytes = resources[new URL(url).pathname.split("/").at(-1)];
...
function glb(json, bin) {
  ...
  const output = new Uint8Array(12 + 8 + length + 8 + binLength),
...
test("runtime external loading decodes meshopt and skips even enormous fallback placeholders", async () => {
  ...
  const source = encode(f.model),
    calls = [];
  const result = await loadGltfAsset(source, {
    baseURL: jsonURL,
  ...
  assert.equal(calls[0].url, "https://assets.example.test/models/compressed.bin");
  assert.equal(calls[0].options.credentials, "omit");
  assert.equal(calls[0].options.redirect, "error");
  assert.equal(result.bytesLoaded, source.length + f.encoded.length);
  ...
  assert.equal(result.sourceJson.buffers[1].byteLength, 2 ** 40);
  assert.ok(Object.isFrozen(result.buffers));
});

test("complete GLB loads and decompresses without fetching or creating dummy fallback storage", async () => {
  const f = fixture();
  delete f.model.buffers[0].uri;
  const source = glb(f.model, f.encoded);
  const result = await loadGltfAsset(source, {
    meshoptDecoder: decoder,
    fetch() {
      assert.fail("GLB has no external resources");
    },
  });
  assert.equal(result.bytesLoaded, source.length);
  assert.deepEqual(result.buffers[2], f.v.expected);
  assert.equal(result.buffers[0].length, f.encoded.length);
  assert.equal(result.sourceJson.bufferViews[0].buffer, 1);
...
test("optional meshopt without decoder fetches only the actual fallback and keeps correct data", async () => {
  ...
  const result = await loadGltfAsset(encode(f.model), {
    baseURL: jsonURL,
    fetch: transport({ "fallback.bin": f.buffers[1] }, calls),
  });
  assert.equal(calls.length, 1);
  assert.ok(calls[0].url.endsWith("/fallback.bin"));
  assert.equal(result.buffers[0], null);
  assert.deepEqual(result.buffers[1], f.v.expected);
  assert.equal(result.decodedBytes, 0);
  assert.equal(result.decodedBufferViews, 0);
});
...
test("missing decoder, expansion limits and malformed compressed ranges stop before dependency I/O", async () => {
  for (const options of [
    {},
    { meshoptDecoder: decoder, maxDecodedBytes: 47 },
    { meshoptDecoder: decoder, maxDecodedBufferBytes: 47 },
  ]) {
    const f = fixture();
    let calls = 0;
    await assert.rejects(
      loadGltfAsset(encode(f.model), {
        baseURL: jsonURL,
        ...options,
        fetch() {
          calls++;
  ...
  const f = fixture();
  f.model.bufferViews[0].extensions[f.ext].byteOffset = f.encoded.length;
  await assert.rejects(
    loadGltfAsset(encode(f.model), {
      baseURL: jsonURL,
      meshoptDecoder: decoder,
...
test("uncompressed data URI buffers and lazy images keep their existing import and byte-count behavior", async () => {
  const png = Uint8Array.from(
    Buffer.from(
      "iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mP8/x8AAwMCAO+aN3kAAAAASUVORK5CYII=",
      "base64",
    ),
  );
  const json = {
  ...
  const result = await loadGltfAsset(source, {
    ...
    meshoptDecoder: {
      get supported() {
        assert.fail("no codec for plain input");
      },
    },
    fetch: transport({ "image.png": png }, calls),
  });
  assert.deepEqual(result.buffers[0], new Uint8Array([1, 2, 3, 4]));
  assert.equal(result.bytesLoaded, source.length + 4);
  assert.equal(result.json, result.sourceJson);
  assert.equal(result.decodedBytes, 0);
...
test("decoded image bufferViews remain readable after compression normalization", async () => {
  ...
  const stride = Math.ceil(png.length / 4) * 4,
    expected = new Uint8Array(stride);
  expected.set(png);
  // Valid ATTRIBUTES v0: one element, all zero deltas, baseline in the tail.
  const encoded = new Uint8Array(1 + stride + Math.max(32, stride));
  encoded[0] = 0xa0;
  encoded.set(expected, encoded.length - stride);
  const f = fixture({ stride, count: 1, mode: "ATTRIBUTES", encoded, expected });
  delete f.model.bufferViews[0].byteStride;
  f.model.images = [{ bufferView: 0, mimeType: "image/png" }];
  delete f.model.buffers[0].uri;
  ...
  assert.equal(asset.sourceJson.images[0].bufferView, 0);
  assert.equal(asset.json.images[0].bufferView, 0);
});

test("compressed resources retain declared origin and response-redirect restrictions", async () => {
  const f = fixture();
  f.model.buffers[0].uri = "https://other.example.test/geometry.bin";
  let calls = 0;
  await assert.rejects(
    loadGltfAsset(encode(f.model), {
      baseURL: jsonURL,
...
test("root URLs and shared compressed resource URLs load once, but each decoded view owns its output", async () => {
  ...
  const source = encode(f.model);
  const result = await loadGltfAsset(jsonURL, {
    meshoptDecoder: decoder,
    fetch: transport({ "scene.gltf": source, "compressed.bin": f.encoded }, calls),
  });
  assert.equal(calls.length, 2);
  assert.equal(result.decodedBytes, 96);
  assert.equal(result.bytesLoaded, source.length + f.encoded.length);
  result.buffers[2][0] = 99;
  assert.equal(result.buffers[3][0], 0);
});

test("input streams remain byte-bounded; malformed compressed data cannot publish a partial asset", async () => {
  const f = fixture(),
    source = encode(f.model),
    original = source.slice();
  await assert.rejects(
    loadGltfAsset(source, {
      baseURL: jsonURL,
      meshoptDecoder: decoder,
      maxBytes: source.length + f.encoded.length - 1,
      fetch: async () => new Response(f.encoded),
    }),
...
test("abort during codec readiness rejects the loading operation without publishing source-mutated data", async () => {
  const f = fixture(),
    ...
    reason = new Error("cancel decoding");
  let ready;
  ...
  const pending = loadGltfAsset(encode(f.model), {
    baseURL: jsonURL,
    ...
    meshoptDecoder: {
      ...
      decodeGltfBuffer: decoder.decodeGltfBuffer,
    },
    fetch: async () => new Response(f.encoded),
  });
  await began;
...
test("unused tagged fallback declarations are skipped without fetching or allocating them", async () => {
  const f = fixture();
  f.model.buffers.push({ byteLength: 2 ** 40, extensions: { [f.ext]: { fallback: true } } });
  const calls = [];
