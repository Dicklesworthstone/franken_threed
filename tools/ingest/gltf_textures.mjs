/** Owned core glTF image uploads and sampler/mipmap realization for WebGPU.
 ...
 * maxTextureBytes bounds RGBA8 texel storage (including mips), not driver overhead.
...
export async function createGltfTextureResources(device,requests,readImage,{
  createImageBitmap:decodeImage=globalThis.createImageBitmap,signal,
  maxTextureBytes=256*1024*1024,maxImagePixels=16*1024*1024,maxTextures=4096,
  defaultMagFilter=9729,defaultMinFilter=9987,
}={}) {
  positive(maxTextureBytes,'texture byte limit');positive(maxImagePixels,'image pixel limit');positive(maxTextures,'texture count');
  ...
  let disposed=false,terminal=null,textureBytes=0;
  ...
  if(entries.size) {
    if(!device?.queue||typeof device.createTexture!=='function'||typeof device.pushErrorScope!=='function'||typeof device.popErrorScope!=='function'||
       typeof device.queue.copyExternalImageToTexture!=='function'||typeof device.queue.onSubmittedWorkDone!=='function'||typeof decodeImage!=='function')fail('DEVICE','WebGPU texture upload and ImageBitmap decoding are required');
    ...
    if(device.lost&&typeof device.lost.then==='function')device.lost.then(info=>{
      terminal=new GltfTextureError('GLTF_TEXTURE_DEVICE_LOST',info?.message ?? 'WebGPU device lost');release();
    },error=>{terminal=error;release();});
  }
