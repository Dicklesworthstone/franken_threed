...
export class GltfTextureError extends Error {
  constructor(code, message) {
    super(`${code}: ${message}`);
    this.name = "GltfTextureError";
    this.code = code;
  }
}
const fail = (code, message) => {
  throw new GltfTextureError("GLTF_TEXTURE_" + code, message);
};
...
const index = (n, label) => {
  if (!Number.isSafeInteger(n) || n < 0) fail("REQUEST", `Invalid ${label}`);
  return n;
};
const wraps = new Map([
  [33071, "clamp-to-edge"],
...
export function gltfImageDimensions(bytes, mimeType) {
  if (
    ...
    bytes.buffer.resizable
  )
    fail("IMAGE", "Expected fixed image bytes");
  ...
  if (mimeType === "image/png") {
    if (
      ...
      fail("IMAGE", "Invalid PNG IHDR");
    return {
      width: positive(view.getUint32(16), "PNG width"),
      height: positive(view.getUint32(20), "PNG height"),
    };
  }
  if (mimeType === "image/jpeg" && bytes.length >= 4 && bytes[0] === 255 && bytes[1] === 216) {
    ...
    while (at < bytes.length) {
      ...
      if (
        ...
        return {
          width: positive(view.getUint16(at + 5), "JPEG width"),
          height: positive(view.getUint16(at + 3), "JPEG height"),
        };
      }
      at += length;
    }
 ...
 */
export async function createGltfTextureResources(
  device,
  requests,
  readImage,
  {
    createImageBitmap: decodeImage = globalThis.createImageBitmap,
    ktx2Loader = null,
    signal,
    maxTextureBytes = 256 * 1024 * 1024,
    maxImagePixels = 16 * 1024 * 1024,
    maxTextures = 4096,
    maxTranscodeBytes = 256 * 1024 * 1024,
    defaultMagFilter = 9729,
    defaultMinFilter = 9987,
  } = {},
) {
  positive(maxTranscodeBytes, "transcode byte limit");
  positive(maxTextureBytes, "texture byte limit");
  positive(maxImagePixels, "image pixel limit");
  positive(maxTextures, "texture count");
  if (!Array.isArray(requests) || requests.length > maxTextures || typeof readImage !== "function")
    fail("REQUEST", "Invalid texture requests or image provider");
  if (![9728, 9729].includes(defaultMagFilter) || !filters.has(defaultMinFilter))
    fail("SAMPLER", "Invalid default filters");
  // Snapshot and validate every request before fetching or allocating anything.
  const entries = new Map(),
    images = new Map(),
    expectedMimeTypes = new Map();
  for (const request of requests) {
    const textureIndex = index(request?.textureIndex, "texture index"),
      imageIndex = index(request?.imageIndex, "image index");
    ...
    if (mimeType !== undefined) {
      if (!["image/png", "image/jpeg", "image/ktx2"].includes(mimeType))
        fail("REQUEST", "Unsupported requested image MIME type");
      if (expectedMimeTypes.has(imageIndex) && expectedMimeTypes.get(imageIndex) !== mimeType)
        fail("REQUEST", "Conflicting image MIME requirements");
      expectedMimeTypes.set(imageIndex, mimeType);
    }
    const colorSpace = request.colorSpace;
    if (colorSpace !== "srgb" && colorSpace !== "linear")
      fail("REQUEST", "Invalid texture color space");
    const input = request.sampler ?? {},
      mag = input.magFilter ?? defaultMagFilter,
      min = input.minFilter ?? defaultMinFilter;
    const u = wraps.get(input.wrapS ?? 10497),
      v = wraps.get(input.wrapT ?? 10497),
      filter = filters.get(min);
    if (!u || !v || !filter || ![9728, 9729].includes(mag))
      fail("SAMPLER", "Unsupported glTF sampler");
    const sampler = {
      addressModeU: u,
      addressModeV: v,
      magFilter: mag === 9728 ? "nearest" : "linear",
      minFilter: filter[0],
      mipmapFilter: filter[1],
      ...(filter[2] ? {} : { lodMaxClamp: 0 }),
    };
    const key = textureIndex + ":" + colorSpace,
      signature = JSON.stringify([imageIndex, sampler]);
    if (entries.has(key)) {
      if (entries.get(key).signature !== signature) fail("REQUEST", "Conflicting texture identity");
      continue;
    }
    const entry = { key, imageIndex, colorSpace, sampler, signature, mips: filter[2] };
    ...
    const group = groups.get(colorSpace);
    group.mips ||= entry.mips;
  ...
  if (entries.size) {
    if (
      ...
      fail("DEVICE", "WebGPU texture upload is required");
    positive(device.limits?.maxTextureDimension2D, "device texture dimension limit");
    if (device.lost && typeof device.lost.then === "function")
      device.lost.then(
        (info) => {
          terminal = new GltfTextureError(
            "GLTF_TEXTURE_DEVICE_LOST",
            info?.message ?? "WebGPU device lost",
          );
          release();
          transcodeCancel?.abort(terminal);
        },
        (error) => {
          terminal = error;
          release();
          transcodeCancel?.abort(error);
        },
      );
  }
  ...
  async function checked(operation) {
    ...
    if (v || m) fail("GPU", (v || m).message ?? "WebGPU upload failed");
    live();
    return result;
  }
  ...
  async function uploadKtx2(image, imageIndex, groups) {
    if (typeof device.queue.writeTexture !== "function")
      fail("DEVICE", "KTX2 uploads require queue.writeTexture");
    ...
    const dfd = new DataView(
      image.bytes.buffer,
      image.bytes.byteOffset,
      image.bytes.byteLength,
    ).getUint32(48, true);
    if (
      [...groups.keys()].some((space) => space !== header.colorSpace) ||
      image.bytes[dfd + 13] !== (header.colorSpace === "srgb" ? 1 : 0)
    ) {
    ...
    transcodeCancel = controller;
    const cancel = () => controller.abort(signal.reason);
    signal?.addEventListener("abort", cancel, { once: true });
    let decoded;
    try {
      live();
      decoded = await transcodeGltfKtx2(image.bytes, ktx2Loader, {
        ...limits,
        colorSpace: header.colorSpace,
        features: device.features ?? new Set(),
        maxDecodedBytes: maxTranscodeBytes,
    ...
    live();
    if (decoded.byteLength > maxTextureBytes - textureBytes)
      fail("LIMIT", "KTX2 mip storage exceeds texture byte budget");
    ...
    await checked(() => {
      ...
      const texture = device.createTexture({
        label: `glTF KTX2 image ${imageIndex}`,
        size: [decoded.width, decoded.height, 1],
        format: decoded.format,
        mipLevelCount: decoded.levelCount,
        usage: 2 | 4,
      });
      owned.add(texture);
      for (const [mipLevel, mip] of decoded.mipmaps.entries()) {
        device.queue.writeTexture(
          ...
          [mip.copyWidth, mip.copyHeight, 1],
        );
      }
  ...
  try {
    for (const [imageIndex, groups] of images) {
      live();
      const image = await readImage(imageIndex);
      live();
      if (
        expectedMimeTypes.has(imageIndex) &&
        image?.mimeType !== expectedMimeTypes.get(imageIndex)
      )
        fail("IMAGE", "Loaded image MIME type disagrees with the selected texture route");
      if (image?.mimeType === "image/ktx2") {
        await uploadKtx2(image, imageIndex, groups);
        continue;
      }
      if (
        ...
        fail("DEVICE", "PNG/JPEG uploads require ImageBitmap and external image copy");
      ...
      if (
        ...
        fail("LIMIT", "Image dimensions exceed decode/device budget");
      ...
      for (const group of groups.values()) {
        ...
        let bytes = 0,
          w = width,
          h = height;
        for (let level = 0; level < group.levels; level++) {
          bytes += w * h * 4;
          w = Math.max(1, Math.floor(w / 2));
          h = Math.max(1, Math.floor(h / 2));
        }
        if (bytes > maxTextureBytes - textureBytes)
          fail("LIMIT", "Texture mip storage exceeds byte budget");
        textureBytes += bytes;
      }
      ...
      try {
        ...
        if (
          ...
          fail("IMAGE", "Decoded image dimensions disagree with header");
        for (const [colorSpace, group] of groups) {
          ...
          await checked(() => {
            ...
            const texture = device.createTexture({
              label: `glTF image ${imageIndex} ${colorSpace}`,
              size: [width, height, 1],
              format: "rgba8unorm",
              viewFormats: colorSpace === "srgb" ? ["rgba8unorm-srgb"] : [],
              mipLevelCount: group.levels,
              usage: 2 | 4 | 16,
            });
            owned.add(texture);
            ...
            if (mipPipeline) {
              const encoder = device.createCommandEncoder({ label: "glTF mip chain" }),
    ...
    if (entries.size) await device.queue.onSubmittedWorkDone();
    live();
    return Object.freeze({
      get textureBytes() {
        return owned.size ? textureBytes : 0;
      },
      get textureCount() {
        return owned.size;
      },
      ...
      resolveTexture(request) {
        live();
        const entry = entries.get(request.textureIndex + ":" + request.colorSpace);
        if (!entry || entry.imageIndex !== request.imageIndex)
          fail("REQUEST", "Texture was not prepared");
        return resolved.get(entry.key);
      },
      dispose() {
        if (!disposed) {
          disposed = true;
