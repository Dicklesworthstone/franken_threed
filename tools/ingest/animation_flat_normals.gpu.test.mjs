...
import {
  ANIMATION_DEFORM_WGSL,
  ANIMATION_FLAT_NORMALS_WGSL,
  createGpuAnimationDeformer,
} from "./animation_webgpu.mjs";
...
function deviceSpy() {
  ...
  const device = {
    ...
    createBuffer(options) {
      const buffer = {
        ...options,
        bytes: new ArrayBuffer(options.size),
        destroyed: 0,
        getMappedRange() {
          return this.bytes;
        },
    ...
    createShaderModule(options) {
      assert.ok([ANIMATION_DEFORM_WGSL, ANIMATION_FLAT_NORMALS_WGSL].includes(options.code));
      return options;
    },
    ...
    createBindGroup(options) {
      if (device.bindFailure && options.entries.length === 2)
        throw new Error("normal binding failed");
      ...
      return {
        beginComputePass() {
          ...
          return {
            ...
            dispatchWorkgroups(...values) {
              pass.dispatch = values;
            },
            end() {
              assert.ok(pass.dispatch);
              pass.ended = true;
            },
          };
        },
    ...
    queue: {
      ...
      submit(commands) {
        for (const command of commands) {
          ...
          submissions.push({
            ...command,
            palette: new Float32Array(deform[3].resource.buffer.bytes).slice(),
...
test("dynamic flat normals add an ordered pass without extra buffers or a changed vertex ABI", async () => {
  const d = deviceSpy(),
    { pose, geometry } = fixture(),
    gpu = await createGpuAnimationDeformer(d, pose, geometry);
  assert.equal(d.buffers.length, 7);
  assert.equal(d.pipelines.length, 2);
  assert.equal(d.bindings.length, 2);
  const passes = d.submissions[0].passes;
  assert.deepEqual(
    passes.map((p) => p.pipeline.compute.entryPoint),
    ["deform", "flat_normals"],
  );
  ...
  assert.deepEqual(gpu.vertexLayout.attributes, [
    { shaderLocation: 0, offset: 0, format: "float32x3" },
    { shaderLocation: 1, offset: 12, format: "float32x3" },
  ]);
  assert.equal(
    gpu.bufferBytes,
    d.buffers.reduce((sum, b) => sum + b.size, 0),
  );
...
test("every uploaded pose submits both passes before palette inputs change again", async () => {
  ...
  await gpu.whenIdle();
  assert.deepEqual(
    d.submissions.map((s) => s.weights[0]),
    [0, 0.25, 0.75],
  );
  assert.deepEqual(
    d.submissions.map((s) => s.palette[30]),
    [0, 0.5, 1.5],
  );
...
for (const [triangles, limit] of [
  ...
  test(`triangle dispatch covers ${triangles} faces with dimension limit ${limit}`, async () => {
    const d = deviceSpy();
    d.limits.maxComputeWorkgroupsPerDimension = limit;
    const positions = Array.from({ length: triangles }, () => plainGeometry().positions).flat();
    const gpu = await createGpuAnimationDeformer(d, plainPose(), {
      node: 0,
      flatNormals: true,
      positions,
    });
    const config = new Uint32Array(d.buffers[6].bytes),
      pass = d.submissions[0].passes[1];
    const gx = Math.min(Math.ceil(triangles / 64), config[3]),
      gy = Math.ceil(Math.ceil(triangles / 64) / gx);
    assert.deepEqual(pass.dispatch, [gx, gy]);
    assert.ok(gx <= limit && gy <= limit);
    const visited = [];
    for (let y = 0; y < gy; y++)
      for (let x = 0; x < gx; x++)
        for (let lane = 0; lane < 64; lane++) {
          const triangle = (y * gx + x) * 64 + lane;
          if (triangle < triangles) visited.push(triangle);
        }
    assert.deepEqual(
      visited,
...
test("static or authored-normal geometry keeps one pipeline and one pass", async () => {
  for (const normal of [false, true]) {
    ...
    delete geometry.flatNormals;
    if (normal) geometry.normals = [0, 0, 1, 0, 0, 1, 0, 0, 1];
    const gpu = await createGpuAnimationDeformer(d, plainPose(), geometry);
    assert.equal(d.pipelines.length, 1);
    assert.equal(d.bindings.length, 1);
    assert.equal(d.submissions[0].passes.length, 1);
    assert.equal(gpu.vertexLayout.attributes.length, normal ? 2 : 1);
    gpu.dispose();
  }
});
...
test("flat flag and geometry snapshot survive asynchronous construction and later input edits", async () => {
  const d = deviceSpy(),
    { pose, geometry } = fixture();
  d.gates = { flat_normals: deferred() };
  let reads = 0;
  Object.defineProperty(geometry, "flatNormals", {
    get() {
      reads++;
      return true;
    },
  ...
  geometry.positions.fill(99);
  geometry.morphTargets[0].positions.fill(99);
  d.gates.flat_normals.resolve();
  const gpu = await pending;
  assert.equal(reads, 1);
  assert.deepEqual([...new Float32Array(d.buffers[0].bytes).slice(0, 3)], [0, 0, 0]);
  gpu.update();
  assert.equal(d.submissions.at(-1).passes.length, 2);
  gpu.dispose();
});
...
test("generated normals use the exact existing GPU buffer budget and charged CPU component budget", async () => {
  const d = deviceSpy(),
    p = plainPose(),
    g = plainGeometry(),
    gpu = await createGpuAnimationDeformer(d, p, g);
  const bytes = gpu.bufferBytes;
  gpu.dispose();
  const tooSmall = deviceSpy();
  await assert.rejects(
    createGpuAnimationDeformer(tooSmall, p, g, { maxBytes: bytes - 1 }),
    code("ANIMATION_GPU_LIMIT"),
  );
  assert.equal(tooSmall.buffers.length, 0);
  const exact = await createGpuAnimationDeformer(deviceSpy(), p, g, {
    maxBytes: bytes,
    maxComponents: 18,
  });
  exact.dispose();
  const components = deviceSpy();
  await assert.rejects(
    createGpuAnimationDeformer(components, p, g, { maxComponents: 17 }),
    code("ANIMATION_DEFORM_LIMIT"),
  );
  assert.equal(components.buffers.length, 0);
});
...
test("ambiguous or invalid flat-normal geometry is rejected before device allocation", async () => {
  for (const patch of [
    ...
    const d = deviceSpy(),
      { pose, geometry } = fixture();
    await assert.rejects(createGpuAnimationDeformer(d, pose, { ...geometry, ...patch }));
    assert.equal(d.buffers.length, 0);
  }
});

for (const failure of ["pipelineThrow", "pipelineReject", "bindFailure"])
  test(`${failure} in the normal pass releases every buffer`, async () => {
    const d = deviceSpy(),
      { pose, geometry } = fixture();
    d[failure] = failure === "bindFailure" ? true : "flat_normals";
...
test("loss while waiting for the second pipeline cancels construction and frees shared buffers", async () => {
  ...
  d.gates = { flat_normals: deferred() };
  const pending = createGpuAnimationDeformer(d, pose, geometry);
  d.loss.resolve({ message: "lost during normals" });
...
test("invalid dynamic input publishes no new matrices, writes or commands", async () => {
  ...
  const writes = d.writes.length,
    commands = d.submissions.length;
  pose.morphWeights[0] = Infinity;
  assert.throws(() => gpu.update(), code("ANIMATION_GPU_VALUE"));
  assert.equal(d.writes.length, writes);
  assert.equal(d.submissions.length, commands);
...
test("second-pass recording failure cannot submit the first pass alone", async () => {
  const d = deviceSpy(),
    ...
    gpu = await createGpuAnimationDeformer(d, pose, geometry);
  const writes = d.writes.length;
  d.secondPassError = new Error("normal encoder refused");
  pose.sample(1);
  assert.throws(() => gpu.update(), /normal encoder refused/);
...
test("completion errors remain observable after a later valid two-pass submission", async () => {
  const d = deviceSpy(),
    { pose, geometry } = fixture(),
    gpu = await createGpuAnimationDeformer(d, pose, geometry);
  d.scopeError = { message: "normal validation refused" };
  pose.sample(0.5);
  gpu.update();
  pose.sample(1);
  gpu.update();
