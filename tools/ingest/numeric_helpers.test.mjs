...
const artifact = (fn, helpers, parameterTypes = ['f64[]']) => compileNumericKernel(fn.toString(), {
  parameterTypes, helperSources: sources(...helpers),
});
  ...
  for (let frame = 0; frame < repeats; frame++) {
    ...
    actual.forEach((value, p) => {
      if (ArrayBuffer.isView(value)) {
        value.forEach((number, i) => assert.ok(Object.is(number, expected[p][i]), `parameter ${p}, element ${i}`));
...
test('compiles a transitive helper graph once into private deterministic Wasm functions', () => {
  ...
  assert.deepEqual(a.helpers, [{ name: 'smooth', arity: 1 }, { name: 'clamp', arity: 3 }, { name: 'lerp', arity: 3 }]);
...
test('runs 10,000-element animation updates for 120 frames with f32 and f64 storage', () => {
  for (const [ArrayType, type] of [[Float32Array, 'f32[]'], [Float64Array, 'f64[]']]) {
    compare(animate, [smooth, clamp, lerp], [type, type, 'f64'], [
      ArrayType.from({ length: 10000 }, (_, i) => (i - 5000) / 7),
      ArrayType.from({ length: 10000 }, (_, i) => (5000 - i) / 11), 1 / 60,
    ], 120);
  }
});
...
test('preserves nested lexical locals, parameter mutation, increments and early returns', () => {
  ...
  function update(x, delta) { for (let i = 0; i < x.length; i++) x[i] = scalar(x[i], delta); }
  compare(update, [scalar], ['f64[]', 'f64'], [new Float64Array([-50, -3, -0, 0, 1, 7, NaN]), 0.5], 7);
});

test('accepts all-branch returns and numeric truthiness without boolean/number confusion', () => {
  function scalar(x) {
    if ((x !== 0 && x === x) || false) { return x < 0 ? -x : x; }
    else { if (!x) return -0; else return 7; }
  }
  function update(x) { for (let i = 0; i < x.length; i++) x[i] = scalar(x[i]); }
  compare(update, [scalar], ['f64[]'], [new Float64Array([0, -0, NaN, Infinity, -Infinity, 1e-300, -1])]);
});

test('helper calls preserve signed zeros, NaN, infinities and arithmetic grouping', () => {
  function ordered(x, y) { const first = x + y; return (first + -y) / y; }
  function update(x, divisor) { for (let i = 0; i < x.length; i++) x[i] = ordered(x[i], divisor); }
  for (const divisor of [0, -0, 1, -1, 1e20, Infinity, -Infinity, NaN]) {
    compare(update, [ordered], ['f64[]', 'f64'], [new Float64Array([0, -0, 1, -1, 1e-300, Infinity, -Infinity, NaN]), divisor]);
  }
});
...
test('f32 stores still round individually when read back into a scalar helper', () => {
  ...
  function update(x, gain) {
    for (let i = 0; i < x.length; i++) { x[i] = times(x[i], gain); x[i] = times(x[i], 1 / gain); }
  }
  compare(update, [times], ['f32[]', 'f64'], [new Float32Array([1 / 3, 1e-30, 1e30, -0]), 1.123456789], 30);
});

test('calls helpers from ordered reductions, indexed expressions and the final result', () => {
  function square(x) { return x * x; }
  function divide(x, count) { return x / count; }
  function energy(x) {
    let sum = 0;
  ...
  for (const count of [0, 1, 100000]) {
    compare(energy, [square, divide], ['f64[]'], [Float64Array.from({ length: count }, (_, i) => (i % 71) / 7)]);
  }
});

test('works with streamed vec3 geometry and shared fixed-index matrix inputs', () => {
  function transform(x, y, z, a, b, c, d) { return a * x + b * y + c * z + d; }
  ...
  compare(update, [transform], ['f32[]', 'f64[]'], [
...
test('supports zero-argument helpers and more than 127 scalar locals', () => {
  ...
  const built = compileNumericKernel('function f(x){for(let i=0;i<x.length;i++)x[i]=scalar(x[i])+constant();}', {
    parameterTypes: ['f64[]'], helperSources: new Map([['scalar', helper], ['constant', 'function constant(){return 42;}']]),
  });
  const engine = instantiateNumericKernel(built.wasm);
  const x = new Float64Array([1, 2]); engine.run(x);
...
test('ignores unreachable helper sources rather than evaluating or parsing application code', () => {
  ...
  const withHelpers = compileNumericKernel(source, { parameterTypes: ['f64[]'], helperSources: new Map([
    ['unused', 'this is not valid JavaScript'], ['oversized', ' '.repeat(300000)],
  ]) });
...
test('retains transactional alias/scalar/length fallback for helper-containing kernels', () => {
  ...
  function update(x, y, delta) { for (let i = 0; i < x.length; i++) x[i] = add(x[i], y[i] * delta); }
  const built = artifact(update, [add], ['f64[]', 'f64[]', 'f64']);
  let calls = 0;
  const engine = instantiateNumericKernel(built.wasm, { fallback(...args) { calls++; return update(...args); } });
  ...
  let coercions = 0;
  engine.run(new Float64Array([1, 2]), new Float64Array([2, 3]), { valueOf() { coercions++; return 2; } });
  assert.equal(coercions, 2);
  assert.equal(engine.diagnostics.lastGuardFailure, 'KERNEL_SCALAR_TYPE');
  const x = new Float64Array([1, 2]); engine.run(x, new Float64Array([2]), 1);
...
test('refuses effects, implicit returns, free variables, invalid scopes and non-scalar helpers', async t => {
  ...
  for (const helper of refused) await t.test(helper, () => {
    assert.throws(() => compileNumericKernel('function f(x){for(let i=0;i<x.length;i++)x[i]=scalar(x[i]);}', {
      parameterTypes: ['f64[]'], helperSources: new Map([['scalar', helper], ['other', 'function other(x){return scalar(x);}']]),
...
test('refuses shadowed helpers, wrong arity, optional calls, spreads and array arguments', async t => {
  ...
  for (const body of snippets) await t.test(body, () => {
    assert.throws(() => compileNumericKernel(`function f(x){${body}}`, {
      parameterTypes: ['f64[]'], helperSources: new Map([['scalar', 'function scalar(x){return x;}']]),
    }), NumericKernelCompileError);
  });
  const helperSources = new Map([
    ['scalar', 'function scalar(x){return other(x);const other=1;}'], ['other', 'function other(x){return x;}'],
  ]);
  assert.throws(() => compileNumericKernel('function f(x){for(let i=0;i<x.length;i++)x[i]=scalar(x[i]);}', {
    parameterTypes: ['f64[]'], helperSources,
  }), /unshadowed/);
});

test('refuses excessively deep or exponentially expanding acyclic helper graphs', () => {
  for (const [length, double] of [[34, false], [15, true]]) {
    const helperSources = new Map(Array.from({ length }, (_, i) => [
      `h${i}`, `function h${i}(x){return ${i === length - 1 ? 'x' : `h${i + 1}(x)${double ? `+h${i + 1}(x)` : ''}`};}`,
    ]));
    assert.throws(() => compileNumericKernel('function f(x){for(let i=0;i<x.length;i++)x[i]=h0(x[i]);}', {
      parameterTypes: ['f64[]'], helperSources,
    }), /limit/);
  }
});
...
test('validates the explicit helper-source ABI instead of accepting dynamic providers', () => {
  assert.throws(() => compileNumericKernel('function f(x){for(let i=0;i<x.length;i++)x[i]=1;}', {
    parameterTypes: ['f64[]'], helperSources: {},
