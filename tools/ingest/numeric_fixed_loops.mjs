...
function writesName(pattern, name) {
  ...
  if (pattern.type === 'ArrayPattern') return pattern.elements.some(item => writesName(item, name));
  if (pattern.type === 'ObjectPattern') return pattern.properties.some(item =>
    writesName(item.type === 'RestElement' ? item.argument : item.value, name));
  return false;
}

function fixedLoop(node, maxIterations) {
  if (node.init?.type !== 'VariableDeclaration' || node.init.kind !== 'let' || node.init.declarations.length !== 1) return null;
  const binding = node.init.declarations[0];
  if (binding.id.type !== 'Identifier') return null;
  const name = binding.id.name, start = literalInteger(binding.init), end = literalInteger(node.test?.right);
  if (start === undefined || end === undefined || node.test?.type !== 'BinaryExpression' ||
      node.test.left.type !== 'Identifier' || node.test.left.name !== name ||
      !['<', '<=', '>', '>='].includes(node.test.operator)) return null;
  const update = node.update;
  let step;
  if (update?.type === 'UpdateExpression' && update.argument.type === 'Identifier' && update.argument.name === name) {
    if (update.operator === '++') step = 1;
    if (update.operator === '--') step = -1;
  } else if (update?.type === 'AssignmentExpression' && update.left.type === 'Identifier' && update.left.name === name) {
    const delta = literalInteger(update.right);
    if (delta !== undefined && ['+=', '-='].includes(update.operator)) step = update.operator === '+=' ? delta : -delta;
  }
  ...
  const test = value => node.test.operator === '<' ? value < end : node.test.operator === '<=' ? value <= end
    : node.test.operator === '>' ? value > end : value >= end;
  const values = [];
  ...
  walk.full(node.body, child => {
    if (['BreakStatement', 'ContinueStatement', 'FunctionDeclaration', 'ClassDeclaration'].includes(child.type) ||
        (child.type === 'VariableDeclaration' && child.kind === 'var') ||
        (child.type === 'CallExpression' && child.callee.type === 'Identifier' && child.callee.name === 'eval') ||
        (child.type === 'AssignmentExpression' && writesName(child.left, name)) ||
        (child.type === 'UpdateExpression' && writesName(child.argument, name)) ||
        (['ForInStatement', 'ForOfStatement'].includes(child.type) && writesName(child.left, name))) safe = false;
  });
 ...
 */
export function expandNumericFixedLoops(source, { maxIterations = 16, maxExpandedIterations = 256,
  ...
  if (!Number.isInteger(maxIterations) || maxIterations < 1 || maxIterations > 64 ||
      !Number.isInteger(maxExpandedIterations) || maxExpandedIterations < 1 || maxExpandedIterations > 4096 ||
      !Number.isInteger(maxSourceLength) || maxSourceLength < 1 || maxSourceLength > 1048576) {
    throw new RangeError('Invalid fixed-loop expansion budget');
  }
  const unchanged = reason => ({ source, changed: false, loops: [], expandedIterations: 0, reason });
  ...
  catch { return unchanged('PARSE_UNSUPPORTED'); }
  const loops = [];
  let work = 0;
  ...
  function render(root, start, end, multiplicity = 1) {
    ...
    walk.recursive(root, null, {
      ForStatement(node, state, visit) {
        ...
        if (!fixed) { walk.base.ForStatement(node, state, visit); return; }
        const copies = fixed.values.length * multiplicity;
        work += copies;
        if (work > maxExpandedIterations || loops.length >= 64) throw budgetExceeded;
        loops.push({ indexName: fixed.name, iterations: fixed.values.length,
          sourceSpan: { start: node.start, end: node.end, line: node.loc.start.line, column: node.loc.start.column } });
        const body = copies ? render(node.body, node.body.start, node.body.end, copies) : '';
        const iterations = fixed.values.map(value => `\n{ const ${fixed.name} = ${Object.is(value, -0) ? '-0' : value};\n${body}\n}`).join('');
    ...
    for (const replacement of replacements) {
      ...
      pieces.push(prefix, replacement.text); cursor = replacement.end;
    }
  ...
  try {
    ...
    return loops.length ? { source: expanded, changed: true, loops, expandedIterations: work, reason: null } : unchanged('NO_FIXED_LOOPS');
  } catch (error) {
