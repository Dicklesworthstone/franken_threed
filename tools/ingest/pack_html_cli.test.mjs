...
function fixture(files = {}) {
  ...
  for (const [name,data] of Object.entries({'index.html':'<script type="module" src="entry.mjs"></script>','entry.mjs':'export const value=42;',...files})) {
...
const sourceModules = html => Object.values(JSON.parse(/<script type="importmap">([\s\S]*?)<\/script>/.exec(html)[1]).imports)
...
test('CLI exports a standalone HTML entry without importing Rollup and writes the requested report',()=>{
  const f=fixture(),report=path.join(f.root,'report.json');
  const result=run(['--entry',f.entry,'--pack-html',f.out,'--output',report]);
  assert.equal(result.status,0,result.stderr);assert.match(result.stdout,/Single HTML exported/);
  ...
  assert.ok(fs.existsSync(f.entry));assert.ok(fs.existsSync(path.join(f.root,'entry.mjs')));
});
...
test('CLI exposes the export mode and rejects incompatible modes before building',()=>{
  ...
  for(const args of [
    ...
    ['--entry',f.entry,'--pack-html',f.out,'--build-kernel',path.join(f.root,'kernel'),'--parameter-types','f64[]'],
    ['--entry',f.entry,'--pack-html',f.out,'--package-root','https://example.com/'],
  ]){assert.equal(run(args).status,1);assert.equal(fs.existsSync(f.out),false);}
});
...
test('CLI preflight rejects existing files, symlinks and output collisions without partial output',()=>{
  ...
  assert.equal(run(['--entry',f.entry,'--build-app',build,'--pack-html',path.join(build,'index.html')]).status,1);
...
test('CLI refuses an unclosed graph but retains the source directory',()=>{
  ...
  assert.equal(result.status,1);assert.match(result.stderr,/DYNAMIC_IMPORT_OPEN/);
  assert.equal(fs.existsSync(f.out),false);assert.ok(fs.existsSync(f.entry));
});
...
test('packs the actual finite-import mapper expression emitted by the existing bundler',()=>{
  ...
  const f=fixture({'entry.mjs':source,'left-ABC.js':`export const value='left';`,'right-DEF.js':`export const value='right';`});
  const result=packHtml(f.entry,f.out);assert.equal(result.moduleCount,3);
...
test('nested imports in dynamic selectors retain disjoint source edits',()=>{
  ...
  const f=fixture({'entry.mjs':source,'selector.mjs':'export const value=true;','lazy.mjs':'export const value=3;'});
...
test('JSON modules remain JSON while import attributes remain in the importing source',()=>{
  const f=fixture({'entry.mjs':`import config from './config.json' with {type:'json'}; export {config};`,'config.json':'{"answer":42}'});
...
test('responsive srcset keeps descriptors and data-URL commas without losing alternatives',()=>{
  const f=fixture({'index.html':'<picture><source srcset="small.png 400w, large.png 800w" sizes="50vw"><img src="small.png" srcset="small.png 1x, large.png 2x"></picture>',
    'small.png':Buffer.from([1,2,3]),'large.png':Buffer.from([4,5,6])});
  assert.equal(packHtml(f.entry,f.out).assetCount,2);
  const output=fs.readFileSync(f.out,'utf8');
  assert.match(output,/srcset="data:image\/png;f3d-resource=[0-9a-f]+;base64,AQID 400w, data:image\/png;f3d-resource=[0-9a-f]+;base64,BAUG 800w"/);
  assert.match(output,/srcset="data:image\/png;f3d-resource=[0-9a-f]+;base64,AQID 1x, data:image\/png;f3d-resource=[0-9a-f]+;base64,BAUG 2x"/);
...
test('legacy CLI option validation remains unchanged when packing is not requested',()=>{
  ...
  assert.match(run(['--entry',f.entry,'--max-memory-pages','4']).stderr,/requires --build-kernel or --specialize-numeric/);
  assert.match(run(['--entry',f.entry,'--parameter-types','f64[]']).stderr,/--parameter-types requires --build-kernel/);
});
