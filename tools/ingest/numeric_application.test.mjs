...
function fixture(files = { 'update.mjs': UPDATE, 'entry.mjs': ENTRY }) {
  ...
  for (const [name, code] of Object.entries(files)) {
    const dest = path.join(source, name); fs.mkdirSync(path.dirname(dest), { recursive: true }); fs.writeFileSync(dest, code);
  }
...
function observeWasm(t) {
  ...
  t.after(() => { WebAssembly.Instance = NativeInstance; });
  WebAssembly.Instance = function (...args) {
    const instance = Reflect.construct(NativeInstance, args); counts.instances++;
    return { exports: { memory: instance.exports.memory,
      run(...params) { counts.calls++; return instance.exports.run(...params); } } };
  };
  return counts;
}
function reportFrom(result) {
  return JSON.parse(fs.readFileSync(path.join(result.outDir, 'f3d-numeric-specialization.json'), 'utf8'));
}
async function importEntry(result) { return import(pathToFileURL(path.join(result.outDir, result.entryFiles[0]))); }

test('normal application build specializes calls across linked source modules with no source API changes', async t => {
  const counts = observeWasm(t), app = fixture();
  ...
  const actual = new Float64Array([0, -0, NaN, Infinity, -Infinity, 1]), expected = actual.slice();
  const velocity = new Float64Array([1, -1, 2, 3, -3, 4]);
  for (let frame = 0; frame < 240; frame++) {
    module.tick(actual, velocity, 1 / 60); module.integrate(expected, velocity, 1 / 60); assert.deepEqual(actual, expected);
  }
...
test('specialization is opt-in and standalone bundle API reports the same assets', async () => {
  ...
  for (const asset of result.numericSpecialization.runtimeAssets) assert.equal(typeof result.files[asset.fileName], 'string');
});
...
test('HTML document, assets and SRI use the specialized final bytes', async t => {
  ...
  t.after(() => { delete globalThis.__f3d_app_test_value; });
  const result = await buildApplication(path.join(app.source, 'index.html'), app.out, { specializeNumeric: true });
  ...
  await importEntry(result); assert.equal(globalThis.__f3d_app_test_value, 7); assert.equal(counts.calls, 1);
});
...
test('dynamic chunks specialize their local calls and remain loadable after relocation', async t => {
  ...
  assert.ok(result.numericSpecialization.units.every(unit => fs.existsSync(path.join(app.out, unit.fileName))));
  const relocated = path.join(app.root, 'relocated'); fs.cpSync(app.out, relocated, { recursive: true });
  const entry = await import(pathToFileURL(path.join(relocated, result.entryFiles[0])));
  const module = await entry.load(); const a = new Float64Array([1]); module.tick(a, new Float64Array([2]), 3);
...
test('unsupported loops, wrong typed arrays and unavailable Wasm retain working application behavior', async t => {
  ...
  for (const a of [[1, 2], new Float32Array([1, 2]), new Float64Array([1, 2])]) {
    module.tick(a, [2, 3], 2); assert.deepEqual([...a], [5, 8]);
  }
  globalThis.WebAssembly = native;
  const refused = fixture({ 'entry.mjs': `function sine(a) { for (let i = 0; i < a.length; i++) a[i] = Math.sin(a[i]); }
  ...
  const a = [1]; (await importEntry(retained)).tick(a); assert.equal(a[0], Math.sin(1));
});
...
test('CLI --build-app --specialize-numeric emits executable output and a build manifest', async () => {
  ...
  const result = spawnSync(process.execPath, [fileURLToPath(new URL('./cli.mjs', import.meta.url)),
    '--entry', app.entry, '--build-app', app.out, '--specialize-numeric', '--max-memory-pages', '4',
    '--output', path.join(app.root, 'build.json')], { encoding: 'utf8' });
  ...
  const a = new Float64Array([1]); (await importEntry(manifest)).tick(a, new Float64Array([2]), 3); assert.equal(a[0], 7);
});

test('invalid specialization configuration and output collisions fail without replacing files', async () => {
  const app = fixture();
  await assert.rejects(buildApplication(app.entry, app.out, { specializeNumeric: { maxKernels: 0 } }), /maxKernels/);
  assert.equal(fs.existsSync(app.out), false);
  fs.mkdirSync(app.out); fs.writeFileSync(path.join(app.out, 'entry.js'), 'original data');
  await assert.rejects(buildApplication(app.entry, app.out, { specializeNumeric: true }), /overwrite/);
...
test('actual Rollup plugin supports repeated generation and preserves requested source-map output', async () => {
  const app = fixture(); const plugin = numericKernelRollupPlugin();
  ...
  try {
    ...
    assert.equal(plugin.api.getReport().compiledKernels, 1);
    assert.deepEqual(first.output.map(item => item.fileName), second.output.map(item => item.fileName));
