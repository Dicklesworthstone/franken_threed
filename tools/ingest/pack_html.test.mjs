...
function build(files, options) {
  const f = fixture(files), result = packHtml(f.entry, f.out, options);
  return { ...f, result, html: fs.readFileSync(f.out, 'utf8') };
}
const dataText = url => Buffer.from(url.split(',')[1].split('#')[0], 'base64').toString('utf8');
const mapOf = html => JSON.parse(/<script type="importmap">([\s\S]*?)<\/script>/.exec(html)[1]).imports;

test('packs circular native ESM, live bindings, finite lazy imports and shared roots', () => {
  const f = build({
    'index.html': '<!doctype html><title>App</title><script type="module" src="./entry.mjs"></script><script type="module" src="./entry.mjs"></script>',
  ...
  const imports = mapOf(f.html); assert.equal(Object.keys(imports).length, 3);
  for (const url of Object.values(imports)) { assert.ok(url.startsWith('data:text/javascript;base64,')); assert.ok(!dataText(url).includes("'./")); }
...
test('same-byte modules and query/fragment variants remain distinct identities', () => {
  const f = build({ 'index.html': '<script type="module" src="a.mjs"></script>',
    'a.mjs': `import './same.mjs?one'; import './same.mjs?two'; import './same.mjs#other'; import './copy.mjs';`,
    'same.mjs': 'export const value={};', 'copy.mjs': 'export const value={};' });
...
test('resolves source import-map scopes and aliases before assigning embedded identities', () => {
  const f = build({ 'index.html': `<script type="importmap">{"imports":{"shared":"./shared.mjs"},"scopes":{"./nested/":{"shared":"./special.mjs"}}}</script><script type="module" src="entry.mjs"></script>`,
    'entry.mjs': `import 'shared'; import './nested/child.mjs';`, 'nested/child.mjs': `import 'shared';`,
    'shared.mjs': 'export const tag="normal";', 'special.mjs': 'export const tag="special";' });
...
test('embeds transitive CSS, image assets, module-relative Wasm and original script attributes', () => {
  ...
  const f = build({ 'index.html': `<link rel="stylesheet" href="main.css" integrity="sha384-${hash(css)}"><img src="pixel.png"><script type="module" src="app.mjs" nonce="abc" data-label='a"b' integrity="sha384-${hash(js)}"></script>`,
    'main.css': css, 'other.css': 'p{color:rgb(10,20,30)}', 'pixel.png': Buffer.from([1,2,3]), 'mesh.bin': Buffer.from([0,255,3]), 'app.mjs': js });
  assert.equal(f.result.assetCount, 4);
  assert.ok(f.html.includes('nonce="abc"')); assert.ok(f.html.includes(`data-label='a"b'`));
  assert.ok(!f.html.includes(`sha384-${hash(js)}`)); assert.ok(!f.html.includes(`sha384-${hash(css)}`));
...
test('HTML raw text and comments are not scanned as resource tags; closing script text is encoded', () => {
  const f = build({ 'index.html': `<!doctype html><!-- <img src="missing.png"> --><textarea><img src="missing.png"></textarea><script>const fake='<img src="missing.png">';</script><script type="module" src="app.mjs"></script>`,
...
test('inline module bodies, classic dynamic imports and comments are preserved without eval', () => {
  const f = build({ 'index.html': `<script>globalThis.run=()=>import('./lazy.mjs');</script><script type="module">import {value} from './lazy.mjs'; globalThis.value=value;</script>`,
    'lazy.mjs': 'export const value=7;' });
  assert.equal(f.result.moduleCount, 2); assert.match(f.html, /globalThis.run=\(\)=>import\("f3d-packed\//);
...
for (const [label, files, code] of [
  ['dynamic imports', { 'app.mjs': `export const load=name=>import(name);` }, 'DYNAMIC_IMPORT_OPEN'],
  ['module URL reflection', { 'app.mjs': `export const source=import.meta.url;` }, 'MODULE_URL_OBSERVATION'],
  ['remote modules', { 'app.mjs': `import 'https://example.com/module.mjs';` }, 'EXTERNAL_RESOURCE'],
  ['worker realm', { 'app.mjs': `new Worker('./worker.mjs');` }, 'HOST_RESOURCE'],
  ['runtime fetch', { 'app.mjs': `fetch('./bytes.bin');` }, 'FETCH_URL'],
  ['base URL', { 'index.html': '<base href="./"><script type="module" src="app.mjs"></script>', 'app.mjs': '' }, 'BASE_URL'],
  ['CSP', { 'index.html': '<meta http-equiv="Content-Security-Policy" content="script-src \'self\'"><script type="module" src="app.mjs"></script>', 'app.mjs': '' }, 'CSP_POLICY'],
  ['blocked import', { 'index.html': '<script type="importmap">{"imports":{"blocked":null}}</script><script type="module" src="app.mjs"></script>', 'app.mjs': `import 'blocked';` }, 'BLOCKED_IMPORT'],
  ['bad integrity', { 'index.html': '<script type="module" src="app.mjs" integrity="sha384-wrong"></script>', 'app.mjs': 'export const x=1;' }, 'INTEGRITY'],
  ['CSS cycle', { 'index.html': '<link rel="stylesheet" href="a.css">', 'a.css': '@import "b.css";', 'b.css': '@import "a.css";' }, 'CSS_CYCLE'],
  ...
  assert.equal(fs.existsSync(f.out), false); assert.deepEqual(fs.readFileSync(f.entry), before);
});
...
test('path and symlink escapes cannot read outside the application root', () => {
  ...
  fs.writeFileSync(f.entry, `<script type="module" src="${path.relative(f.root, path.join(other, 'private.mjs'))}"></script>`);
...
test('output collisions and packing limits never overwrite or partially publish', () => {
  const f = fixture({ 'index.html': '<script type="module" src="app.mjs"></script>', 'app.mjs': 'export const x=1;' });
  ...
  fs.writeFileSync(f.out, 'existing'); assert.throws(() => packHtml(f.entry, f.out), {code:'OUTPUT_EXISTS'});
...
test('same logical application in two directories has byte-identical output', () => {
  const files = { 'index.html': '<script type="module" src="app.mjs"></script>', 'app.mjs': `import './side.mjs';`, 'side.mjs': 'export const value=1;' };
...
test('the generated import map precedes module preloads as well as script execution', () => {
  const f = build({ 'index.html': '<head><link rel="modulepreload" href="a.mjs"></head><body><script type="module" src="a.mjs"></script></body>',
    'a.mjs': `import './b.mjs';`, 'b.mjs': 'export const x=1;' });
...
test('unused external source import mappings do not create a false network dependency', () => {
  const f = build({ 'index.html': '<script type="importmap">{"imports":{"unused":"https://example.com/not-loaded.mjs"}}</script><script type="module" src="a.mjs"></script>',
    'a.mjs': 'export const x=1;' });
  assert.equal(f.result.moduleCount,1); assert.ok(!f.html.includes('example.com'));
});
