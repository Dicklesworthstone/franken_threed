...
function affine(a, m) {
  const scale = m[0];
  for (let i = 0; i < a.length; i++) a[i] = a[i] * scale + m[1];
}
...
const matrixValues = [
  1, 0.001, 0, 0.00001, -0.001, 1, 0.001, 0, 0.001, 0, 1, 0.00002, 0.0001, -0.0001, 0.0002, 1,
];
...
for (const [ArrayType, type] of [
  ...
  test(`${type} geometry projects through a 16-element f64 matrix with exact operator order`, () => {
    ...
    const expected = actual.slice(),
      matrix = new Float64Array(matrixValues);
    for (let frame = 0; frame < 120; frame++) {
      matrix[12] = frame / 100000; // Uniforms are fresh on every invocation.
      project(expected, matrix);
      kernel.run(actual, matrix);
...
test("pre-loop declarations share lexical scope and nested loop bindings retain their TDZ", () => {
  ...
  for (const setup of [
    ...
    assert.throws(() =>
      compileNumericKernel(`function update(a) { ${setup} for(let i=0;i<a.length;i++) a[i]=x; }`, {
        parameterTypes: ["f64[]"],
      }),
    );
  }
  assert.throws(
    () =>
      compileNumericKernel(
        `function update(a) { const x=1; for(let i=0;i<a.length;i++) {
    a[i]=x; const x=2; } }`,
        { parameterTypes: ["f64[]"] },
      ),
...
test("an array used only for its loop bound is not copied or assigned a vertex-sized scratch region", () => {
  function update(bound, out, m) {
    for (let i = 0; i < bound.length; i++) out[i] = m[0];
  }
  ...
  assert.ok(out.every((value) => value === 7));
  assert.equal(kernel.diagnostics.wasmCalls, 1);
...
test("uniform read extents can coexist with streamed reads on a read-only parameter", () => {
  function update(out, input) {
    for (let i = 0; i < out.length; i++) out[i] = input[i] + input[7];
  }
  const kernel = compiled(update, ["f32[]", "f64[]"]);
  const out = new Float32Array(3);
  kernel.run(out, new Float64Array([1, 2, 3, 4, 5, 6, 7, 8]));
  assert.deepEqual([...out], [9, 10, 11]);
  assert.equal(kernel.diagnostics.wasmCalls, 1);
...
test("undersized uniforms fallback once before any output is published", () => {
  ...
  kernel.run(a, new Float64Array([2]));
  assert.ok(a.every(Number.isNaN));
...
test("zero records still execute original prelude effects when guards require fallback", () => {
  const events = [],
    kernel = compiled(affine, ["f32[]", "f64[]"]);
  const m = {
    get 0() {
      events.push("setup");
      return 2;
    ...
    get 1() {
      throw Error("loop must not run");
    },
  };
  kernel.run(new Float32Array(0), m);
...
test("incomplete geometry records, growing scratch memory, and changed uniforms match JS", () => {
  const kernel = compiled(project, ["f32[]", "f64[]"]);
  const matrix = new Float64Array(matrixValues);
...
test("fixed writes, mutable uniforms, unbounded indices and setup effects stay refused", () => {
  ...
  for (const body of bodies)
    assert.throws(() =>
      compileNumericKernel(`function update(a,m) { ${body} }`, {
...
test("the maximum fixed index uses bounded allocation and unsigned Wasm offsets", () => {
  function update(a, m) {
    for (let i = 0; i < a.length; i++) a[i] = m[65535];
  }
  const kernel = compiled(update, ["f64[]", "f64[]"]);
...
test("automatic specialization admits four bounded stream/uniform layouts and executes each natively", async (t) => {
  ...
  assert.equal(result.report.candidates[0].variants.length, 4);
  ...
  for (let frame = 0; frame < 12; frame++)
    for (const A of [Float32Array, Float64Array])
      for (const M of [Float32Array, Float64Array]) {
        const a = new A([1, 2, 3, 4, 5, 6]),
          expected = a.slice(),
          m = new M(matrixValues);
        module.tick(a, m);
        project(expected, m);
        assert.deepEqual(a, expected);
      }
...
test("setup without uniforms retains only two variants and conservative mixed-stream fallback", () => {
  function update(a, b, dt) {
    const factor = dt * 2;
    for (let i = 0; i < a.length; i++) a[i] += b[i] * factor;
  }
...
test("uniform access manifests are deeply immutable and reject malformed extents", (t) => {
  ...
  for (const mutate of [
    (m) => {
      m.parameters[1].access.minimumLength = -1;
    },
    (m) => {
      m.parameters[1].access.minimumLength = 65537;
    },
    (m) => {
      m.parameters[1].access.indexed = 1;
    },
    (m) => {
      delete m.parameters[1].access;
    },
    (m) => {
      m.parameters[0].access.indexed = false;
    },
    (m) => {
      m.parameters[0].access.minimumLength = 1;
    },
    (m) => {
      m.loopStride = 0;
    },
