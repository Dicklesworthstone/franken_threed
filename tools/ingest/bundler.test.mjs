...
test("Multi-entry HTML preserves all entries in order and executes script effects", async () => {
  ...
  fs.writeFileSync(
    path.join(scratch, "entry1.js"),
    `(globalThis.__f3d_order = globalThis.__f3d_order || []).push('entry1');\n`,
  );
  ...
  assert.ok(
    res.files[res.entryFiles[0]] && res.files[res.entryFiles[1]],
    "Both entry chunks must exist in files map",
  );
  ...
  assert.equal(
    res.outputChunks.filter((c) => c.isEntry).length,
    2,
    "Second HTML entry must not be omitted",
  );
  ...
  assert.deepEqual(
    globalThis.__f3d_order,
    ["entry1", "entry2:from_dep"],
    "Both HTML script effects must execute in admitted order",
  );
});
...
test("Literal dynamic import preserves chunk, defers side effects, returns real exports", async () => {
  ...
  fs.writeFileSync(
    path.join(scratch, "lazy_dep.js"),
    `globalThis.__f3d_lazy = true;\nexport function getPayload() { return 'payload_ok'; }\n`,
  );
  fs.writeFileSync(
    path.join(scratch, "main.js"),
    `export async function loadFeature() { const mod = await import('./lazy_dep.js'); return mod.getPayload(); }\n`,
  );
  ...
  assert.ok(
    dynChunk && res.files[dynChunk.fileName],
    "Dynamic chunk must exist with isDynamicEntry: true",
  );
  ...
  const mainMod = await import(
    pathToFileURL(path.join(fullDir, res.entryFiles[0])).href + `?v=${Date.now()}`
  );
  assert.equal(
    globalThis.__f3d_lazy,
    false,
    "Deferred side effect must remain deferred on initial entry load",
  );
  ...
  assert.equal(
    globalThis.__f3d_lazy,
    true,
    "Dynamic chunk side effect must execute when dynamic import evaluates",
  );
  ...
  const singleMod = await import(
    pathToFileURL(path.join(singleDir, res.entryFiles[0])).href + `?v=${Date.now() + 1}`
  );
  await assert.rejects(
    async () => {
      await singleMod.loadFeature();
    },
...
test("Import-map null mapping is rejected while nearest permitted mapping resolves and executes", async () => {
  ...
  fs.writeFileSync(
    path.join(pkgDir, "allowed", "tool.js"),
    `export const toolVal = 'nearest_permitted_ok';\n`,
  );
  ...
  fs.writeFileSync(
    blockedHtml,
    `<!DOCTYPE html><html><head><script type="importmap">${mapJson}</script><script type="module">import 'pkg/blocked/secret.js';</script></head><body></body></html>`,
  );

  await assert.rejects(
    async () => {
      await bundleWithRollup(blockedHtml);
    },
    (err) =>
      err instanceof IngestionResolutionError ||
      err?.name === "IngestionResolutionError" ||
      String(err?.message || "").includes("blocked") ||
      String(err?.message || "").includes("mapped to null"),
  ...
  fs.writeFileSync(
    permittedHtml,
    `<!DOCTYPE html><html><head><script type="importmap">${mapJson}</script><script type="module">import { toolVal } from 'pkg/allowed/tool.js'; globalThis.__f3d_permitted = toolVal;</script></head><body></body></html>`,
  );
  ...
  assert.equal(
    globalThis.__f3d_permitted,
    "nearest_permitted_ok",
    "Nearest permitted mapping must resolve and execute",
  );
});
...
test("Repeated HTML module scripts preserve document-order entries to unique chunk and execute once", async () => {
  ...
  fs.writeFileSync(
    ...
    `export const sharedState = { id: 'shared_identity_singleton', calls: 0 };\n`,
  );
  fs.writeFileSync(
...
`,
  ...
  assert.equal(
    res.entryFiles.length,
    3,
    "Must preserve all 3 HTML module script entries in document order",
  );
  assert.equal(
    res.entryFiles[0],
    res.entryFiles[2],
    "Repeated module script tags must map to the same emitted chunk file",
  );
  assert.notEqual(
    res.entryFiles[0],
    res.entryFiles[1],
    "Distinct module scripts must map to distinct chunk files",
  );
  ...
  assert.equal(
    entryChunks.length,
    2,
    "Must emit exactly 2 unique entry chunks, avoiding duplicate code emission",
  );
  ...
  assert.equal(
    globalThis.__f3d_repeat.sameExec,
    1,
    "Module with repeated script references must execute only once (browser module cache identity)",
  );
  assert.deepEqual(
    globalThis.__f3d_repeat.order,
    ["same", "other"],
    "Execution order must match first occurrence in document order",
  );
  ...
  assert.equal(
    importedModules[0],
    importedModules[2],
    "Module namespaces for repeated entries must share identical module cache identity",
  );
  assert.equal(
    importedModules[0].sharedState,
    importedModules[1].sharedState,
    "Shared dependency must preserve identical object reference across entry chunks",
  );
  ...
  assert.equal(
    pureRes.entryFiles.length,
    2,
    "Pure duplicate HTML must produce 2 entryFile mappings",
  );
  assert.equal(
    pureRes.entryFiles[0],
    pureRes.entryFiles[1],
    "Both mappings must point to identical chunk",
  );
  assert.equal(
    Object.keys(pureRes.files).length,
    1,
    "Pure duplicate HTML must emit exactly 1 unique chunk file",
  );
  assert.equal(
    pureRes.outputChunks.length,
    1,
    "Pure duplicate HTML must have exactly 1 output chunk",
  );
  ...
  assert.equal(
    queryRes.entryFiles.length,
    3,
    "Must maintain exact 3 entry files matching HTML script count",
  );

  // Identical URL repeats (tag 0 and tag 2) map to the same emitted chunk file
  assert.equal(
    queryRes.entryFiles[0],
    queryRes.entryFiles[2],
    "Identical URL module scripts must map to the same emitted chunk",
  );

  // Distinct URL query variant (tag 1) must map to its own distinct emitted chunk file
  assert.notEqual(
    queryRes.entryFiles[0],
    queryRes.entryFiles[1],
    "URL query/fragment variant has distinct module identity and must emit a separate chunk",
  );
  ...
  assert.equal(
    queryEntryChunks.length,
    2,
    "Must emit exactly 2 distinct entry chunks for the two distinct module URL identities",
  );
  ...
  for (const entryFile of queryRes.entryFiles) {
    const mod = await import(
      pathToFileURL(path.join(queryEmitDir, entryFile)).href + `?v=${queryNonce}`
    );
  ...
  assert.equal(
    queryModules[0],
    queryModules[2],
    "Identical URL repeat must share the exact same module namespace",
  );
  ...
  assert.notEqual(
    queryModules[0],
    queryModules[1],
    "Query variant must instantiate a distinct module namespace",
  );
  ...
  assert.equal(
    globalThis.__f3d_repeat.sameExec,
    2,
    "Both distinct module URL identities must execute, while repeated identical URL executes only once",
  );
  assert.deepEqual(
    globalThis.__f3d_repeat.order,
    ["same", "same"],
    "Both distinct module chunks must execute their top-level code",
  );
});
...
`,
  ...
  fs.writeFileSync(path.join(scratch, "branch_proto.js"), `export const branchName = 'PROTO';\n`);
...
`,
  ...
  assert.equal(
    res.isMultiChunk,
    true,
    "Conditional dynamic import must produce multi-chunk output",
  );
  ...
  const nonce = Date.now();
  const mainMod = await import(
    pathToFileURL(path.join(emitDir, res.entryFiles[0])).href + `?v=${nonce}`
  );
  ...
  assert.deepEqual(
    globalThis.__f3d_finite_effects,
    [".branch_a"],
    ".branch_a must execute when loaded",
  );
  ...
  assert.equal(
    modA1,
    modA2,
    "Repeated dynamic import must return identical module namespace instance",
  );
  assert.equal(
    modA1.singleton,
    mainMod.singleton,
  ...
  const modWithOptions = await mainMod.loadWithOptions(
    () => {
      evalOrder.push("condition");
      return true;
    },
    () => {
      evalOrder.push("options");
      return {};
    },
  );
  assert.deepEqual(
    evalOrder,
    ["condition", "options"],
    "Condition must evaluate before options expression",
  );
  ...
  assert.equal(
    optionsEvaluated,
    false,
    "Options expression must not evaluate when condition throws",
  );
  ...
  assert.equal(
    modProto.branchName,
    "PROTO",
    "Bare __proto__ candidate must resolve via import map and execute",
  );
});
...
test("Finite dynamic import: blocked import-map candidate propagates IngestionResolutionError", async () => {
  ...
  await assert.rejects(
    async () => {
      await bundleWithRollup(htmlFile);
    },
    (err) =>
      err instanceof IngestionResolutionError ||
      err?.name === "IngestionResolutionError" ||
...
`,
  ...
  assert.ok(
    res.assets.length >= 1,
    "Nested asset inside dynamic import must be emitted as an asset",
  );
...
`,
  ...
  const dataUrlA =
    'data:text/javascript,(globalThis.__f3d_ext_effects=globalThis.__f3d_ext_effects||[]).push("data_a");export const branchName="DATA_A";export const count=1;';
  const dataUrlB =
    'data:text/javascript,(globalThis.__f3d_ext_effects=globalThis.__f3d_ext_effects||[]).push("data_b");export const branchName="DATA_B";export const count=2;';
...
`,
  ...
  assert.ok(
    !res.outputChunks.some((c) => c.fileName.includes("data:")),
    "External data: modules must not be emitted as file chunks",
  );
  assert.ok(
  ...
  assert.equal(
    mainMod.staticVal,
    "STATIC_DATA",
    "Static external data: import must resolve and export correctly",
  );
  assert.deepEqual(
    globalThis.__f3d_ext_effects,
    [],
  ...
  assert.equal(
    modLit.literalVal,
    "LITERAL_DATA",
    "Literal dynamic data: import must resolve external route and load",
  );
  assert.deepEqual(
    globalThis.__f3d_ext_effects,
    [],
  ...
  assert.equal(
    modA1,
    modA2,
    "Repeated dynamic import of data URL must return identical module namespace instance",
  );
  assert.deepEqual(
    globalThis.__f3d_ext_effects,
    ["data_a", "data_b"],
    "Data URL module must execute only once",
  );
  ...
  assert.equal(
    modAliased,
    modA1,
    "Aliased data URL must share namespace identity with direct data URL",
  );
  ...
  await assert.rejects(
    async () => {
      await bundleWithRollup(blockedHtmlFile);
    },
    (err) =>
      err instanceof IngestionResolutionError ||
      err?.name === "IngestionResolutionError" ||
...
test("bundleWithRollup handles HTML entries with external root module scripts (mixed and all-external)", async () => {
  ...
  assert.ok(
    !mixedRes.chunks.some((c) => c.fileName.includes("ext.js") || c.fileName.includes("https:")),
    "External root script must not emit a chunk",
  );
  ...
  assert.equal(
    allExtRes.entryFiles.length,
    0,
    "All-external HTML entry must emit zero entry files",
  );
