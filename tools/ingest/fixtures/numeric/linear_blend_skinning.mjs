  ...
  for (let i = 0; i < positions.length; i += 3) {
    ...
    for (let influence = 0; influence < 4; influence++) {
      ...
      if (weight !== 0) {
        ...
        resultX += weight * (matrices[base] * x + matrices[base + 4] * y + matrices[base + 8] * z + matrices[base + 12]);
        resultY += weight * (matrices[base + 1] * x + matrices[base + 5] * y + matrices[base + 9] * z + matrices[base + 13]);
        resultZ += weight * (matrices[base + 2] * x + matrices[base + 6] * y + matrices[base + 10] * z + matrices[base + 14]);
      }
    }
    output[i] = resultX;
    output[i + 1] = resultY;
