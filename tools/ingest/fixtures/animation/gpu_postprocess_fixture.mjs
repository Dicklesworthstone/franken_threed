...
export function gpuPostprocessFixture() {
  ...
  function texture(width = 16, height = 8, format = "rgba16float", usage = 4 | 16, owned = false) {
    const result = {
      ...
      createView(descriptor = {}) {
        ...
        return { texture: result, descriptor };
      },
      destroy() {
        result.destroyed++;
      },
  ...
  const device = {
    limits: { maxTextureDimension2D: 8192, maxSampledTexturesPerShaderStage: 16 },
    lost: lost.promise,
    createTexture(descriptor) {
      textureAttempt++;
      if (controls.textureFailure === textureAttempt)
    ...
    popErrorScope() {
      calls.popped.push(calls.scopes.pop());
      const error = controls.scopeError;
      controls.scopeError = null;
    ...
    createCommandEncoder() {
      ...
      return {
        beginRenderPass(descriptor) {
          const pass = { descriptor, bindings: null, pipeline: null, vertices: null, ended: false };
          passes.push(pass);
          return {
            setPipeline(value) {
              pass.pipeline = value;
            ...
            setBindGroup(index, value) {
              ...
              for (const entry of value.entries)
                if (entry.resource.texture)
                  assert.notEqual(entry.resource.texture, output, "read/write texture feedback");
            },
            draw(vertices) {
        ...
        finish() {
          for (const pass of passes) assert.equal(pass.ended, true);
          return { passes };
        },
      };
    },
    queue: {
      ...
      submit(commands) {
        calls.submissions.push(commands);
      },
      onSubmittedWorkDone() {
        return controls.completion?.promise ?? Promise.resolve();
      },
    },
    destroy() {
