...
for (const name of unary)
  for (const ArrayType of [Float32Array, Float64Array]) {
    test(`${name}: Number parity over IEEE edge cases and 25,000 bit patterns in ${ArrayType.name}`, () => {
      ...
      const artifact = compile(source, [type(ArrayType), type(ArrayType)]),
      ...
      for (const count of [0, 1, 7, inputs.length]) {
        const input = new ArrayType(inputs.slice(0, count)),
          actual = new ArrayType(count),
          expected = new ArrayType(count);
        original(source)(expected, input);
        kernel.run(actual, input);
...
for (const name of ["min", "max"])
  for (const ArrayType of [Float32Array, Float64Array]) {
    test(`${name}: every ordered IEEE edge pair, ${ArrayType.name}`, () => {
      ...
      const actual = new ArrayType(left.length),
        expected = new ArrayType(left.length);
      original(source)(expected, left, right);
      kernel.run(actual, left, right);
      same(actual, expected);
      assert.equal(kernel.diagnostics.wasmCalls, 1);
    });
  }
for (const name of ["min", "max"])
  for (const arity of [0, 1, 3, 64]) {
    test(`${name}: ${arity} arguments, including empty extrema and zero signs`, () => {
      ...
      const actual = new Float64Array(edges),
        expected = actual.slice();
      const kernel = host(compile(source));
...
for (const ArrayType of [Float32Array, Float64Array]) {
  test(`private helper graph + ordered normalization/clamping pipeline, ${ArrayType.name}`, () => {
    const helpers = new Map([
      ...
      [
        "quantize",
        "function quantize(x) { const r=Math.round(x*127); return Math.fround(r/127); }",
      ],
    ]);
    ...
    const actual = new ArrayType(30000),
      expected = new ArrayType(30000);
    for (let i = 0; i < actual.length; i++) actual[i] = expected[i] = ((i % 113) - 56) / 31;
    for (let frame = 0; frame < 30; frame++) {
      const result = fn(expected, 1.1);
      assert.ok(Object.is(kernel.run(actual, 1.1), result));
      same(actual, expected);
    }
...
for (const name of names) {
  test(`replacing Math.${name} after native execution falls back once, then recovers`, () => {
    ...
    const a = new Float64Array([1, 4, 9]);
    kernel.run(a);
    ...
    try {
      ...
      Object.defineProperty(Math, name, saved);
    }
    assert.equal(methodCalls, 3);
...
test("method getters are only called by the original function, including an empty loop", () => {
  ...
  const kernel = host(compile(source), { fallback: original(source) }),
    saved = Object.getOwnPropertyDescriptor(Math, "sqrt");
  let gets = 0;
  try {
    ...
    assert.equal(gets, 0);
    kernel.run(new Float64Array([1, 4, 9]));
    assert.equal(gets, 3);
  } finally {
    Object.defineProperty(Math, "sqrt", saved);
  }
...
test("global Math getters receive no extra guard reads", () => {
  const source = "function update(a) { for(let i=0;i<a.length;i++) a[i]=Math.sqrt(a[i]); }";
  const kernel = host(compile(source), { fallback: original(source) }),
    saved = Object.getOwnPropertyDescriptor(globalThis, "Math");
  let gets = 0;
  try {
    ...
    Object.defineProperty(globalThis, "Math", saved);
  }
  assert.equal(gets, 2);
  assert.equal(kernel.diagnostics.fallbackCalls, 2);
});
...
for (const lexical of [false, true])
  test(`replacement Math proxy: no guard traps (${lexical ? "lexical" : "global"})`, () => {
    let gets = 0,
      descriptors = 0;
    ...
    const kernel = host(compile(source), {
      resolveMath: lexical ? () => proxy : () => Math,
      fallback: fn,
    });
    const saved = globalThis.Math;
    try {
      if (!lexical) globalThis.Math = proxy;
      kernel.run(new Float64Array([4, 9]));
    } finally {
      globalThis.Math = saved;
    }
    assert.equal(gets, 2);
...
test("throwing original getter preserves preceding pass effects and exception identity", () => {
  const source =
    "function update(a) { for(let i=0;i<a.length;i++) a[i]+=1; for(let i=0;i<a.length;i++) a[i]=Math.sqrt(a[i]); }";
  const kernel = host(compile(source), { fallback: original(source) }),
    saved = Object.getOwnPropertyDescriptor(Math, "sqrt");
  const sentinel = {
    get code() {
      throw new Error("must not inspect original error");
    },
  };
...
test("revalidates Math after memory growth before executing or publishing", () => {
  ...
  const grow = WebAssembly.Memory.prototype.grow,
    sqrt = Math.sqrt;
  const a = new Float64Array(10000).fill(9);
  let grows = 0,
    methodCalls = 0;
  try {
    WebAssembly.Memory.prototype.grow = function (...args) {
      ...
      return Reflect.apply(grow, this, args);
    };
    kernel.run(a);
  } finally {
  ...
  assert.equal(methodCalls, a.length);
  same(a, new Float64Array(a.length).fill(16));
  assert.equal(kernel.diagnostics.wasmCalls, 0);
  assert.equal(kernel.diagnostics.fallbackCalls, 1);
});

test("missing or TDZ Math resolver fails closed before effects; original receiver preserved", () => {
  const source = "function update(a) { for(let i=0;i<a.length;i++) a[i]=Math.abs(a[i]); }",
    artifact = compile(source);
  const a = new Float64Array([-1]);
  assert.throws(() => instantiateNumericKernel(artifact.wasm).run(a), {
    code: "KERNEL_MATH_BINDING",
  });
  same(a, [-1]);
  const receiver = {};
  let calls = 0;
  ...
  assert.equal(kernel.run.call(receiver, a), 1);
  assert.equal(calls, 1);
  same(a, [-1]);
});
...
test("unneeded intrinsic replacements and resolvers do not affect native legacy calls", () => {
  ...
  const kernel = host(compile(source), {
    resolveMath() {
      throw new Error("not a dependency");
    },
  });
  const saved = Object.getOwnPropertyDescriptor(Math, "max");
  let gets = 0;
  ...
  try {
    ...
    Object.defineProperty(Math, "max", saved);
  }
  same(a, [2, 3]);
  assert.equal(gets, 0);
  assert.equal(kernel.diagnostics.wasmCalls, 1);
...
test("disabled Math compilation remains explicit and metadata is immutable", () => {
  const source = "function update(a) { for(let i=0;i<a.length;i++) a[i]=Math.sqrt(a[i]); }";
  assert.throws(
    () => compileNumericKernel(source, { parameterTypes: ["f64[]"] }),
    NumericKernelCompileError,
  );
  assert.throws(() => compile(source, ["f64[]"], { allowMath: 1 }), { code: "INVALID_KERNEL_ABI" });
  const artifact = compile(source),
    kernel = host(artifact);
  assert.ok(Object.isFrozen(artifact.manifest.mathIntrinsics));
...
for (const value of rejected)
  test(`retains unsupported Math expression: ${value.slice(0, 70)}`, () => {
    assert.throws(
      () => compile(`function update(a) { for(let i=0;i<a.length;i++) a[i]=${value}; }`),
      NumericKernelCompileError,
    );
  });
for (const source of [
  ...
  test(`rejects shadowed intrinsic: ${source.slice(0, 65)}`, () => {
    const types = source.includes("a,Math") ? ["f64[]", "f64"] : ["f64[]"];
    assert.throws(() => compile(source, types), NumericKernelCompileError);
  });
for (const body of ["return Math.sqrt(x); const Math=1;", "const Math=1;return Math.sqrt(x);"])
  test("helper TDZ/local Math is not global Math: " + body, () => {
    assert.throws(
      () =>
        compile("function update(a) { for(let i=0;i<a.length;i++) a[i]=f(a[i]); }", ["f64[]"], {
...
test("guard failures retain coercions, aliases, and plain-array behavior", () => {
  ...
  const update =
    "function update(a,b) { for(let i=0;i<a.length;i++) { a[i]=Math.abs(b[i]); b[i]=a[i]+1; } }";
  const alias = host(compile(update, ["f64[]", "f64[]"]), { fallback: original(update) });
  const data = new Float64Array([-2, -3]);
  alias.run(data, data);
  same(data, [3, 4]);
  assert.equal(alias.diagnostics.lastGuardFailure, "KERNEL_ARRAY_ALIAS");
});
...
function rewriteManifest(bytes, edit) {
  ...
  while (offset < bytes.length) {
    const start = offset,
      id = bytes[offset++];
    const [length, payload] = read(offset);
    offset = payload + length;
    if (id !== 0) continue;
    const [nameLength, nameStart] = read(payload),
      jsonStart = nameStart + nameLength;
    const manifest = JSON.parse(new TextDecoder().decode(bytes.slice(jsonStart, offset)));
    edit(manifest);
    const body = [
      ...bytes.slice(payload, jsonStart),
...
for (const [label, edit] of [
  ["missing list", (m) => delete m.mathIntrinsics],
  ["empty list", (m) => (m.mathIntrinsics = [])],
  ["duplicate", (m) => (m.mathIntrinsics = ["sqrt", "sqrt"])],
  ["unknown method", (m) => (m.mathIntrinsics = ["sin"])],
  ["invalid member", (m) => (m.mathIntrinsics = [{}])],
  ["legacy downgrade", (m) => (m.numericSemantics = "f64-operator-order")],
  ["unknown semantics", (m) => (m.numericSemantics = "approximate-math")],
])
  test("rejects malformed intrinsic contract: " + label, () => {
    const artifact = compile(
      "function update(a) { for(let i=0;i<a.length;i++) a[i]=Math.sqrt(a[i]); }",
    );
    assert.throws(() => instantiateNumericKernel(rewriteManifest(artifact.wasm, edit)), {
      code: "KERNEL_ABI_MISMATCH",
    });
  });
