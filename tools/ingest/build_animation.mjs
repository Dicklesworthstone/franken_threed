...
function json(value, depth = 0) {
  ...
  if (typeof value === "number") {
    if (!Number.isFinite(value)) fail("GLTF_ANIMATION_VALUE", "Non-finite JSON metadata");
  ...
  if (value && typeof value === "object")
    return (
      ...
      Object.entries(value)
        .map(([key, item]) => JSON.stringify(key) + ":" + json(item, depth + 1))
        .join(",") +
      "}"
    );
...
function parseContainer(bytes) {
  ...
  if (bytes.length >= 4 && bytes.readUInt32LE(0) === 0x46546c67) {
    if (bytes.length < 20 || bytes.readUInt32LE(4) !== 2 || bytes.readUInt32LE(8) !== bytes.length)
      fail("GLTF_ANIMATION_GLB", "Invalid GLB header");
    let cursor = 12,
      chunks = 0;
    while (cursor < bytes.length) {
      if (cursor + 8 > bytes.length) fail("GLTF_ANIMATION_GLB", "Truncated GLB chunk");
      const length = bytes.readUInt32LE(cursor),
        kind = bytes.readUInt32LE(cursor + 4),
        end = cursor + 8 + length;
      if (length % 4 || end > bytes.length) fail("GLTF_ANIMATION_GLB", "Invalid GLB chunk extent");
      if (chunks === 0) {
        if (kind !== 0x4e4f534a) fail("GLTF_ANIMATION_GLB", "JSON must be the first GLB chunk");
        source = bytes.subarray(cursor + 8, end);
      } else if (kind === 0x4e4f534a) fail("GLTF_ANIMATION_GLB", "Duplicate JSON chunk");
      else if (kind === 0x004e4942) {
        if (bin || chunks !== 1) fail("GLTF_ANIMATION_GLB", "BIN must be the second chunk");
        bin = bytes.subarray(cursor + 8, end);
      }
      chunks++;
      cursor = end;
    }
  ...
  let model;
  try {
    model = JSON.parse(new TextDecoder("utf-8", { fatal: true }).decode(source));
  } catch {
    fail("GLTF_ANIMATION_JSON", "Invalid model JSON");
  }
  return { model, bin };
}
function decodeDataUri(uri) {
  ...
  if (match[1]) {
    if (!/^[A-Za-z0-9+/]*={0,2}$/.test(body) || body.replace(/=+$/, "").length % 4 === 1)
      fail("GLTF_ANIMATION_URI", "Invalid base64 buffer");
    const bytes = Buffer.from(body, "base64");
    if (bytes.toString("base64").replace(/=+$/, "") !== body.replace(/=+$/, ""))
      fail("GLTF_ANIMATION_URI", "Invalid base64 buffer");
    return bytes;
  }
  const out = [];
  for (let i = 0; i < body.length; i++) {
    if (body[i] === "%") {
      if (!/^[\da-f]{2}$/i.test(body.slice(i + 1, i + 3)))
        fail("GLTF_ANIMATION_URI", "Invalid escaped binary data");
      out.push(parseInt(body.slice(i + 1, i + 3), 16));
      i += 2;
    } else {
      const byte = body.charCodeAt(i);
      if (byte > 127) fail("GLTF_ANIMATION_URI", "Binary data URI must contain escaped octets");
      out.push(byte);
    }
  }
...
export function buildAnimation(
  ...
  const entry = path.resolve(entryPath),
    destination = path.resolve(outDir),
    root = fs.realpathSync(rootDir);
  try {
    fs.lstatSync(destination);
    fail("ANIMATION_OUTPUT_EXISTS", "Destination must be a fresh directory");
  } catch (error) {
  ...
  function read(file) {
    ...
    if (!inside(root, canonical))
      fail("GLTF_ANIMATION_ROOT", "Buffer symlink escapes the application root");
    if (!files.has(canonical)) {
      const stat = fs.statSync(canonical);
      if (!stat.isFile()) fail("GLTF_ANIMATION_FILE", "Animation input is not a file");
      if (stat.size + inputBytes > maxBytes)
        fail("GLTF_ANIMATION_LIMIT", "Input byte budget exceeded");
      const data = fs.readFileSync(canonical);
      inputBytes += data.length;
      if (inputBytes > maxBytes) fail("GLTF_ANIMATION_LIMIT", "Input grew beyond byte budget");
      files.set(canonical, data);
    }
    return files.get(canonical);
  }
  const source = read(entry),
    { model, bin } = parseContainer(source),
    loaded = new Map();
  const loadBuffer = (index) => {
    if (loaded.has(index)) return loaded.get(index);
    const buffer = model.buffers[index];
    let bytes, logical;
    if (buffer.uri === undefined) {
      if (index !== 0 || !bin) fail("GLTF_ANIMATION_BUFFER", "Missing GLB BIN buffer");
      bytes = bin;
      logical = "#BIN";
      if (bin.length - buffer.byteLength > 3)
        fail("GLTF_ANIMATION_BUFFER", "GLB BIN padding exceeds three bytes");
    } else if (typeof buffer.uri !== "string" || !buffer.uri)
      fail("GLTF_ANIMATION_URI", "Invalid buffer URI");
    else if (buffer.uri.startsWith("data:")) {
      bytes = decodeDataUri(buffer.uri);
      logical = `#data-buffer-${index}`;
    } else {
      let url;
      try {
        url = new URL(buffer.uri, pathToFileURL(entry));
      } catch {
        fail("GLTF_ANIMATION_URI", "Invalid buffer URL");
      }
      if (url.protocol !== "file:")
        fail("GLTF_ANIMATION_NETWORK", "Animation build does not fetch network resources");
      bytes = read(fileURLToPath(url));
      logical =
        path.relative(root, fileURLToPath(url)).split(path.sep).join("/") + url.search + url.hash;
    }
  ...
  const encoded = json(definition),
    runtime = fs.readFileSync(new URL("./animation_runtime.mjs", import.meta.url), "utf8");
...
`,
  ...
  if (threeScene) {
    ...
    outputs.set("gpu_playback.mjs", outputs.get("gpu_playback.mjs") +
      ...
      "export {createGpuThreeCanvas,createGpuThreeHdrCanvas} from './three_canvas.mjs';\n" +
  ...
  if (rigidGeometry) {
    outputs.set(
      ...
      fs.readFileSync(new URL("./animation_rigid_geometry.mjs", import.meta.url), "utf8"),
    );
    outputs.set(
      "gpu_playback.mjs",
  ...
  const manifest = {
    ...
    entry: "animation.mjs",
    playbackEntry: "playback.mjs",
    profile: "core-gltf-animation-pose",
    ...(webgpu
      ? {
          gpuEntry: "gpu_playback.mjs",
          gpuExecution: "webgpu-compute-f32",
          gpuRendering:
            "explicit-unlit-triangle-list; caller-owned geometry, materials and attachments",
        }
      : {}),
    ...(rigidGeometry
    ...
    morphWeightCount: validated.morphWeights.length,
    ignoredChannels: definition.ignoredChannels,
    execution: "javascript-cpu-pose",
    accelerationClaim: false,
    ...(expanded.instanceCount
      ? {
          meshInstanceCount: expanded.instanceCount,
          instanceOrigins: expanded.instanceOrigins,
          instanceExecution: "expanded-node-mesh",
        }
      : {}),
    playback:
      "explicit sampling, per-binding blending and action controls; imported rest-relative additive layers",
    deformation:
      "optional CPU morph-then-skin over loader-decoded geometry; mesh-local outputs and matching world transform",
    artifacts: [...outputs].map(([file, data]) => ({
      file,
  ...
  if (outputBytes > maxBytes)
    fail("GLTF_ANIMATION_LIMIT", "Generated animation package exceeds byte budget");
  fs.mkdirSync(path.dirname(destination), { recursive: true });
  fs.mkdirSync(destination);
  for (const [file, data] of outputs)
    fs.writeFileSync(path.join(destination, file), data, { flag: "wx" });
  return {
    ...manifest,
    outDir: destination,
    emittedFiles: [...outputs.keys()],
    inputBytes,
    outputBytes,
  };
}
