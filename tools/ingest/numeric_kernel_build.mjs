...
function compileFile(source, sourceName, options) {
  const refuse = (message, code = "KERNEL_NOT_CLOSED") => {
    throw new NumericKernelCompileError(code, message);
  };
  if (source.length > 1048576)
    refuse("Numeric source file exceeds the 1 MiB limit", "INVALID_KERNEL_SOURCE");
  let ast;
  try {
    ast = acorn.parse(source, { ecmaVersion: "latest", sourceType: "module" });
  } catch (error) {
    refuse(error.message, "INVALID_KERNEL_SOURCE");
  }
  const functions = ast.body.map((statement) =>
    statement.type === "ExportNamedDeclaration" ? statement.declaration : statement,
  );
  if (!functions.length || functions.some((fn) => fn?.type !== "FunctionDeclaration" || !fn.id)) {
    refuse(
      "Numeric package source must contain only named function declarations and optional named exports",
    );
  }
  ...
  if (
    functions.length === 1 &&
    (options.functionName === undefined || options.functionName === functions[0].id.name)
  ) {
    return {
  ...
  function assigned(node) {
    ...
    if (node.type === "Identifier") mutations.add(node.name);
      ...
      node.properties.forEach((item) =>
        assigned(item.type === "RestElement" ? item.argument : item.value),
      );
  }
  walk.full(ast, (node) => {
  ...
  const artifact = compileNumericCandidate(single ? source : source.slice(fn.start, fn.end), {
    ...
    helperSources: new Map(
      functions
        .filter((helper) => !mutations.has(helper.id.name))
