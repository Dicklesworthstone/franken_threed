...
function ordinaryBuffer(array) {
  if (
    ...
    return false;
  try {
    new Uint8Array(array.buffer, 0, 0);
    return true;
  } catch {
    return false;
  }
...
function budget(value, limit, name) {
  requireThat(
    Number.isSafeInteger(value) && value <= limit,
    "DEFORMATION_BUDGET",
    `${name} exceeds ${limit}`,
  );
}
function attribute(attribute, itemSize, count, name) {
  ...
  requireThat(
    attribute &&
      ...
      !target?.isInstancedInterleavedBuffer &&
      ordinaryBuffer(target?.array),
    "DEFORMATION_ATTRIBUTE",
    `${name} needs ${count} ordinary ${itemSize}-component vertices`,
  );
  requireThat(
    !Object.hasOwn(target, "onUploadCallback") &&
      (!target.onUploadCallback ||
        target.onUploadCallback === Object.getPrototypeOf(target)?.onUploadCallback),
    "UNSUPPORTED_UPLOAD_CALLBACK",
    `${name} has a custom upload callback`,
  );
  for (const getter of ["getX", "getY", "getZ", "getW"].slice(0, itemSize)) {
    requireThat(
      typeof attribute[getter] === "function",
      "DEFORMATION_ATTRIBUTE",
      `${name}.${getter} is required`,
    );
  }
  return attribute;
}
    ...
    for (let axis = 0; axis < size; axis++) {
      ...
      requireThat(
        Number.isFinite(value) &&
          (!integers || (Number.isInteger(value) && value >= 0 && value <= 0xffffffff)),
        "DEFORMATION_VALUE",
        `${name}[${vertex},${axis}] is not ${integers ? "a u32" : "finite"}`,
      );
...
function matrix(matrix, destination, offset, name) {
  ...
  requireThat(
    (Array.isArray(source) || ordinaryBuffer(source)) && source.length === 16,
    "DEFORMATION_BIND",
    `${name} needs sixteen numeric matrix elements`,
  );
  for (let i = 0; i < 16; i++) {
 ...
 */
export function captureDeformationBatch(
  meshes,
  ...
  requireThat(
    ...
    "EMPTY_MESH_BATCH",
    "an ordered nonempty mesh array is required",
  );
  for (const [name, value] of Object.entries({ maxVertices, maxMorphComponents, maxJoints }))
    checkedSize(value, name);
  const rows = [];
  let vertexCount = 0,
    morphComponents = 0,
    morphCount = 0,
  ...
  for (let index = 0; index < meshes.length; index++) {
    ...
    requireThat(
      mesh?.isMesh && !mesh.isInstancedMesh && !mesh.isBatchedMesh && geometry?.isBufferGeometry,
      "DEFORMATION_MESH",
      `mesh ${index} needs scalar BufferGeometry`,
    );
    ...
    for (const [name, targets] of Object.entries(families)) {
      requireThat(
        Array.isArray(targets) &&
          (name === "position" || name === "normal" || targets.length === 0),
        "DEFORMATION_MORPH",
        `mesh ${index}: unsupported ${name} morph family`,
      );
    }
    const targets = families.position ?? [],
      normals = families.normal ?? [];
    const familyCount = Math.max(targets.length, normals.length);
    const weights = mesh.morphTargetInfluences ?? [];
    requireThat(
      (Array.isArray(weights) || ordinaryBuffer(weights)) &&
      ...
      "DEFORMATION_MORPH",
      `mesh ${index}: morph families and influences disagree`,
    );
    ...
    for (let i = 0; i < familyCount; i++)
      requireThat(Number.isFinite(weights[i]), "DEFORMATION_MORPH", "morph weight is not finite");
    ...
    if (skinned) {
      requireThat(
        Array.isArray(mesh.skeleton?.bones),
        "DEFORMATION_SKELETON",
        `mesh ${index}: missing skeleton bones`,
      );
      ...
      requireThat(
        palette instanceof Float32Array &&
          ordinaryBuffer(palette) &&
          palette.length >= paletteCount * 16,
        "DEFORMATION_SKELETON",
        `mesh ${index}: an evaluated Float32 boneMatrices bank is required`,
      );
      skinIndex = attribute(geometry.attributes.skinIndex, 4, count, "skinIndex");
      requireThat(
        !skinIndex.normalized,
        "DEFORMATION_ATTRIBUTE",
        "skinIndex must not be normalized",
      );
    ...
    rows.push({
      ...
      morphOffset: morphComponents,
      weightOffset: morphCount,
      jointOffset: jointCount * 16,
      influenceOffset: influences,
      bindOffset: skinnedCount * 32,
    });
    vertexCount += count;
    morphComponents += count * 3 * targets.length;
    morphCount += targets.length;
  ...
  const layout = new Uint32Array(rows.length * 4),
    positions = new Float64Array(vertexCount * 3);
  const morphPositions = new Float64Array(morphComponents),
    morphWeights = new Float64Array(morphCount);
  const jointIndices = new Uint32Array(influences),
    jointWeights = new Float64Array(influences);
  const jointPalettes = new Float64Array(jointCount * 16),
    bindMatrices = new Float64Array(skinnedCount * 32);
  for (let i = 0; i < rows.length; i++) {
    ...
    layout.set(
      [
        ...
        row.targets.length,
        row.paletteCount,
        (row.geometry.morphTargetsRelative === true ? 1 : 0) | (row.skinned ? 2 : 0),
      ],
      i * 4,
    );
    ...
    for (let target = 0; target < row.targets.length; target++) {
      values(
        ...
        row.count,
        morphPositions,
        row.morphOffset + target * row.count * 3,
        "morph position",
      );
    ...
    if (row.skinned) {
      ...
      for (let j = 0; j < row.paletteCount * 16; j++) {
        requireThat(
          Number.isFinite(row.palette[j]),
          "DEFORMATION_SKELETON",
          `joint palette component ${j} is not finite`,
        );
  ...
  return {
    ...
    vertexCount,
    skinnedCount,
    deformedMeshCount: rows.filter((row) => row.skinned || row.targets.length > 0).length,
  };
}
...
export function evaluateDeformationBatch(input, wasm) {
  ...
  requireThat(
    ...
    "DEFORMATION_MISSING_WASM",
    "compiled f3d_deform_position_batch is required",
  );
  const output = native.call(
    wasm,
    input.layout,
    input.positions,
    input.morphPositions,
    input.morphWeights,
    input.jointIndices,
    input.jointWeights,
    input.jointPalettes,
  ...
  requireThat(
    output instanceof Float32Array &&
      ordinaryBuffer(output) &&
      output.length === input.positions.length,
    "DEFORMATION_WASM_OUTPUT",
    "compiled result must have one finite xyz tuple per input vertex",
  );
  for (const value of output)
    requireThat(Number.isFinite(value), "DEFORMATION_WASM_OUTPUT", "compiled output is not finite");
