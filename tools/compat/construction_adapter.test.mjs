...
test('Defect D: SPECIALIZED_WEBGPU selection invokes registered specialized implementation, NOT source constructorFn', () => {
  ...
  assert.equal(sourceConstructorCalls, 0, 'Source constructorFn must NOT be invoked when specialized implementation is registered');
...
test('Defect D: SPECIALIZED_WEBGPU fallback to RETAINED_UPSTREAM when no specialized implementation is registered', () => {
  ...
  assert.equal(decision.reasons.includes('specialized-island-admitted'), false, 'Must not report admission and unavailability together');
...
test('Defect D: SPECIALIZED_WEBGPU fallback selects registered RETAINED_UPSTREAM when constructorFn is absent', () => {
  ...
  assert.equal(getRendererRoute(instance), ExecutionRoute.RETAINED_UPSTREAM);
  assert.ok(getRendererDecision(instance).reasons.includes(EscapeReason.SPECIALIZATION_UNAVAILABLE));
  assert.equal(getRendererDecision(instance).reasons.includes('specialized-island-admitted'), false, 'Must not report admission and unavailability together');
  assert.equal(router.getCanvasLock(canvas)?.route, ExecutionRoute.RETAINED_UPSTREAM);
});
...
test('Defect D: SPECIALIZED_WEBGPU targetImplementation override takes highest precedence', () => {
  class RegisteredSpecialized {
    constructor() { this.type = 'registered'; }
  }
  class ExplicitSpecialized {
    constructor() { this.type = 'explicit'; }
  }
  class UpstreamSource {
    constructor() { this.type = 'upstream'; }
  }
...
test('Defect D: Qualified registered constructor takes precedence over default and bypasses source constructorFn', () => {
  ...
  assert.equal(sourceConstructorCalls, 0, 'Source constructorFn must not be called when qualified replacement is registered');
  assert.ok(instance instanceof QualifiedSpecializedRenderer, 'Must select qualified constructor over default');
  ...
  assert.ok(instanceDefault instanceof DefaultSpecializedRenderer, 'Must select default constructor when registry lacks qualified key');
...
test('Defect D: Two calls on same canvas with specializationAvailable=true both construct retained without lock collision', () => {
  ...
  assert.equal(getRendererDecision(r1).reasons.includes('specialized-island-admitted'), false, 'Must not report admission and unavailability together');
  ...
  assert.equal(getRendererDecision(r2).reasons.includes('specialized-island-admitted'), false, 'Must not report admission and unavailability together');
});
...
test('Defect D: Re-routing canvas from RETAINED_UPSTREAM to SPECIALIZED_WEBGPU when replacement exists throws RouteLockError', () => {
  class UpstreamWebGPURenderer {
    constructor(opts) { this.canvas = opts?.canvas; }
  }
  class AdmittedSpecializedRenderer {
    constructor(opts) { this.canvas = opts?.canvas; }
  }
