...
function matrixElements(matrix, label, writable) {
  ...
  for (let i = 0; i < 16; i++) {
    ...
    if (
      ...
      throw fail(
        "HIERARCHY_MATRIX",
        `${label}[${i}] must be an ordinary ${writable ? "writable " : ""}numeric element`,
      );
    }
  }
...
function readComponents(value, names, label) {
  ...
  return names.map((name) => {
    ...
    if (typeof component !== "number")
      throw fail("HIERARCHY_TRS", `${label}.${name} must be numeric`);
    return component;
  });
}
...
function checkedOutput(value, count, label) {
  if (
    ...
    throw fail(
      "HIERARCHY_WASM_OUTPUT",
      `${label} must be an owned Float64Array of length ${count}`,
    );
  }
  return value;
}
...
export function bindThreeTransformHierarchy(wasm, objects) {
  ...
  for (const node of nodes) {
    if (
      !node ||
      node.isObject3D !== true ||
      node.isCamera ||
      node.isSkinnedMesh ||
      Object.hasOwn(node, "updateMatrixWorld") ||
      Object.hasOwn(node, "updateWorldMatrix")
    ) {
      throw fail(
        "HIERARCHY_NODES",
        "ordinary Object3D matrices without camera, skin, or own update hooks are required",
      );
    }
  ...
  function snapshot() {
    ...
    for (let index = 0; index < nodes.length; index++) {
      ...
      if (
        typeof node.matrixAutoUpdate !== "boolean" ||
        typeof node.matrixWorldAutoUpdate !== "boolean"
      ) {
        throw fail(
          "HIERARCHY_FLAGS",
          "matrixAutoUpdate and matrixWorldAutoUpdate must be explicit booleans",
        );
      }
      const parent = node.parent;
      if (parent !== null && !slots.has(parent))
        throw fail("HIERARCHY_PARENT", "parent is outside the bound node membership");
      ...
      if (destinations.has(local) || destinations.has(world) || local === world) {
        throw fail(
          "HIERARCHY_ALIAS",
          "local and world matrix arrays must have independent identities",
        );
      }
      ...
      locals.set(local, index * 16);
      worlds.set(world, index * 16);
      ...
      const row = {
        ...
        local,
        world,
        autoLocal: node.matrixAutoUpdate,
        autoWorld: node.matrixWorldAutoUpdate,
        originalLocal: Array.from(local),
        originalWorld: Array.from(world),
      };
      if (row.autoLocal) {
        row.position = node.position;
        row.quaternion = node.quaternion;
        ...
        autoNodes.push(index);
        positions.push(...row.p);
        rotations.push(...row.q);
        scales.push(...row.s);
      }
      if (!row.autoWorld) {
        worldNodes.push(index);
        externalWorlds.push(...world);
      }
      rows.push(row);
    }
    ...
    if (autoNodes.length) {
      if (typeof wasm.f3d_batch_compose_scalar !== "function") {
        throw fail(
          "HIERARCHY_MISSING_WASM",
          "automatic local matrices require compiled f3d_batch_compose_scalar",
        );
      }
      const composed = checkedOutput(
        wasm.f3d_batch_compose_scalar(
          new Float64Array(positions),
          new Float64Array(rotations),
          new Float64Array(scales),
        ),
        autoNodes.length * 16,
        "composed local matrices",
      );
    ...
    return {
      ...
      flags,
      rows,
      worldNodes: new Uint32Array(worldNodes),
      externalWorlds: new Float64Array(externalWorlds),
    };
  }
  ...
  function checkSnapshot(input) {
    for (const row of input.rows) {
      ...
      if (
        n.parent !== row.parent ||
        n.matrix !== row.localMatrix ||
        n.matrixWorld !== row.worldMatrix ||
        n.matrixAutoUpdate !== row.autoLocal ||
        n.matrixWorldAutoUpdate !== row.autoWorld ||
        n.matrix.elements !== row.local ||
        n.matrixWorld.elements !== row.world ||
  ...
  try {
    ...
    native = new wasm.F3dTransformHierarchy(
      initial.parents,
      initial.locals,
      initial.worlds,
      initial.flags,
    ...
    for (const method of methods) {
      if (typeof native?.[method] !== "function")
        throw fail("HIERARCHY_MISSING_WASM", `compiled hierarchy lacks ${method}`);
    }
    checkSnapshot(initial);
  } catch (error) {
    disposed = true;
    ...
    try {
      ...
      /* Preserve the constructor's original error. */
    }
    throw error;
  }
    ...
    update() {
      ...
      try {
        ...
        if (typeof revision !== "bigint")
          throw fail("HIERARCHY_WASM_OUTPUT", "revision must be a lossless BigInt");
        const worlds = checkedOutput(
          native.worldMatrices(revision),
          nodes.length * 16,
          "world matrices",
        );
        ...
        if (
          !(changed instanceof Uint32Array) ||
          !(stats instanceof Uint32Array) ||
          stats.length !== 5 ||
          stats.some((value) => value > nodes.length) ||
          stats[0] !== stats[1] + stats[2] + stats[3] ||
          stats[4] !== changed.length ||
          changed.some((node, i) => node >= nodes.length || (i > 0 && node <= changed[i - 1]))
        ) {
          throw fail(
            "HIERARCHY_WASM_OUTPUT",
            "invalid solve statistics or changed-node identities",
          );
        }
        if (native.revision !== revision)
          throw fail("HIERARCHY_STALE_RESULT", "native revision changed before publication");
        ...
        return Object.freeze({
          ...
          multiplied: stats[1],
          copiedRoots: stats[2],
          preservedWorlds: stats[3],
          changedNodes: changed.slice(),
        });
      } finally {
        busy = false;
