...
test('evidence logger appends conforming events and generates summary', () => {
  ...
  const requiredFields = [
    'ts_wall', 'ts_app', 'lane', 'bead', 'test', 'step',
    'level', 'owner', 'route', 'browser', 'msg', 'data'
  ];

  for (const line of lines) {
    const ev = JSON.parse(line);
    for (const field of requiredFields) {
      assert.ok(Object.prototype.hasOwnProperty.call(ev, field), `Event missing required field ${field}`);
    }
  }
  ...
  assert.ok(typeof summary.commit === 'string' && summary.commit.length > 0, 'commit must be non-empty string');
