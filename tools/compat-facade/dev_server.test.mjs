...
const H1_PATH = path.resolve(
  REPO_ROOT,
  "upstream/three.js/examples/webgpu_performance_renderbundle.html",
);
...
test("Import Map: createDevImportMap produces correct package routing entries", () => {
  ...
  const h1DevMap = createDevImportMap({
    baseUrl: "http://127.0.0.1:8080",
    sourceImportMap: h1SourceMap,
  });
  assert.equal(
    h1DevMap.imports["three"],
    "http://127.0.0.1:8080/compat-facade/webgpu.js",
    "H1 three must route to webgpu facade",
  );
  assert.equal(
    h1DevMap.imports["three/webgpu"],
    "http://127.0.0.1:8080/compat-facade/webgpu.js",
    "H1 three/webgpu must route to webgpu facade",
  );

  // Explicit routeThreeToWebgpu option without sourceImportMap
  const explicitWebgpuMap = createDevImportMap({
    baseUrl: "http://127.0.0.1:8080",
    routeThreeToWebgpu: true,
  });
  assert.equal(explicitWebgpuMap.imports["three"], "http://127.0.0.1:8080/compat-facade/webgpu.js");
  assert.equal(
    explicitWebgpuMap.imports["three/webgpu"],
    "http://127.0.0.1:8080/compat-facade/webgpu.js",
  );
});
...
test("HTML Transformation: transformHtmlImportMap preserves H1 application code while routing import map", () => {
  ...
  assert.ok(
    transformedHtml.includes('"three": "/compat-facade/webgpu.js"'),
    "Import map must preserve H1 alias mapping three to webgpu facade",
  );
  assert.ok(
    transformedHtml.includes('"three/webgpu": "/compat-facade/webgpu.js"'),
    "Import map must route three/webgpu to webgpu facade",
  );
  assert.ok(
    transformedHtml.includes('"three/tsl": "/compat-facade/tsl.js"'),
    "Import map must route three/tsl to tsl facade",
  );
  assert.ok(
    transformedHtml.includes('"three/addons/": "/compat-facade/addons/"'),
    "Import map must route three/addons/ to addons facade",
  );

  // 2. Upstream application script must be 100% preserved
  assert.ok(
    transformedHtml.includes("import * as THREE from 'three/webgpu';"),
    "App import from three/webgpu preserved",
  );
  assert.ok(
    transformedHtml.includes("import { Inspector } from 'three/addons/inspector/Inspector.js';"),
    "Inspector import preserved",
  );
  assert.ok(
    transformedHtml.includes(
      "import { OrbitControls } from 'three/addons/controls/OrbitControls.js';",
    ),
    "OrbitControls import preserved",
  );
  assert.ok(
    transformedHtml.includes(
      "renderer = new THREE.WebGPURenderer( { antialias: true, forceWebGL: ! api.webgpu } );",
    ),
    "Renderer construction preserved",
  );
  assert.ok(
    transformedHtml.includes("function randomizeMatrix( matrix )"),
    "randomizeMatrix function preserved",
  );
  assert.ok(
    transformedHtml.includes("renderer.inspector = new Inspector();"),
    "Inspector setup preserved",
  );
  assert.ok(
    transformedHtml.includes(
      "gui.add( api, 'renderBundle' ).name( 'render bundle' ).onChange( reload );",
    ),
    "GUI controls preserved",
  );
...
test("Dev Server: Lifecycle, HTTP responses, and H1 delivery", async () => {
  ...
  try {
    ...
    assert.ok(
      webgpuSource.includes("tools/compat/route_types.mjs"),
      "Must directly import route_types.mjs",
    );
    assert.ok(
      webgpuSource.includes("tools/compat/construction_adapter.mjs"),
      "Must directly import construction_adapter.mjs",
    );
    ...
    assert.equal(
      textureResWithPrefix.status,
      200,
      "/examples/textures/uv_grid_opengl.jpg must return 200",
    );
    ...
    assert.equal(
      examplesJsmRes.status,
      200,
      "/examples/jsm/controls/OrbitControls.js must return 200",
    );
    ...
    assert.equal(
      lut3DFacadeRes.status,
      200,
      "/compat-facade/addons/tsl/display/Lut3DNode.js must return 200",
    );
...
test("Exact Backend & WebGPU Mode: Genuine WebGPURenderer retained in both modes", async () => {
  ...
  assert.ok(
    webgpuInstance instanceof PinnedWebGPURenderer,
    "Must construct genuine WebGPURenderer",
  );
  assert.equal(
    webgpuInstance.backend.constructor.name,
    "WebGPUBackend",
    "Default mode must select WebGPUBackend",
  );
  ...
  assert.equal(
    webglInstance.backend.constructor.name,
    "WebGLBackend",
    "forceWebGL must select WebGLBackend, NOT legacy WebGLRenderer",
  );
  ...
  assert.ok(
    !webgpuOwnKeys.some((k) => k.startsWith("__f3d_")),
    "No enumerable __f3d own properties on WebGPU instance",
  );
  assert.ok(
    !webglOwnKeys.some((k) => k.startsWith("__f3d_")),
    "No enumerable __f3d own properties on WebGL instance",
  );
});
...
test("Generated WebGPU Facade Module: Evaluation, Proxy constructor adapter, instanceof, subclassing, and new.target", async () => {
  ...
  assert.equal(
    typeof facadeModule.WebGPURenderer,
    "function",
    "WebGPURenderer must be exported as a constructor",
  );
  assert.equal(
  ...
  assert.ok(
    rendererWebGPU instanceof facadeModule.WebGPURenderer,
    "Instance must be instanceof facade WebGPURenderer",
  );
  assert.ok(
    rendererWebGPU instanceof UpstreamWebGPURenderer,
    "Instance must be instanceof Upstream WebGPURenderer",
  );
  ...
  assert.equal(
    rendererWebGPU.constructor,
    UpstreamWebGPURenderer,
    "Direct instance constructor references pristine upstream class",
  );
  assert.equal(
    rendererWebGPU.backend.constructor.name,
    "WebGPUBackend",
    "Default mode selects WebGPUBackend",
  );
  assert.ok(
  ...
  assert.ok(
    rendererWebGL instanceof facadeModule.WebGPURenderer,
    "WebGL instance must be instanceof facade WebGPURenderer",
  );
  assert.ok(
    rendererWebGL instanceof UpstreamWebGPURenderer,
    "WebGL instance must be instanceof Upstream WebGPURenderer",
  );
  assert.equal(
    rendererWebGL.constructor,
    UpstreamWebGPURenderer,
    "Direct WebGL instance constructor references pristine upstream class",
  );
  assert.equal(
    rendererWebGL.backend.constructor.name,
    "WebGLBackend",
    "forceWebGL selects WebGLBackend (never legacy WebGLRenderer)",
  );
  ...
  assert.ok(
    derivedInstance instanceof CustomAppRenderer,
    "Subclass instance must be instanceof CustomAppRenderer",
  );
  assert.ok(
    derivedInstance instanceof facadeModule.WebGPURenderer,
    "Subclass instance must be instanceof facade WebGPURenderer",
  );
  assert.ok(
    derivedInstance instanceof UpstreamWebGPURenderer,
    "Subclass instance must be instanceof Upstream WebGPURenderer",
  );
  assert.equal(
    derivedInstance.constructor,
    CustomAppRenderer,
    "Subclass instance constructor identity must equal CustomAppRenderer",
  );
  assert.equal(
    derivedInstance.customFlag,
    "app-custom-renderer",
    "Subclass own property preserved",
  );
  assert.equal(derivedInstance.customMethod(), 999, "Subclass method callable");
  assert.equal(
    derivedInstance.backend.constructor.name,
    "WebGPUBackend",
    "Subclass backend created",
  );
  ...
  try {
    ...
    assert.equal(
      subclassGetterCount,
      0,
      "Construction must NOT invoke observable domElement getter on subclass",
    );
    ...
    assert.ok(
      exposedDomElement && typeof exposedDomElement === "object",
      "Exposed canvas must be an object",
    );
    assert.equal(
...
test("Served Module Graph Closure: Ingest tools resolve served H1 and Inspector reachable static imports with zero unresolved specifiers", async () => {
  ...
  try {
    ...
    assert.ok(
      ...
      'H1 must contain at least one <script type="module"> entry',
    );
    ...
    assert.ok(
      fetchedModules.length >= 25,
      `Expected at least 25 fetched modules, got ${fetchedModules.length}`,
    );
    ...
    assert.ok(
      fetchedPaths.has("/compat-facade/addons/inspector/Inspector.js"),
      "Must fetch Inspector.js",
    );
    assert.ok(
      fetchedPaths.has("/compat-facade/addons/controls/OrbitControls.js"),
      "Must fetch OrbitControls.js",
    );
    assert.ok(
      fetchedPaths.has("/compat-facade/addons/inspector/RendererInspector.js"),
      "Must fetch RendererInspector.js",
    );
    assert.ok(
      fetchedPaths.has("/upstream/three.js/build/three.webgpu.js"),
      "Must fetch upstream three.webgpu.js",
    );
    ...
    assert.ok(
      fetchedPaths.has("/tools/compat/construction_adapter.mjs"),
      "Must fetch construction_adapter.mjs",
    );
    assert.ok(
      fetchedPaths.has("/tools/compat/connected_groups.mjs"),
      "Must fetch connected_groups.mjs",
    );
    assert.ok(fetchedPaths.has("/tools/compat/route_decider.mjs"), "Must fetch route_decider.mjs");
    assert.ok(
      !fetchedPaths.has("/tools/compat/exact_backend.mjs"),
      "Must NOT fetch exact_backend.mjs (prevents three.module load)",
    );
    assert.ok(!fetchedPaths.has("/tools/compat/index.mjs"), "Must NOT fetch barrel index.mjs");
    assert.ok(
      !fetchedPaths.has("/tools/compat/route_report.mjs"),
      "Must NOT fetch route_report.mjs",
    );
    assert.ok(
      !fetchedPaths.has("/upstream/three.js/build/three.module.js"),
      "Must NOT fetch three.module.js",
    );

    // Inspector tabs
    assert.ok(
      fetchedPaths.has("/compat-facade/addons/inspector/tabs/Performance.js"),
      "Must fetch Performance tab",
    );
    assert.ok(
      fetchedPaths.has("/compat-facade/addons/inspector/tabs/Memory.js"),
      "Must fetch Memory tab",
    );
    assert.ok(
      fetchedPaths.has("/compat-facade/addons/inspector/tabs/Console.js"),
      "Must fetch Console tab",
    );
    assert.ok(
      fetchedPaths.has("/compat-facade/addons/inspector/tabs/Parameters.js"),
      "Must fetch Parameters tab",
    );
    assert.ok(
      fetchedPaths.has("/compat-facade/addons/inspector/tabs/Settings.js"),
      "Must fetch Settings tab",
    );
    assert.ok(
      fetchedPaths.has("/compat-facade/addons/inspector/tabs/Viewer.js"),
      "Must fetch Viewer tab",
    );
    assert.ok(
      fetchedPaths.has("/compat-facade/addons/inspector/tabs/Timeline.js"),
      "Must fetch Timeline tab",
    );
    ...
    assert.ok(
      ...
      "example.css must inline logo via data URI",
    );
    ...
    assert.equal(
      textureRes.status,
      200,
      "Texture asset /examples/textures/uv_grid_opengl.jpg must return 200",
    );
...
test("Regression (Topaz 6953): Inspector bare import three/addons/tsl/display/Lut3DNode.js and three/tsl resolve to served 200 modules", async () => {
  ...
  try {
    ...
    const tslSource = await tslRes.text();
    assert.ok(
      tslSource.includes("build/three.tsl.js"),
      "TSL facade must re-export from build/three.tsl.js",
    );
  } finally {
    await close();
  }
...
test("Honesty Guard (Plan §5.1, Mails 6869/6879): Actual generated facade router implementations contain exactly exact-backend and retained-upstream, pristine prototype constructor, and zero enumerable __f3d_ properties", async () => {
  ...
  const f3dOwnPropsWebGPU = Object.getOwnPropertyNames(instanceWebGPU).filter((k) =>
    k.startsWith("__f3d_"),
  );
  assert.deepEqual(
    f3dOwnPropsWebGPU,
  ...
  assert.deepEqual(
    forInKeysWebGPU,
    [],
    "for..in loop on WebGPU instance must yield 0 __f3d_ properties",
  );
  ...
  const f3dOwnPropsWebGL = Object.getOwnPropertyNames(instanceWebGL).filter((k) =>
    k.startsWith("__f3d_"),
  );
  assert.deepEqual(
    f3dOwnPropsWebGL,
  ...
  assert.deepEqual(
    forInKeysWebGL,
    [],
    "for..in loop on WebGL instance must yield 0 __f3d_ properties",
  );
});
...
test("Served H1 HTML Import Map: Contains exactly intended routed specifiers (three, three/webgpu, three/tsl, three/addons/) and nothing else", async () => {
  ...
  try {
    ...
    assert.ok(
      ...
      "Import map must parse to an object",
    );
    assert.ok(
      parsedImportMap.imports && typeof parsedImportMap.imports === "object",
      "Import map must have imports object",
    );
    ...
    assert.deepEqual(
      topLevelKeys,
      ["imports"],
      "Import map must not contain unexpected keys like scopes",
    );
...
test("Dynamic Import Closure: Dynamic import() sites reachable from served H1 and Inspector resolve with HTTP 200 and JavaScript content-type", async () => {
  ...
  try {
    ...
    const settingsSite = dynamicImportSites.find((s) =>
      s.referrerUrl.includes("/inspector/tabs/Settings.js"),
    );
