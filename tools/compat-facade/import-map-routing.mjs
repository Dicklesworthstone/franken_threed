...
function routeTarget(specifier, target, base) {
  ...
  let url;
  try {
    url = new URL(target, "https://f3d.invalid/");
  } catch {
    return target;
  }
...
export function routeImportMap(source, { baseUrl = "" } = {}) {
  ...
  const integrity =
    source.integrity === undefined ? null : record(source.integrity, "Import map integrity");
  const hasIntegrity = integrity !== null && Object.keys(integrity).length > 0;
  const routeEntries = (entries, label) =>
    Object.fromEntries(
      Object.entries(record(entries, label)).map(([specifier, target]) => {
        ...
        if (hasIntegrity && routed !== target) {
          throw new Error(
            "Cannot reroute an integrity-pinned import map without regenerated facade integrity hashes",
          );
        }
  ...
  if (source.imports !== undefined)
    result.imports = routeEntries(source.imports, "Import map imports");
  if (source.scopes !== undefined) {
    result.scopes = Object.fromEntries(
      Object.entries(record(source.scopes, "Import map scopes")).map(([scope, imports]) => [
        scope,
        routeEntries(imports, `Import map scope ${JSON.stringify(scope)}`),
      ]),
    );
  }
  return result;
...
export function createDevImportMap({
  baseUrl = "",
  sourceImportMap = null,
  routeThreeToWebgpu = false,
} = {}) {
  const base = baseUrl.replace(/\/+$/, "");
  ...
  if (routeThreeToWebgpu && imports.three !== null) {
    if (
      routed.integrity &&
      Object.keys(routed.integrity).length &&
      Object.hasOwn(routed.imports ?? {}, "three") &&
    ...
    !Object.hasOwn(routed.imports ?? {}, "three/webgpu") &&
    typeof imports.three === "string" &&
...
function inlineImportMaps(html) {
  ...
  for (let tag; (tag = tags.exec(html)); ) {
    ...
    if (attrs.has("src"))
      throw new TypeError("Import maps must be inline, not script src references");
