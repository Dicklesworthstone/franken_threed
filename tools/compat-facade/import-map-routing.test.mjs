...
const wrap = (map, attributes = 'type="importmap"') => `<script ${attributes}>${JSON.stringify(map)}</script>`;
...
test('scoped routes preserve aliases, per-scope selection and non-Three dependencies', () => {
  const source = {
    ...
    scopes: {
      '/gpu/': {three:'../build/three.webgpu.js', 'three/webgpu':'../build/three.webgpu.js', utility:'/gpu/utility.mjs'},
...
test('programmatic development maps preserve custom entries, scopes and explicit blocks', () => {
  const source = {imports:{three:null, 'three/webgpu':null, utility:'/utility.mjs'},scopes:{'/private/':{three:null}}};
  ...
  assert.equal(createDevImportMap({routeThreeToWebgpu:true}).imports.three, '/compat-facade/webgpu.js');
});

test('HTML routing never replaces an explicit null with an unblocked default', () => {
  const map = readMap(transformHtmlImportMap(wrap({imports:{three:null,'three/webgpu':null}})));
...
test('WebGPU aliases keep identical query and fragment module identity', () => {
  const source = {imports:{three:'../build/three.webgpu.js?revision=186#entry', alias:'../build/three.webgpu.js?revision=186#entry'}};
  const result = createDevImportMap({baseUrl:'https://local.test/dev/', sourceImportMap:source});
  assert.equal(result.imports.three, 'https://local.test/dev/compat-facade/webgpu.js?revision=186#entry');
...
test('bundle-like query text and unrelated jsm directories are not rerouted', () => {
  const source = {imports:{utility:'/tools/inspect.js?file=three.webgpu.js',other:'/vendor/jsm/',fake:'/not-three.module.js'}};
...
test('attributes, CSP nonces, application code and outer HTML are unchanged', () => {
  ...
  const html = prefix + opening + '{"imports":{"three":"../build/three.module.js"}}' + '</ScRiPt>' + suffix;
...
test('comments, raw text, inert templates and data-type attributes are not import maps', () => {
  ...
  const result = transformHtmlImportMap(prefix + wrap({imports:{three:'../build/three.module.js'}}));
...
test('multiple maps are rewritten independently without shadowing later explicit mappings', () => {
  const html = wrap({imports:{utility:'/utility.mjs'}}) + '\n' + wrap({imports:{three:'../build/three.webgpu.js'}});
  const result = transformHtmlImportMap(html);
  const maps = [...result.matchAll(/<script[^>]*>([\s\S]*?)<\/script>/g)].map((m) => JSON.parse(m[1]));
...
test('malformed map JSON and invalid structures fail rather than fabricate a new map', () => {
  assert.throws(() => transformHtmlImportMap('<script type="importmap">{bad}</script>'), /Invalid import map JSON/);
  for (const value of [null, [], 1, {imports:[]}, {scopes:null}, {scopes:{'/':[]}}, {integrity:[]}]) {
  ...
  assert.throws(() => transformHtmlImportMap('<script type="importmap" src="map.json">{}</script>'), /must be inline/);
});

test('integrity metadata is never silently discarded or copied onto different bytes', () => {
  const untouched = {imports:{utility:'/utility.mjs'},integrity:{'/utility.mjs':'sha384-existing'}};
  assert.deepEqual(routeImportMap(untouched), untouched);
  assert.throws(() => routeImportMap({...untouched, imports:{three:'../build/three.module.js'}}), /integrity.*regenerated/);
  assert.throws(() => routeImportMap({...untouched, scopes:{'/gpu/':{three:'../build/three.webgpu.js'}}}), /integrity.*regenerated/);
});
...
test('prototype-shaped import keys remain inert own entries', () => {
  const source = JSON.parse('{"imports":{"__proto__":"/prototype.mjs","constructor":"/ctor.mjs"},"scopes":{"__proto__":{"three":null}}}');
  const result = createDevImportMap({sourceImportMap:source});
...
test('routing is idempotent and does not mutate the input map', () => {
  const source = {imports:{three:'../build/three.webgpu.js'}, scopes:{'/gpu/':{tsl:'../build/three.tsl.js'}}};
  const snapshot = structuredClone(source);
...
test('forcing WebGPU cannot bypass an existing root integrity mapping', () => {
  assert.throws(() => createDevImportMap({
    routeThreeToWebgpu:true,
    sourceImportMap:{imports:{three:'/custom.mjs'},integrity:{'/custom.mjs':'sha384-existing'}},
  }), /integrity.*regenerated/);
});

test('noscript fallback content is not treated as an active import map', () => {
  const fallback = '<noscript>' + wrap({imports:{three:'/fallback/three.js'}}) + '</noscript>';
