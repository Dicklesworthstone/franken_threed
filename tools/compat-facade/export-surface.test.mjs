...
test("namespace diamonds retain the spec surface without forcing host-ambiguous imports", async () => {
  ...
  fs.writeFileSync(
    ...
    generateFacadeModule(
      { exportKey: ".", condition: "import", target: "root.mjs", moduleType: "esm" },
      surface,
...
test("ordinary imports are not evaluated or resolved", () => {
  const f = fixture({
    "root.mjs": "import 'uninstalled-package'; throw new Error('must not run'); export const x=1;",
  });
...
test("missing roots and CJS keep the established empty static surface", () => {
  ...
  assert.deepEqual(extract(f("missing.mjs")), {
    named: [],
    hasDefault: false,
    totalCount: 0,
    isCJS: false,
...
test("declared named exports from external modules do not require evaluating them", async () => {
  const f = fixture({
    "root.mjs":
      "export {readFile as read} from 'node:fs'; import {writeFile as write} from 'node:fs'; export {write};",
  });
  await matchesNode(f("root.mjs"));
});
...
test("caller-supplied module resolution follows package ESM bindings, not require conditions", async () => {
  ...
  await matchesNode(f("root.mjs"), {
    resolveModule: (specifier) =>
      specifier === "example" ? f("node_modules/example/esm.mjs") : undefined,
  });
});
...
test("generated facades load ambiguous stars and preserve live bindings and identity", async () => {
  const { extractModuleExportSurface, generateFacadeModule, parseFacadeExportSurface } =
    await import("./index.mjs");
  const f = fixture({
    "leaf.mjs": "export let value=1; export function increment(){value++} export const object={};",
    "other.mjs": "export const conflict=2;",
    "root.mjs":
      "export * from './leaf.mjs'; export const conflict=1; export {object as default} from './leaf.mjs';",
    "ambiguous.mjs":
      "export * from './root.mjs'; export * from './other.mjs'; export {default} from './root.mjs';",
  });
  const surface = extractModuleExportSurface(f("ambiguous.mjs"));
  const source = generateFacadeModule(
    { exportKey: ".", condition: "import", target: "ambiguous.mjs", moduleType: "esm" },
    surface,
    { retainedSpecifier: "./ambiguous.mjs" },
  );
...
test("generated string export names and quoted specifiers survive real module loading", async () => {
  ...
  const source = generateFacadeModule(
    { exportKey: ".", condition: "import", target: "it's.mjs", moduleType: "esm" },
    surface,
    { retainedSpecifier: "./it's.mjs" },
  );
  fs.writeFileSync(f("facade.mjs"), source);
  assert.deepEqual(parseFacadeExportSurface(source).named, surface.named);
...
test("generated CJS retains synchronous require and singleton identity with quoted paths", async () => {
  ...
  const source = generateFacadeModule(
    { exportKey: ".", condition: "require", target: "it's.cjs", moduleType: "cjs" },
    { named: [], hasDefault: false },
    { retainedSpecifier: "./it's.cjs" },
  );
...
test("package self-references use reconciled ESM targets across the complete facade map", async () => {
  ...
  const f = fixture({
    "package.json": JSON.stringify({
      name: "three",
      type: "module",
      exports: { import: "./root.js", require: "./root.cjs" },
    }),
  ...
  const result = buildFacadeModuleMap({
    packageDir: path.dirname(f("root.js")),
...
test("path metadata cannot terminate generated facade comments", async () => {
  ...
  const source = generateFacadeModule(
    {
      exportKey: "./dir*/source",
      condition: "import",
      target: "dir*/source.mjs",
      moduleType: "esm",
    },
    extract(f("dir*/source.mjs")),
    { retainedSpecifier: "./dir*/source.mjs" },
  );
  fs.writeFileSync(f("facade.mjs"), source);
