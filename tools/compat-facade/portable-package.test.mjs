...
function fixture(t, overrides = {}) {
  ...
  put(
    ...
    'import { Mesh } from "three"; export { Mesh }; export default Mesh; export const worker = new URL("../workers/decoder.js", import.meta.url);',
  );
...
test("relocated package executes ESM, CJS, self-references and worker URLs without the source", (t) => {
  ...
  assert.equal(
    run(
      f.root,
      `const assert = require('node:assert/strict'); const api = require('@franken/three'); assert.equal(api.value,42); assert.equal(api.synchronous,true); assert.equal(api.then,undefined); console.log('cjs-ok');`,
      "commonjs",
    ),
    "cjs-ok",
  );
...
test("preserves binary assets, private export blockers and original nested package metadata", (t) => {
  ...
  assert.equal(
    JSON.parse(fs.readFileSync(path.join(f.out, "_retained/package.json"))).name,
    "three",
  );
...
test("rewrites legacy bundler, declaration, bin and browser fields without touching external dependencies", () => {
  const result = portablePackageManifest({
    exports: "./index.js",
    main: "index.cjs",
    types: "./types/index.d.ts",
    bin: { tool: "bin/tool.js" },
    browser: { "./index.js": "./web.js", fs: false },
    imports: { "#local": "./src/local.js", "#external": "external" },
  });
...
test("rejects traversal, malformed targets and unsupported package contracts", () => {
  ...
  assert.throws(
    () => portablePackageManifest({ exports: "./x.js" }, { packageName: "../bad" }),
    /package name/,
  );
});
...
test("refuses recursive output even through a symlinked parent", (t) => {
  const f = fixture(t);
  assert.throws(
    () => emitPortablePackage(path.join(f.source, "dist"), { packageDir: f.source }),
    /outside/,
  );
  fs.symlinkSync(f.source, path.join(f.root, "alias"), "dir");
  assert.throws(
    () => emitPortablePackage(path.join(f.root, "alias/dist"), { packageDir: f.source }),
    /outside/,
  );
...
test("CLI produces a working package and reports retained execution honestly", (t) => {
  const f = fixture(t);
  const result = spawnSync(process.execPath, [cli, "--out", f.out, "--package-dir", f.source], {
    encoding: "utf8",
  });
  assert.equal(result.status, 0, result.stderr);
  const report = JSON.parse(result.stdout);
  assert.equal(report.execution, "retained-upstream-js");
...
test("relocates tree-shaking globs and versioned declaration paths", () => {
  ...
  assert.deepEqual(result.typesVersions, {
    ">=5.0": { "*": ["./_retained/types/*", "./_retained/fallback/*"] },
  });
  assert.deepEqual(source.sideEffects, ["./src/nodes/**/*", "*.css", "**/*.wasm"]);
  for (const value of [true, false])
    assert.equal(
      portablePackageManifest({ exports: "./index.js", sideEffects: value }).sideEffects,
      value,
    );
  assert.throws(
    () => portablePackageManifest({ exports: "./index.js", sideEffects: "invalid" }),
    /sideEffects/,
  );
  assert.throws(
    () => portablePackageManifest({ exports: "./index.js", sideEffects: ["!src/*"] }),
    /sideEffects/,
  );
});
...
test("excludes nested packaging controls while leaving source files untouched", (t) => {
  ...
  for (const name of [".npmignore", ".gitignore", ".npmrc"])
    f.put(`assets/${name}`, "original config");
