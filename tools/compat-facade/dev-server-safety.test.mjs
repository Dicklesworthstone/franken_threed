...
function fixture() {
  ...
  const files = {
    "repo/upstream/three.js/examples/webgpu_performance_renderbundle.html":
      '<!doctype html><script nonce="keep-nonce" type="importmap">{"imports":{"three":"../build/three.webgpu.js"},"scopes":{"/cpu/":{"three":"../build/three.module.js"}}}</script><script type="module">window.applicationIsUnchanged = true;</script>',
...
function request(port, target, method = "GET") {
  return new Promise((resolve, reject) => {
    ...
    const req = http.request(
      { host: "127.0.0.1", port, path: target, method, agent: false },
      (res) => {
        const chunks = [];
...
test("real HTTP H1 routing preserves scopes, CSP nonce and application bytes", async () => {
  await withServer(async (port) => {
    ...
    assert.ok(
      res.body.endsWith('<script type="module">window.applicationIsUnchanged = true;</script>'),
    );
...
test("real HTTP aliases, repository files and example fallback serve original bytes", async () => {
  await withServer(async (port) => {
    for (const [url, expected] of [
      ...
      ["/textures/example.bin", "TEXTURE"],
      ["/examples/textures/example.bin", "TEXTURE"],
      ["/example.css", "body { margin: 0; }"],
      ["/examples/example.css", "body { margin: 0; }"],
    ]) {
...
test("external symlinks cannot leak through any static route", async () => {
  await withServer(async (port, { parent, repoRoot }) => {
    ...
    fs.symlinkSync(
      path.join(repoRoot, "private.txt"),
      path.join(repoRoot, "upstream/three.js/build/alias-escape.txt"),
    );
...
test("in-root symlinks, relative repository roots and generated endpoints still work", async () => {
  const files = fixture();
  fs.symlinkSync(
    path.join(files.repoRoot, "tools/example.mjs"),
    path.join(files.repoRoot, "tool-link.mjs"),
  );
  const server = await startDevServer({
    repoRoot: path.relative(process.cwd(), files.repoRoot),
    port: 0,
  });
  try {
...
test("missing paths and directories return 404 without disrupting subsequent requests", async () => {
  await withServer(async (port) => {
    for (const target of [
      "/missing.bin",
      "/compat-facade/addons/missing.js",
      "/build/missing.js",
      "/tools/",
      "/textures/",
    ]) {
...
test("unsafe configured H1 paths and symlinks cannot leave the repository", async () => {
  ...
  const files = fixture();
  fs.symlinkSync(
    path.join(files.parent, "repo-private/outside.txt"),
    path.join(files.repoRoot, "outside.html"),
  );
  const server = await startDevServer({ ...files, port: 0, h1RelativePath: "outside.html" });
  try {
    assert.equal((await request(server.port, "/")).status, 403);
  } finally {
    await server.close();
  }
});
...
test("HTML transformation failures return 500 without disabling static assets", async () => {
  await withServer(async (port, { repoRoot }) => {
    fs.writeFileSync(
      path.join(repoRoot, "upstream/three.js/examples/webgpu_performance_renderbundle.html"),
