...
test("Positive: enumeratePackageExportEntries covers all 1778 pinned r186 package export entries", () => {
  ...
  assert.equal(
    categories.examples_jsm_wildcard,
    494,
    "examples/jsm/* wildcard code exports must be 494",
  );
...
test("Positive: extractModuleExportSurface extracts accurate export surface for core bundles", () => {
  ...
  const threeModule = extractModuleExportSurface("upstream/three.js/build/three.module.js", {
    cache,
  });
  ...
  // 2. three.webgpu.js
  const threeWebgpu = extractModuleExportSurface("upstream/three.js/build/three.webgpu.js", {
    cache,
  });
  ...
  const orbitControls = extractModuleExportSurface(
    "upstream/three.js/examples/jsm/controls/OrbitControls.js",
    { cache },
  );
  ...
  const webglCapability = extractModuleExportSurface(
    "upstream/three.js/examples/jsm/capabilities/WebGL.js",
    { cache },
  );
...
test("Core: Every generated ESM facade re-exports exactly the upstream export names (Acorn parse-only, no Rollup)", () => {
  ...
  for (const [key, item] of result.map.entries()) {
    ...
    if (expectedNamed.size !== actualNamed.size) {
      failures.push({
        key,
        reason: `Named export count mismatch: expected ${expectedNamed.size}, got ${actualNamed.size}`,
      });
      continue;
    }
    ...
    if (parsed.hasDefault !== item.exportSurface.hasDefault) {
      failures.push({
        key,
        reason: `Default export mismatch: expected ${item.exportSurface.hasDefault}, got ${parsed.hasDefault}`,
      });
      continue;
    }
  ...
  assert.equal(
    failures.length,
    0,
    `All ESM facades must match export surface exactly. Failures: ${JSON.stringify(failures.slice(0, 3))}`,
  );
...
test("Core: Root CommonJS facade synchronously re-exports without Promises or GPU access", () => {
  ...
  assert.ok(
    !cjsEntry.facadeSource.includes("Promise"),
    "CJS facade must not create or return Promises",
  );
  assert.ok(!cjsEntry.facadeSource.includes("async"), "CJS facade must be synchronous");
  assert.ok(
    !cjsEntry.facadeSource.includes("navigator.gpu"),
    "CJS facade must not reference navigator.gpu",
  );
...
test("Node Execution: require() CJS and import() root ESM, webgpu, tsl, and addons facades with strict guards", async () => {
  ...
  try {
    ...
    assert.equal(
      typeof cjsThree.Vector3,
      "function",
      "CJS export must include Vector3 constructor",
    );
    assert.equal(typeof cjsThree.Mesh, "function", "CJS export must include Mesh constructor");
    assert.equal(
      typeof cjsThree.WebGLRenderer,
      "function",
      "CJS export must include WebGLRenderer constructor",
    );
    assert.ok(
      !(cjsThree instanceof Promise),
      "CJS entry must be strictly synchronous, never a Promise",
    );
    ...
    const actualEsmKeys = Object.keys(esmThree)
      .filter((k) => k !== "default")
      .sort();
    assert.deepEqual(
      actualEsmKeys,
      rootEsmMapItem.exportSurface.named,
      "Root ESM export names must match map",
    );
    ...
    const actualWebgpuKeys = Object.keys(webgpuThree)
      .filter((k) => k !== "default")
      .sort();
    assert.deepEqual(
      actualWebgpuKeys,
      webgpuMapItem.exportSurface.named,
      "WebGPU export names must match map",
    );
    ...
    const actualTslKeys = Object.keys(tslThree)
      .filter((k) => k !== "default")
      .sort();
    assert.deepEqual(
      actualTslKeys,
      tslMapItem.exportSurface.named,
      "TSL export names must match map",
    );
    ...
    const actualAddonsKeys = Object.keys(addonsThree)
      .filter((k) => k !== "default")
      .sort();
    assert.deepEqual(
      actualAddonsKeys,
      addonsMapItem.exportSurface.named,
      "Addons export names must match map",
    );
    ...
    Object.defineProperty(globalThis, "document", {
      get() {
        throw new Error(
          "ILLEGAL_ACCESS: global document accessed during CPU-only module execution",
        );
      },
      configurable: true,
    });
    ...
    if (typeof globalThis.navigator !== "undefined") {
      Object.defineProperty(globalThis.navigator, "gpu", {
        get() {
          throw new Error(
            "ILLEGAL_ACCESS: navigator.gpu accessed during CPU-only module execution",
          );
        },
        configurable: true,
      });
    ...
    WebAssembly.instantiate = () => {
      throw new Error(
        "ILLEGAL_ACCESS: WebAssembly.instantiate called during CPU-only module execution",
      );
    };
    WebAssembly.instantiateStreaming = () => {
      throw new Error(
        "ILLEGAL_ACCESS: WebAssembly.instantiateStreaming called during CPU-only module execution",
      );
    };
