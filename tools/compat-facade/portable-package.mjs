...
function retainedTarget(value, { external = false, relative = false } = {}) {
  ...
  try {
    ...
    throw new Error(`Invalid encoded package target: ${value}`);
  }
  if (
    !decoded ||
    /[\\\0]/.test(decoded) ||
    decoded
      .split("/")
      .some((part) => part === ".." || part === "." || part === "node_modules" || part === "")
  ) {
    throw new Error(`Unsupported or escaping package target: ${value}`);
  }
...
function mapTargets(value, transform) {
  ...
  return Object.fromEntries(
    Object.entries(value).map(([key, target]) => [key, mapTargets(target, transform)]),
  );
}
...
export function portablePackageManifest(upstream, { packageName = "@franken/three" } = {}) {
  ...
  if (
    ...
    !Object.hasOwn(upstream, "exports")
  ) {
  ...
  for (const key of ["main", "module", "types", "typings"]) {
    if (upstream[key] !== undefined)
      manifest[key] = retainedTarget(upstream[key], { relative: true });
  }
  if (upstream.imports)
    manifest.imports = mapTargets(upstream.imports, (target) =>
      retainedTarget(target, { external: true }),
    );
  if (typeof upstream.browser === "string")
    manifest.browser = retainedTarget(upstream.browser, { relative: true });
  else if (upstream.browser && typeof upstream.browser === "object") {
    manifest.browser = Object.fromEntries(
      Object.entries(upstream.browser).map(([key, value]) => [
  ...
  if (upstream.typesVersions) {
    manifest.typesVersions = mapTargets(upstream.typesVersions, (target) =>
      retainedTarget(target, { relative: true }),
    );
  }
  if (typeof upstream.bin === "string")
    manifest.bin = retainedTarget(upstream.bin, { relative: true });
  else if (upstream.bin)
    manifest.bin = Object.fromEntries(
      Object.entries(upstream.bin).map(([key, value]) => [
        key,
        retainedTarget(value, { relative: true }),
      ]),
    );
  // Upstream development/publish hooks must not run when installing this facade.
  for (const key of ["scripts", "devDependencies", "workspaces", "directories", "publishConfig"])
    delete manifest[key];
...
export function emitPortablePackage(targetDir, options = {}) {
  if (typeof targetDir !== "string" || !targetDir)
    throw new TypeError("A destination directory is required");
  ...
  if (within(source, destination))
    throw new Error("Portable output must be outside the source package");
  ...
  try {
    ...
    for (const relative of files.filter(
      (file) => !file.includes(path.sep) && /^(?:licen[cs]e|copying|notice)(?:[.-]|$)/i.test(file),
    )) {
  ...
  return {
    ...
    manifest,
    files: files.map((file) => `${RETAINED}/${file.split(path.sep).join("/")}`),
    count: files.length,
  };
}
...
function main(args) {
  ...
  for (let i = 0; i < args.length; i++) {
    const flag = args[i];
    if (
      !["--out", "--package-dir", "--name"].includes(flag) ||
      !args[i + 1] ||
      args[i + 1].startsWith("--")
    ) {
      throw new Error(
        "Usage: node portable-package.mjs --out NEW_DIRECTORY [--package-dir UPSTREAM] [--name @franken/three]",
      );
    }
    const value = args[++i];
    ...
    JSON.stringify(
      {
        ...
        retainedFiles: result.count,
        execution: "retained-upstream-js",
        accelerated: false,
      },
      null,
      2,
